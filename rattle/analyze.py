...
class ControlFlowGraph(object):
    ...
    def __iter__(self):
        """Allows iteration over the basic blocks in the function."""
        return iter(self.function.blocks)  # Ensures iteration over SSA blocks
    ...
    def dot(self) -> str:
        ...
        for block in self.function.blocks:  # Changed from self.function to self.function.blocks
            block_id = f'block_{block.offset:#x}'
            block_body = '\\l'.join([f'{insn.offset:#x}: {insn}' for insn in block])
            block_body = block_body.replace('<', '\\<').replace('>', '\\>')
            block_dot = f'{block_id} [label="{block_body}\\l", shape="record"];'
