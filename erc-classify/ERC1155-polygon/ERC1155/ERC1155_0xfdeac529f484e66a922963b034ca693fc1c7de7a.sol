...
contract Valts721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    // For Opensea to return contract-level metadata
    function contractURI()
    public
    pure
    returns (string memory) {
        return "https://www.valts.link/swagger/721.json";
    }
    ...
    override
    returns (string memory) {
        return "https://www.valts.link/swagger/nfts/";
    }
    ...
    returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        string memory tokenIdHex = toHexString(tokenId);
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenIdHex, ".json")) : "";
    }

    // Same as ERC1155 token's URI formatting pattern. Equivalent to python expression: {:064x}.format(1)
    ...
    returns (string memory)
    {
        ...
        // This is ridiculous.
        // Because uint cannot be set to -1, we can only loop to 1, need extra process for 0 index
        // And array only accepts uint as index
        for (uint256 i = length-1; i > 0; i--){
            buffer[i] = _HEX_SYMBOLS[value & 0xf];
            value >>= 4;
        }
        // Have to process the 0 index here
