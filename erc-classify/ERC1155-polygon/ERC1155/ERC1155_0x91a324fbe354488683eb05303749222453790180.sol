...
// File: @ensdomains/ens-contracts/contracts/utils/BytesUtils.sol
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// File: contracts/EnstoolsAuction.sol
...
contract EnstoolsAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC1155Holder {
    ...
    function buy(uint256 _auctionId, uint256 _paymentValue) external payable {
        require(_auctionId < auctionCount, concatenateStrings("Auction id must be less than ", uint2str(auctionCount)));
        require(_paymentValue == msg.value);

        AuctionEntry storage _auctionEntry = auctionEntries[_auctionId];
        require(getTimestamp() < _auctionEntry.auctionUntil, "Auction has ended");
        require(!_auctionEntry.cancelled, "Auction has been canceled");
        ...
        IERC1155(ensNFT).safeTransferFrom(_auctionEntry.creator, address(this), _auctionEntry.tokenId, 1, "0x");
        PublicResolver(ensPublicResolver).setAddr(bytes32(_auctionEntry.tokenId), msg.sender);
        IERC1155(ensNFT).safeTransferFrom(address(this), msg.sender, _auctionEntry.tokenId, 1, "0x");
    ...
    function cancelAuction(uint256 _auctionId) external {
        AuctionEntry storage _auctionEntry = auctionEntries[_auctionId];
        require(_auctionEntry.creator == msg.sender, "It should be the creator");
        require(_auctionEntry.cancelled == false, "It has already been canceled");
        require(_auctionEntry.selectedAt == 0, "It has already ended");
