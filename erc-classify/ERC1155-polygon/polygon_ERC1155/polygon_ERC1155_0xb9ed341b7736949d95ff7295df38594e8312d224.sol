...
// Chain: POLYGON - File: @openzeppelin/contracts/utils/Pausable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/GlinERC1155.sol
// SPDX-License-Identifier: MIT
...
/**
 * @title GlinERC1155
 * @dev GlinERC1155 is an ERC1155 contract, inheriting various features from OpenZeppelin contracts.
    ...
    ReentrancyGuard
{
     ...
     */
    mapping(uint256 => uint256) public singleAddressMaxMint;
     ...
     */
    mapping(uint256 => uint256) public singleNftMaxSupply;
     ...
     * Also sets default maximum mint limits per address and default maximum supplies for each NFT.
     *
     *
     * Requirements:
     *
     * - `_initialBaseURI` must be a non-empty string. This will be the base URI for all token metadata.
     * - `_owner` must be a valid address. This address will be set as the owner of the contract.
     *
    ...
    constructor(
        ...
        address _owner
    ) ERC1155(_initialBaseURI) Ownable(_owner) {
        ...
        for (uint256 i = 0; i < allCollectionUniqueSupply; i++) {
            singleAddressMaxMint[i] = 10;
            singleNftMaxSupply[i] = 1000;
        }
    }
     ...
     * Can only be called by the contract owner.
     */
    function setAllCollectionUniqueSupply(
        uint256 newSupply
    ) external onlyOwner {
     ...
     */
    function setSingleAddressMaxMint(
        uint256 tokenId,
        uint256 newMaxMint
    ) external onlyOwner {
        singleAddressMaxMint[tokenId] = newMaxMint;
    }

    /**
     * @dev Sets the maximum supply for a single NFT.
     * Can only be called by the contract owner.
     *
     * @param newMaxSupply The new maximum supply.
     */
    function setSingleNftMaxSupply(
        uint256 tokenId,
        uint256 newMaxSupply
    ) external onlyOwner {
        singleNftMaxSupply[tokenId] = newMaxSupply;
    }
    ...
    /**
     * @dev Adds an address to the allow list for a specific NFT ID.
     */
    function addToAllowList(address _address, uint256 _id) external onlyOwner {
        allowList[_address][_id] = true;
        emit AllowListUpdated(_address, _id, true);
    }

    /**
     * @dev Removes an address from the allow list for a specific NFT ID.
     */
    function removeFromAllowList(
        address _address,
        uint256 _id
    ) external onlyOwner {
        allowList[_address][_id] = false;
        emit AllowListUpdated(_address, _id, false);
    }
     ...
     * - The caller must send enough ETH to cover the minting cost.
     * - The caller can mint 1 amount until added again to list
     * - Hardcode the minting of exactly 1 unit
     * - After mint caller removing from allow list for given nft id
     * - Token id must be a valid number.
     */
    function allowListMint(uint256 id) external payable nonReentrant {
        require(allowListMintOpen, "Allow list mint is closed");
        require(
            allowList[msg.sender][id],
            "You are not eligible for allow list mint for this NFT ID"
        );
        require(msg.value >= allowListPrice, "Not enough money sent");

        _mintToken(id, 1, msg.sender);
        allowList[msg.sender][id] = false;
    }
     ...
     */
    function _mintToken(uint256 id, uint256 amount, address recipient) private {
        require(id < allCollectionUniqueSupply, "Nft not found");
        require(
            totalSupply(id) + amount <= singleNftMaxSupply[id],
            "Max supply for this NFT ID cannot be exceeded"
        );
        require(
            addressTokenMinted[recipient][id] + amount <=
                singleAddressMaxMint[id],
