...
contract PolygonCubes16 is ERC1155Burnable, Ownable {
    using Strings for uint256;
    using Strings for uint128;

    constructor(string memory contractUri) ERC1155("") {
        _contractURI = contractUri;
        _mint(_msgSender(), 0, 256, "");
    }
    ...
    function _attributes() private view returns(string memory) {
        return string(
            abi.encodePacked(
                '[',
                    '{"trait_type":"Current Block Number","value":"',block.number.toString(),'"}',
                ']'
            )
        );
    }
    ...
    function _shape(bytes32 seed) private pure returns(string memory) {
        ...
        return string(
            abi.encodePacked(
                "<rect fill='hsl(",(uint256(seed) % 360).toString(),",99%,65%)' width='512' height='512' x='",(uint128(xy[0]) % 2048).toString(),"' y='",(uint128(xy[1]) % 2048).toString(),"' />"
            )
        );
    }
    ...
    function _svg() private view returns(string memory) {
        ...
        return string(
            abi.encodePacked(
                "<svg width='2048' height='2048' viewPort='0 0 2048 2048' style='background:hsl(",(uint256(blockhash(block.number - 1)) % 360).toString(),",99%,80%)' xmlns='http://www.w3.org/2000/svg'>",
        ...
        return string(
                abi.encodePacked(
                    ...
                    Base64.encode(string(abi.encodePacked(
                        '{"name":"Polygon Cubes 16",', 
                        '"description":"A dynamic NFT generating an image based on 16 recent blocks.\\nEvery cube is representing a block, positioned and colored based on the block hash.\\nBackground color is based on the current block hash.\\n\\nAll of the item metadata of this NFT lives on the blockchain.",',  
                        '"image_data":', '"', _svg(), '",',
