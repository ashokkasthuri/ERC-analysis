...
contract TwTDefendFacet {
    function twtShadowcornDefendRegion(
        ...
        LibTwTShadowcornDefense.twtShadowcornDefendRegion(
            regionId,
...
library LibTwTShadowcornDefense {
    ...
    function twtSetMaxBatchQuantityForMinionSquads(
        ...
        require(maxBatchQuantity > 0, "SD-013");
    ...
    function twtValidateUserCanDefendRegionAndGetData(
    ...
    ) internal view returns (DraftSquadPlacement memory draftSquadPlacement) {
        {
            ...
            for (uint256 i = 0; i < poolIds.length; ++i) {
                if (someZeroAppeared) {
                    require(poolIds[i] == 0 && minionAmounts[i] == 0, "SD-012");
                } else {
                    if (poolIds[i] > 0) {
                        require(minionAmounts[i] > 0, "SD-011");
                        draftSquadPlacement.amountOfDifferentClasses++;
                    } else {
                        someZeroAppeared = true;
                        require(minionAmounts[i] == 0, "SD-010");
                    }
                }
            }
        }
        ...
        require(shadowcornOverseerTokenId > 0, "SD-008");
    ...
    function twtRemoveMinionSquadFromUserSquads(
        ...
        require(squads.length > 0, "SD-014");
    ...
    function getShadowcornOverseerTotalStats(
        uint24 shadowcornOverseerTokenId
    ) internal view returns (uint24 shadowcornOverseerTotalStats) {
        (
            uint256 might,
            uint256 wickedness,
            uint256 tenacity,
        ...
        shadowcornOverseerTotalStats = uint24(
            ((might + wickedness + tenacity + cunning + arcana) *
                LibTwTAdmin.twtGetOverseerStatsBonusForCurrentSeason()) / 100
        );
    }
...
library LibExternalAddress {
    ...
    function externalAddressStorage()
        internal
        pure
...
interface IMinionStats {
    function getMinionPoolStats(
        uint256 poolId
    ) external view returns (MinionStats memory);

    function getMinionPoolStatsAndClassesMultiple(
        uint256[] memory poolIds
    ) external view returns (MinionStats[] memory, uint8[] memory);
...
library LibTwTModifier {
    ...
    function twtGetRegionModifiersByCurrentSeason(
        uint8 regionId
    ) internal view returns (TwTRegionModifier memory) {
        (, uint16 currentSeasonId) = LibTwTSeason.twtGetCurrentSeason();

        return
            twtModifierStorage().regionModifiersBySeason[currentSeasonId][
                regionId
            ];
    }

    function twtGetModifiersForAllRegions(
        uint16 seasonId
    ) internal view returns (LibTwTModifier.TwTRegionModifier[5] memory) {
        LibTwTModifier.TwTRegionModifier[5] memory regionModifiers;

        for (uint8 i = 0; i < 5; ++i) {
            regionModifiers[i] = twtModifierStorage().regionModifiersBySeason[
                seasonId
            ][i + 1];
        }
    ...
    function twtGetRegionModifiersBySeason(
        uint16 seasonId,
        uint8 regionId
    ) internal view returns (TwTRegionModifier memory) {
    ...
    function twtSetRegionModifiersBySeason(
        uint16 seasonId,
        uint8 regionId,
        TwTRegionModifier memory regionModifier
    ) internal {
        // Check if shadowcorn class is valid
        require(
            regionModifier.affectedMinionClass > 0 &&
                regionModifier.affectedMinionClass <= 5,
            "M-001"
        );
        ...
        for (
            uint8 i = 0;
            i < regionModifier.affectedUnicornClasses.length;
            ++i
        ) {
        ...
        twtModifierStorage().regionModifiersBySeason[seasonId][
            regionId
        ] = regionModifier;
    }
     ...
     */
    function getRPSModifier(
        IUnicornStatCache.Stats[] memory unicornsInformation,
        LibTwTShadowcornDefense.ShadowcornSquad memory shadowcornSquad
    )
        internal
        view
        returns (uint24 shadowcornTeamRPSBonus, uint24 unicornTeamRPSBonus)
    {
        uint24[] memory minionAmountsThatLostRPSAgainstUnicorns = new uint24[](
            shadowcornSquad.minionAmounts.length
        );
        uint24[] memory minionAmountsThatWonRPSAgainstUnicorns = new uint24[](
            shadowcornSquad.minionAmounts.length
        );

        LibTwTShadowcornDefense.ShadowcornSquad
            memory shadowcornSquadTemporaryForShadowcorns = shadowcornSquad;
        ...
        for (uint256 i = 0; i < unicornsInformation.length; ++i) {
            ...
            for (uint256 j = 0; j < shadowcornSquad.minionClasses.length; ++j) {
                if (
                    !unicornAlreadyWonRPS &&
                    (shadowcornSquad.minionAmounts[j] -
                        minionAmountsThatLostRPSAgainstUnicorns[j]) >
                    0 &&
                    unicornWinsRPSMatch(
                        unicornsInformation[i].class,
                        shadowcornSquad.minionClasses[j]
                    )
                ) {
    ...
    function twtGetTotalModifiersAmountForMinion(
        uint8 minionClass,
        uint256 shadowcornOverseerClass,
        uint256 shadowcornOverseerRarity,
        uint8 regionId
        ...
        )
    {
        ...
        uint24 shadowcornModifier = twtGetShadowcornModifierForMinion(
            minionClass,
            shadowcornOverseerClass,
            shadowcornOverseerRarity
        );
        // Get region modifier to minion stats: % Modifier to selected stats based on Region combat occurs in
        uint24 regionModifier = twtGetRegionModifierForMinion(
            minionClass,
            regionId,
            shadowcornOverseerRarity
        );
    ...
    function twtGetTotalModifiersAmountForUnicorn(
        IUnicornStatCache.Stats memory unicornInfo,
        uint8 regionId
    ) internal view returns (uint24) {
        uint24 genesisBonus = twtGetGenesisBonusForUnicorn(unicornInfo.origin);
        uint24 mythicBonus = twtGetMythicBonusForUnicorn(
            unicornInfo.mythicCount
        );
        uint24 regionBonus = twtGetRegionBonusForUnicorn(
            unicornInfo.class,
            unicornInfo.origin,
            unicornInfo.mythicCount,
            regionId
    ...
    function twtGetGenesisBonusForUnicorn(
        bool isGenesis
    ...
    function twtGetMythicBonusForUnicorn(
    ...
    function twtGetShadowcornModifierForMinion(
        uint8 minionClass,
        uint256 shadowcornOverseerClass,
        uint256 shadowcornOverseerRarity
    ...
    function twtGetRegionModifierForMinion(
        uint8 minionClass,
        uint8 regionId,
        uint256 shadowcornRarityId
    ) private view returns (uint24) {
        LibTwTModifier.TwTRegionModifier memory regionModifier = LibTwTModifier
            .twtGetRegionModifiersByCurrentSeason(regionId);

        uint24 modifierAmount = 0;

        if (regionModifier.affectedMinionClass == minionClass) {
            modifierAmount += regionModifier
                .affectedMinionClassModifierPercentage;
        }

        if (regionModifier.affectedShadowcornRarity == shadowcornRarityId) {
            modifierAmount += regionModifier
                .affectedShadowcornRarityModifierPercentage;
        }

        return modifierAmount;
    }

    function twtGetRegionBonusForUnicorn(
        uint8 unicornClass,
        bool isGenesis,
        uint8 amountOfMythicParts,
        uint8 regionId
        ...
        for (
            ...
            if (regionModifier.affectedUnicornClasses[i] == unicornClass) {
                bonus += regionModifier.affectedUnicornClassModifierPercentages[
                    i
                ];
                break;
            }
        ...
        if (amountOfMythicParts > 0) {
            bonus += regionModifier.modifierPercentageForMythicUnicorns;
        }

        if (isGenesis) {
            bonus += regionModifier.modifierPercentageForGenesisUnicorns;
        }
...
library LibTwTSeason {
    ...
    function twtUpdateSeasonState(
        ...
        require(oldState != TwTSeasonState.NONE, "S-005");

        // Check new state is advancing the season's current state
        require(uint256(newState) == uint256(oldState) + 1, "S-004");
    ...
    function twtSetCurrentSeason(uint16 seasonId) internal {
        ...
        // Check if season exists
        require(seasonState != TwTSeasonState.NONE, "S-003");
...
library LibEvents {
    ...
    event TwTManualBurn(
        uint56 indexed matchFrom,
        uint56 indexed matchTo,
        uint56[] matchIds,
        uint24[5] poolIds,
        uint24[5] amounts
    );
    event TwTClaimableRewardsProcessed(
...
library LibTwTRewards {
    ...
    function twtAddSeasonReward(
        ...
        require(splits.length == 4, "R-022");

        require(splits[0] + splits[1] <= component.amount, "R-021");
        require(splits[2] + splits[3] <= component.amount, "R-020");
        require(splits[0] + splits[1] + splits[2] <= component.amount, "R-019");
        require(splits[0] + splits[2] + splits[3] <= component.amount, "R-018");
    ...
    function twtClaimUnicornRewards(
        ...
        // begin the next wave if the current wave is over
        twtBeginNextWaveIfCurrentIsOver(seasonId, regionId);

        uint256[] memory playerRewards = new uint256[](
            twtRewardsStorage().regionRewards[seasonId][regionId].length
        );

        // reduce unicorn claimable rewards from each wave and get player rewards
        playerRewards = twtReduceAndRetrievePlayerRewards(
            seasonId,
            regionId,
            player,
            false
        ...
        // reward distribution
        twtDistributeRewards(seasonId, regionId, playerRewards, player);
    ...
    function twtClaimShadowcornRewards(
        ...
        // reduce shadowcorn claimable rewards from each wave and get player rewards
        playerRewards = twtReduceAndRetrievePlayerRewards(
            seasonId,
            regionId,
            player,
    ...
    function twtReduceAndRetrievePlayerRewards(
        uint16 seasonId,
        uint8 regionId,
        address player,
        bool isShadowcorn
    ) internal returns (uint256[] memory playerRewards) {
        ...
        if (isShadowcorn) {
            for (
                ...
                playerRewards = addArrays(
                    playerRewards,
                    twtReduceAndRetrieveRewardsByWaveId(
                        seasonId,
                        regionId,
                        waveCounter,
                        player,
                ...
                minionQueue.removeMinionSquadsAndPtsByWaveId(
                    waveCounter,
                    player
                );
            }
            ...
            for (
                ...
                unicornQueue.removeAccountWavePts(waveCounter, player);
            }
        }
    ...
    function twtRetrieveRewardsByWaveId(
        uint16 seasonId,
        uint8 regionId,
        uint256 waveId,
        address player,
        bool isShadowcorn
    ) internal view returns (uint256[] memory playerWaveRewards) {
        LibTwTRewardsStorage storage ltrs = twtRewardsStorage();
        SeasonReward[] storage regionRewards = ltrs.regionRewards[seasonId][
            regionId
        ];

        playerWaveRewards = new uint256[](regionRewards.length);
        ...
        return playerWaveRewards;
    }

    function twtReduceAndRetrieveRewardsByWaveId(
        uint16 seasonId,
        uint8 regionId,
        uint256 waveId,
        address player,
        bool isShadowcorn
    ) internal returns (uint256[] memory playerWaveRewards) {
        LibTwTRewardsStorage storage ltrs = twtRewardsStorage();
        ...
        if (isShadowcorn) {
            ...
            for (uint256 i = 0; i < waveRewards.length; ++i) {
                ...
                if (
                    ...
                    // get faction control for the wave
                    FactionType controllingFactionType = ltrs.factionControlMap[
                        seasonId
                    ][regionId][waveId];
                    ...
                    waveRewards[i].claimableShadowcornRewards = 0;

                    emit LibEvents.TwTClaimableRewardsProcessed(
                        waveRewards[i].claimableShadowcornRewards,
                        playerWaveRewards[i],
                        seasonId,
                        regionId,
                        waveId,
                        waveRewards[i].component.assetType,
                        waveRewards[i].component.poolId,
                        waveRewards[i].component.asset
                    );
                } else {
                    require(
                        waveRewards[i].claimableShadowcornRewards >=
    ...
    function twtRemoveMinionWavePts(
        ...
        for (
            ...
            minionQueue.removeMinionSquadsAndPtsByWaveId(waveCounter, user);
        }
    }
    ...
    function twtGetShadowcornPlayerRewards(
        ...
        for (
            ...
            require(playerWaveRewards.length == regionRewards.length, "R-004");
    ...
    function twtDeclareFactionControl(
        ...
        uint256 lastWaveId = getWaveIdByPeek(unicornQueue, minionQueue, false);
        ...
        )
    {
        ...
        uint256 firstWaveId = getWaveIdByPeek(unicornQueue, minionQueue, true);

        require(firstWaveId <= lastClaimableWaveId, "R-003");
        ...
        for (uint256 i = firstWaveId; i < lastClaimableWaveId; ++i) {
            ...
            unclaimedShadowcornRewards = new uint256[](
                unclaimedShadowcornRewardsByWaveId.length
            );
            unclaimedUnicornRewards = new uint256[](
                unclaimedUnicornRewardsByWaveId.length
            );
    ...
    function getWaveIdByPeek(
        LibUnicornQueue.QueueStorage storage unicornQueue,
        LibMinionQueue.QueueStorage storage minionQueue,
        bool isFirstWave // true for first waveId, false for last waveId
    ) internal view returns (uint256 waveId) {
        uint256 unicornWaveId;
        uint256 minionWaveId;

        // If we are looking for the first waveId
        if (isFirstWave) {
            (unicornWaveId, ) = unicornQueue.peek();
            (minionWaveId, , ) = minionQueue.peek();
            if (unicornWaveId != 0 && minionWaveId != 0) {
                waveId = unicornWaveId <= minionWaveId
                    ? unicornWaveId
                    : minionWaveId;
            } else if (unicornWaveId != 0) {
                waveId = unicornWaveId;
            } else {
                waveId = minionWaveId;
            }
        }
        // If we are looking for the last waveId
        else {
            (unicornWaveId, ) = unicornQueue.peekLast();
            (minionWaveId, , ) = minionQueue.peekLast();
            waveId = unicornWaveId >= minionWaveId
                ? unicornWaveId
                : minionWaveId;
        }
    ...
    function twtDequeueUnicornQueue(
        ...
        for (uint256 i = 0; i < dequeueCount; ++i) {
            (uint256 waveIdFromQueue, uint56 points) = unicornQueue.dequeue();
            emit LibEvents.TwTUnicornDequeue(
                seasonId,
                regionId,
                waveIdFromQueue,
                points
    ...
    function twtDequeueMinionQueue(uint16 seasonId, uint8 regionId) internal {
        ...
        for (uint256 i = 0; i < dequeueCount; ++i) {
            ...
            emit LibEvents.TwTMinionDequeue(
                seasonId,
                regionId,
                waveIdFromQueue,
                stakingPts,
    ...
    function twtGetPlayerRewardsForAllRegionsV2(
        uint16 seasonId,
        address player,
        bool isShadowcorn
    ) internal view returns (uint256[] memory playerRewards) {
        // Get rewards length from the first region since they all have the same length
        playerRewards = new uint256[](
            twtRewardsStorage().regionRewards[seasonId][1].length
        );
    ...
    function twtGetUnicornPlayerRewards(
        ...
        for (
            ...
            require(playerWaveRewards.length == regionRewards.length, "R-002");
            for (uint256 j = 0; j < regionRewards.length; ++j) {
                playerRewards[j] = playerRewards[j] + playerWaveRewards[j];
            }
        }
    ...
    function twtShadowcornPlayerRewardsByWaveId(
        ...
        ) = twtGetShadowcornPlayerPtsByWaveId(
                seasonId,
                regionId,
                waveId,
                player
            );
        SeasonReward[] memory dailyRewards = twtRewardsStorage().waveRewards[
            seasonId
        ][regionId][waveId];

        SeasonReward[] memory regionRewards = twtRewardsStorage().regionRewards[
            seasonId
        ][regionId];
    ...
    function debugTwTClaimShadowcornRewards(
        uint16 seasonId,
        uint8 regionId,
        address player
    ) internal {
        ...
        require(playerRewards.length == regionRewards.length, "R-009");
    ...
    function getAccountStakingPtsByWaveId(
        ...
        return minionQueue.getAccountStakingPtsByWaveId(waveId, user);
    }
    ...
    function getAccountCombatPtsByWaveId(
        ...
        return minionQueue.getAccountCombatPtsByWaveId(waveId, user);
    }
    ...
    function twtGetShadowcornPtsForPlayerByWaveId(
        ...
        (stakingPts, combatPts) = minionQueue.getAccountWavePts(waveId, player);
        return (stakingPts, combatPts);
    }
    ...
    function twtGetShadowcornPlayerPtsByWaveId(
        uint16 seasonId,
        uint8 regionId,
        uint256 waveId,
        address player
        ...
        )
    {
        ...
        (myStakingPts, myCombatPts) = minionQueue.getAccountWavePts(
            waveId,
            player
        );

        totalStakingPts = minionQueue.getStakingPtsByWaveId(waveId);
        totalCombatPts = minionQueue.getCombatPtsByWaveId(waveId);
    }
    ...
    function enforceRegionIdIsValid(uint8 regionId) internal pure {
        require(regionId >= 1 && regionId <= 5, "R-001");
    }
    ...
    function twtGetShadowcornPtsByWave(
        uint16 seasonId,
        uint8 regionId,
        uint256 wave
    ) internal view returns (uint56 stakingPts, uint56 combatPts) {
    ...
    function twtGetClaimableRewardsByRegion(
        uint16 seasonId,
        uint8 regionId
    )
        internal
        view
        returns (
            uint256[] memory claimableShadowcornRewards,
            uint256[] memory claimableUnicornRewards
        )
    {
        LibTwTRewardsStorage storage ltrs = twtRewardsStorage();
        SeasonReward[] memory regionRewards = ltrs.regionRewards[seasonId][
        ...
        claimableShadowcornRewards = new uint256[](regionRewards.length);
        claimableUnicornRewards = new uint256[](regionRewards.length);

        for (uint256 i = 0; i < regionRewards.length; ++i) {
            claimableShadowcornRewards[i] += regionRewards[i]
                .claimableShadowcornRewards;
            claimableUnicornRewards[i] += regionRewards[i]
                .claimableUnicornRewards;
        }
        return (claimableShadowcornRewards, claimableUnicornRewards);
    }

    function twtGetBaseAndControlAmountByWaveId(
        uint16 seasonId,
        uint8 regionId,
        uint256 waveId
    )
        ...
        view
        returns (
            uint256[] memory shadowcornBaseAmount,
            uint256[] memory shadowcornControlAmount,
            uint256[] memory unicornBaseAmount,
            uint256[] memory unicornControlAmount
        )
    {
        LibTwTRewardsStorage storage ltrs = twtRewardsStorage();
        ...
        shadowcornBaseAmount = new uint256[](waveRewards.length);
        shadowcornControlAmount = new uint256[](waveRewards.length);
        unicornBaseAmount = new uint256[](waveRewards.length);
        unicornControlAmount = new uint256[](waveRewards.length);

        for (uint256 i = 0; i < waveRewards.length; ++i) {
            shadowcornBaseAmount[i] = waveRewards[i].shadowcornBaseAmount;
            shadowcornControlAmount[i] = waveRewards[i].shadowcornControlAmount;
            unicornBaseAmount[i] = waveRewards[i].unicornBaseAmount;
            unicornControlAmount[i] = waveRewards[i].unicornControlAmount;
        }
    }
...
library LibUnicornQueue {
    ...
    function enforceQueueInitialized(QueueStorage storage queue) internal view {
        require(isInitialized(queue), "UQ-002");
    }
...
library LibMinionQueue {
    ...
    function addToWaveCombatPts(
        ...
        if (queue.stakedMinionSquadToWaveId[minionSquadId] != waveId) {
            queue.accountToWaveIdToMinionSquads[user][waveId].push(
                minionSquadId
            );
        }
    ...
    function removeMinionSquadsAndPtsByWaveId(
        QueueStorage storage queue,
        uint256 waveId,
        address user
    ) internal {
    ...
    function getAccountWavePts(
        ...
        for (uint256 i = 0; i < minionSquadIds.length; ++i) {
            if (queue.stakedMinionSquadToWaveId[minionSquadIds[i]] == waveId) {
                stakingPts += queue.minionSquadToStakingPts[minionSquadIds[i]];
            }
            combatPts += queue.minionSquadToWaveIdToCombatPts[
                minionSquadIds[i]
            ][waveId];
        }
    }
    ...
    /// @param queue The queue to check for initialization.
    function enforceQueueInitialized(QueueStorage storage queue) internal view {
        require(isInitialized(queue), "MQ-001");
    }
...
import {LibGasReturner} from "LibGasReturner.sol";
import {IShadowcornStatsFacet} from "IShadowcornStatsFacet.sol";
import {ITerminusBurnMinions} from "ITerminusBurnMinions.sol";
...
library LibTwTUnicornAttack {
    ...
    function twtSanitizeMatchAfterFailedFulfillment(
        uint256 vrfRequestId,
        Match memory playedMatch
    ) internal {
        LibTwTUnicornAttackStorage storage uas = twtUnicornAttackStorage();
        uas.totalMatchesBySeasonAndRegion[playedMatch.seasonId][
            playedMatch.regionId
        ]--;
        uas.matchIdByVRFRequestId[vrfRequestId] = 0;
        for (uint8 i = 0; i < playedMatch.unicornIds.length; ++i) {
            if (playedMatch.unicornIds[i] != 0) {
                uas.unicornLastBattleTimestampByTokenId[
                    playedMatch.unicornIds[i]
                ] = 0;
            }
        }
    }

    function twtGetMatchByVRFRequestId(
        uint256 vrfRequestId
    ) internal returns (Match memory) {
        return
            twtUnicornAttackStorage().matchById[
                twtUnicornAttackStorage().matchIdByVRFRequestId[vrfRequestId]
            ];
    }
    ...
    function enforceUnicornsAreValid(
        ...
        require(!unicornsInformation[0].gameLocked, "UA-011");
        for (uint8 i = 0; i < 3; ++i) {
            if (tokenIds[i] > 0) {
                require(
                    unicornsInformation[i].lifecycleStage ==
                        uint8(UnicornDNA.LIFECYCLE_ADULT),
                    "UA-010"
                );
                require(
                    IUnicornERC721Facet(LibExternalAddress.getUnicornAddress())
                        .ownerOf(tokenIds[i]) == user,
                    "UA-009"
                );
            }
        }
    ...
    function enforceUnicornsAreNotCoolingDown(
        ...
        for (uint8 i = 0; i < 3; ++i) {
            if (tokenIds[i] != 0) {
                require(
                    (twtUnicornAttackStorage()
                        .unicornLastBattleTimestampByTokenId[tokenIds[i]] +
    ...
    function setUnicornNerfPercentageForSeason(
        ...
        require(newPercentage < 100, "UA-007");
    ...
    function twtValidateUnicornAttack(
        ...
        require(tokenIds[0] != 0, "UA-005");
        require(
            (tokenIds[0] != tokenIds[1] && tokenIds[0] != tokenIds[2]) &&
                (tokenIds[1] != tokenIds[2] || tokenIds[1] == 0),
            "UA-012"
        );
        LibTwTRewards.enforceRegionIdIsValid(regionId);
    ...
    function twtUnicornAttackRegion(
        ...
        LibGasReturner.GasReturnerTransactionType gasReturnerTransactionType = twtGetGasReturnerTransactionType(
                tokenIds
            );
        ...
        LibGasReturner.returnGasToUser(
            gasReturnerTransactionType,
            (availableGas - gasleft()),
            payable(user)
        );
    }

    function twtGetGasReturnerTransactionType(
        uint64[3] memory tokenIds
    ) private returns (LibGasReturner.GasReturnerTransactionType) {
        ...
        (, bool[] memory cachedUnicorns) = IUnicornStatCache(
            LibExternalAddress.getUnicornAddress()
        ).checkUnicornStatsCachedBatch(castedTokenIds);
        ...
        if (cachedUnicornsAmount == 3) {
            return
                LibGasReturner
                    .GasReturnerTransactionType
                    .ATTACK_WITH_3_CACHED_UNICORNS;
        }

        return
            LibGasReturner
                .GasReturnerTransactionType
                .ATTACK_WITH_NO_CACHED_UNICORNS;
    }
    ...
    function twtGetUnicornsInfoMultipleAndValidate(
        uint64[3] memory tokenIds,
        address user
    ) private returns (IUnicornStatCache.Stats[] memory unicornsInformation) {
        ...
        unicornsInformation = IUnicornStatCacheAdvanced(
            LibExternalAddress.getUnicornAddress()
        ).getAndCacheUnicornEnhancedStatsBatch(tempTokenIds);
    ...
    function simulateMatchWithMaxLuckForShadowcornTeam(
        ...
        unicornsWin =
            uint256(
                matchData.unicornTeamStamina /
                    ((matchData.shadowcornTeamDamage * 125) / 100)
            ) >
            uint256(
                matchData.shadowcornTeamStamina /
                    ((matchData.unicornTeamDamage * 75) / 100)
            );
    }
    ...
    function getTotalStatsForUnicorns(
        ...
        for (uint8 i = 0; i < unicornsInformation.length; ++i) {
            totalStats += getTotalStatsForUnicorn(
                unicornsInformation[i],
                relevantUnicornStats
            );
        }
    }

    function getTotalStatsForUnicorn(
        IUnicornStatCache.Stats memory unicornInformation,
        uint256[] memory relevantUnicornStats
    ) private pure returns (uint256 totalStats) {
        ...
        returns (Match memory, DamageCalculationExtraInfo memory extraInfo)
    {
        ...
        for (uint256 i = 0; i < unicornsInformation.length; ++i) {
            ...
            extraInfo.totalStatsPerUnicorn[i] = uint24(
                getTotalStatsForUnicorn(
                    unicornsInformation[i],
                    relevantUnicornStats
                )
            );
    ...
    {
        require(side != LibTwTRewards.FactionType.NONE, "UA-002");
    ...
    function twtGetMatchAndUnicornSquadDetails(
        uint256 vrfReqId
    ) internal view returns (Match memory matchDetail) {
        LibTwTUnicornAttackStorage storage uas = twtUnicornAttackStorage();
    ...
    function twtUnicornAttackRegionFulfillRandomness(
        ...
        Match memory matchDetail = twtGetMatchAndUnicornSquadDetails(vrfReqId);
        ...
        ITerminusBurnMinions(LibExternalAddress.getShadowcornItemsAddress())
            .twtBurnMinions(
                matchDetail.deadMinionPoolIds,
                matchDetail.deadMinionAmounts
            );
        ...
        pushMatchesByPlayerAndSide(
            matchDetail.attackerAddress,
            LibTwTRewards.FactionType.UNICORN,
            matchDetail
        );
        pushMatchesByPlayerAndSide(
            squad.sender,
        ...
        twtUnicornAttackStorage().matchById[matchDetail.matchId] = matchDetail;
    ...
    function resolveShadowcornTeamWinnerCase(
        ...
        matchDetail.dominationPointsShadowcorns = uint56(
            ((uint256(shadowcornsDamageDealt) * uint256(winnerMultiplier)) /
                10000)
        );

        (matchDetail) = regroupNewSquad(
            squad,
            matchDetail,
            unicornsDamageDealt
        );
        ...
        if (
            ...
            twtUnicornAttackStorage()
                .deadUnicornsByPlayer[matchDetail.attackerAddress]
                .push(matchDetail.unicornIds[0]);
        } else {
            returnLeaderToUser(
                matchDetail.attackerAddress,
                matchDetail.unicornIds[0]
    ...
    function resolveUnicornTeamWinnerCase(
        ...
        matchDetail.dominationPointsShadowcorns = uint56(
            ((uint256(shadowcornsDamageDealt) * uint256(loserMultiplier)) /
                10000)
        );
    ...
    {
        uint24 squadMinionQuantity = 0;
        for (uint256 i = 0; i < squadPoolIds.length; ++i) {
            squadMinionQuantity += squadMinionAmounts[i];
        ...
        minionsToKill =
            unicornsDamageDealt /
            (shadowcornTeamStamina / squadMinionQuantity);
        ...
        if (minionsToKill >= squadMinionQuantity) {
            minionsToKill = squadMinionQuantity - 1;
        }
    }
    ...
    function regroupNewSquad(
        ...
        uint40[5] memory priorUserDeadMinions = getDeadMinionsByPools(
            squad.poolIds,
            squad.sender
        );

        (newSquadInformation) = killMinionsAndUpdateNewSquadInformation(
            newSquadInformation,
            matchDetail,
            squad.sender,
            minionsToKill
        ...
        uint40[5] memory postUserDeadMinions = getDeadMinionsByPools(
            squad.poolIds,
            squad.sender
        );
        (
        ...
        NewSquadOverseerData
            memory newSquadOverseerData = getMinionStatsAndOverseerData(
                newSquadInformation.newSquadPoolIds,
                squad.shadowcornOverseerTokenId
            );

        NewSquadTotals memory newSquadTotals;
        if (nonZeroMinionAmounts > 5) {
            nonZeroMinionAmounts = 5;
        }
        ...
        newSquadTotals.totalStats += LibTwTShadowcornDefense
            .getShadowcornOverseerTotalStats(
                uint24(squad.shadowcornOverseerTokenId)
            );
        //convert minion stats to unicorn stats
        newSquadTotals.totalStats = newSquadTotals.totalStats * 8;
        ...
        sds.squadById[sds.lastSquadId] = LibTwTShadowcornDefense
            .ShadowcornSquad({
                squadId: sds.lastSquadId,
        ...
        LibTwTMinions.twtSetPartiallyKilledMinionSquadUnlockableWaveId(
            sds.lastSquadId,
            LibTwTMinions.twtGetMinionSquadUnlockableWaveId(squad.squadId)
        );
        ...
        )
    {
        ...
        uint24[5] memory orderedAndUpdatedSquadMinionAmounts;
        uint24[5] memory orderedAndUpdatedSquadPoolIds;
        nonZeroMinionAmounts = 1;
        for (uint256 i = 0; i < priorUserDeadMinions.length; ++i) {
            newDeadMinionsDifference = uint24(
                postUserDeadMinions[i] - priorUserDeadMinions[i]
            );
            matchDetail.deadMinionAmounts[i] = newDeadMinionsDifference;
            if (newDeadMinionsDifference > 0) {
                matchDetail.deadMinionPoolIds[i] = squadPoolIds[i];
            }
            newSquadMinionAmount =
                squadMinionAmounts[i] -
                newDeadMinionsDifference;
            if (newSquadMinionAmount > 0) {
                orderedAndUpdatedSquadPoolIds[
                    nonZeroMinionAmounts - 1
                ] = squadPoolIds[i];
                orderedAndUpdatedSquadMinionAmounts[
                    nonZeroMinionAmounts - 1
                ] = newSquadMinionAmount;
                nonZeroMinionAmounts++;
        ...
        newSquadInformation.newSquadPoolIds = orderedAndUpdatedSquadPoolIds;
        newSquadInformation
            .newSquadMinionAmounts = orderedAndUpdatedSquadMinionAmounts;
        newSquadInformation.newSquadSize = newSquadSize;
        return (newSquadInformation, matchDetail, nonZeroMinionAmounts);
    }

    function getDeadMinionsByPools(
        uint24[5] memory poolIds,
        address player
    ) internal view returns (uint40[5] memory deadMinions) {
        for (uint256 i = 0; i < poolIds.length; ++i) {
            LibTwTUnicornAttackStorage storage uas = twtUnicornAttackStorage();
            deadMinions[i] = uas.deadMinionsByPlayerAndPoolId[player][
                poolIds[i]
            ];
        }
    }
    ...
    function killMinionsAndUpdateNewSquadInformation(
        ...
        address squadSender,
        uint24 minionsToKill
    ) internal returns (NewSquadInformation memory) {
        uint256 indexToKill;
        uint256 i = 0;
        while (minionsToKill > 0) {
            indexToKill = LibRNG.expand(5, matchDetail.randomness, i);
            if (newSquadInformation.newSquadMinionAmounts[indexToKill] != 0) {
                newSquadInformation.newSquadMinionAmounts[indexToKill]--;
                twtUnicornAttackStorage().deadMinionsByPlayerAndPoolId[
                    squadSender
                ][newSquadInformation.newSquadPoolIds[indexToKill]]++;
                minionsToKill--;
            }
            ++i;
        }
    ...
    function getMinionStatsAndOverseerData(
        uint24[5] memory newSquadPoolIds,
        uint256 shadowcornOverseerTokenId
    ) internal view returns (NewSquadOverseerData memory overseerData) {
        ...
        uint256[] memory castedNewSquadPoolIds = new uint256[](
            newSquadPoolIds.length
        );

        for (uint256 j = 0; j < castedNewSquadPoolIds.length; ++j) {
            castedNewSquadPoolIds[j] = uint256(newSquadPoolIds[j]);
        }

        (overseerData.minionStats, overseerData.minionClasses) = IMinionStats(
            LibExternalAddress.getShadowcornItemsAddress()
        ...
        )
    {
        ...
        while (!combat.ended) {
            ...
            combat.shadowcornTeamRoundDamage = uint24(
            ...
            combat.unicornTeamRoundDamage = uint24(
    ...
    function twtGetBattlePreview(
        ...
        IUnicornStatCache.Stats[]
            memory unicornsInformation = IUnicornStatCacheAdvanced(
                LibExternalAddress.getUnicornAddress()
            ).getUnicornEnhancedStatsBatch(castedUnicornTokenIds);
        ...
        uint24 totalUnicornsStats = uint24(
            getTotalStatsForUnicorns(unicornsInformation, relevantUnicornStats)
        );
    ...
    function pushMatchesByPlayerAndSide(
        address sender,
        LibTwTRewards.FactionType faction,
        Match memory matchDetail
    ) private {
        twtUnicornAttackStorage().matchesByPlayerAndSide[sender][faction].push(
            matchDetail
        );
    }
}
...
library LibTwTUnicorn {
    ...
    function twtGetRerollCosts(
        uint16 seasonId
    ) internal view returns (uint256 rbwCost) {
    ...
    function twtRescueUnicorn(uint256[] memory vrfRequestIds) internal {
        for (uint256 i = 0; i < vrfRequestIds.length; ++i) {
            ...
            LibTwTUnicornAttack.Match memory playedMatch = LibTwTUnicornAttack
                .twtGetMatchByVRFRequestId(vrfRequestId);
            ...
            address currentOwner = IERC721(
                LibExternalAddress.getUnicornAddress()
            ).ownerOf(playedMatch.unicornIds[0]);
            ...
            // Readd squad to the list of attackable squads and user squads. Also recalculate the unlockable wave id
            twtReaddSquadAfterFailedFulfillment(playedMatch);

            // Sanitize match
            LibTwTUnicornAttack.twtSanitizeMatchAfterFailedFulfillment(
                vrfRequestId,
                playedMatch
            );
    ...
    function twtReaddSquadAfterFailedFulfillment(
        LibTwTUnicornAttack.Match memory playedMatch
    ) private {
        LibTwTShadowcornDefense.ShadowcornSquad
            memory minionSquad = LibTwTShadowcornDefense
                .twtGetShadowcornSquadById(playedMatch.shadowcornSquadId);

        LibTwTMinions.twtAddMinionSquadToArray(
            playedMatch.seasonId,
            playedMatch.regionId,
            playedMatch.shadowcornSquadId
        );
        ...
        returns (LibTwTShadowcornDefense.ShadowcornSquad[3] memory squads)
    {
        ...
        uint256 stableMinionSizeIncrements = LibTwTAdmin
            .twtGetStableMinionSquadSizeIncrements();
        if (length > stableMinionSizeIncrements) {
            length =
                (length / stableMinionSizeIncrements) *
                stableMinionSizeIncrements;
        }
        squads = twtGetAttackableSquads(
            length,
            seasonId,
            regionId,
            player,
    ...
    function twtGetAttackableSquads(
        uint256 length,
        uint16 seasonId,
        uint8 regionId,
        address player,
        ...
        returns (LibTwTShadowcornDefense.ShadowcornSquad[3] memory squads)
    {
        ...
        LibTwTShadowcornDefense.LibTwTShadowcornDefenseStorage
            storage sds = LibTwTShadowcornDefense.twtShadowcornDefenseStorage();
        for (uint8 i = 0; i < 3; ++i) {
            while (squadIndexes[i] == type(uint256).max) {
                squadIndex = LibRNG.expand(length, playerSeed, salt);
                if (
                    squadIndexes[0] != squadIndex &&
                    squadIndexes[1] != squadIndex &&
                    squadIndexes[2] != squadIndex
                ) {
                    squadId = LibTwTMinions.twtGetMinionSquadIdsArray(
                        seasonId,
                        regionId
                    )[squadIndex];
                    LibTwTShadowcornDefense.ShadowcornSquad memory squad = sds
                        .squadById[squadId];
    ...
    function twtRerollShadowcornOpponentsFulfillRandomness(
        ...
        emit LibEvents.TwTRNGSeedChanged(
            player,
            LibRNG.getPlayerSeed(player),
            randomness
        );
    ...
    function twtCanCombatMinionSquad(
        ...
        LibTwTShadowcornDefense.ShadowcornSquad[3]
            memory squads = twtGetAttackableSquads(
                length,
                seasonId,
                regionId,
                player,
...
library LibRNG {
    ...
    bytes32 private constant RNGVRF_STORAGE_POSITION =
        keccak256("diamond.DarkForest.LibRNGVRFV2.storage");
    ...
    function vrfV2Storage()
        internal
        pure
        returns (LibRNGVRFV2Storage storage vrf)
    {
    ...
    function requestRandomWordsFor(
        ...
        uint32 callbackGasLimit = vrfs.callbackGasLimitForMechanicId[
            mechanicId
        ];
        uint16 requestConfirmations = vrfs.confirmationsForMechanicId[
            mechanicId
        ];
        uint256 requestId = VRFCoordinatorV2Interface(vrfs.vrfCoordinator)
            .requestRandomWords(
                vrfs.keyHash,
                vrfs.subscriptionId,
    ...
    function getMechanicIdByVRFRequestId(
        uint256 requestId
    ) internal view returns (uint256) {
...
library LibTwTMinions {
    ...
    function twtRemoveMinionSquadFromArray(
        ...
        uint40 lastMinionSquadId = ltms.minionSquadIdsArray[seasonId][regionId][
...
library LibTwTAdmin {
    ...
    function twtGetDominationPointsWinnerAndLoserMultiplierBySeason(
        uint16 seasonId
    ) internal view returns (uint32 winnerMultiplier, uint32 loserMultiplier) {
    ...
    function twtSetAdmin(address wallet) internal {
        ...
        require(adminIndex > lts.adminWallets.length, "ADM-001");
    ...
    function twtUnsetAdmin(address wallet) internal {
        LibTwTAdminStorage storage lts = twtAdminStorage();
        require(lts.adminWallets.length > 0, "ADM-002");
        uint256 adminIndex = twtGetAdminIndex(wallet);
        require(adminIndex < lts.adminWallets.length, "ADM-003");
    ...
    function twtAddMinionPool(uint24 poolId) internal {
        ...
        LibTwTAdminStorage storage lts = twtAdminStorage();
        uint24[] memory oldPools = lts.minionPoolIds;
        require(minionPoolIndex > oldPools.length, "ADM-005");
    ...
    function enforcePoolIdIsMinionPoolId(uint24 poolId) internal view {
        require(twtAdminStorage().poolIdIsMinionPoolId[poolId], "ADM-006");
    }
...
interface IUnicornStatCache {
    ...
    function getUnicornFullStats(
        ...
        external
        view
        returns (Stats memory naturalStats, Stats memory enhancedStats);
}
...
interface IUnicornStatCacheAdvanced is IUnicornStatCache {
    ...
    function cacheUnicornNaturalStats(
        uint256 tokenId
    ) external returns (Stats memory naturalStats);
...
// Chain: POLYGON - File: ITerminusBurnMinions.sol
//SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.20;

interface ITerminusBurnMinions {
    function twtBurnMinions(
        uint24[5] memory poolIds,
        uint24[5] memory amounts
    ) external;
}
