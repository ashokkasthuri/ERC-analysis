...
// Chain: POLYGON - File: contracts/SvobodaShockNFT.sol
// SPDX-License-Identifier: MIT
...
contract SvobodaShockNFT is ERC1155, Pausable, Ownable {
    using ECDSA for bytes32;
    using Strings for uint256;

    constructor() ERC1155("https://nft.eddie.ws/metadata/{id}.json") {}

    function redeemAsset(uint256 _assetId, bytes memory _signature) external {
        require(
            _verifySignedMessage(_assetId.toString(), _signature),
            "This asset is not redeemable by you."
        );
        _mintAsset(msg.sender, _assetId);
    }

    function mintAsset(address _receiver, uint256 _assetId) external {
        _mintAsset(_receiver, _assetId);
    }
    ...
    function _mintAsset(address _receiver, uint256 _assetId) internal {
        _mint(_receiver, _assetId, 1, "0x0");
    }
    ...
    function _verifySignedMessage(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                bytes(_message).length.toString(),
                _message
            )
        );
        return hash.recover(_signature) == owner();
    }
}