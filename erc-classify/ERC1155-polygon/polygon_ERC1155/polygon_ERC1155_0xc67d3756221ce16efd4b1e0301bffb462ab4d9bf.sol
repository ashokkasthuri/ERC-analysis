// Chain: POLYGON - File: /contracts/RC.sol
...
contract RC is Ownable {
    ...
    address public constant QUOTE_TOKEN_ADDRESS =
        0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;// USD Coin MATIC
    uint256 public constant BASE_POINT = 10000;
    Penalty public currentPaymentPenalty =
        Penalty({
            feeAmountBP: 10000, // +100%
            erc20AmountBP: 2000 // 20%
        });

    address public feeAddress;
    address public paymentAdmin;
    mapping(address => Insurance) public insurances;
    mapping(address => mapping(address => bool)) public validators;
    mapping(address => address[]) public backupWalletOwners;
    mapping(address => Propose) public proposals;
    Subscription[] public subscriptions;

    modifier correctID(uint256 _sid) {
        require(_sid < subscriptions.length, "bad sid");
        _;
    }

    modifier onlyValidator(address insuranceOwner) {
        require(
            validators[insuranceOwner][msg.sender] == true,
            "only validators"
        );
        _;
    }
    ...
    event UpgradeInsurancePlan(address user, uint256 newSId);
    event BillPayment(
        address payer,
    ...
    constructor(address _feeAddress, address _paymentAdmin) {
    ...
    function setPaymentPenalty(
        Penalty calldata _currentPaymentPenalty
    ) external onlyOwner {
        require(
            _currentPaymentPenalty.erc20AmountBP < BASE_POINT,
            "bad erc20AmountBP"
        );
        currentPaymentPenalty = _currentPaymentPenalty;
    }
    ...
    function addSubscription(
        ...
        IPair pair = IPair(_subscription.busdTokenPair);
        address token0 = pair.token0();
        address token1 = pair.token1();
        require(
            (token0 == QUOTE_TOKEN_ADDRESS || token1 == QUOTE_TOKEN_ADDRESS) &&
                (token0 == _subscription.paymentToken ||
                    token1 == _subscription.paymentToken),
            "bad busdTokenPair address"
        );
    ...
    function removeSubscription(
        uint256 _sId
    ) external onlyOwner correctID(_sId) {
        subscriptions[_sId] = subscriptions[subscriptions.length - 1];
        subscriptions.pop();
    }
    ...
    function getPriceInPaymentToken(
        ...
        if (_priceInBusd > 0 && _pair != address(0)) {
            ...
            if (reserveBase > 0) {
                return (_priceInBusd * reserveQuote) / reserveBase + 1;
            } else {
                return 1;
            }
        } else {
    ...
    function upValidators(address _validator, bool status) external {
        Insurance memory userInsurance = insurances[msg.sender];
        require(
            userInsurance.backupWallet != address(0),
            "insurance not exist"
        );
        validators[msg.sender][_validator] = status;
    }

    function setAutopayment(bool _autopayment) external {
        Insurance storage userInsurance = insurances[msg.sender];
        require(
            userInsurance.backupWallet != address(0),
            "insurance not exist"
        );
        userInsurance.autopayment = _autopayment;
    }
    ...
    function createInsurance(
        ...
        uint256 _sId,
        bool autopayment
    ) external correctID(_sId) {
        Insurance memory userInsurance = insurances[msg.sender];
        Subscription memory paymentPlan = subscriptions[_sId];
        require(
            userInsurance.backupWallet == address(0) ||
                (userInsurance.plan.priceInBusd == 0 &&
                    paymentPlan.priceInBusd > 0),
            "already created"
        );
        require(
        ...
        backupWalletOwners[_backupWallet].push(msg.sender);

        for (uint256 i = 0; i < _validators.length; i++) {
            validators[msg.sender][_validators[i]] = true;
        }

        userInsurance.autopayment = autopayment;
        userInsurance.voteQuorum = _voteQuorum;
        userInsurance.backupWallet = _backupWallet;
        userInsurance.plan = paymentPlan;
        userInsurance.expirationTime =
            block.timestamp +
            paymentPlan.validityPeriod;
        userInsurance.paymentPenalty = currentPaymentPenalty;

        uint256 priceInPaymentToken = getPriceInPaymentToken(
            paymentPlan.busdTokenPair,
            paymentPlan.priceInBusd
        );
    ...
    function upgradeInsurancePlan(uint256 _sId) external correctID(_sId) {
        Insurance memory userInsurance = insurances[msg.sender];
        require(
            userInsurance.backupWallet != address(0),
            "insurance not exist"
        ...
        require(
            userInsurance.expirationTime > block.timestamp,
            "current insurance expired,payment require"
        );

        userInsurance.plan = subscriptions[_sId];
        require(
            userInsurance.plan.priceInBusd > 0,
        ...
        userInsurance.expirationTime =
            block.timestamp +
            userInsurance.plan.validityPeriod;

        uint256 priceInPaymentToken = getPriceInPaymentToken(
            userInsurance.plan.busdTokenPair,
            userInsurance.plan.priceInBusd
        );

        if (priceInPaymentToken > 0) {
            IERC20(userInsurance.plan.paymentToken).safeTransferFrom(
                msg.sender,
                feeAddress,
                priceInPaymentToken
            );
        ...
        emit UpgradeInsurancePlan(msg.sender, _sId);
    }
    ...
    function autoPayment(address insuranceOwner) external {
        ...
        require(userInsurance.expirationTime < block.timestamp, "too early");
    ...
    function _billPayment(
        ...
        uint256 paymentDebtInBusd;
        uint256 debtPeriods;
        if (userInsurance.expirationTime < block.timestamp) {
            debtPeriods =
                (block.timestamp - userInsurance.expirationTime) /
                userInsurance.plan.validityPeriod;
            paymentDebtInBusd = debtPeriods * userInsurance.plan.priceInBusd;
        }
        ...
        uint256 amountInPaymentToken = getPriceInPaymentToken(
            userInsurance.plan.busdTokenPair,
            userInsurance.plan.priceInBusd + paymentDebtInBusd
        );

        if (penalty) {
            amountInPaymentToken +=
                (amountInPaymentToken *
                    userInsurance.paymentPenalty.feeAmountBP) /
                BASE_POINT;
        }
    ...
    function insuranceEvent(
        ...
        if (userInsurance.plan.recoveryTokenLimit > 0) {
            ...
            require(
                userInsurance.rcCount <= userInsurance.plan.recoveryTokenLimit,
        ...
        for (uint256 i = 0; i < erc20Tokens.length; i++) {
            ...
            if (balance > 0) {
                erc20PenaltyAmount = penalty
                    ? (balance * userInsurance.paymentPenalty.erc20AmountBP) /
                        BASE_POINT
                    : 0;
    ...
    function proposeChangeBackupWallet(
        address insuranceOwner,
        address newBackupWallet
    ) external onlyValidator(insuranceOwner) {
        require(
    ...
    function confirmProposal(
        ...
        for (uint256 i = 0; i < proposal.voters.length; i++) {
            if (msg.sender == proposal.voters[i]) {
                revert("already voted");
            }
        }
        proposal.voters.push(msg.sender);

        emit Vote(insuranceOwner, proposal.newBackupWallet, msg.sender);

        if (proposal.voters.length >= userInsurance.voteQuorum) {
