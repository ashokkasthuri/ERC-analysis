...
// Chain: POLYGON - File: contracts/MySoccerTeamNFT.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
contract MySoccerTeamNFT is ERC1155, ERC1155Burnable, Ownable, ERC2981 {

    string public name = "My Crypto Soccer Team NFT";
    string public symbol = "MCST";

    constructor(address initialOwner, address _receiver) ERC1155("") Ownable(initialOwner) {
    ...
    mapping(uint256 => MetadataInfo) metadata;

    function mint(
        address _account,
        uint256 _id,
        uint256 _amount,
        string memory _uri_base,
        bytes memory _data
    ) public onlyOwner {
        metadata[_id] = MetadataInfo(_uri_base);
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            metadata[ids[i]] = MetadataInfo(_uri_base);
        }
        _mintBatch(to, ids, amounts, data);
    }

    function setURI(uint256 _tokenId, string memory _uri_base) public onlyOwner {
        metadata[_tokenId] = MetadataInfo(_uri_base);
    }
