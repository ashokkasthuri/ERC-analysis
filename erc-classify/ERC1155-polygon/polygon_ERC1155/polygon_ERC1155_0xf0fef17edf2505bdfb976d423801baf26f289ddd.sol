...
// Chain: POLYGON - File: contracts/aCO2Token.sol
...
contract aCO2Token is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    // Event to track burned tokens
    event BurnedTokens(address indexed nftreeAddress, uint256 indexed nftreeId, uint256 amount, address indexed owner, uint256 timestamp);
    ...
    function getBatchIndex(address batchAddress) internal view returns (uint256) {
    ...
    // Function to burn multiple aCO2 tokens and issue corresponding carbon certificates
    function _batchBurn(address owner, address[] memory nftreeAddress, uint256[] memory tokenIds, uint256[] memory amounts) public {
        require(tokenIds.length == amounts.length, "Token IDs and amounts length mismatch");
        require(nftreeAddress.length == tokenIds.length, "Addresses and IDs length mismatch");

        // Arrays to accumulate token IDs and amounts for batch burning
        uint256[] memory idsToBurn = new uint256[](tokenIds.length);
        uint256[] memory amountsToBurn = new uint256[](amounts.length);
        
        // Accumulate tokens and verify balances before burning
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(owner == IERC721(nftreeAddress[i]).ownerOf(tokenIds[i]), "Not authorized to burn");
            uint256 tokenId = getTokenId(nftreeAddress[i], tokenIds[i]);
            idsToBurn[i] = tokenId;
            require(balanceOf(owner, tokenId) >= amounts[i], "Insufficient token balance to burn");
            amountsToBurn[i] = amounts[i];
        }

        // Perform the batch burn
        _burnBatch(owner, idsToBurn, amountsToBurn);

        // Emit event for each token burned
        for (uint256 i = 0; i < tokenIds.length; i++) {
            emit BurnedTokens(nftreeAddress[i], tokenIds[i], amounts[i], owner, block.timestamp);
        }
    }

    function batchBurn(address[] memory nftreeAddress, uint256[] memory tokenIds, uint256[] memory amounts) public {
        _batchBurn(msg.sender, nftreeAddress, tokenIds, amounts);
    }

    function batchBurnDelegate(address owner, address[] memory nftreeAddress, uint256[] memory tokenIds, uint256[] memory amounts) public {
        require(isApprovedForAll(owner, msg.sender), "Not approved to burn");
        _batchBurn(owner, nftreeAddress, tokenIds, amounts);
    }
    ...
    function claimRegularMerkleReward(uint256[] calldata nftreeIds, uint256[] calldata amounts, address[] calldata nftreeAddresses, bytes32[][] calldata merkleProofs) public {
        ...
        for (uint256 i = 0; i < nftreeIds.length; i++) {
            ...
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender, nftreeAddresses[i], nftreeIds[i], amounts[i]));
            ...
            emit ClaimedNormalMerkle(msg.sender, nftreeAddresses[i], nftreeIds[i], amounts[i]);
        }

        _mintBatch(msg.sender, aco2TokenIds, mintAmounts, "");
    }
    ...
    function getPendingTokenAmount(address nftreeAddress, uint256 nftreeId) public view returns (uint256) {
        uint256 lastClaimed = lastClaimTime[nftreeAddress][nftreeId] == 0 
            ? startTime 
            : lastClaimTime[nftreeAddress][nftreeId];
        uint256 currentTime = block.timestamp;
        uint256 earnRate = tokenEarnRates[nftreeAddress] > 0 
            ? tokenEarnRates[nftreeAddress] 
            : GLOBAL_TOKENS_PER_DAY;
        uint256 tokenAmount = ((currentTime - lastClaimed) * earnRate) / 86400;
        return tokenAmount;
    }
