...
// Chain: POLYGON - File: contracts/kcashMarketplaceUpgradble.sol
// SPDX-License-Identifier: MIT
...
/**
 * @title KCashMarketplace
 * @dev This contract implements the Kcash Marketplace by inheriting ERC1155.
 */
contract KCashMarketplaceUpgradble is ERC1155Upgradeable, Ownable2StepUpgradeable {
    ...
    // The Kcash token
    IERC20Upgradeable public kcashToken;
    ...
    function startContract(
        address _kcashAddress,
        string memory _baseURI
    ) external initializer {
        __Ownable2Step_init();
        __ERC1155_init(_baseURI);
        kcashToken = IERC20Upgradeable(_kcashAddress);
    }
     ...
     * @param _tokenId The ID of the token representing the item.
     * @param _name The name of the item.
     * @param _productId The ID of the product associated with the item.
     * @param _productDetails The details of the product associated with the item.
     * Requirements:
     * - Only the contract owner can call this function.
     * - The item with the given tokenId must not already exist.
     *  Emits an {ItemAdded} event indicating the details of the item.
     */
    function addItem(
        uint256 _tokenId,
    ...
    /**
     * @dev Edit an existing item in the marketplace.
     ...
     * Emits an {ItemEdited} event indicating the updated details of the item.
     */
    function editItem(
        uint256 _tokenId,
        string calldata _name,
    ...
    /**
     * @dev Withdraws all the Kcash tokens from the contract and transfers them to the owner's address.
     */
    function withdraw() external onlyOwner {
        kcashToken.transfer(owner(), kcashToken.balanceOf(address(this)));
    }

    /**
     * @dev Withdraws the native currency from the contract and transfers it to the owner's address.
     * Only the owner of the contract can call this function.
     */
    ...
    /**
     * @dev Sets the address of the Kcash token contract.
     * @param _kcashAddress The address of the Kcash token contract.
     * Requirements:
     * - Only the owner of the contract can call this function.
     */
    function setKcashAddress(address _kcashAddress) external onlyOwner {
        kcashToken = IERC20Upgradeable(_kcashAddress);
    }
    ...
    // override to make sure only owner can call
    function _beforeTokenTransfer(
        address operator,
        address from,
        address to,
