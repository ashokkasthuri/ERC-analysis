...
contract MRASSETS is ERC1155, ERC2981, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    constructor(address _minter,address owner,string memory name_, string memory symbol_) ERC1155("ipfs://CID/{id}.json") {
    ...
    function setURI(string memory newuri,uint256 _tokenID) public onlyOwner {
        _tokenURIs[_tokenID] = newuri;
    }

    function createCollection(uint256 maxSupply, string memory collection_uri,address _royaltyReciever,uint96 _feeNumerator) public onlyOwner returns (uint256) {
        supplies[_currentTokenID] = maxSupply;
        _tokenURIs[_currentTokenID] = collection_uri;
        _setTokenRoyalty(_currentTokenID, _royaltyReciever, _feeNumerator);
        _currentTokenID++;
        return _currentTokenID - 1;
    }

    function mint(address wallet, uint256 id, uint256 amount)
        public
        onlyMinter
    {   
  ...
  //      require(_tokenSupply[tokenID] > 0, "ERC1155: invalid token ID");
