...
// percentages of wallets
    uint256 marketting = 9;
    uint256 landAssestLease = 25;
    uint256 adminstration = 10;
    uint256 remuneration = 5;
    uint256 finanaceLegal = 1;
// addresses 
    ...
    modifier onlyfarmOperatorWallet() {
        require(
            msg.sender == farmOperatorWallet,
            "farmOperatorWallet can be called"
        );
        _;
    }
    ...
    function setPaymentAddress(
        address _paymentToken
    ) public onlyfarmOperatorWallet {
        paymentToken = _paymentToken;
    }

    // terminate the sale by putting the initialCounter
    function terminateSale() public onlyfarmOperatorWallet {
        initialized = false;
        emit TERMINATESALE();
    }

    // update the price of NFT by putting initailCounter
    function updatePrice(uint256 newPrice) public onlyfarmOperatorWallet {
    ...
    function markettingReward(uint256 noOfcopies, uint256 percent) internal {
        ...
        distribute(_price.sub(referralReward));
    }
    ...
    function withdraw(address _token, address to) public onlyfarmOperatorWallet {
    ...
    function upateName(
        string memory _name
    ) public onlyfarmOperatorWallet returns (string memory) {
        name = _name;
        return _name;
    }
    ...
    function distribute(uint256 amount) internal {
        uint256 _marketting = (amount.mul(marketting)).div(100);
        IERC20(paymentToken).transfer(markettingWallet, amount.sub(_marketting));

        uint256 _landAssestLease = (amount.mul(landAssestLease)).div(100);
        IERC20(paymentToken).transfer(landAssestLeaseWallet, amount.sub(_landAssestLease));

        uint256 _adminstration = (amount.mul(adminstration)).div(100);
        IERC20(paymentToken).transfer(administrationWallet, amount.sub(_adminstration));

        uint256 _remuneration = (amount.mul(remuneration)).div(100);
        IERC20(paymentToken).transfer(remunerationWallet, amount.sub(_remuneration));

        uint256 _finanaceLegal =  (amount.mul(finanaceLegal)).div(100);
        IERC20(paymentToken).transfer(finanaceLegalWallet, amount.sub(_finanaceLegal));

        uint256 _farmOperator = _marketting.add(_landAssestLease).add(_adminstration).add(_remuneration).add(_finanaceLegal);
        IERC20(paymentToken).transfer(farmOperatorWallet, amount.sub(_farmOperator));
    }
}