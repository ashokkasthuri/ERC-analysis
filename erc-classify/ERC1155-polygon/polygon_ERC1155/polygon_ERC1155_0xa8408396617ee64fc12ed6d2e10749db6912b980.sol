...
contract CricketCup is
    OwnableUpgradeable,
    ERC1155SupplyUpgradeable,
    ERC1155URIStorageUpgradeable,
    UUPSUpgradeable
{
    uint256 public constant LOWER_BOUND = 1;
    uint256 public constant UPPER_BOUND = 11;
    ...
    modifier idRange(uint256 id) {
        require(id >= LOWER_BOUND && id <= UPPER_BOUND, "Token id out of range");
        _;
    }
    ...
    modifier onlyServer() {
        require(
            serverAccounts[msg.sender],
            "Only server can operate"
        );
        _;
    }    
    ...
    function setURI(uint256 _tokenId, string memory _tokenURI) external onlyAdminOrOwner {
    ...
    function setServers(address[] calldata _servers, bool knob) external onlyAdminOrOwner {
    ...
    function mintBatch(
        ...
        uint256[] memory ids,
        uint256[] memory amounts
    ) public onlyServer idsRange(ids) {
    ...
    function _beforeTokenTransfer(
        ...
        if (isTransferable) {
            ...
            revert("NFT is not transferable");
        }
    }
