...
// Chain: POLYGON - File: @openzeppelin/contracts/access/IAccessControlEnumerable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/eip/ERC165.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/interfaces/IWETH.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/openzeppelin-presets/token/ERC20/utils/SafeERC20.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/openzeppelin-presets/utils/cryptography/ECDSA.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/openzeppelin-presets/utils/cryptography/EIP712.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/openzeppelin-presets/utils/ERC1155/ERC1155Holder.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/openzeppelin-presets/utils/ERC1155/ERC1155Receiver.sol
// SPDX-License-Identifier: Apache 2.0
...
// Chain: POLYGON - File: contracts/EventTicketsNFT.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
contract EventTicketsNFT is ERC1155Short, SignatureMintERC1155, EventOrgAccess, ERC1155Holder {
    ...
    mapping(uint256 => mapping(string => uint256)) public balanceOfByEmail;
    ...
    function _paginationData(uint arrLength, uint offset, uint limit) private view returns (uint, uint) {
        uint256 length = limit;
        if (arrLength < offset) {
            return (arrLength, 0);
        }
        if (length > arrLength - offset) {
            length = arrLength - offset;
        }
        return (offset, length);
    }

    function getReceipts(uint _offset, uint _limit) external view onlyOwner returns (Receipt[] memory, uint) {
        (uint offset, uint length) = _paginationData(_receipts.length, _offset, _limit);
        Receipt[] memory result = new Receipt[](length);
        for (uint256 i = 0; i < length; i++) {
            result[i] = _receipts[offset + i];
        }
        return (result, offset + length);
    }

    function getEmailsByTokenId(uint256 _tokenId, uint _offset, uint _limit) external view onlyOwner returns (string[] memory, uint) {
        (uint offset, uint length) = _paginationData(emailsByTokenId[_tokenId].length, _offset, _limit);
        string[] memory result = new string[](length);
        for (uint256 i = 0; i < length; i++) {
            result[i] = emailsByTokenId[_tokenId][offset + i];
        }
        return (result, offset + length);
    }
    ...
    function getOazizCommissionByPrice(uint256 totalPrice) public view returns (uint256) {
    ...
    function cancelTicketByOrg(CancelTicket[] memory cancelTickets) external onlyAdminOrContributorOrgMembers {
        for (uint256 i = 0; i < cancelTickets.length; i++) {
            ...
            if (bytes(cancelTicket.email).length > 0 && cancelTicket.owner == address(0)) {
                require(cancelTicket.quantity <= balanceOfByEmail[cancelTicket.tokenId][cancelTicket.email]);
                _burn(address(this), cancelTicket.tokenId, cancelTicket.quantity);
                balanceOfByEmail[cancelTicket.tokenId][cancelTicket.email] -= cancelTicket.quantity;
            } else if (bytes(cancelTicket.email).length == 0 && cancelTicket.owner != address(0)) {
                _burn(cancelTicket.owner, cancelTicket.tokenId, cancelTicket.quantity);
            } else {
                revert("Provide both data for delete. Email and address");
            }
    ...
    function _mintProcess(MintRequest calldata _req, address signerOrMinter) internal {
        ...
        if (_req.to == address(0)) {
            ...
            balanceOfByEmail[tokenIdToMint][_req.email] += _req.quantity;
        ...
        _receipts.push(Receipt(receiver, _req.email, tokenIdToMint, _req.quantity, block.timestamp));
    ...
    function transferFromEmail(string memory email, address _to, uint tokenId) external {
        ...
        uint amount = balanceOfByEmail[tokenId][email];
        _safeTransferFrom(address(this), _to, tokenId, amount, "");
        balanceOfByEmail[tokenId][email] -= amount;
    }
...
// Chain: POLYGON - File: contracts/extension/ERC1155Short.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
contract ERC1155Short is ERC1155, Ownable, Royalty, Multicall, ContractMetadata {
...
// Chain: POLYGON - File: contracts/extension/EventOrgAccess.sol
// SPDX-License-Identifier: MIT
...
abstract contract EventOrgAccess is Ownable {
    ...
    mapping(string => mapping(address => bool)) private isAccessGrantedByProductForAddress;
    ...
    function grandAccessForUsers(address[] memory _addresses, string memory externalProductId) public {
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            isAccessGrantedByProductForAddress[externalProductId][_addresses[i]] = true;
        }
    }
    ...
    function removeAccessForUsers(address[] memory _addresses, string memory externalProductId) public {
        ...
        require(existProductExternalId[externalProductId], "Product should be exist");
        for (uint i = 0; i < _addresses.length; i++) {
            if (isAccessGrantedByProductForAddress[externalProductId][_addresses[i]]) {
                isAccessGrantedByProductForAddress[externalProductId][_addresses[i]] = false;
            }
        }
    }
    ...
    function isAddressHasAccessToTicket(address _address, string memory externalProductId) public view returns (bool) {
        ...
        return isAccessGrantedByProductForAddress[externalProductId][_address];
    }
...
// Chain: POLYGON - File: contracts/extension/interfaces/ISignatureMintERC1155.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: contracts/extension/SignatureMintERC1155.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: contracts/interfaces/IOazizOrganization.sol
// SPDX-License-Identifier: MIT
