...
// Chain: POLYGON - File: contracts/MGGTradingCard01.sol
// SPDX-License-Identifier: MIT
    ...
    ERC1155
{
    ...
    constructor() ERC1155("") {
        ...
        setBaseURI("https://arweave.net/rf1Z79d5M_THNl9nECSlQwqIyLVzOttwAVXtaTA_I5Y/json/");
        ...
        whenNotPaused
    {
        ...
        uint256[] memory mintAmountEachTokens = new uint256[](12);
        uint256 countIds;

        for (uint256 i=0; i<_mintAmount; i++) {
            uint256 time = block.timestamp;
            bytes32 randomValue = keccak256(abi.encodePacked(msg.sender, "-", time, "-", i));
            uint256 tokenSeed = uint256(randomValue) % 12 + 1;
            
            mintAmountEachTokens[tokenSeed - 1]++;
        }
        ...
        onlyRole(ADMIN)
    {
        ...
        (bool dev, ) = payable(devAddress).call{value: balance * 25 / 100}('');
        require(dev);

        // withdrawing for contributor's wallet
        (bool cont1, ) = payable(contributorAddress1).call{value: balance * 25 / 100}('');
        require(cont1);

        // withdrawing for contributor's wallet
        (bool cont2, ) = payable(contributorAddress2).call{value: balance * 25 / 100}('');
        require(cont2);

        // withdrawing for dao wallet remainder
        (bool os, ) = payable(withdrawAddress).call{value: balance * 25 / 100}('');
        require(os);
    }
