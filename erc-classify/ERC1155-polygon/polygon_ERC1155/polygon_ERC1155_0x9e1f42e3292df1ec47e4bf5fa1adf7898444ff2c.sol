// Chain: POLYGON - File: src/components/Medallions.sol
...
contract Medallions is ERC1155, OwnableRoles {

    // =============================================================
    //                           EVENTS
    // =============================================================

    // Event to be emitted when an item is consumed
    event ItemConsumed(uint256 indexed playerID, uint256 indexed itemID, uint256 quantity);
    ...
    // @dev Emitted Upon Aidrop
    event AirdropMedallions(address indexed to, uint256 indexed id, uint256 amount);

    // @dev Emitted upon Batch Mint
    event BatchMint(address indexed to, uint256[] ids, uint256[] amounts);

    // @dev emitted upon Burn
    event RedemptionBurn(address indexed from, uint256 indexed id, uint256 amount);
    ...
    // Maximum amount that can be minted in one transaction
    uint256 public MAX_AIRDROP_AMOUNT = 20; 

    // Mapping from token ID to its URI
    mapping (uint256 => string) public tokenURIs;

    //Owner approved contract for burning mechanics
    address private _authorizedContract;
    
    // Mapping from token ID to total supply
    mapping(uint256 => uint256) private _totalSupplies;

    //Incase funds end up in contract by mistake
    address public fundingRecipient = 0x3816A52eC73F494e856625a8612C232DeC83Bf94;
    ...
    function Airdrop(
        ...
        uint256 id,
        uint256 amount
    )   public onlyRolesOrOwner(ADMIN_ROLE | MINTER_ROLE) {
        require(amount > 0 && amount <= MAX_AIRDROP_AMOUNT, "Mint: Invalid amount");
        ...
        emit AirdropMedallions(to, id, amount);

    }

    // Public or external function to batch mint new tokens
    function batchMint(
        address to, 
        uint256[] memory ids, 
        uint256[] memory amounts
    ) public onlyRolesOrOwner(ADMIN_ROLE | MINTER_ROLE) {
        require(to != address(0), "BatchMint: recipient is the zero address");
        require(ids.length == amounts.length, "BatchMint: ids and amounts length mismatch");

        // Call the internal batch mint function
    ...
    function burn(address from, uint256 id, uint256 amount) external { 
        // Check if the caller is the authorized contract
        if (msg.sender == _authorizedContract) {
            // Bypass certain checks and directly burn the token
            _burn(from, id, amount);
        } else {
            // Perform the regular checks before burning
            require(
            ...
            emit RedemptionBurn(from, id, amount);
        }
    }
    
    // Override for URI getter
    ...
    // Function to set URIs for existing item types
    ...
    //Withdrawal functions incase anyone accidently sends assets to the ccontract
    function withdrawETH() external onlyRolesOrOwner(ADMIN_ROLE) {
    ...
    function withdrawERC20(address[] calldata tokens) external onlyRolesOrOwner(ADMIN_ROLE) {
    unchecked {
...
// Chain: POLYGON - File: lib/solady/src/tokens/ERC1155.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/solady/src/auth/OwnableRoles.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: lib/solady/src/auth/Ownable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
