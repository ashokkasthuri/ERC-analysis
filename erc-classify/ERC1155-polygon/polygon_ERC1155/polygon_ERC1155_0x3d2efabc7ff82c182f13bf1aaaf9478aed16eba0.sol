// Chain: POLYGON - File: src/InstantRewards.sol
...
contract InstantRewards is
    ...
    UUPSUpgradeable
{
    error NotEnoughInstantRewards();
    error NotEnoughEth();
    error NotHaveThisReward();

    address internal s_bot;
    address internal s_manager;

    uint256[] internal INSTANT_REWARDS_IDS;
    mapping(uint256 => uint256) internal INSTANT_REWARDS;

    event RewardBurnt(address indexed account, uint256 indexed tokenId, bytes32 messageId);

    modifier OnlyOwnerOrBotOrManager() {
        if (_msgSender() != owner() && _msgSender() != s_bot && _msgSender() != s_manager) {
    ...
    function initialize(address initialOwner, address bot, address manager) public initializer {
        __ERC1155_init("https://nft.junkyard.wtf/InstantRewards/{id}.json");
    ...
    function mint(address account, uint256 tokenId, uint256, bytes calldata) public OnlyOwnerOrBotOrManager {
        uint256 amount = 1;
        _mint(account, tokenId, amount, "");
    }

    function claim(uint256 tokenId, bytes32 messageId) public {
        address account = _msgSender();
        if (balanceOf(account, tokenId) == 0) {
            revert NotHaveThisReward();
        }

        _burn(account, tokenId, 1);

        emit RewardBurnt(account, tokenId, messageId);
    }

    function setBotAddress(address newAddress) public onlyOwner {
        s_bot = newAddress;
    }

    function setManagerAddress(address newAddress) public onlyOwner {
        s_manager = newAddress;
    }
