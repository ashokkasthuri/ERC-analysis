// Chain: POLYGON - File: VTRUCommunityPass_flattened.sol
...
contract VTRUCommunityPass is ERC1155, ERC1155Burnable, AccessControl {
    ...
    mapping(uint256 => address) public _tokenIdOwner;
    mapping(address => uint256) public _ownerTokenId;
    ...
    bytes32 public constant AIRDROP_ROLE = keccak256("AIRDROP_ROLE"); // RoleID = 2

    event PassAirdropped(
        address indexed to,
        uint256 indexed passType,
        uint256 amount
    );

    event BulkPassAirdropped(
        address[] indexed to,
        uint256 indexed passType,
        uint256 amount
    );
    ...
    function _init() internal {
        ...
        _grantRole(AIRDROP_ROLE, 0x0AE8D18bddbFaa5dDbbA4475CAE0A77432FE2EE6);
    }

    function AirdropPass(
        address to,
        uint256 passType,
        uint256 amount
        ...
        _mintedSupply[passType] += amount;
        _minted[to][passType] += amount;

        _mint(to, passType, amount, "");
        emit PassAirdropped(to, passType, amount);
    }

    function BulkAirdropPass(
        uint256 passType,
        uint256 amount,
        address[] calldata recipients
    ) external onlyRole(AIRDROP_ROLE) {
        uint256 quantity = recipients.length;
        _mintedSupply[passType] += quantity;
        ...
        emit BulkPassAirdropped(recipients, passType, amount);
    }
    ...
    function burnToRedeem(
        ...
        require(
            msg.sender == _redeemAddress,
            "Sender not authorized to redeem"
        );
        _burnedSupply[passType] += amount;
        _burned[to][passType] += amount;

        _burn(to, passType, amount);
        emit BurnedToRedeem(to, passType, amount);
    }
    ...
    function burnBatchToRedeem(
        ...
        emit BurnedBatchToRedeem(passType, recipients, amount);
    }
    ...
    function recoverFunds() external onlyRole(DEFAULT_ADMIN_ROLE) {
