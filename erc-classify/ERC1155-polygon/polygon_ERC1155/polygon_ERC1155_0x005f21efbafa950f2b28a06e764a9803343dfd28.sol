// Chain: POLYGON - File: contracts/nft/ ReadlStories.sol
// SPDX-License-Identifier: MIT
import "./Story_resources.sol";
import "@openzeppelin/contracts/token/common/ERC2981.sol";

pragma solidity ^0.8.0;

contract  ReadlStories is ERC1155Tradable, ReentrancyGuard, ERC2981 {

    using TokenIdentifiers for uint256;

    uint96 constant public MAX_AUTHOR_FEE = 2000; // 20%
    uint96 constant public DEFAULT_FEE = 200; // 2%
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "AssetContractShared#creatorOnly"
        );
        _;
    }
    ...
    function mint(
        ...
        _setTokenRoyalty(_id, creator(_id), DEFAULT_FEE);
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "AssetContractShared#_batchMint: ONLY_CREATOR"
            );
            _setTokenRoyalty(_ids[i], creator(_ids[i]), DEFAULT_FEE);
        }
    ...
    */
    ...
    function safeTransferFrom(
        ...
        if(mintedBalance < _amount && _remainingSupply(_id) > 0) {
            // Only mint what _from doesn't already have
            mint(_from, _id, _remainingSupply(_id), _data);
        }
    ...
    function setTokenRoyalty(
        ...
        address receiver,
        uint96 feeNumerator
    ) external virtual creatorOnly(tokenId) {
        require(
            feeNumerator <= MAX_AUTHOR_FEE,
            "setTokenRoyalty: FEE_TOO_HIGH"
        );
...
// Chain: POLYGON - File: contracts/nft/Story_resources.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
import "../common/TokenIdentifiers.sol";
    ...
    Ownable
{
    ...
    function safeBatchTransferFrom(
        ...
        require(
            ...
            "AssetContract#safeBatchTransferFrom: INVALID_A_LENGTH"
        );
        for (uint256 i = 0; i < _ids.length; i++) {
    ...
    function _mint(
        ...
        if (_data.length > 1 && bytes(_tokenURI[_id]).length == 0) {
    ...
    function _batchMint(
        ...
        require(
            _ids.length == _amounts.length,
            "ERC1155Tradable#batchMint: INVALID_A_LENGTH"
        );
        ...
        if (_data.length > 1) {
            for (uint256 i = 0; i < _ids.length; i++) {
                if(bytes(_tokenURI[_ids[i]]).length == 0) {
    ...
    function creator(uint256 _id) public pure returns (address) {
        return _id.tokenCreator();
    }

    // PROXY HELPER METHODS

    function _isProxyForUser(address, address _address)
        internal
        view
        returns (bool)
    {
...
// Chain: POLYGON - File: contracts/common/TokenIdentifiers.sol
...
// Chain: POLYGON - File: contracts/common/meta-transactions/Initializable.sol
// SPDX-License-Identifier: MIT
