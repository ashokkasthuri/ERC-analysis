...
contract TerminusFacet is ERC1155WithTerminusStorage {
    ...
    event EmptyPoolBurnBatchIgnored();
    ...
    function twtBurnMinions(uint24[5] memory poolIds, uint24[5] memory amounts) external {
        uint256[] memory nonZeroPoolIds = new uint256[](5);
        uint256[] memory nonZeroAmounts = new uint256[](5);
        uint256 nonZeroIndex = 0;
        for(uint256 i = 0; i < poolIds.length; i++) {
            if(poolIds[i] != 0 && amounts[i] != 0) {
                nonZeroPoolIds[nonZeroIndex] = uint256(poolIds[i]);
                nonZeroAmounts[nonZeroIndex] = uint256(amounts[i]);
                nonZeroIndex++;
            }
        }
        if(nonZeroIndex == 0) {
            emit EmptyPoolBurnBatchIgnored();
            return;
        }
        assembly {
            mstore(nonZeroPoolIds, nonZeroIndex)
            mstore(nonZeroAmounts, nonZeroIndex)
        }
        
        _burnBatch(msg.sender, nonZeroPoolIds, nonZeroAmounts);
    }
}
