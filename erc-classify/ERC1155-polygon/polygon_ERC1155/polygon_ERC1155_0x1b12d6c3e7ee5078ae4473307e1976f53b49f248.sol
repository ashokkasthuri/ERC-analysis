...
// Chain: POLYGON - File: contracts/Coupons.sol
...
contract Coupons is ERC1155URIStorage, Ownable {
  ...
  event CouponClaimed(uint256 tokenId, address claimer);

  constructor() ERC1155('') {}

  function createCoupon(address _creator, string memory _tokenURI) public {
    varieties += 1;
    _mint(_creator, varieties, 1, 'BALI BETA FOOD COUPON');
    _setURI(varieties, _tokenURI);
    emit CouponCreated(varieties, _creator);
  }

  function claimCoupon(uint256 _tokenID, address _claimer) public {
    require(_tokenID <= varieties && _tokenID > 0, 'INVALID COUPON');
    _mint(_claimer, _tokenID, 1, 'BALI BETA FOOD COUPON');
    emit CouponClaimed(_tokenID, _claimer);
  }
