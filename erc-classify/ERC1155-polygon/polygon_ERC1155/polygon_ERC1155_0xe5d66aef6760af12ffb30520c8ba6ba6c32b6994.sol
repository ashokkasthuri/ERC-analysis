...
// Chain: POLYGON - File: contracts/OKXTemplate/OKX1155Template.sol
    ...
    UUPSUpgradeable
{
    ...
    mapping(address => bool) public isMinted; //address -> status
    ...
    function setServerAccount(
        ...
        for (uint256 i = 0; i < _servers.length; i++) {
            // Already set the same value
            if (serverAccounts[_servers[i]] == knob) {
                continue;
            }
            serverAccounts[_servers[i]] = knob;
        }
    }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < length; ) {
            require(!isMinted[tos[i]], "Address has minted");
            require(
                amounts[i] <= mintLimitationPerAddress,
                "Amount exceed limit"
            );

            isMinted[tos[i]] = !isMinted[tos[i]];
