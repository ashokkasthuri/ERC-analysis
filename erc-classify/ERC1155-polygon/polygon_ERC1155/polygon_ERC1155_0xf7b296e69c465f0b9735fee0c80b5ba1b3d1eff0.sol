// Chain: POLYGON - File: contracts/deployed/ArayaNft.sol
...
contract ArayaNft is ERC1155, AccessControl, OpenSeaCompatible, NativeMetaTransaction, ContextMixin
{
    ...
    uint64 public constant MINTING_CHAINID = 137;

    mapping (uint256 => uint256) public mintedCounts;
    mapping (uint256 => uint256) public mintingLimits;

    constructor(string memory domainName) ERC1155("ArayaNft", "ARAYALIGHT", domainName) {
    ...
    */
    function initialMint(address account, uint256 id, uint256 amount, bytes calldata data) external onlyRole(MINTER_ROLE) {
        require(block.chainid == MINTING_CHAINID || block.chainid == 31337, "Minting is allowed only on Polygon."); // or hardhat internal (for testing)
