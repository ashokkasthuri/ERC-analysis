...
// Chain: POLYGON - File: contracts/StakesGear.sol
...
contract StakesGear is ERC1155URIStorage, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;

    Counters.Counter private _tokenIds;
    mapping(uint256 => uint256) private _circulating;
    ...
    constructor() ERC1155("") {
        contractURI = "https://firebasestorage.googleapis.com/v0/b/stakes-3fde0.appspot.com/o/sports_gods_gear.json?alt=media";
    }
    ...
    function mintFor(
        ...
        _circulating[newTokenId] = _circulating[newTokenId].add(amount);

        if (tokenId <= 0) {
            _setURI(newTokenId, uri);
        }

        emit Minted(recipient, newTokenId, amount, uri);
        return newTokenId;
    }

    function burn(uint256 tokenId, uint256 amount) public {
        _burn(msg.sender, tokenId, amount);
        _circulating[tokenId] = _circulating[tokenId].sub(amount);
    }
    ...
    function getTokensHeld(
        ...
        tokenIds = new uint256[](uniqueTokenCount);
        balances = new uint256[](uniqueTokenCount);

        uint256 index = 0;
        for (uint256 i = 1; i <= lastId; i++) {
            uint256 balance = balanceOf(walletAddress, i);
            if (balance > 0) {
                tokenIds[index] = i;
