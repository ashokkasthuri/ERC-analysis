...
contract FCBarcelona is ERC1155Supply, Ownable {
    using Strings for uint256;

    uint256 public MAX_SUPPLY = 300;
    string public name = 'FCBarcelona';
    string public symbol = 'FCB';
    string private _uri;
    
    constructor() ERC1155('ipfs://bafybeihlpakbfjnrqcmxjocvldiqjg4w43ohsfrz7wnkzsyb6qxiv5ddne/') Ownable(msg.sender) {
        uint[] memory ids = new uint[](MAX_SUPPLY);
        uint[] memory values = new uint[](MAX_SUPPLY);
        for (uint i = 0; i < MAX_SUPPLY; i++) {
            ids[i] = i + 1;
            values[i] = 1;
        }
        _mintBatch(msg.sender, ids, values, '');
        setURI('ipfs://bafybeihlpakbfjnrqcmxjocvldiqjg4w43ohsfrz7wnkzsyb6qxiv5ddne/');
    }
