// Chain: POLYGON - File: contracts/IStakeValidator.sol
...
// Chain: POLYGON - File: contracts/Staking.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.19;
...
contract Staking is IERC165, IERC721Receiver, IERC1155Receiver, Ownable, Initializable {
     ...
     */
    uint256 constant public MAX_ACTIVE_STAKES = 100;
    ...
    /**
     * @dev Event emitted whenever the validator changes, i.e. `setValidator` is called
     ...
     * @param initialOwner The owner/admin of this contract. This address is allowed to call setValidator
     * @param _unstakePeriodSeconds Initial unstaking time in seconds
     */
    function initialize(
        address _validator, address initialOwner, uint256 _unstakePeriodSeconds
    ) initializer external {
        require(_validator != address(0x0), "Can not set validator to zero address");
        require(initialOwner != address(0x0), "Can not set owner to zero address");
    ...
    function setValidator(address _validator) external onlyOwner {
        require(_validator != address(0x0), "Can not set validator to zero address");
        validator = IStakeValidator(_validator);
        emit ValidatorChanged(_validator);
    }
    ...
    function unstake(uint256[] calldata recordIds) external {
        ...
        for (uint256 i = 0; i < recordIds.length; i++) {
            ...
            record.claimableAt = uint64(block.timestamp + unstakePeriodSeconds);
    ...
    function _recordStake(
        ...
        nextId.increment();
        StakeRecord memory record = StakeRecord(id, tokenId, 0, tokenContract, tokenType);
...
// Chain: POLYGON - File: openzeppelin-contracts/contracts/proxy/utils/Initializable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: openzeppelin-contracts/contracts/token/ERC721/IERC721.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: openzeppelin-contracts/contracts/token/ERC721/IERC721Receiver.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: openzeppelin-contracts/contracts/utils/Counters.sol
// SPDX-License-Identifier: MIT
