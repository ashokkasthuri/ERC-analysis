...
// Chain: POLYGON - File: contracts/common/Base.sol
...
abstract contract Base is Ownable {
    event ReceivedNativeToken(address indexed receiver, uint256 amount);
    event ReceivedERC20Token(
        address indexed receiver,
        address indexed token,
        uint256 amount
    );
    event ReceivedERC721Token(
        address indexed receiver,
        address indexed token,
        uint256 tokenId
    );
     ...
     */
    function withdrawNativeToken(uint256 amount) external virtual onlyOwner {
        ...
        emit ReceivedNativeToken(msg.sender, amount);
    }
    ...
    function withdrawErc20(
        ...
        emit ReceivedERC20Token(msg.sender, address(erc20token), amount);
    }

    /**
     * withdraw ERC721 Tokens in case of accidentally transfer - owner only
     */
    function withdrawErc721(
        ...
        emit ReceivedERC721Token(msg.sender, address(erc721Token), tokenId);
    }
}


// Chain: POLYGON - File: contracts/polygon/ChibiTreasureChest.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
...
import {IChibiTreasureChest} from "./interfaces/IChibiTreasureChest.sol";
import {Base} from "../common/Base.sol";

error InvalidMinter();
error InvalidTokenId();

contract ChibiTreasureChest is
    ERC1155,
    ERC1155Burnable,
    Base,
    IChibiTreasureChest
{
    uint256 public constant HASH = 1;
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (ids[i] < BRONZE || ids[i] > PLATINUM) {
                revert InvalidTokenId();
            }
        }
    ...
    function uri(uint256 id) public view override returns (string memory) {
        return
            string(
                abi.encodePacked(_baseUri, Strings.toString(id), _extension)
            );
    }
}


// Chain: POLYGON - File: contracts/polygon/interfaces/IChibiTreasureChest.sol
// SPDX-License-Identifier: UNLICENSED

pragma solidity ^0.8.0;

interface IChibiTreasureChest {
    function mint(
        address account,
        uint256 id,
        uint256 amount,
    ...
    function setURI(string memory baseUri_, string memory extension_) external;
