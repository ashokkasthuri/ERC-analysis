// Chain: POLYGON - File: node_modules/@openzeppelin/contracts/utils/structs/EnumerableSet.sol
...
// Chain: POLYGON - File: node_modules/@thirdweb-dev/contracts/base/ERC1155LazyMint.sol
...
// Chain: POLYGON - File: node_modules/@thirdweb-dev/contracts/extension/interface/IClaimableERC1155.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: node_modules/@thirdweb-dev/contracts/openzeppelin-presets/security/ReentrancyGuard.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: src/ScavengerHunt1155Drop.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Swap functions
    ...
    */
    function adminSwap(address _from, address _to, uint256[] memory _fromTokenIds, uint256[] memory _toTokenIds) public onlyOwner {
        ...
        // Can't trade 0 tokens from either side
        require(_fromTokenIds.length > 0 && _toTokenIds.length > 0, "CANNOT TRADE 0 TOKENS");
  
        uint256 fromLen = _fromTokenIds.length;
        uint256 toLen = _toTokenIds.length;

        uint256[] memory amountsFrom = new uint256[](fromLen);
        uint256[] memory amountsTo = new uint256[](toLen);
        
        // Create an array of '1' for the From tokens
        for (uint256 i = 0; i < fromLen; i++) {
            
            amountsFrom[i] = 1;
        }

        // Create an array of '1' for the To tokens
        for (uint256 i = 0; i < toLen; i++) {
            
            amountsTo[i] = 1;
        }
        
        _safeBatchTransferFrom(_from, _to, _fromTokenIds, amountsFrom, bytes(""));
        _safeBatchTransferFrom(_to, _from, _toTokenIds, amountsTo, bytes(""));
    }
