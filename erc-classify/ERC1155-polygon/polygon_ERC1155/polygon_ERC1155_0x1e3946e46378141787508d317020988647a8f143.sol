...
// Chain: POLYGON - File: contracts/TicketCollection.sol
...
contract TicketCollection is ERC1155URIStorage {
    ...
    function ownerSafeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public onlyOwner {
        require(to != address(0), "Invalid recipient address");
        require(balanceOf(from, id) >= amount, "Insufficient ticket balance");
    ...
    function mintBatch(address to, uint256[] memory tokenIds, uint256[] memory amounts) external onlyOwner returns (uint256) {
        require(tokenIds.length > 0, "No tokenIds provided");
        // Use _mintBatch to mint all tickets in one transaction
        _mintBatch(to, tokenIds, amounts, new bytes(0));
        ticketsMinted += tokenIds.length;
        return ticketsMinted;   
    }
}