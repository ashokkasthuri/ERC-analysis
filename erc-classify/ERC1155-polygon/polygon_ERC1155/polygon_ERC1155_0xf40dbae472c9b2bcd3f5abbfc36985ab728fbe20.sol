...
contract OneSlicepizza is ERC1155Enumerable, OwnWithAuth, ERC1155URI {
/*
    address public BUCKETBEAR;
    address public MIMI;
...
*/
    constructor() ERC1155Enumerable("0ne Sl1ce pizza", "0NESL1CE") {
    ...
    function burn(address from_, uint256 id_, uint256 amount_) external onlyAdmin("BURNER") {
        /*
        require(shopId_ == 0 || shopId_ == 1, "shop id has to be 0 or 1");
        _burn(from_, id_, amount_);
        ShopTotal[shopId_] += amount_;
        RedeemAddress[from_][shopId_] += amount_;
        if (pause) {
            IBUCKETBEAR(BUCKETBEAR).mint(from_, 1, amount_, data_); /// for bucket bear
            IMIMI(MIMI).mint(from_, 1, amount_, data_); /// for mimi
        }
        */
        _burn(from_, id_, amount_);
    }
...
/*
    function setPause(bool bool_) external onlyAdmin("ADMIN") {
        pause = bool_;
    }

    function setAddresses(address addressBucket_, address addressMIMI_) external onlyAdmin("ADMIN") { 
        BUCKETBEAR = addressBucket_;
        MIMI = addressMIMI_;
    }
*/
