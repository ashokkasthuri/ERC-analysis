...
/**
 * @title ERC1155 Token & ERC1155 BURNER
...
contract OneSlicepizzaMIMI is ERC1155Enumerable, OwnWithAuth, ERC1155URI {

    address public ERC1155 = 0xF40dBae472C9B2bcd3F5ABbFc36985Ab728fBE20;

    /// address_ => shopId_ => amount_
    mapping(address => mapping(uint256 => uint256)) public RedeemAddress;

    /// shopId_ => amount_
    mapping(uint256 => uint256) public ShopTotal;

    /// event
    event MUNCHED(address owner_, uint256 id_, uint256 amount_, uint256 shopId_);

    constructor() ERC1155Enumerable("0ne Sl1ce pizza mimi", "0NESL1CEMIMI") {
         _setAdmin("TRANSFER", msg.sender, true);
        _setAdmin("MINTER", msg.sender, true);
        _setAdmin("BURNER", msg.sender, true);
        _setAdmin("ADMIN", msg.sender, true);
        _setAdmin("TRANSFER", 0xf59FCc62Ed0cf8118493092eB31ef4B2546c3c6b, true);
        _setAdmin("BURNER", 0xf59FCc62Ed0cf8118493092eB31ef4B2546c3c6b, true);
        _setAdmin("MINTER", 0xf59FCc62Ed0cf8118493092eB31ef4B2546c3c6b, true);
        _setAdmin("ADMIN", 0xf59FCc62Ed0cf8118493092eB31ef4B2546c3c6b, true);
        _setTokenToURI(1, "ipfs://Qma6ENsYsWJX4HzzPKZGG5mQrMmAoCxyorG8j7kVhTSxCh");
    }
    ...
    function eatPizza(address owner_, uint256 id_, uint256 amount_, uint256 shopId_) external onlyAdmin("BURNER") {
        IERC1155(ERC1155).burn(owner_, id_, amount_);
        _mint(owner_, 1, amount_, "0x00");
        RedeemAddress[owner_][shopId_] += amount_;
        ShopTotal[shopId_] += amount_;
        emit MUNCHED(owner_, id_, amount_, shopId_);
    }

    function setPizza(address address_) external onlyAdmin("ADMIN") {
        ERC1155 = address_;
    }
    ...
    function safeTransferFrom(address from_, address to_, uint256 id_, uint256 amount_, bytes memory data_) public virtual override onlyAdmin("TRANSFER") {
        super.safeTransferFrom(from_, to_, id_, amount_, data_);
    }

    function safeBatchTransferFrom(address from_, address to_, uint256[] memory ids_, uint256[] memory amounts_, bytes memory data_) public virtual override onlyAdmin("TRANSFER") {
