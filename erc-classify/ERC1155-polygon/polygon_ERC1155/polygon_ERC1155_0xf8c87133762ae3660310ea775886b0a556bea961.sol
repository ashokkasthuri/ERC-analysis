// Chain: POLYGON - File: tests/1155-0107.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";

contract VCITY is ERC1155 {
    uint256 private constant DB = 1001;
    uint256 private constant MAX_SUPPLY = 10**12;  // 设定 DB 代币的最大供应量
    mapping(uint256 => uint256) private _totalSupply;  // 用于跟踪每种代币的总供应量
    ...
    // 自定义的铸造函数
    function nftMint(address to) public onlyOwner {        
        require(_totalSupply[DB] + 100 <= MAX_SUPPLY, "Exceeds maximum supply"); // 检查总供应量
        _totalSupply[DB] += 100;  // 更新总供应量
        _mint(to, DB, 100, bytes(""));  // 调用内部 _mint 函数进行铸造
    }

    // 公共函数，用于返回特定代币的当前总供应量
    function totalSupply() public view returns (uint256) {
        return _totalSupply[DB];
    }
}
