...
// Chain: POLYGON - File: contracts/WePoap.sol
// SPDX-License-Identifier: MIT
...
contract WePoap is ERC1155, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private supplies;
    ...
    constructor(string memory _metadataUri, address _airdropWallet, string memory _collectionName)
    ERC1155(_metadataUri)
    {
        name = _collectionName;
        symbol = _collectionName;
        addToken(1, _metadataUri);
    ...
    function addToken(uint256 _tokenId, string memory _metadataUri) public onlyOwner {
        require(tokens[_tokenId].supply.current() == 0, "Token is already added");
        tokens[_tokenId] = Token(_metadataUri, Counters.Counter(0));
    }
    ...
    function supply(uint256 _tokenId) public view returns (uint256) {
        return tokens[_tokenId].supply.current();
    }

    modifier onlyAirdropWallet() {
        require(msg.sender == airdropWallet, "Only airdrop wallet");
        _;
    }

    function airdrop(address _receiver, uint256 _tokenId) public onlyAirdropWallet {
        require(balanceOf(_receiver, _tokenId) == 0, "This wallet already has this NFT");
        tokens[_tokenId].supply.increment();
        _mint(_receiver, _tokenId, 1, "");
    }

    function changeAirdropWallet(address _newAirdropWallet) public onlyOwner {
        airdropWallet = _newAirdropWallet;
    }
}