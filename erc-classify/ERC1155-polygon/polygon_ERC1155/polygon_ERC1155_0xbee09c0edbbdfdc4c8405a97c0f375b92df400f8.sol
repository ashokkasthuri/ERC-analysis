 ...
 * by @NFTChef
 */
contract FireworksAR is ERC1155, ERC1155Supply, Ownable {
    ...
    /**
     * @notice Sends multiple tokens to addresses list
     * @param ads The addresses to receive the tokens
     * @param typeId The address to receive the tokens
     * @param quantity How many to send she receiver
     */
    function batchMint(
        ...
        require(ads.length == quantity.length, "length error");
        for (uint256 i = 0; i < ads.length; i++) {
            _mint(ads[i], typeId, quantity[i], "");
        }
    }

    function burnFromList(address[] calldata _list, uint256 typeId, uint256 quantity)
        external
        onlyOwner
    {
        require(typeId > 0 && typeId < 4, "type err");
