...
contract METARUFFY_ASSETS is ERC1155, ERC2981, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    function addMinter(address _minterAddress,bool _access) external onlyOwner{
        minters[_minterAddress] = _access;
    }
    function setURI(string memory _newuri,uint256 _tokenID) external onlyOwner {
        _tokenURIs[_tokenID] = _newuri;
    }

    
    function createCollection(uint256 _maxSupply, string memory _collection_uri) external onlyOwner returns (uint256) {
    ...
    function tokenInfo(uint256 tokenID) external view returns (uint256 , uint256 , string memory) {
