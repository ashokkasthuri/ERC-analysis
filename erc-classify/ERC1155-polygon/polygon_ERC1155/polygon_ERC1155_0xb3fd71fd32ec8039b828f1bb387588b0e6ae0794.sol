...
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";
import {MercuryVirgin} from "./MercuryVirgin.sol";
    ...
    UUPSUpgradeable
{
    ...
    MercuryVirgin public mercuryVirgin;
    uint256[] private _tokenDistribution;
    uint256 private _storkFeatherTokenId;
    mapping(address => uint256) public lastMintTimestamp;
    uint256 public lastResetTimestamp;
    mapping(uint256 => bool) public gamePlayable;
    ...
    function setMercuryVirgin(address _address) public onlyOwner {
        mercuryVirgin = MercuryVirgin(_address);
    }

    function setTokenDistribution(
        uint256[] calldata newtokenDistribution
    ) public onlyOwner {
        _tokenDistribution = newtokenDistribution;
    }

    function setStorkFeatherTokenId(uint256 _tokenId) public onlyOwner {
        _storkFeatherTokenId = _tokenId;
    }

    function resetMintingForAll() public onlyOwner {
        lastResetTimestamp = block.timestamp;
    }

    function setGamePlayable(uint256 tokenId, bool playable) public onlyOwner {
        gamePlayable[tokenId] = playable;
    }

    function setGamePlayableBatch(
        uint256[] calldata tokenIds,
        bool playable
    ) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            gamePlayable[tokenIds[i]] = playable;
        }
    }
    ...
    {
        return _tokenDistribution;
    }

    function getStorkFeatherTokenId() public view onlyOwner returns (uint256) {
        return _storkFeatherTokenId;
    }

    // Minting
    function mint(address account) public onlyOwner nonReentrant whenNotPaused {
        require(_tokenDistribution.length > 0, "Token distribution is not set");
        require(
            address(mercuryVirgin) != address(0),
            "MercuryVirgin address is not set"
        );
        require(mercuryVirgin.isMercuryPresent(), "Mercury is not present");
        require(
            lastMintTimestamp[account] <= lastResetTimestamp,
            "You have already minted an item"
        );

        uint256 randomIndex = _randModulus(_tokenDistribution.length);
        uint256 tokenId = _tokenDistribution[randomIndex];
        _mint(account, tokenId, 1, "");
        lastMintTimestamp[account] = block.timestamp;
    }
    ...
    function mintOwner(
        ...
        uint256 id,
        uint256 amount
    ) public onlyOwner nonReentrant onlyOwner {
    ...
    // Utils
    function _randModulus(uint256 modulus) internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
...
// Chain: POLYGON - File: contracts/MercuryVirgin.sol
// SPDX-License-Identifier: MIT
    ...
    UUPSUpgradeable
{
    ...
    string public baseURI;
    Timeframe private _mercuryPresenceTimeframe;
    ...
    function initialize() public initializer {
        __ERC721_init("MERCURY VIRGIN", "MV");
    ...
    function tokenURI(
        ...
        return
            string(
                abi.encodePacked(ERC721Upgradeable.tokenURI(tokenId), ".json")
            );
    }
    ...
    // Getter
    function getMercuryPresenceTimeframe()
        public
        view
        onlyOwner
        returns (Timeframe memory)
    {
        return _mercuryPresenceTimeframe;
    }

    function isMercuryPresent() public view returns (bool) {
        uint256 currentTime = block.timestamp;
        return
            _mercuryPresenceTimeframe.startTime <= currentTime &&
            currentTime < _mercuryPresenceTimeframe.endTime;
    }
    ...
    function setMercuryPresenceTimeframe(
        uint256 _startTime,
        uint256 _endTime
    ) public onlyOwner {
        require(_startTime < _endTime, "Start time must be before end time");
        _mercuryPresenceTimeframe.startTime = _startTime;
        _mercuryPresenceTimeframe.endTime = _endTime;
    }
