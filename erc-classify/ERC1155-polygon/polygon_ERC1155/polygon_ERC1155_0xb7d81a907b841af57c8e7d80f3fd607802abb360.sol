...
// Chain: POLYGON - File: contracts/GoGoShikibuKitchen.sol
// SPDX-License-Identifier: MIT

//
// 🚚 Go!Go! Shikibu Kitchen
//
...
contract GoGoShikibuKitchen is ERC1155, AccessControl, ERC1155Supply {
    bytes32 public constant ADMIN = keccak256("ADMIN");
    bool public paused = true;
    string public name;
    string public symbol;

    address public withdrawAddress = 0xde1d12d569184Dd1d1fb376CfA9763228Eb6392f;
    ...
    constructor() ERC1155("") {
        name = "Go!Go! Shikibu Kitchen";
        symbol = "GSK";
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(ADMIN, msg.sender);
        _grantRole(ADMIN, 0xDFF35863CD9986E2d4271C0aCa86a235A1774e24);
    }
    ...
    function setWithdrawAddress(address newAddress) external onlyRole(ADMIN) {
    ...
    function mint(address account, uint256 amount) external payable {
        ...
        require(amount > 0, "Mint amount must be gt 0");
        require(mintCost * amount <= msg.value, "Not enough eth");
        _mint(account, tokenIdToMint, amount, "");
    }

    /**
     * URI関数
     */
    /// @dev tokenIdごとにtokenURIを設定
    function setURI(
        uint256 tokenId,
    ...
    function setApprovalForAll(
        ...
        require(operator == address(0), "this is SBT");
     ...
     * その他の関数
     */
    function togglePaused() external onlyRole(ADMIN) {
        paused = !paused;
    }

    function setTokenIdToMint(uint256 id) external onlyRole(ADMIN) {
        require(bytes(tokenURIs[id]).length != 0, "tokenURI not set");
        tokenIdToMint = id;
    }

    function setMintCost(uint256 newCost) external onlyRole(ADMIN) {
        mintCost = newCost;
    }
