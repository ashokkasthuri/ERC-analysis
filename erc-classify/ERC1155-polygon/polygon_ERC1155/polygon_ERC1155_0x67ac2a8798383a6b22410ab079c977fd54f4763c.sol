...
// Root file: contracts/tokens/appreciation_badge_soulbound.sol
...
// Appreciation Badge contract: Molecule ERC1155 token, soul-bound
// with AccessControl for allowing server accounts to mint
contract AppreciationBadge is ERC1155, Ownable, AccessControl {
    enum MoleculeType {
        Approve,
        Burn,
        Mint,
    ...
    // Default Openzepplin contract does not emit `data` in event.
    // Modified mint function to emit `data` so we can associate with id
    function mintWithDataEvent(
        address account,
        uint256 id,
        uint256 amount,
        bytes memory data
        ...
        _mint(account, id, amount, data);
        emit TransferSingleData(msg.sender, address(0), account, id, amount, data);
    }
    ...
    function _beforeTokenTransfer(
        ...
        // If no molecule specified, make it a soul-bound token by default
        // Allow mint or burn only
