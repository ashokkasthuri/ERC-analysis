...
contract Poli is ERC1155, Ownable {
  ...
  /**
   * @dev Mapping of balances per tokenId.
   */
  ...
  function mintBatch(Recipient[] memory recList) external onlyAuthorized() {
    for (uint i = 0; i < recList.length; i++) {
      Recipient memory rec = recList[i];
      require (tokenIdBalance[rec.tokenId] + rec.amount <= 100, 'Amount 100 exceeded');

      tokenIdBalance[rec.tokenId] += rec.amount;
      _mint(rec.wallet, rec.tokenId, rec.amount, "");
    }
  }

  function setURI(string memory _uri) external onlyAuthorized() {
    _setURI(_uri);
  }
