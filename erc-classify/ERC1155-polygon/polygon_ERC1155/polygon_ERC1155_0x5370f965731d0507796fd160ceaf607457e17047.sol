...
// Chain: POLYGON - File: src/ERC1155/ERC1155BloomPad.sol
// SPDX-License-Identifier: MIT
  ...
  Ownable
{
  ...
  ) ERC1155(_uri) ERC2771Context(trustedForwarder) {
    royaltyBasis = _royaltyBasis;
    royaltyWallet = payable(receiver);
  }

  modifier mintCompliance(uint id, uint256 mintAmount) {
    uint256 total = totalSupply(id);
    uint256 max = maxSupply(id);
    require(max == 0 || total + mintAmount <= max, "Max supply exceeded!");
    _;
  }

  modifier mintBatchCompliance(uint256[] memory ids, uint256[] memory mintAmounts) {
    for (uint256 i = 0; i < ids.length; i++) {
      uint256 id = ids[i];
      uint256 total = totalSupply(id);
      uint256 max = maxSupply(id);
  ...
  function mint(
    ...
    uint256 amount,
    bytes memory data
  ) public mintCompliance(id, amount) onlyOwner {
  ...
  function mintBatch(
    ...
    uint256[] memory amounts,
    bytes memory data
  ) public mintBatchCompliance(ids, amounts) onlyOwner {
...
// Chain: POLYGON - File: src/ERC1155/interfaces/IERC1155Mintable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
// Chain: POLYGON - File: src/ERC1155/interfaces/IRoyalty.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
interface IRoyalty is IERC2981 {
  function setRoyaltyBasis(uint256 _royaltyBasis) external;

  function setRoyaltyWallet(address payable _royaltyWallet) external;
}