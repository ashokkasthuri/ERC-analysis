...
// Chain: POLYGON - File: contracts/MboToys.sol
// SPDX-License-Identifier: MIT
    ...
    ERC1155URIStorage
{
    ...
    constructor(
        ...
        uint256[] memory initMaxSupply,
        bytes32[] memory initRoot,
        ...
        addTokens(initUri, initMaxSupply, initRoot);
    }
    ...
    function isTokenExists(uint256 tokenId) public view returns (bool) {
        return tokenId < nextTokenId.current();
    }
    ...
    function setURI(
        ...
        require(isTokenExists(tokenId), "Token not exists");
    ...
    function addTokens(
        string[] memory tokenUri,
        uint256[] memory tokenMaxSupply,
        bytes32[] memory tokenRoot
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            tokenUri.length == tokenMaxSupply.length &&
                tokenMaxSupply.length == tokenRoot.length,
            "Invalid data length"
        );
        ...
        for (uint256 i = 0; i < tokenUri.length; i++) {
            ...
            maxSupply.push(tokenMaxSupply[i]);
            root.push(tokenRoot[i]);
            nextTokenId.increment();
        }
    }

    function tokenSize() public view returns (uint256) {
    ...
    function mint(
        ...
        string memory code = string(abi.encodePacked(leaf));
        require(isValidCode(tokenId, proof, leaf), "Invalid code");
        require(!usedCodes[code], "Used code");
        require(
