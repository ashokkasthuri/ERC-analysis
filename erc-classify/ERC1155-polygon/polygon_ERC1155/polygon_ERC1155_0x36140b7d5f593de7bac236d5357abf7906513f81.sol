// Chain: POLYGON - File: metaverse.sol
...
contract MyNFTCollection is ERC1155, Ownable {
    uint256 public constant MAX_SUPPLY = 111994;
    uint256 public totalMinted = 0;

    constructor() ERC1155("https://vcity.app/upfile/metaverse/{id}.json") {}

    function mintBatch(uint256 amount, bytes memory data) public onlyOwner {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ids[i] = totalMinted + i + 1;  // 生成唯一ID
            amounts[i] = 1;  // 每个NFT的数量设为1
        }
    ...
    function mintInChunks(uint256 totalAmount, uint256 chunkSize, bytes memory data) public onlyOwner {
        require(totalAmount <= MAX_SUPPLY - totalMinted, "Exceeds maximum supply");

        uint256 remainingAmount = totalAmount;
        while (remainingAmount > 0) {
            uint256 currentBatchSize = remainingAmount > chunkSize ? chunkSize : remainingAmount;
            mintBatch(currentBatchSize, data);
            remainingAmount -= currentBatchSize;
        }
    }
}
