// Chain: POLYGON - File: contracts/NTR.sol
    ...
    ContextMixin
{
    ...
    function _readQuotaUsedFreePaid(
        address account
    ) private view returns (bool, bool, uint32, uint32) {
        uint32 passFilter = ((uint32(1) << QUOTA_BIT) - 1);
    ...
    function readFreeQuota(address account) public view returns (uint32) {
        (, , uint32 res, ) = _readQuotaUsedFreePaid(account);
        return res;
    }

    function readPaidQuota(address account) public view returns (uint32) {
        (, , , uint32 res) = _readQuotaUsedFreePaid(account);
        return res;
    }
    ...
    {
        activeMintMode = block.timestamp >= _whitelist.startMintTime
            ? _whitelist.mintMode
            : MintMode.NOTOPEN;
    ...
    function burn(
        ...
        require(
            account.length == id.length && account.length == amount.length,
            "length mismatch"
        );
        ...
        for (uint i = 0; i < account.length; i++) {
            require(
                _msgSender() == owner() ||
                    account[i] == _msgSender() ||
                    isApprovedForAll(account[i], _msgSender()),
...
// Chain: POLYGON - File: @openzeppelin/contracts-upgradeable/utils/math/SafeCastUpgradeable.sol
// SPDX-License-Identifier: MIT
