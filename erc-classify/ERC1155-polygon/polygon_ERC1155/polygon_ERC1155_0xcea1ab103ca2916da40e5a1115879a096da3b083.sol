...
// Chain: POLYGON - File: contracts/EvedenBundleStorage.sol
// SPDX-License-Identifier: MIT
...
contract EvedenBundleStorage is Ownable, ERC1155Receiver {
  IERC1155 immutable public collection;
  address public issuer;

  constructor(address collection_, address issuer_) {
  ...
  event Issue(uint indexed nonce);

  function issue(address account, uint[] memory ids, uint[] memory amounts, uint nonce, bytes32 sigR, bytes32 sigS, uint8 sigV) external {
    require(issuer != address(0), "no issuer");
    uint blk = nonce / 256;
    uint idx = nonce % 256;
    require(((_nonces[blk] >> idx) & 1) == 0, "duplicate");
    bytes32 h = keccak256(abi.encodePacked(block.chainid, this, account, ids, amounts, nonce));
  ...
  function send(address account, uint[] memory ids, uint[] memory amounts, uint nonce) external {
    require(_msgSender() == issuer, "bad issuer");
