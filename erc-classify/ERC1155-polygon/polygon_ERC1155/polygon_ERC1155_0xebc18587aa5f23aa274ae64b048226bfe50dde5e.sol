// Chain: POLYGON - File: /contracts/WeddingTokenV3.sol
...
contract WeddingTokenV3 is ERC1155Upgradeable, AccessControlUpgradeable {
    ...
    struct PropositionData {
        ...
        uint256 prevBlockNumber;    // number of block, which was right before block with create proposition transaction
    }
    ...
    event Proposition(address indexed from, address indexed to, uint256 indexed id,
        string metaUri, string condData, bool authorAccepted, bool receiverAccepted);

    /// @dev Emitted when Wedding NFT is minted.
    event Wedding(address indexed author, address indexed receiver, uint256 indexed id,
        string metaUri, string condData);
     ...
     */
    event DivorceRequest(address indexed author, address indexed receiver, uint256 indexed id,
        uint256 timestamp, uint256 timeout, bool byAuthor);

    /// @dev Emitted when divorce is accepted.
    event Divorce(address indexed author, address indexed receiver, uint256 indexed id);
    ...
    */
    function propose(address to, string memory metaUri, string memory condData) external onlyNotInMarriage {
        ...
        propositions[_msgSender()][to] = PropositionData(metaUri, condData,
            defaultDivorceTimeout, block.timestamp, true, false, id, block.number);
        _from[_msgSender()].add(to);
        _to[to].add(_msgSender());

        emit Proposition(_msgSender(), to, id, metaUri, condData, true, false);
    }
    ...
    */
    function updateProposition(address to, string memory metaUri, string memory condData) external onlyNotInMarriage {
        ...
        if (isAuthor) {
            ...
            emit Proposition(_msgSender(), to, prop.tokenId, metaUri, condData, true, false);
        } else {
            (prop.authorAccepted, prop.receiverAccepted) = (false, true);
            emit Proposition(to, _msgSender(), prop.tokenId, metaUri, condData, false, true);
        }
    }
    ...
    function acceptProposition(address to, bytes32 metaHash, bytes32 condHash) external onlyNotInMarriage {
        ...
        if (isAuthor) {
            marriages[id] = Marriage(_msgSender(), to, DivorceState.NotRequested,
                ...
                prop.tokenId, prop.prevBlockNumber);
        ...
        if (isAuthor) {
            emit Wedding(_msgSender(), to, id, marriages[id].metaUri, marriages[id].conditionsData);
        } else {
            emit Wedding(to, _msgSender(), id, marriages[id].metaUri, marriages[id].conditionsData);
        }
    }
    ...
    */
    function requestDivorce() external onlyInMarriage {
        ...
        emit DivorceRequest(marriage.author, marriage.receiver, marriage.tokenId, marriage.divorceRequestTimestamp,
    ...
    */
    function confirmDivorce() external onlyInMarriage {
        ...
        emit Divorce(author, receiver, id);
    }
