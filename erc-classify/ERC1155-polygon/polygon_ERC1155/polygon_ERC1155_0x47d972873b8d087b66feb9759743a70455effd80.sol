// Chain: POLYGON - File: contracts/stickerbook/StickerbookCollection.sol
...
interface IUtilityTrait {
    function hasTrait(uint16 tokenId) external view returns(bool);
    function removeTraits(uint16[] calldata tokenIds,uint256 numberToRemove) external returns (uint256[] memory numberRemoved);
}

contract StickerbookCollection is ERC1155, Ownable  {
    ...
    mapping(uint16 => Joker)                           public jokerInfo;
    ...
    mapping(uint16 => IUtilityTrait)                    public _implementers;
    ...
    function _newStickerBook(
        ...
        if (_clearBadge) {
            console.log("granting access to trait ",_traitToClear);
        ...
        jokerInfo[bookPos] = Joker(acceptsJoker,jokerID);

        (,,,,,address storageImplementer,,) = reg.traits(_traitToClear);
        if (address(_implementers[_traitToClear]) == address(0)) {
            if (storageImplementer == address(0)) {  // boolean
                _implementers[_traitToClear] = IUtilityTrait(address(reg));
            } else {
                _implementers[_traitToClear] = IUtilityTrait(storageImplementer);
            }
        }
    }
    ...
    function convert(stickerInput memory si) internal returns (sticker memory sm) {
        ...
        if (sm.visual) {
            if (address(_implementers[si.traitID])== address(0)) {
                _implementers[si.traitID] = IUtilityTrait(ts.storageImplementer);
            }
            if (
        ...
        if (address(_implementers[si.traitID]) == address(0)) {
            if (ts.storageImplementer == address(0)) {  // boolean
                _implementers[si.traitID] = IUtilityTrait(address(reg));
            } else {
                _implementers[si.traitID] = IUtilityTrait(ts.storageImplementer);
            }
    ...
    function claim(string calldata name, uint16[] calldata tokenIds) external {
        ...
        if (!_clearBadge) {
            ...
            for (uint16 pos = 0; pos < tokenIds.length; pos++) {
                if (nft.ownerOf(tokenIds[pos]) != msg.sender) 
                    revert NotOwnerOfToken(tokenIds[pos], msg.sender);
            ...
            address imp = address(_implementers[_traitToClear]);
            
            if (imp == address(reg)) { // boolean
                badgeImplementer.HandleBadges(tokenIds,_traitToClear);
            } else {
                
                IUtilityTrait implementer = IUtilityTrait(imp);
                console.log("removing traits");
                uint256[] memory removed = implementer.removeTraits(tokenIds,1);
                console.log("removed ",removed.length,"traits");
                for (uint rm = 0; rm < removed.length; rm++) {
                    require(removed[rm] == 1,"StickerbookCollection : badge removal failed");
                }
            }
            for (uint16 pos = 0; pos < tokenIds.length; pos++) {
        ...
        for (uint16 pos = 0; pos < tokenIds.length; pos++) {
            ...
            for (uint256 criteria = 0; criteria < numberOfCriteria; criteria++) {
                ...
                if (st.visual) {
                    ...
                    //console.log("checking trait ",st.traitID,"for token",tokenId);
                    traitNum = _implementers[st.traitID].hasTrait(tokenId) ? 1 : 0;
                    //console.log("trait ",st.traitID,"has trait ",traitNum);
                }
    ...
    function addFullStickerBook(fullStickerBookData calldata fsbd) external onlyAllowed(STICKERBOOK_ADMIN) {
        _newStickerBook(fsbd.name,fsbd.maxRedemptions,fsbd.clearBadge, fsbd.traitToClear, fsbd.ipfsHash,fsbd.acceptsJoker,fsbd.jokerID);
    ...
    function hasRole(bytes32 key, address user) public view returns (bool) {
        return 
            myCommunityRegistry.isUserCommunityAdmin(key, user) ||
            myCommunityRegistry.hasRole(key,user);
    }
...
// Chain: POLYGON - File: contracts/stickerbook/IStickerbookCollection.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/stickerbook/StickerbookUtils.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
contract StickerbookUtils {
    ...
    function getBadges(string memory name,uint16[] memory tokenIds) internal view returns(bool[] memory result) {
        ...
        if (storageImplementer == address(0)) {
            for(uint j = 0; j < tokenIds.length; j++) {
                result[j] = reg.hasTrait(st.traitToClear,tokenIds[j]);
            }
            return result;
        }
        IUtilityTrait iut = IUtilityTrait(storageImplementer);
        for(uint j = 0; j < tokenIds.length; j++) {
    ...
    function meetsCriteria(string memory name, uint16 tokenId, uint16 pos) public view returns (bool) {
        ...
        for (uint256 criteria = 0; criteria < numberOfCriteria; criteria++) {
            ...
            IUtilityTrait imp = stickerbook()._implementers(st.traitID);
            ...
            if (st.visual) {
                ...
                traitNum = stickerbook()._implementers(st.traitID).hasTrait(tokenId) ? 1 : 0; 
    ...
    function _getStickerbookData(uint16 id) internal view returns (fullStickerBookInfo memory fsbd) {
        ...
        fsbd.ipfsHash = st.ipfsHash;
         ...
         (fsbd.acceptsJoker, fsbd.jokerID) = stickerbook().jokerInfo(id);
    ...
    function _checkConditional(uint16 stickerBookID, uint16 pos, uint16 tokenId) internal view returns (bool) 
    {
        ...
        for (uint256 criteria = 0; criteria < numberOfCriteria; criteria++) {
            ...
            IUtilityTrait imp = stickerbook()._implementers(stc.traitID);
            ...
            if (stc.visual) {
                ...
                traitNum = stickerbook()._implementers(stc.traitID).hasTrait(tokenId) ? 1 : 0; 
    ...
    function checkConditionalsForSet(uint16 stickerBookID, uint16[] memory tokenIds) external view returns (bool result,bool[] memory details) {
        stickerBookInfo memory st= getStickerbookData(stickerBookID);
        ...
        for (uint16 cpos = 0; cpos < count; cpos++) {
            ...
            uint8 required = stickerbook().conditionalCount(stickerBookID,cpos);
            if(conditionalCounter < required) {
                result = false;
            } else {
                details[cpos] = true;
...
// Chain: POLYGON - File: contracts/traits/traitregistry/ECRegistryV3c.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
contract ECRegistryV3c is ProxyProtection, Ownable {
    ...
    uint                    public constant version              = 20230705;
    ...
    */
    ...
    function _setTraitUnchecked(uint16 traitID, uint16 tokenId, bool _value) internal {
        ...
        if(
            traits[traitID].traitType == 1 || 
            (
                (traits[traitID].traitType == 2) && 
                (traits[traitID].start <= tokenId && tokenId <= traits[traitID].end)
            )
        ) {
            _value = !_value; 
        }
    ...
    */
    ...
    function hasTrait(uint16 traitID, uint16 tokenId) public view returns (bool result) {
        return _hasTrait(traitID,tokenId);
    }
    function _hasTrait(uint16 traitID, uint16 tokenId) internal view returns (bool result)
    {
        (uint16 byteNum, uint8 bitPos) = getByteAndBit(tokenId);
    ...
    function haveTrait(uint16 traitID, uint16[] calldata tokenIDs) external view returns (bool[] memory result) {
        ...
        for (uint pos = 0; pos < tokenIDs.length; pos++) {
            result[pos] = _hasTrait(traitID,tokenIDs[pos]);
        }
    }
...
// Chain: POLYGON - File: contracts/traits/VisualTraitRegistry/VisualTraitRegistry.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/traits/implementers/TraitUint8ValueImplementer.sol
//SPDX-License-Identifier: Unlicensed
...
// Chain: POLYGON - File: contracts/traits/interfaces/IRegistryConsumer.sol
pragma solidity ^0.8.13;
...
// Chain: POLYGON - File: contracts/@galaxis/registries/contracts/ICommunityList.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/@galaxis/registries/contracts/CommunityRegistry.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/traits/extras/recovery/BlackHolePrevention.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/stickerbook/StickerbookData.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/stickerbook/IBadgeImplementer.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/traits/interfaces/IECRegistry.sol
//SPDX-License-Identifier: Unlicensed
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: contracts/@galaxis/registries/contracts/CommunityList.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.13;
...
// Chain: POLYGON - File: @openzeppelin/contracts/access/AccessControlEnumerable.sol
// SPDX-License-Identifier: MIT
