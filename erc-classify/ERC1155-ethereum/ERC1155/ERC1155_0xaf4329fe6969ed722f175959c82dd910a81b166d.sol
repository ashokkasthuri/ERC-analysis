...
contract ArtCard is ERC1155, IManifold {
  
  address payable constant _RUNES = payable(0x521f9C7505005CFA19A8E5786a9c3c9c9F5e6f42);

  constructor() ERC1155("") {}

  function mintFromRunes(uint256 id, uint256 amount) public {
    require(IERC721(_RUNES).ownerOf(id) == msg.sender, "Not owner");
  ...
  function getRoyalties(uint256 tokenId) external view returns (address payable[] memory, uint256[] memory) {
    ...
    recipients[0] = _RUNES;
    recipients[1] = payable(IERC721(_RUNES).ownerOf(tokenId));
