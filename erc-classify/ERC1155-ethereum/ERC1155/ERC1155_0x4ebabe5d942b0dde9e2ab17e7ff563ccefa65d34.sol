...
// File: @nftculture/nftc-contracts-private/contracts/token/v4/ERC1155_NFTC.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.11;
 ...
 */
abstract contract ERC1155_NFTC is ERC1155Burnable, OperatorFilterer, OwnableDeferral {
     ...
     */
    function setOperatorFilteringEnabled(bool value) public isOwner {
...
// File: @nftculture/nftc-contracts-private/contracts/token/v4/minter-extensions/FullInteractiveNFTMinter_1155Fixed.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.11;

// Local References
import '../ERC1155_NFTC.sol';
...
/**
 * @title FullInteractiveNFTMinter_1155Fixed
 * @author @NiftyMike | @NFTCulture
 * @dev Minter for mints that can be directly interacted with by users, the full version.
 *
 * Note: This implementation should only be used by a subset of 1155 collections that use a fixed tokenType.
 * By enforcing a fixed tokenType, we can apply the same API for minting dApps.
 *
 *  - INFTCNonFungibleToken: Important mint functions for Base URI Management
 *    and Query Helpers
 */
abstract contract FullInteractiveNFTMinter_1155Fixed is ERC1155_NFTC, INFTCNonFungibleToken, INFTCInteractiveMintFull {
    string private _artUri; // For tracking changes to the base uri()
    uint256 private _fixedTokenId;
    uint64 private _totalSupply;
    uint64 private _numberMinted;

    constructor(uint256 __fixedTokenId) {
        _fixedTokenId = __fixedTokenId;
    }

    function _getFixedTokenId() internal view returns (uint256) {
        return _fixedTokenId;
    }

    function setArtURI(string memory newuri) external override isOwner {
        _artUri = newuri;
        _setURI(newuri); // also set it as the contract's base uri.
    }
    ...
    function totalTokensExist() external view override returns (uint256) {
        return _totalTokensExist();
    }

    function _totalTokensExist() internal view returns (uint256) {
        return _totalSupply;
    }

    function balanceOwnedBy(address tokenOwner) external view override returns (uint256) {
        return super.balanceOf(tokenOwner, _fixedTokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            for (uint256 i = 0; i < tokenIds.length; ) {
                unchecked {
                    numberMinted += uint64(amounts[i]);
                    ++i;
                }
            }

            _numberMinted += numberMinted;
            _totalSupply += numberMinted;
        }
        ...
        if (to == address(0)) {
            ...
            for (uint256 i = 0; i < tokenIds.length; ) {
                unchecked {
                    numberBurned += uint64(amounts[i]);
                    ++i;
                }
            }

            if (_totalSupply < numberBurned) revert InvalidBurnAmount();
            _totalSupply -= numberBurned;
        }
    }
}
...
// File: contracts/NBNFTA2/NFTBookArtistTwoBase.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.11;

// NFTC Prerelease Contracts
import '@nftculture/nftc-contracts-private/contracts/metadata/v2/metadata-consumers/SingletonMetadataConsumer.sol';
import '@nftculture/nftc-contracts-private/contracts/token/v4/minter-extensions/FullInteractiveNFTMinter_1155Fixed.sol';
...
/**
 * @title NFTBookArtistTwoBase
 * @author @NiftyMike | @NFTCulture
 * @dev ERC721a Implementation with @NFTCulture standardized components.
 *
 * Public Mint is Phase Three.
 */
abstract contract NFTBookArtistTwoBase is
    IMintable,
    FullInteractiveNFTMinter_1155Fixed,
    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        uint256 __fixedTokenId,
        uint256[] memory __phasePrices
    )
        ERC1155(__baseURI)
        FullInteractiveNFTMinter_1155Fixed(__fixedTokenId)
        StaticPhasedMintThree(__phasePrices[0], __phasePrices[1], __phasePrices[2])
    {
        _registerForOperatorFiltering();
        operatorFilteringEnabled = true;
    }
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        if (tokenId != _getFixedTokenId()) revert InvalidTokenId();

        // ERC1155: All tokens get the exact same metadata.
        uint64 tokenType = _getMetadataProducer(address(this), tokenId).getTokenTypeForToken(address(this), tokenId);
        return
            _getMetadataProducer(address(this), tokenId).getJsonAsEncodedString(address(this), tokenType, tokenId, 0);
    }
    ...
    function _internalMintTokens(
        ...
        // ERC1155 does not distinguish between bookNumbers.
        _mint(minter, _getFixedTokenId(), count, '');
    }
}


// File: contracts/NBNFTA2/NFTBookArtistTwoSplitsAndRoyalties.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.11;
...
abstract contract NFTBookArtistTwoSplitsAndRoyalties is NFTCSplitsAndRoyalties {
...
// File: contracts/NBNFTA2/OnNFTsDmitriCherniak.sol
// SPDX-License-Identifier: MIT
...
// Local References
import './NFTBookArtistTwoBase.sol';
import './NFTBookArtistTwoSplitsAndRoyalties.sol';
 ...
 *
 * On NFTs Art Edition - Dmitri Cherniak - Published by Taschen
 */
contract OnNFTsDmitriCherniak is
    NFTBookArtistTwoSplitsAndRoyalties,
    NFTBookArtistTwoBase,
    ERC7572_ContractMetadata,
    OwnableDeferralResolution
{
    // Mainnet
    string private constant DEFAULT_CONTRACT_METADATA_URI =
        'https://arweave.net/dKFDckMXIyJ0os3KZYLh2Ung9dJd5DoTME-Re0Q7LAU/onnfts-ae2.json'; // contract-metadata v1.0.4
    ...
    uint256 private constant FIXED_TOKEN_ID = 1; // This is an ERC1155_Fixed contract, and will host 1 single tokenId.

    constructor()
        NFTBookArtistTwoBase(DEFAULT_ART_URI, FIXED_TOKEN_ID, DEFAULT_PHASE_PRICES)
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC1155, ERC2981, ERC7572_ContractMetadata) returns (bool) {
        return
