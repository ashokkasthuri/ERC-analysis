// File: /contracts/JardinXV1_0_2.sol
...
import "./AdministrationV1_0_1.sol";

contract JardinXV1_0_2 is Initializable, AdministrationV1_0_1, ERC1155Upgradeable, ReentrancyGuardUpgradeable
{
    // libraries
    ...
    uint256 public currentEscrow;

    uint256 internal TOKEN_VALUE;

    Stake[] public _stakeList;
    Collections[] public _collectionList;
    ...
    event TradeSucceed(address[] bidder, address[] asker, uint256[] cid, uint256[] amountBid, uint256[] amountAsk);
    ...
    function stake(ERC721Upgradeable nft, uint256[] calldata tokenIds) external whenPauseSA onlyWhitelisted {
        _stake(nft, tokenIds);
    ...
    function unstake(address from, ERC721Upgradeable nft, uint256 tokenIds, uint256 status) external onlySAnA {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
    ...
    function deposit(address to, uint256 amount) external payable nonReentrant whenPauseSA onlyWhitelisted {
        uint256 sentWei = msg.value;
        require(msg.sender == to, "not owner");
        require(sentWei == amount.add(_txFee), "amount must equal to payable amount");
    ...
    function requestWithdrawApproval(uint256 amount) external payable nonReentrant whenPauseSA onlyWhitelisted {
    ...
    function requestUnstakeApproval(ERC721Upgradeable nft, uint256 tokenId) external payable nonReentrant whenPauseSA onlyWhitelisted {
        require(msg.sender != address(0), "not valid sender");
        require(msg.value >= _txFee, "not enough value");
    ...
    function requestTransferApproval(address to, ERC721Upgradeable[] calldata nft, uint256[] calldata amount) external payable nonReentrant whenPauseSA onlyWhitelisted {
    ...
    function trade(address[] memory bidder, address[] memory asker, uint256[] memory cid, uint256[] memory amountBid, uint256[] memory amountAsk, uint256[] memory sumAmountBid, address[] memory distinctAddr) external onlySAnA {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
        
        for(uint256 i = 0; i < bidder.length; i++){
            _transferTrade(asker[i], bidder[i], cid[i], amountAsk[i], "0x");
            _transferTrade(bidder[i], asker[i], 0, amountBid[i], "0x");
        }
        for(uint256 j = 0; j < distinctAddr.length; j++){
            _transferTrade(distinctAddr[j], owner(), 0, uint256((sumAmountBid[j] * _txRate) / 10000).add(_txFee), "0x");
        }
    ...
    function addCollection(ERC721Upgradeable nft, string memory currencyName) external onlySAnA {
        ...
        _collectionList.push(Collections({
            nft: nft,
            cid: _cid,
            currencyName: currencyName,
            totalStaked: 0,
    ...
    function deleteCollection(ERC721Upgradeable nft) external onlySuperAdmin whenPauseSA {
        ...
        for (uint256 i = 0; i < _collectionList.length; i++) {
            if (_collectionList[i].cid == collections[nft].cid) {
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public override nonReentrant onlySAnA {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
        require(keccak256(abi.encodePacked(string(data))) == keccak256(abi.encodePacked("\x01")), "transfer denied");
        
        super.safeTransferFrom(from, to, id, amount, data);
    }

    function safeBatchTransferFrom(address from, address to, uint256[] memory id, uint256[] memory amount, bytes memory data) public override nonReentrant onlySAnA {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
        require(keccak256(abi.encodePacked(string(data))) == keccak256(abi.encodePacked("\x01")), "batch transfer denied");
    ...
    function showCollectionList() public view returns (Collections[] memory) {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
    ...
    function showStakeList() public view returns (Stake[] memory) {
        require(_pause == 0 || admin[msg.sender] == 1 || owner() == msg.sender, "Pausable: paused");
    ...
    function _stake(ERC721Upgradeable nft, uint256[] calldata tokenIds) internal virtual whenPauseSA {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            vault[nft][tokenIds[i]] = Stake({
                tokenId: uint48(tokenIds[i]),
                cid: uint48(collections[nft].cid)
            });
    ...
    function _unstake(address from, ERC721Upgradeable nft, uint256 tokenIds) internal virtual whenPauseSA {    
        ...
        collections[nft].totalStaked = collections[nft].totalStaked.sub(1);
        ...
        for(uint256 i =0; i < _stakeList.length; i++){
            if(collections[nft].cid == _stakeList[i].cid && tokenIds == _stakeList[i].tokenId){
                delete _stakeList[i];
...
// File: /node_modules/@openzeppelin/contracts-upgradeable/utils/math/SafeMathUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// File: /contracts/AdministrationV1_0_1.sol
// SPDX-License-Identifier: MIT
 ...
 * the owner.
 */
abstract contract AdministrationV1_0_1 is Initializable, ContextUpgradeable {
    address private _owner;
    mapping(address => uint256) internal admin;
    mapping(address => uint256) internal blacklist;
    address[] internal adminArr;
    address[] internal blacklistArr;
    ...
    function deleteAdmin(address _addr) public onlySuperAdmin {
        require(admin[_addr] == 1, "Admin not found.");
    ...
    function addBlacklist(address _addr) public onlySAnA {
        require(
            blacklist[_addr] == 0,
            "Address already registered"
        );
        require(_addr != owner(), "cannot blacklist owner");

        blacklistArr.push(_addr);
        blacklist[_addr] = 1;
    }
    ...
    function deleteBlacklist(address _addr) public onlySuperAdmin {
        require(blacklist[_addr] == 1, "Admin not found.");

        delete blacklist[_addr];
        for (uint256 i = 0; i < blacklistArr.length; i++) {
            if (blacklistArr[i] == _addr) {
                delete blacklistArr[i];
    ...
    modifier onlySAnA() {
        require(admin[msg.sender] == 1 || owner() == msg.sender, "OSANA");
        _;
    }

    modifier onlyWhitelisted() {
        require(blacklist[msg.sender] == 0, "Blacklisted");
        _;
    }
