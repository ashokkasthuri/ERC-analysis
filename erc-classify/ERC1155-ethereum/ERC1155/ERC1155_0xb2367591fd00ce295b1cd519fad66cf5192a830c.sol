// File: src/ethereum/eth-vaults/convex-frax-asset/asset-2/HourglassFraxBPReceipt.sol
...
contract HourglassFraxBPReceipt is AccessControl, ERC1155 {
    ...
    function earned(uint256[] calldata tokenIds) external view returns (address[][] memory rewardTokens, uint256[][] memory amounts) {
        ...
        for (uint256 i; i < numIds; i++) {
            if (tokenMetadata[tokenIds[i]].depositVault == address(0)) revert TokenIdNotInitialized();
    ...
    function _beforeTokenTransfer(
        ...
        if (to == address(0)) {
            for (uint256 i; i < numIds; ++i) {
                ...
                if (supply < amount) revert ERC1155SupplyGTAmount();
                unchecked {
        ...
        for (uint256 i; i < numIds; i++) {
            if(block.timestamp < ids[i]) {
                ...
                if(!ICustodian(assetMetadata.custodian).vaultMatured(tokenMetadata[ids[i]].depositVault)) {
...
// File: src/interfaces/IProxyVault.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;
...
// File: src/interfaces/ICustodian.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;
