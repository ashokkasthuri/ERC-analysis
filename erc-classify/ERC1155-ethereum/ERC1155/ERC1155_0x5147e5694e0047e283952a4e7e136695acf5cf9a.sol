// File: /contracts/RengaTreasures.sol
...
/// @custom:security-contact admin@pagzi.com
contract RengaTreasures is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155SupplyUpgradeable {
     ...
     * All methods are onlyOwner, tested for up to 300 drops per tx.
     * @dev Airdrop a token to each address in the calldata list,
     * setting the supply to the length of the list + already minted.
     * @param _dropId The tokenID to airdrop.
     * @param _list address[] list of wallets to airdrop to.
     */
    function airdrop(uint256 _dropId, address[] calldata _list)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _list.length; i++) {
            _mint(_list[i], _dropId, 1, "");
        }
    }

    /**
     * @notice Sends multiple tokens to a single address.
     * @param _dropId The token id to send.
     * @param _address The address to receive the tokens.
     * @param _quantity The amount of tokens to send.
     */
    function batchMint(
        uint256 _dropId,
        address _address,
        ...
        _mint(_address, _dropId, _quantity, "");
    }
