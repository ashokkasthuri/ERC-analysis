...
// File @openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol@v4.7.3
...
// File contracts/CryptorviaERC1155.sol
    ...
    ERC1155Supply
{
    ...
    uint256 public minReserveBalance = 2 ether;
    address private masterWallet;
    address private fundWallet;
    mapping(string => uint256) public SkuToId;
    mapping(uint256 => string) public IdToSku;
    mapping(address => bool) supportedTokens;
    string public URIExtension;

    // Events
    event MasterWalletUpdated(address masterWallet);
    event FundWalletUpdated(address fundWallet);
    event MinReserveBalanceUpdated(uint256 minReserveBalance);
    event URIUpdated(string URI);
    event URIExtensionUpdated(string extension);
    event FundsTransferred(address wallet, address token, uint256 value);
    event RefundBatch(address to, uint256[] ids, uint256[] amounts);
    ...
    function getMinReserveBalance() public view returns (uint256) {
        return minReserveBalance;
    }

    // Min Reserve Balance
    function setMinReserveBalance(
        uint256 newMinReserveBalance
    ) public onlyOwner {
    ...
    // Fund Wallet
    function setFundWallet(address newFundWallet) public onlyOwner {
        fundWallet = newFundWallet;
        emit FundWalletUpdated(fundWallet);
    }

    function getFundWallet() public view returns (address) {
        return fundWallet;
    }
    ...
    function contractURI() public view returns (string memory) {
        string memory _uri = string(
            abi.encodePacked(super.uri(0), "main", URIExtension)
        );
        return _uri;
    }
    ...
    function setExtension(string memory newURIExtension) public onlyOwner {
        ...
        emit URIExtensionUpdated(newURIExtension);
    }
    ...
    function uri(
        ...
        string memory _uri = string(
            abi.encodePacked(super.uri(id), IdToSku[id], URIExtension)
        );
        return _uri;
    }
    ...
    function _mintToken(
        ...
        uint256 _paymentToken = uint256(uint160(paymentToken));
        uint256 _to = uint256(uint160(to));
        require(
            verifySigner(_to, sku, _paymentToken, price, timestamp, signature),
            "Invalid Signer"
        );
    ...
    function mintBatch(
        ...
        // Check arrays length
        require(skus.length == prices.length, "SKUs and prices size mismatch.");
        require(
            skus.length == amounts.length,
            "SKUs and amounts size mismatch."
        );
        require(
            skus.length == signatures.length,
            "SKUs and signature size mismatch."
        );
        ...
        uint256 _to = uint256(uint160(to));
        // return;
        uint256[] memory ids = new uint256[](skus.length);
        for (uint i = 0; i < skus.length; i++) {
            ...
            _verifyTimestamp(timestamps[i]);
            uint256 id = SkuToId[skus[i]];
            if (id == 0) {
                _tokenIdCounter.increment();
                id = _tokenIdCounter.current();
                SkuToId[skus[i]] = id;
            ...
            _totalPrice = _totalPrice + (prices[i] * amounts[i]);
            ids[i] = id;
        }
        if (paymentToken == address(0)) {
            require(msg.value >= _totalPrice, "Batch: Insufficient value");
            transferFunds();
        } else {
            require(
                supportedTokens[paymentToken],
    ...
    function refundBatch(
        ...
        uint256[] memory ids = new uint256[](skus.length);
        uint256 _paymentToken = uint256(uint160(paymentToken));
        uint256 _from = uint256(uint160(from));
        for (uint i = 0; i < skus.length; i++) {
            // Verify Signature
            require(
                verifySigner(
        ...
        if (paymentToken != address(0)) {
            ...
            emit FundsTransferred(from, paymentToken, _totalPrice);
        } else {
            require(
                address(this).balance >= _totalPrice,
                "Insufficient reserve balance"
            );
            payable(from).transfer(_totalPrice);
            emit FundsTransferred(from, address(0), _totalPrice);
        }
        _burnBatch(from, ids, amounts);
        emit RefundBatch(from, ids, amounts);
    }
    ...
    function verifySigner(
        ...
        return signer == masterWallet;
    }

    function _verifyTimestamp(uint256 _timestamp) private view {
        require(_timestamp <= block.timestamp, "future timestamp");
        require(
            _timestamp >= (block.timestamp - (10 minutes)),
            "signature expired"
        );
    }
    ...
    function withdraw() public onlyOwner {
        ...
        emit FundsTransferred(fundWallet, address(0), balance);
        return;
    }

    function transferFunds() internal {
        if (address(this).balance > minReserveBalance) {
            payable(fundWallet).transfer(
                address(this).balance - minReserveBalance
            );
