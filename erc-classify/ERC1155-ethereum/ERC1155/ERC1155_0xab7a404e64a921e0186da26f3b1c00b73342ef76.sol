// File: contracts/SolaNFT1155.sol
...
interface ISolarrWhitelistInterface {
    function isAccountWhitelisted(address _address) external view returns (bool);
}

contract SolaNFT1155 is ERC1155 {

    ISolarrWhitelistInterface solarrWhitelist;
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix, address _whitelist) ERC1155 (name, symbol) {
        owner = msg.sender;
        solarrWhitelist = ISolarrWhitelistInterface(_whitelist);
    ...
    modifier isAccountWhitelisted(address _accountAddress) {
        require(solarrWhitelist.isAccountWhitelisted(_accountAddress), "Only whitelisted user can mint!");
        _;
    }
    ...
    */
    ...
    function mint(string memory uri, uint256 supply, uint256 fee, Sign memory sign)  external isAccountWhitelisted(msg.sender) returns (uint256) {
