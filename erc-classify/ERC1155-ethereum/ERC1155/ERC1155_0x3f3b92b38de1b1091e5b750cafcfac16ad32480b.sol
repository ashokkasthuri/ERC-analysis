...
╚█████╔╝╚█████╔╝██║░╚███║╚█████╔╝███████╗██║░░░░░░░░██║░░░
░╚════╝░░╚════╝░╚═╝░░╚══╝░╚════╝░╚══════╝╚═╝░░░░░░░░╚═╝░░░
https://tao404.network
https://t.me/tao404_network
https://twitter.com/tao404network
 */
...
contract Token is ERCX {
    ...
    uint256 private constant _totalTokens = 21000;
    uint256 private constant _tokensPerNFT = 1;
    string private constant _name = "New Tensor Concept";
    string private constant _ticker = "TAO404";
    ...
    function switchDelay() external onlyOwner {
    ...
    function nftURI(uint256 id) public view returns (string memory) {
        ...
        if (bytes(baseTokenURI).length > 0)
            ...
            string memory image = "";
            string memory neuralType = "";
            string memory description = "";

            if (seed < 37) {
                image = "ANHECEU.jpeg";
                neuralType = "1";
                description = "Bridging NFTs with fungible tokens for a revolutionary leap on the Tao network. Explore a realm where each token is unique yet interconnected, shaping the future of finance, art, and technology";
            } else if (seed < 73) {
                image = "amtqH5w.jpeg";
                neuralType = "2";
                description = "Bridging NFTs with fungible tokens for a revolutionary leap on the Tao network. Explore a realm where each token is unique yet interconnected, shaping the future of finance, art, and technology";
            ...
            string memory jsonPreImage = string(
                abi.encodePacked(
                    '{"name": "New Tensor Concept #',
                    id.toString(),
                    '","description":"',
                    description,
                    '","external_url":"https://tao404.network","image":"',
                    dataURI,
                    image
                )
            );
            return
                string(
                    abi.encodePacked(
                        ...
                        jsonPreImage,
                        '","attributes":[{"trait_type":"NeuralType","value":"',
                        neuralType,
                        '"}]}'
                    )
                );
