    ...
    ReentrancyGuard
{
    ...
    uint256 public unwrapFee = 500; // e.g., 500 for 5%
    uint256 public _accFee;
    uint256 private _feeThreshold = 10; // e.g., 10 for 0.1%
    ...
    function setFeeRecipient(address payable wallet_) external onlyOwner {
        _feeRecipient = wallet_;
    }
    ...
    /**
     * @dev Return expected token amount to unwrap
     *
     * @param amount_ amount of MFTs to unwrap
     */
    function prepareUnwrap(uint256 amount_) external view returns (uint256) {
        if (amount_ == 0) revert UnwrapZeroAmount();
        uint256 fee = _calculateFee(amount_, unwrapFee);
        require(fee > 0, "Invalid Operation");

        return _tokenAmount * amount_ - fee;
    }

    /**
     * @dev Unwrap mft to token
     *
    ...
    function unwrap(uint256 amount_) public virtual nonReentrant {
        ...
        _erc20Token.transfer(msg.sender, _tokenAmount * amount_ - fee);

        _accFee = _accFee + fee;
        if (_accFee >= ((_erc20Token.totalSupply() * _feeThreshold) / 10000)) {
            _swapTokensForEth(_accFee);
            _accFee = 0;
        }
    ...
    /**
     * @dev Calculate Fee with presets
     *
     * @param amount_ amount of tokens
     * @param feePercentage_ percentage of amount as fee
     */
    function _calculateFee(
        uint256 amount_,
        uint256 feePercentage_
    ) private view returns (uint256) {
        return (amount_ * _tokenAmount * feePercentage_) / 10000;
    }
    ...
    function _swapTokensForEth(uint256 tokenAmount_) private {
        ...
        path[0] = address(_erc20Token);
        path[1] = uniswapV2Router.WETH();
        _erc20Token.approve(address(uniswapV2Router), tokenAmount_);
    ...
    function manualSwap() external {
        require(_msgSender() == _feeRecipient, "Invalid operation");
        if (_accFee > 0) {
            _swapTokensForEth(_accFee);
            _accFee = 0;
        }
...
// File contracts/RIZOMFTERC1155.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity 0.8.25;

contract RIZOMFTERC1155 is ERC69_1155 {
    constructor(
        string memory name,
        string memory symbol,
        string memory baseURI,
        address erc20Token,
        uint256 tokenAmountPerMFT,
        uint256 maxSupply
    ) ERC69_1155(name, symbol, baseURI, erc20Token, tokenAmountPerMFT, maxSupply) {}
}