// File: contracts/ParalismOpenToken.sol
 ...
 * ParalismOpenToken - ERC1155 contract that whitelists an operator address, has create and mint functionality, and supports useful standards from OpenZeppelin,
  like _exists(), name(), symbol(), and totalSupply()
 */
contract ParalismOpenToken is ContextMixin, ERC1155, NativeMetaTransaction, Ownable {
  ...
  modifier creatorOnly(uint256 _id) {
    require(creators[_id] == _msgSender(), "ParalismOpenToken#creatorOnly: ONLY_CREATOR_ALLOWED");
    _;
  }
  ...
  modifier ownersOnly(uint256 _id) {
    require(balanceOf(_msgSender(), _id) > 0, "ParalismOpenToken#ownersOnly: ONLY_OWNERS_ALLOWED");
    _;
  }
  ...
  function uri(
    ...
    require(_exists(_id), "ParalismOpenToken#uri: NONEXISTENT_TOKEN");
  ...
  /**
   * @dev Sets a new proxy contract to enable gas-free listings
   * @param _proxyRegistryAddress New proxy contract address
   */
  ...
  function batchMint(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      uint256 _id = _ids[i];
      require(creators[_id] == _msgSender(), "ParalismOpenToken#batchMint: ONLY_CREATOR_ALLOWED");
  ...
  function setCreator(
    ...
    require(_to != address(0), "ParalismOpenToken#setCreator: INVALID_ADDRESS.");
    for (uint256 i = 0; i < _ids.length; i++) {
      uint256 id = _ids[i];
      _setCreator(_to, id);
    }
  ...
  function isApprovedForAll(
    ...
    // Whitelist Paralism proxy contract for easy trading.
  ...
  function exists(
    ...
    /**
     * This is used instead of msg.sender as transactions won't be sent by the original token owner, but by Paralism.
     */
    function _msgSender()
        internal
        override
