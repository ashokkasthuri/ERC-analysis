...
// File: contracts/contracts-upgradeable/interfaces/IRegistry.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/contracts-upgradeable/ShipParts.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function claimParts(
    ...
    uint256[] calldata partAmounts,
    bytes memory signature
  )
  external
  nonReentrant
  {
    if (!_verify(msg.sender, nonce, partIds, partAmounts, signature)) {
      revert MismatchedSignature();
    }

    if (currentNonce[msg.sender] >= nonce) {
  ...
  internal
  {
    currentNonce[msg.sender] = nonce;
    _mintBatch(msg.sender, partIds, partAmounts, "");
  }
