...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function safeTransferFrom(
        ...
        require(true == false, "reverted for security");
...
contract WalletBuddyLock is ERC1155, ERC1155Supply {
    ...
    constructor() ERC1155("https://bafybeigghh45qy7vxctvawqrxigqoow4ny7laophwq3jcqescjo6xkwtzq.ipfs.nftstorage.link/") {
        WalletBuddyMaker = 0x928bA2d5ee8a6780e8e32115e37C62DE0a038ffE;
        Developer = 0xF5226f23063F3a40Ef21cdA06EB0226d72cfB57E;
        ClonePrice = 1*10**15; //0.001 ETH
        ThisNFT = IERC1155(address(this));
        name = "Wallet Buddy Keys";
    }


    function setURI(string memory newuri) public {
        address user = msg.sender;
        require(user == Developer, "Sender is not an operator");
        _setURI(newuri); // incase i need to add more metadata / update the looks (this can't change token ID)
    }
    ...
    function safeTransferFrom(
        ...
        require(LockedWalletBuddies[id] == false, "Owner has locked the transfer of all their wallet buddy keys for this wallet buddy");
    ...
    function ArrayLength(address user) public view returns(uint256) {
        return allOwnedTokens[user].length;
    }
    function find(uint256 id, address user) public view returns(uint256) {
        uint256[] storage array = allOwnedTokens[user];
        uint256 result = 0;
    ...
    for (uint i = 0; i < allOwnedTokens[user].length; i++) {
        if (allOwnedTokens[user][i] == num) {
            return true;
        }
    }
    ...
    function PauseTransfers(uint256 id) public {
        address user = msg.sender;
        require(WalletBuddyCreator[id] == user, "This wallet did not create this wallet buddy");
    ...
    function CreateLock(address account, uint256 id, bytes memory data) public {
        ...
        require(CreatedWalletBuddies[id] != true, "This wallet buddy lock was already created");
    ...
    function withdrawETH(address payable _reciever) public {
        require(msg.sender == Developer && _reciever == Developer, "Only the developer has permission to rescue eth");
