...
// File: contracts/CostaRicaStamps.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.20;
...
contract CostaRicaStamps is AccessControl, ReentrancyGuard,ERC1155URIStorage, ERC2981 {
    ...
    function mintBatch(uint256[] memory id) external payable nonReentrant isPaused {
        ...
        uint finalAmount = price * amount;
         if(msg.value != finalAmount){
            revert wrongPrice();
        }

        safeTransferNative(finalAmount);
    ...
    function updatePrice(uint256 newPrice) public admin {
     ...
     function safeTransferNative( uint256 value) private {
        (bool success, ) = _owner.call{value: value}(new bytes(0));
