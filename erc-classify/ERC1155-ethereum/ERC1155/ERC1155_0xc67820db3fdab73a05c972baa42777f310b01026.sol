...
// File: contracts/ClickToken.sol
//SPDX-License-Identifier: NONE
pragma solidity ^0.8.18;

import "./manifold-creator-core/ERC1155Creator.sol";

error ClickIsSoulbound();
error MarketplaceCanOnlyMoveClick();

contract ClickToken is ERC1155Creator {

    uint256 public constant CLICK_TOKEN_ID = 1;
    ...
    // Adds an immutable pre-approval for CLICK marketplace contract.
    ...
    function safeBatchTransferFrom(
        ...
        } else if (!(ids.length == 1 && ids[0] == CLICK_TOKEN_ID)) {
            revert MarketplaceCanOnlyMoveClick();
        }
    ...
    // Ensures CLICK is soulbound, except for approved operators.
    ...
    function _arrayHasClickToken(uint256[] memory ids) internal pure returns (bool) {
    ...
    function setAllowedOperator(address operator, uint256 status) external onlyOwner {
        allowedOperators[operator] = status;
    }

    function airdropClicks(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
...
// File: contracts/manifold-creator-core/ERC1155Creator.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold-creator-core/token/ERC1155/ERC1155Base.sol
// SPDX-License-Identifier: MIT
