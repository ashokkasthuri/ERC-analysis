...
// File: contracts/interfaces/IRetrobridgeNFT.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

interface IRetrobridgeNFT {
...
// File: contracts/RetrobridgeNFT.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "./interfaces/IRetrobridgeNFT.sol";
import "@openzeppelin/contracts/access/Ownable2Step.sol";
import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155URIStorage.sol";

contract RetrobridgeNFT is Ownable2Step, ERC1155Supply, ERC1155URIStorage, IRetrobridgeNFT {
    ...
    function setWhitelist(uint256 nftId, address account, bool whitelisted) public onlyOwner {
        checkId(nftId);
        require(whitelist[nftId][account] != whitelisted, "RetrobridgeNFT: not changing whitelist state");
    ...
    function safeTransferFrom(address, address, uint256, uint256, bytes memory) public pure override {
        revert("RetrobridgeNFT: safeTransferFrom is forbidden");
    }
    ...
    function safeBatchTransferFrom(
        ...
        revert("RetrobridgeNFT: safeBatchTransferFrom is forbidden");
    }
    ...
    function mint(address to, uint256 nftId, uint256 value) public {
        checkId(nftId);
        require(whitelist[nftId][msg.sender], "RetrobridgeNFT: msg.sender not whitelisted");
    ...
    function mintBatch(address to, uint256[] memory nftIds, uint256[] memory values) public {
        for(uint256 i = 0; i < nftIds.length; i++) {
            checkId(nftIds[i]);
            require(whitelist[nftIds[i]][msg.sender], "RetrobridgeNFT: msg.sender not whitelisted");
        }
        _mintBatch(to, nftIds, values, "");
        emit MintBatch(msg.sender, to, nftIds, values);
    }

    function uri(uint256 nftId) public view override(ERC1155, ERC1155URIStorage, IRetrobridgeNFT) returns (string memory) {
        return ERC1155URIStorage.uri(nftId);
    }

    function balanceOf(address account, uint256 nftId) public view override(ERC1155, IRetrobridgeNFT) returns (uint256) {
        checkId(nftId);
    ...
    function balanceOfBatch(
        address[] memory accounts,
        uint256[] memory ids
    ) public view override (ERC1155, IRetrobridgeNFT) returns (uint256[] memory) {
        return super.balanceOfBatch(accounts, ids);
    }

    function totalSupply(uint256 nftId) public view override(ERC1155Supply, IRetrobridgeNFT) returns (uint256) {
        checkId(nftId);
        return ERC1155Supply.totalSupply(nftId);
    }

    function totalSupply() public view override(ERC1155Supply, IRetrobridgeNFT) returns (uint256) {
        return ERC1155Supply.totalSupply();
    }

    function exists(uint256 nftId) public view override(ERC1155Supply, IRetrobridgeNFT) returns (bool) {
        checkId(nftId);
        return ERC1155Supply.exists(nftId);
    }

    function checkId(uint256 nftId) public pure {
        require(nftId <= MIDNIGHT_NFT_ID, "RetrobridgeNFT: invalid nftId");
    }

    function owner() public view override(Ownable, IRetrobridgeNFT) returns (address) {
