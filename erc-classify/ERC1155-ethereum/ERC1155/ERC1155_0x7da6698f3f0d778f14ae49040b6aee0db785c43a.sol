// File: contracts/RAYA_Slmn_X_Ciele.sol
...
contract RAYA_Slmn_X_Ciele is ERC1155, Ownable, Pausable, ReentrancyGuard {
  /*
   * Public Variables
   */

  // Token name
  string public name = "Run As You Are - Salomon X Ciele | Distance Nirvana";

  uint256 public totalSupplyMinted;

  uint256 public totalMintedOne = 0;

  uint256 public totalMintedTwo = 0;
  ...
  constructor() ERC1155("") {
    totalSupplyMinted = 0;
    tokenURI = "ipfs://bafybeibvwyo77qoltj56szaum2z2mzx2lk426w3lchab6c7szwm2dan5cy/";
  }
  ...
  /// @param _tokenId RAYA x Salomon X Ciele  | Distance Nirvana token Id to view
  ...
  function adminMint(
    ...
    //potential mint amounts counter
    uint256 typeOneToMint = 0;
    uint256 typeTwoToMint = 0;

    // quantity checks
    require(totalMintedOne <= maxMints, "over_max_mint");
    require(totalMintedTwo <= maxMints, "over_max_mint");
    require(accounts.length == tokenAmounts.length, "accounts_ne_tokens_entered");

    //check for mint limit if potential mints are minted
    for (uint256 i = 0; i < accounts.length; i++) {
      // only two token types
      require(tokenIds[i] == 1 || tokenIds[i] == 2, "only token 1 or 2 available");

      if (tokenIds[i] == 1) {
        typeOneToMint += tokenAmounts[i];
      } else if (tokenIds[i] == 2) {
        typeTwoToMint += tokenAmounts[i];
      }

      require(totalMintedOne + typeOneToMint <= maxMints, "over_max_mint");
      require(totalMintedTwo + typeTwoToMint <= maxMints, "over_max_mint");
    }

    for (uint256 i = 0; i < accounts.length; i++) {
      for (uint256 j = 0; j < tokenAmounts[i]; j++) {
        //token limits counter
        if (tokenIds[i] == 1) {
          totalMintedOne++;
        } else if (tokenIds[i] == 2) {
          totalMintedTwo++;
