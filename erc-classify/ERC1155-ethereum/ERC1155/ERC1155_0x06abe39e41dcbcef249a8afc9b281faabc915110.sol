...
contract MoveMove is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, IERC1155Receiver {
    ...
    function uniSetLiquidityPair(address _tokenA, address _tokenB) external onlyOwner {
        ...
        if (existingPair == address(0)) {
            ...
            // 如果配对已存在，可以选择更新uniswapPair或仅记录已存在
            uniswapPair = existingPair;
        }
    }

    // 添加流动性
    function uniAddLiquidity(address _tokenA, address _tokenB, uint256 _amountA, uint256 _amountB) external onlyOwner {
        ...
        uint256 deadline = block.timestamp + 15 minutes; // 设置截止时间
    ...
    // 移除流动性
    function uniRemoveLiquidity(address _tokenA, address _tokenB, uint256 _liquidity) external onlyOwner {

        IERC20(uniswapPair).safeIncreaseAllowance(address(uniswapV2Router), _liquidity);
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory base = super.uri(tokenId); // 获取基础 URI
        // 如果为特定 tokenId 设置了 URI，则返回它
        if (bytes(_uri).length > 0) {
            return string(abi.encodePacked(base, _uri));
        }
        // 如果没有为特定 tokenId 设置 URI，则返回基础 URI 和 tokenId
    ...
    function mint(address account, uint256 id, uint256 amount, string memory tokenUri, bytes memory data) public onlyOwner {
        ...
        _mint(account, id, amount, data);  // 铸造代币
    ...
    function mintBatchWithUri(address to, uint256[] memory ids, uint256[] memory amounts, string[] memory tokenUris, bytes memory data) public onlyOwner {
        ...
        _mintBatch(to, ids, amounts, data);  // 批量铸造代币
    ...
    receive() external payable {
        emit Received(msg.sender, msg.value);  // 更新的receive()函数
    }
