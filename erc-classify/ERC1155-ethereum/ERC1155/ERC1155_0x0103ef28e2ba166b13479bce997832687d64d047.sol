// File: contracts/FlowtyUnchained.sol
...
contract FlowtyUnchained is ERC1155Supply, Ownable, ReentrancyGuard {
  ...
  // aging stages
  uint256[] private _stages = [18921674, 21585674, 32241674, 40233674, 50889674, 53553674, 64209674, 69537674, 85521674, 104169674, 146793674, 157449674, 165441674, 178761674, 184089674, 221385674, 226713674, 245361674];
  // Number of mints per each tokenId
  uint256 private _supply;
  // metadata URI
  string private tokenBaseURI;

  constructor(IERC20 INKTokenAddress) ERC1155("FLOWTY UNCHAINED") {
  ...
  {
    for (uint i = 0; i < 18;) {
          if (_stages[i] > block.number) {
              return i - 1; // Return the index as soon as a value less than target is found
          }
          unchecked { i++; }
      }
      return 17;
  ...
  //---------------------------------------------------------------------------------
  // We build URL is a following way: tokenBaseURI + AGE (0...period)
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    string memory _baseTokenURI = tokenBaseURI;
    return bytes(_baseTokenURI).length > 0 ? 
          string(
            abi.encodePacked(abi.encodePacked(_baseTokenURI, getAge().toString()))) : "";  }
}
