...
contract Lucky7pass is ERC1155, Ownable, ERC1155Supply, ReentrancyGuard {
    ...
    function mint(bytes32[] calldata _merkleProof) public payable {
        ...
        if (saleActive) { 
            require(msg.value >= salePrice, "Lucky7: Insufficient tx value.");
            require(!_saleMinted[msg.sender], "Lucky7: You cannot mint more than 2 in the public sale.");
            require(paidSupplyLeft > 0, "Lucky7: Exceeds available supply.");
            _mint(msg.sender, passIndex, 2, "");
            _saleMinted[msg.sender] = true;
            paidSupplyLeft -= 2;
        } else if (presaleActive) {
            require(msg.value >= presalePrice, "Lucky7: Insufficient tx value.");
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            //Check sender will only have 2 total presale mint.
            require(!_presaleMinted[msg.sender], "Lucky: You cannot mint more than 2 in the presale.");
