...
// File: contracts/ERC1155Tradable-new.sol
 ...
 * ERC1155TradableNew - ERC1155 contract that whitelists an operator address, has create and mint functionality, and supports useful standards from OpenZeppelin,
  like _exists(), name(), symbol(), and totalSupply()
 */
contract ERC1155TradableNew is ContextMixin, ERC1155, NativeMetaTransaction, Ownable {
   ...
   */
    modifier creatorOnly(uint256 _id) {
        require(creators[_id] == _msgSender(), "ERC1155TradableNew#creatorOnly: ONLY_CREATOR_ALLOWED");
        _;
    }
   ...
   */
    modifier ownersOnly(uint256 _id) {
        require(balanceOf(_msgSender(), _id) > 0, "ERC1155TradableNew#ownersOnly: ONLY_OWNERS_ALLOWED");
        _;
    }
    ...
    function uri(
        ...
        require(_exists(_id), "ERC1155TradableNew#uri: NONEXISTENT_TOKEN");
    ...
    */
    ...
    /**
      * @dev Returns the what maximum quantity for a token ID can be
    * @param _id uint256 ID of the token to query
    * @return amount of token in existence
    */
   ...
   */
    ...
    */
    function create(
        ...
        if (_supplyLimit > 0) {
            require(_initialSupply <= _supplyLimit, "token supply limit should be greater than initial supply");
        }
        ...
        tokenSupplyLimit[_id] = _supplyLimit;
        return _id;
    }
    ...
    */
    function mint(
        ...
        _increaseSupply(_id, _quantity);
        _mint(_to, _id, _quantity, _data);
    }

    /**
      * @dev supply limiting check, if supply 0 then it's unlimited
    * @param _id        token id
    * @param _quantity  amounts of new tokens
    */
    function _increaseSupply(uint256 _id, uint256 _quantity) private {
        uint256 newSupplyValue = tokenSupply[_id].add(_quantity);
        if (tokenSupplyLimit[_id] > 0) {
            require(tokenSupplyLimit[_id] >= newSupplyValue, "ERC1155Tradable:#_increaseSupply SUPPLY_OVER_LIMIT");
        }
        tokenSupply[_id] = newSupplyValue;
    }
    ...
    */
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 _id = _ids[i];
            require(creators[_id] == _msgSender(), "ERC1155TradableNew#batchMint: ONLY_CREATOR_ALLOWED");
            uint256 quantity = _quantities[i];
            _increaseSupply(_id, quantity);
        }
    ...
    */
    function setCreator(
        ...
        require(_to != address(0), "ERC1155TradableNew#setCreator: INVALID_ADDRESS.");
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 id = _ids[i];
            _setCreator(_to, id);
        }
