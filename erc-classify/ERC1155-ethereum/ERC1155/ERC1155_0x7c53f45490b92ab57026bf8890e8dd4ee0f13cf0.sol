...
contract SBT is ERC1155,Ownable {
    ...
    event SetOperator(address _operator,bool _enable);
    ...
    constructor(string memory _baseURI) Ownable(msg.sender)  ERC1155(""){
        baseURI = _baseURI;
    }

    function setOperator(address _operator, bool _enable) external onlyOwner{
        require(_operator != address(0), "bad address");
        emit SetOperator(_operator, _enable);
        operators[_operator] =  _enable;
    }

    function mint(address _to, uint _tokenId, uint _value) public onlyOperator{
        tokenSupply[_tokenId] += _value;
