...
import {
    ...
    MarketItem,
    SaleDTO
} from "../libraries/MgdMarketPlaceDataTypes.sol";
import {Counters} from "@openzeppelin/contracts/utils/Counters.sol";
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    event NftPurchasedCollaboratorAmount(uint256 indexed saleId, address collaborator, uint256 amount);

    error ItemIsNotListed(address nft);
    error ItemIsNotListedBySeller(
        uint256 tokenId, address market, address contractAddress, address seller, address msgSender
    );
     ...
     */
    mapping(address => mapping(uint256 => mapping(address => MarketItem))) public idMarketItemsByContractByOwner;
    ...
    function initialize(
        ...
        require(
            _mintGoldDustCompany != address(0) && _mintGoldDustERC721Address != address(0)
    ...
    function getManagePrimarySale(address nft, uint256 tokenId) external view returns (ManagePrimarySale memory) {
    ...
    function setMintGoldDustMarketplace(address _mintGoldDustMarketplace) external {
     ...
     */
    function list(uint256 tokenId, uint256 amount, MintGoldDustNFT nft, uint256 price) external virtual;
     ...
     */
    function _list(ListDTO memory listDTO, uint256 auctionId, address sender) internal {
        bool isERC721 = false;
        ...
        ManagePrimarySale memory managePS = listDTO.nft.getManagePrimarySale(listDTO.tokenId);
        ...
        (,,,, uint256 returnedTokenAmount,) =
            mintGoldDustMarketplace.idMarketItemsByContractByOwner(address(listDTO.nft), listDTO.tokenId, sender);

        if (!managePS.soldout && sender == managePS.owner) {
            require(listDTO.amount + returnedTokenAmount <= managePS.amount, "Invalid amount for primary sale");
        }

        AuctionProps memory auctionProps = AuctionProps(auctionId, 0, 0, payable(address(0)), 0, false);

        idMarketItemsByContractByOwner[address(listDTO.nft)][listDTO.tokenId][sender] =
            MarketItem(listDTO.tokenId, sender, listDTO.price, isERC721, realAmount, auctionProps);

        listDTO.nft.transfer(sender, address(this), listDTO.tokenId, realAmount);
    }
     ...
     */
    function _getMarketItem(SaleDTO memory saleDTO) internal view returns (MarketItem memory) {
        return idMarketItemsByContractByOwner[address(saleDTO.nft)][saleDTO.tokenId][saleDTO.seller];
    }
     ...
     */
    function _executePurchaseNftFlow(SaleDTO memory saleDTO, address sender, uint256 value) internal {
        _isTokenListed(saleDTO.tokenId, address(saleDTO.nft), saleDTO.seller);
        _mustBeMintGoldDustERC721Or1155(address(saleDTO.nft));

        _hasEnoughAmountListed(saleDTO.tokenId, address(saleDTO.nft), address(this), saleDTO.amount, saleDTO.seller);
        ...
        _checkIfIsPrimaryOrSecondarySaleAndCall(marketItem, saleDTO, value, sender, realAmount);
    }
    ...
    function _checkIfIsPrimaryOrSecondarySaleAndCall(
        ...
        ManagePrimarySale memory managePS = saleDTO.nft.getManagePrimarySale(saleDTO.tokenId);

        if ((managePS.owner == saleDTO.seller && managePS.soldout) || (managePS.owner != saleDTO.seller)) {
            _isMsgValueEnough(marketItem.price, realAmount, value, marketItem.auctionProps.auctionId);
            _secondarySale(marketItem, saleDTO, value, sender);
        } else {
            _isMsgValueEnoughPrimarySale(marketItem.price, realAmount, value, marketItem.auctionProps.auctionId);
    ...
    function _isBuyingAllListedTokens(SaleDTO memory saleDTO) internal view {
        if (
            saleDTO.amount
                < idMarketItemsByContractByOwner[address(saleDTO.nft)][saleDTO.tokenId][saleDTO.seller].tokenAmount
        ) {
    ...
    function _isNFTowner(uint256 tokenId, address sender) internal view {
        if ((MintGoldDustERC721(mintGoldDustERC721Address)).ownerOf(tokenId) != sender) {
     ...
     */
    function _isTokenListed(uint256 tokenId, address nft, address seller) internal view {
        if (idMarketItemsByContractByOwner[nft][tokenId][seller].tokenAmount == 0) {
            revert ItemIsNotListedBySeller(tokenId, address(this), nft, seller, msg.sender);
        }
        if (
            nft == mintGoldDustERC721Address
                && (MintGoldDustERC721(mintGoldDustERC721Address)).ownerOf(tokenId) != address(this)
        ) {
        ...
        if (
            nft == mintGoldDustERC1155Address
                && (MintGoldDustERC1155(mintGoldDustERC1155Address)).balanceOf(address(this), tokenId) == 0
        ) {
            revert ItemIsNotListed(nft);
        }
    }
    ...
    function _hasEnoughAmountListed(
        ...
        if (
            nft == mintGoldDustERC1155Address
                && (MintGoldDustERC1155(mintGoldDustERC1155Address)).balanceOf(marketPlaceAddress, tokenId) < tokenQuantity
        ) {
            revert LessItemsListedThanTheRequiredAmount();
        }
        if (idMarketItemsByContractByOwner[nft][tokenId][seller].tokenAmount < tokenQuantity) {
            revert LessItemsListedThanTheRequiredAmount();
        }
    }
     ...
     * @param seller is the address of the seller of this tokenId.
     */
    function _isSeller(uint256 tokenId, address nft, address seller) internal view {
        if (msg.sender != idMarketItemsByContractByOwner[nft][tokenId][seller].seller) {
            revert AddressUnauthorized("Not seller!");
        }
    }

    function _isNotListed(uint256 tokenId, address nft, address _seller) internal view {
        if (idMarketItemsByContractByOwner[nft][tokenId][_seller].tokenAmount > 0) {
     ...
     * @param sender is the address that started this flow.
     */
    function _checkBalanceForERC1155(uint256 tokenId, uint256 _tokenAmount, address sender) private view {
        if ((MintGoldDustERC1155(mintGoldDustERC1155Address)).balanceOf(sender, tokenId) < _tokenAmount) {
    ...
    function _primarySale(
        ...
        ManagePrimarySale memory mPSale = saleDTO.nft.getManagePrimarySale(saleDTO.tokenId);

        saleDTO.nft.updatePrimarySaleQuantityToSell(saleDTO.tokenId, realAmount);
        ...
        _checkIfIsSplitPaymentAndCall(saleDTO.nft, marketItem, saleDTO, balance, fee, collFee, true, netValue, sender);

        (bool successOwner,) = payable(mintGoldDustCompany.owner()).call{value: collFee + fee}("");
    ...
    function _checkIfIsSplitPaymentAndCall(
        ...
        if (nft.hasTokenCollaborators(saleDTO.tokenId)) {
            _handleSplitPaymentCall(
                nft, saleDTO, balance, fee, collFeeOrRoyalty, artistOrSeller, isPrimarySale, value, sender
            );
            return;
        }

        if (isPrimarySale) {
            _uniqueOwnerPrimarySale(nft, marketItem, saleDTO, fee, collFeeOrRoyalty, balance, value, sender);
            return;
        }

        _uniqueOwnerSecondarySale(
            marketItem, nft, saleDTO, artistOrSeller, fee, collFeeOrRoyalty, balance, value, sender
        );
    }
    ...
    function _uniqueOwnerPrimarySale(
        ...
        (bool successSeller,) = payable(marketItem.seller).call{value: balance}("");
        require(successSeller, "Transfer to seller failed.");
    }

    function _updateIdMarketItemsByContractByOwnerMapping(SaleDTO memory saleDTO) private {
        MarketItem storage item = idMarketItemsByContractByOwner[address(saleDTO.nft)][saleDTO.tokenId][saleDTO.seller];
    ...
    function _splittedSale(
        ...
        uint256 tokenIdCollaboratorsQuantity = nft.tokenIdCollaboratorsQuantity(saleDTO.tokenId);

        uint256 balanceSplitPart =
            (balanceOrRoyalty * nft.tokenIdCollaboratorsPercentage(saleDTO.tokenId, 0)) / (100e18);

        (bool successArtist,) = payable(artist).call{value: balanceSplitPart}("");
        require(successArtist, "Split tx to artist failed.");

        emit NftPurchasedCollaboratorAmount(itemsSold.current(), artist, balanceSplitPart);

        for (uint256 i = 1; i < tokenIdCollaboratorsQuantity; i++) {
            balanceSplitPart = (balanceOrRoyalty * nft.tokenIdCollaboratorsPercentage(saleDTO.tokenId, i)) / (100e18);
            address collaborator = nft.tokenCollaborators(saleDTO.tokenId, i - 1);

            (bool successCollaborator,) = payable(collaborator).call{value: balanceSplitPart}("");
            require(successCollaborator, "Split tx to collab failed.");

            emit NftPurchasedCollaboratorAmount(itemsSold.current(), collaborator, balanceSplitPart);
        }

        _updateIdMarketItemsByContractByOwnerMapping(saleDTO);
        _emitEventForSplitPayment(
            saleDTO, marketItem, nft, artist, balance, fee, collFeeOrRoyalty, _isPrimarySale, value, sender
        );
    }
    ...
    function _handleSplitPaymentCall(
        ...
        nft.transfer(address(this), sender, saleDTO.tokenId, saleDTO.amount);
        _splittedSale(balance, fee, collFeeOrRoyalty, artistOrSeller, nft, saleDTO, isPrimarySale, value, sender);
    }
     ...
     *         that we get is the highst bidder that is stored in the marketItem struct. So we need to manage this way.
     */
    function _secondarySale(MarketItem memory marketItem, SaleDTO memory saleDTO, uint256 value, address sender)
        private
    {
        itemsSold.increment();
        ...
        fee = (value * mintGoldDustCompany.secondarySaleFeePercent()) / (100e18);
        royalty = (value * saleDTO.nft.tokenIdRoyaltyPercent(saleDTO.tokenId)) / (100e18);

        balance = value - (fee + royalty);

        _checkIfIsSplitPaymentAndCall(saleDTO.nft, marketItem, saleDTO, balance, fee, royalty, false, value, sender);

        (bool successOwner,) = payable(mintGoldDustCompany.owner()).call{value: fee}("");
    ...
    function _isMsgValueEnough(uint256 price, uint256 amount, uint256 value, uint256 _auctionId) private pure {
     ...
     */
    function _isMsgValueEnoughPrimarySale(uint256 price, uint256 amount, uint256 value, uint256 _auctionId)
        private
        pure
    {
        uint256 realAmount = amount;
...
contract MintGoldDustCompany is Initializable, IERC165, OwnableUpgradeable {
    ...
    function setPublicKey(address _mintGoldDustPublicKey) external onlyOwner isZeroAddress(_mintGoldDustPublicKey) {
        publicKey = _mintGoldDustPublicKey;
    }

    /// @notice Add new validators to Mint Gold Dust Company
    function setValidator(address _address, bool _state) external onlyOwner isZeroAddress(_address) {
        isAddressValidator[_address] = _state;
        emit ValidatorAdded(_address, _state);
    }

    /// @notice Whitelist/Blacklist artist
    function whitelist(address _address, bool _state) external isValidatorOrOwner isZeroAddress(_address) {
...
contract MintGoldDustERC721 is Initializable, ERC721URIStorageUpgradeable, MintGoldDustNFT {
    using Counters for Counters.Counter;
     ...
     * @param _mintGoldDustCompany The contract responsible to Mint Gold Dust management features.
     */
    function initializeChild(address _mintGoldDustCompany) external initializer {
     ...
     */
    function transfer(address _from, address _to, uint256 _tokenId, uint256) public virtual override nonReentrant {
    ...
    function _executeMintFlow(
        ...
        emit MintGoldDustNFTMinted(newTokenId, _tokenURI, _sender, _royaltyPercent, 1, true, _collectorMintId, _memoir);
        return newTokenId;
    }
    ...
    function burnToken(uint256 tokenId) external whenNotPaused {
        require(
            _isApprovedOrOwner(msg.sender, tokenId) || msg.sender == mintGoldDustCompany.owner()
...
abstract contract MintGoldDustNFT is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    /**
    ...
    event MintGoldDustNftMintedAndSplitted(
        uint256 indexed tokenId, address[] collaborators, uint256[] ownersPercentage, address contractAddress
    );

    event TokenBurned(uint256 indexed tokenId, bool isERC721, address owner, address burner, uint256 amount);
    ...
    modifier checkArraySize(address[] calldata _newOwners, uint256[] calldata _ownersPercentage) {
    ...
    modifier checkParameters(address _sender, address _artistAddress, uint256 percentage) {
        if (!mintGoldDustCompany.isArtistApproved(_artistAddress) || _artistAddress == address(0)) {
            revert UnauthorizedOnNFT("ARTIST");
        }
     ...
     */
    function initialize(address _mintGoldDustCompany) internal onlyInitializing isZeroAddress(_mintGoldDustCompany) {
        __ReentrancyGuard_init();
        __Pausable_init();
    ...
    function setOverridePrimarySaleQuantityToSell(uint256[] calldata _tokenId) external {
        require(
            msg.sender == mintGoldDustCompany.owner() && address(this) == 0x4B0Dc0900dDe9d4f15115Bee56554857AE0Becb0,
            "Unauthorized"
        );
    ...
    function getManagePrimarySale(uint256 _tokenId) public view returns (ManagePrimarySale memory) {
        ...
        return ManagePrimarySale({owner: tokenIdArtist[_tokenId], soldout: remaining == 0, amount: remaining});
    }
     ...
     */
    function mintNft(string calldata _tokenURI, uint256 _royaltyPercent, uint256 _amount, bytes calldata _memoir)
        public
        payable
        ...
        returns (uint256)
    {
        uint256 newTokenId = _executeMintFlow(_tokenURI, _royaltyPercent, _amount, msg.sender, 0, _memoir);
    ...
    function splitMint(
        ...
        uint256 _amount,
        bytes calldata _memoir
    ) external whenNotPaused checkArraySize(_newOwners, _ownersPercentage) returns (uint256) {
    ...
    function collectorMint(
        ...
        uint256 _collectorMintId,
        address _sender
    ) external onlySetPrice checkParameters(_sender, _artistAddress, _royaltyPercent) whenNotPaused returns (uint256) {
        uint256 newTokenId =
            _executeMintFlow(_tokenURI, _royaltyPercent, _amountToMint, _artistAddress, _collectorMintId, _memoir);
    ...
    {
        uint256 _tokenId =
            _executeMintFlow(_tokenURI, _royalty, _amountToMint, _artistAddress, _collectorMintId, _memoir);
    ...
    function updatePrimarySaleQuantityToSell(uint256 _tokenId, uint256 _amountSold) external virtual {
        require(
            msg.sender == mintGoldDustMarketplaceAuctionAddress || msg.sender == mintGoldDustSetPriceAddress,
            "Unauthorized on NFT"
        );
        if (_primarySaleQuantityToSell[_tokenId] > 0) {
            _primarySaleQuantityToSell[_tokenId] = _primarySaleQuantityToSell[_tokenId] - _amountSold;
        }
    }
    ...
    function setMintGoldDustMarketplaceAuctionAddress(address _mintGoldDustMarketplaceAuctionAddress) external {
        require(msg.sender == mintGoldDustCompany.owner(), "Unauthorized");
    ...
    function _executeSplitMintFlow(
        ...
        require(_ownersPercentage[ownersCount] > 0, "Percentage must be > zero!");
