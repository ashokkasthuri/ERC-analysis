...
// File: contracts/UnikuraMembership/IUnikuraMembership.sol
// SPDX-License-Identifier: MIT
...
interface IUnikuraMembership is IERC1155Upgradeable {
    ...
    function addWhitelistedMinter(address account) external;

    function removeWhitelistedMinter(address account) external;

    function isAllowedToMint(address account) external view returns (bool);
...
// File: contracts/UnikuraMembership/UnikuraMembership.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.16;
...
library UnikuraMembershipWhitelistStorage {
    struct Layout {
        /// @notice The only address that can mint tokens on this contract.
        mapping(address => bool) whitelistedMinters;
    }

    bytes32 internal constant STORAGE_SLOT =
        keccak256("unikura.contracts.storage.unikuraMembership.whitelist");
    ...
    Ownable2StepUpgradeable
{
    ...
    using UnikuraMembershipWhitelistStorage for UnikuraMembershipWhitelistStorage.Layout;
    ...
    event WhitelistMinterAdded(address indexed account);
    event WhitelistMinterRemoved(address indexed account);
     ...
     */
    function addWhitelistedMinter(address account) external onlyOwner {
        UnikuraMembershipWhitelistStorage.layout().whitelistedMinters[
            account
        ] = true;
        emit WhitelistMinterAdded(account);
    }
     ...
     */
    function removeWhitelistedMinter(address account) external onlyOwner {
        UnikuraMembershipWhitelistStorage.layout().whitelistedMinters[
            account
        ] = false;
        emit WhitelistMinterRemoved(account);
    }

    /**
     * @notice Checks whether the specified `account` is allowed to mint SILVER tokens.
     * @dev This function is marked as `view` and does not modify the contract state.
     * @param account The address of the account to check for minting privileges.
     * @return A boolean indicating whether the account is whitelisted for minting.
     */
    function isAllowedToMint(address account) public view returns (bool) {
        return
            UnikuraMembershipWhitelistStorage.layout().whitelistedMinters[
                account
            ];
    ...
    function mint(address account, uint256 id, uint256 amount) public {
        ...
        if (id == SILVER) {
            require(
                ...
                "Only whitelisted minters can mint SILVER tokens"
            );
