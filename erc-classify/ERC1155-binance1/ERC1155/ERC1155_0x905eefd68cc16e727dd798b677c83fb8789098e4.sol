...
// File: contracts/nft/GeneralERC1155.sol
// SPDX-License-Identifier: MIT OR Apache-2.0
pragma solidity 0.8.9;
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC1155/extensions/ERC1155URIStorageUpgradeable.sol";

contract GeneralERC1155 is Initializable, ERC1155URIStorageUpgradeable, AccessControlUpgradeable {
    bytes32 public constant CREATOR_ROLE = keccak256("CREATOR_ROLE");

    uint256 private tokenIDs;
    string private _projectID;
    string private _collectionID;
    bool public isMintingPublic;

    event MintingStatusUpdated(bool isPublic);
    ...
    function mint(address player, string memory tokenURI, uint256 amount) external returns (uint256) {
        if (!isMintingPublic) {
            _checkRole(CREATOR_ROLE);
        }
        uint256 newTokenId = tokenIDs++;
        _mint(player, newTokenId, amount, "");
        _setURI(newTokenId, tokenURI);

        return newTokenId;
    }

    function mintingPublic(bool isPublic) external onlyRole(CREATOR_ROLE) {
        if (isMintingPublic == isPublic) revert AlreadySameStatus();
        isMintingPublic = isPublic;
        emit MintingStatusUpdated(isPublic);
    }
