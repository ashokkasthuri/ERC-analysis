// Chain: BSC - File: src/GPKCard.sol
...
contract GPKCard is ERC1155, Ownable {
    ...
    function airdrop(address _to, uint256[] memory _ids, uint256[] memory _amounts) external onlyOwner {
    ...
    function getMintedSupply(uint256 _cardId) public view returns (uint256 supply) {
        require(isCardValid(_cardId), "Card does not exist");
    ...
    function getSeasonIds() public view returns (uint256[] memory allSeasonIds) {
        return _seasonIds;
    }
    ...
    function getSeasonCards(uint256 _id) public view returns (uint256[] memory seasonCardIds) {
        require(_seasonExists(_id), "Season does not exist");
        ...
        returns (uint256[] memory minted)
    {
        ...
        uint256 tokenAmount = _getGPKFromUSD(season.mysteryPack1Price);

        if (_quantity == 5) {
            tokenAmount = _getGPKFromUSD(season.mysteryPack5Price);
        } else if (_quantity == 10) {
            tokenAmount = _getGPKFromUSD(season.mysteryPack10Price);
        }
    ...
    function _getGPKFromUSD(uint256 _usd) internal view returns (uint256 amount) {
        address[] memory path = new address[](2);
        path[0] = _busdAddress;
        path[1] = _tokenAddress;

        uint256[] memory amounts = IUniswapV2Router02(_routerAddress).getAmountsOut(1, path);
        return amounts[1] * _usd * 1 ether;
    }

    function _getCardPriceInGPK(uint256 _cardId) internal view returns (uint256 tokens) {
        require(isCardValid(_cardId), "Card does not exist");
        Card memory fetchedCard = _cardMap[_cardId];

        return _getGPKFromUSD(fetchedCard.price);
    }
...
// Chain: BSC - File: lib/openzeppelin-contracts/contracts/utils/math/SafeMath.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: lib/v2-periphery/contracts/interfaces/IUniswapV2Router02.sol
...
// Chain: BSC - File: lib/v2-periphery/contracts/interfaces/IUniswapV2Router01.sol
pragma solidity >=0.6.2;
