...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    function openCampaign(
        ...
        uint256[] memory _phases, // start phase 1, end phase 2, start phase 2, end phase 2
    ...
    function getCurrentPhase(uint256 _campaignId) public view returns(uint256) {
        ...
        if (block.timestamp >= campaign.phasesToTime[1] && block.timestamp <= campaign.phasesToTime[2]) {
            return 1;
        }
        if (block.timestamp >= campaign.phasesToTime[3] && block.timestamp <= campaign.phasesToTime[4]) {
            return 2;
        }
        return 0;
    }
