...
contract Mintable is Ownable {
    ...
    constructor()  {
        ...
        emit MinterChanged(_msgSender(), true);
    }
...
contract Runswap1155 is Singleton, Mintable, ERC1155Base {
    ...
    mapping(uint256 => uint256) public priceBNB;
    ...
    constructor(string  memory _name, string memory _symbol, address _tokenPayment) public validAddress(_msgSender()) {
        name = _name;
        symbol = _symbol;
    ...
    function setPriceBNB(uint256 _id, uint256 _price) public onlyOwner {
        priceBNB[_id] = _price;
    }
    ...
    function buy(uint256 _id, uint256 _amount) public {
        require(price[_id] != 0, "nft must have price");
        uint256 totalPrice = price[_id].mul(_amount);
        IBEP20(tokenPayment).safeTransferFrom(msg.sender, address(this), totalPrice);
        _mint(msg.sender, _id, _amount);
    }

    function buyBNB(uint256 _id, uint256 _amount) public payable {
        require(priceBNB[_id] != 0, "nft must have price");
        uint256 totalPrice = priceBNB[_id].mul(_amount);
        require(msg.value >= totalPrice, "not enough bnb");
        _mint(msg.sender, _id, _amount);
    }
    ...
    function clearAllBNB(address payable to) external onlyOwner {
