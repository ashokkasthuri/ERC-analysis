...
// Chain: BSC - File: contracts/TombolaTickets.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
contract TombolaTickets is ERC1155Supply, AccessControl {
    ...
    constructor(string memory baseURI, string memory extension) ERC1155(string.concat(_baseURI,"{id}.",extension)) {
    ...
    function uri(uint256 _tokenId) override public view returns (string memory) {
        return string(
            abi.encodePacked(
                _baseURI,
                Strings.toString(_tokenId),".",_extension
            )
        );
    }
    ...
    function setExtension(string memory extension) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _extension = extension;
    ...
    function mint(address to, uint256 id, uint256 value, bytes memory data) external onlyRole(MINTER_ROLE) {
        _mint(to,id,value,data);
    }

    function mintBatch(address to, uint256[] memory ids, uint256[] memory values, bytes memory data) external onlyRole(MINTER_ROLE) {
