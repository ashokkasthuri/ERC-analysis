...
// Chain: BSC - File: /contracts/Counters1155.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: /contracts/IMarketplace1155.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: /contracts/PentasNFT1155.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.2;
...
import "./Counters1155.sol";
import "./IMarketplace1155.sol";

contract PentasNFT1155 is
    Initializable,
    ERC1155Upgradeable,
    OwnableUpgradeable,
    ...
    ERC1155SupplyUpgradeable
{
     ...
     */
    using Counters1155 for Counters1155.Counter;
    Counters1155.Counter private _tokenIdCounter;
     ...
     */
    function initialize(string memory prefixURI) public initializer {
        __ERC1155_init(prefixURI);
        __Ownable_init();
    ...
    {
        require(exists(tokenId), "Pentas: Query for nonexistent token");
        ...
        external
    {
        ...
        require(
            ...
            "Pentas: Beneficiary is zero address"
        );
    ...
    function safeMint(
        ...
        // Conduct minting
        _mint(_msgSender(), _tokenIdCounter.current(), amount, "");
        _tokenIdCounter.increment();
    }

    /**
     * @dev Mint token on behalf of other address.
     * @param minter Minter or Artist address
     * @param ipfsCID IPFS CID. Obtain from IPFS network.
     * @param royalty Royalty percentage. Royalty will be paid on every sale transaction. Example: 5% = 5000, 2.5% = 2500.
     * @param amount Token amount
     */
    function safeMintFor(
        ...
        _mint(minter, _tokenIdCounter.current(), amount, "");
        _safeTransferFrom(
            minter,
            _msgSender(),
     ...
     * @param tokenId Token ID.
     * @param qty Quantity for listing.
     * @param perUnitPrice Token selling price in wei per unit.
     */
    function approveMarket(
        ...
        IMarketplace1155(_marketplaceAddress).setSalePrice(
            address(this),
            tokenId,
            _msgSender(),
            qty,
    ...
    function burn(
        ...
        // Check if the sender is the same as the account
        require(_msgSender() == account, "Pentas: Caller is not owner.");

        require(exists(tokenId), "Pentas: Query for nonexistent token");

        // Check if sender is the original minter
        // This is to ensure only the minter can burn token
        require(
    ...
    function burnBatch(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            // This is to ensure only the minter can burn token
            require(
                account == _tokenData[tokenId].minter,
                "Pentas: Sender is not the original minter"
            );
        }
