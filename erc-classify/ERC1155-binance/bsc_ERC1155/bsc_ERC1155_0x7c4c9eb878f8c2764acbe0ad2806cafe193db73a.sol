// Chain: BSC - File: contracts/tokens/GameAntzNFT.sol
...
contract GameAntzNFT is Ownable, ERC1155Supply, IGameAntzCardsFeatures {
  using Strings for uint256;

  string public name = "GameAntzNFT";
  string public symbol = "ANTZ";
  ...
  mapping(uint256 => bool) public hasMultipliers;

  mapping(uint256 => uint256) public feeDiscounts;
  mapping(uint256 => bool) public hasFeeDiscounts;

  mapping(uint256 => uint256) public harvestReliefs;
  mapping(uint256 => bool) public hasHarvestReliefs;
  ...
  modifier onlyAuthorizedOrOwner() {
    require(msg.sender == address(this) || msg.sender == authorized || msg.sender == owner(), "Only authorized");
    _;
  }

  constructor(string memory _uri) ERC1155(_uri) {}

  function addHarvestReliefCard(uint256[] memory _tokenId, uint256[] memory _harvestRelief)
    external
    override
    onlyAuthorizedOrOwner
  {
    require(_tokenId.length == _harvestRelief.length, "Invalid lengths");
    for (uint256 i = 0; i < _tokenId.length; i++) {
      if (!hasHarvestReliefs[_tokenId[i]]) {
        hasHarvestReliefs[_tokenId[i]] = true;
        harvestReliefCards.push(_tokenId[i]);
        harvestReliefs[_tokenId[i]] = _harvestRelief[i];
        emit HarvestReliefCardAdded(_tokenId[i], _harvestRelief[i]);
      }
    }
  }

  function addFeeDiscountCard(uint256[] memory _tokenId, uint256[] memory _feeDiscount)
    external
    override
    onlyAuthorizedOrOwner
  {
    require(_tokenId.length == _feeDiscount.length, "Invalid lengths");
    for (uint256 i = 0; i < _tokenId.length; i++) {
      if (!hasFeeDiscounts[_tokenId[i]]) {
        hasFeeDiscounts[_tokenId[i]] = true;
        feeDiscountCards.push(_tokenId[i]);
        feeDiscounts[_tokenId[i]] = _feeDiscount[i];
        emit FeeDiscountCardAdded(_tokenId[i], _feeDiscount[i]);
      }
    }
  }

  function addMultiplierCard(uint256[] memory _tokenId, uint256[] memory _multiplier)
    public
    override
    onlyAuthorizedOrOwner
  {
    require(_tokenId.length == _multiplier.length, "Invalid lengths");
    for (uint256 i = 0; i < _tokenId.length; i++) {
      if (!hasMultipliers[_tokenId[i]]) {
        hasMultipliers[_tokenId[i]] = true;
        multiplierCards.push(_tokenId[i]);
        multipliers[_tokenId[i]] = _multiplier[i];
        emit MutliplierCardAdded(_tokenId[i], _multiplier[i]);
      }
    }
  }
  ...
  function removeHarvestReliefCard(uint256 index) external override onlyAuthorizedOrOwner {
    ...
    require(hasHarvestReliefs[_tokenId], "don't have harvest releif");
    delete hasHarvestReliefs[_tokenId];
  ...
  function removeFeeDiscountCard(uint256 index) external override onlyAuthorizedOrOwner {
    ...
    require(hasFeeDiscounts[_tokenId], "don't have fee discount");
    delete hasFeeDiscounts[_tokenId];
  ...
  function removeMultiplierCard(uint256 index) external override onlyAuthorizedOrOwner {
    ...
    require(hasMultipliers[_tokenId], "don't have multiplier");
    delete hasMultipliers[_tokenId];
  ...
  function mintBatch(
    ...
    addMultiplierCard(ids, _multipliers);
  ...
  function mint(
    ...
    _id[0] = id;
    uint256[] memory _multiplier = new uint256[](1);
    _multiplier[0] = multiplier;
    addMultiplierCard(_id, _multiplier);
...
// Chain: BSC - File: contracts/interfaces/IGameAntzCardsFeatures.sol
// SPDX-License-Identifier: MIT
