...
contract testBase is testAccessControl,ERC1155{
    ...
    uint256 public legendary = 0;
    bool public modifyFlag = false;
    bool private summonFlag = true;
    ...
    function setStake(uint256 id, bool status) external onlyPartner{
        require(id < _tokenIds.current(), "cannot set other");
        setStakeStatus(id, status);
    }
    
    function burn(address _address, uint256 _tokenId, uint256 amount) external onlyPartner {
        _burn(_address,_tokenId, amount);
    }
    
    function getStar(uint256 id) external view returns (uint256) {
        ItemAttr storage token = tokenIdToItemAttr[id];
        return token.star;
    }
    ...
    uint16 itemName,uint16 gameId,bytes memory data, bool hardFlag)public virtual onlyPartner() returns(uint256){
        
        uint256 newItemId = _tokenIds.current();
        _mint(account,newItemId,1,data);
        tokenSupply[newItemId] = 1;
        uint256 genes = _generateRandomDna(newItemId);
        ...
        if (legendary <= 100) {
            if (hardFlag) {
                while ((strength.add(intelligence).add(agility) >= 255|| 
                (strength > 85 && intelligence > 85) || 
                (intelligence > 85 && agility > 85) || 
                (agility > 85 && strength >85)) && time < 4) {
                    genes = _generateRandomDna(time);
                ...
                while ((strength.add(intelligence).add(agility) >= 269|| 
                (strength > 89 && intelligence > 89) || 
                (intelligence > 89 && agility > 89) || 
                (agility > 89 && strength > 89)) && time < 3) {
                    genes = _generateRandomDna(time);
                    strength = genes.mod(100);
                    intelligence = (genes.mod(10000)).div(100);
                    agility = (genes.mod(1000000)).div(10000);
            ...
            while (strength.add(intelligence).add(agility) >= 279) {
                genes = _generateRandomDna(time);
                strength = genes.mod(100);
                intelligence = (genes.mod(10000)).div(100);
                agility = (genes.mod(1000000)).div(10000);  
        ...
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,true,1,genes,strength,intelligence,agility,
            calculateMaxStar(strength.add(intelligence).add(agility)),
            strength.add(intelligence).add(agility));
        setStakeStatus(newItemId, false);
        tokenIdToItemAttr[newItemId] = attr;
        _tokenIds.increment();
        if ((strength.add(intelligence).add(agility) >= 279)) legendary += 1;
        return newItemId;
    }
    
    function calculateMaxStar(uint256 combat) private pure returns(uint256) {
        if (combat < 120) {   // legendary 6.7%
            return 3;
        } else if (combat < 220) {   // epic 8%
            return 5;
        }
        return 100;
    }
    ...
    function getRarity(uint256 _tokenId) public view returns(uint256) {
        ...
        if (token.originalCombat >= 280) {   // legendary 6.7%
            return 0;
        } else if (token.originalCombat >= 255) {   // epic 8%
            return 1;
        } else if (token.originalCombat >= 220) {  // rare 11.6%
            return 2;
        } else if (token.originalCombat >= 120){   // normal 33.3%  - 
            return 3;
        } else {  // base  40% 
            return 4;
        }
    ...
    function levelUpItemHelper(address account, uint256 _tokenId1, uint256 _tokenId2, uint256 probability) private returns(bool, uint256){
        ...
        require(balanceOf(account,_tokenId1) >= 1 && balanceOf(account,_tokenId2) >= 1,"You must have this item");
        require(getStakeStatus(_tokenId1) == false && getStakeStatus(_tokenId2) == false, "one of token is staked");
        
        bool value = true;
        if (isSuccessfulAction(_tokenId1,probability)) {
            value = false;
        }
        uint256 newGenes = 0;
        ...
        uint256 newItemId = _tokenIds.current();
        _mint(account,newItemId,1,"0x03");
        tokenSupply[newItemId] = 1;
        uint256 _tokenId = _tokenId1;
        if (tokenGenes1 % 2 == 1) {
           _tokenId = _tokenId2;
        }
        ...
        tokenIdToItemAttr[newItemId] = 
            ItemAttr(attr.name,attr.gameId,(token1.canRaiseBirth && token2.canRaiseBirth),
            1,newGenes,strength,intelligence,agility,
            calculateMaxStar(strength.add(intelligence).add(agility)),
            strength.add(intelligence).add(agility));
        setStakeStatus(newItemId, false);
        _tokenIds.increment();

        if (getRarity(newItemId) == 0) legendary += 1;
        
        return (value, newItemId);
    }
    
    function levelUpItem(address account, uint256 _tokenId1, uint256 _tokenId2, uint256 probability) public virtual onlyPartner returns(bool, int256){
        (bool value, uint256 id) = levelUpItemHelper(account, _tokenId1, _tokenId2, probability);
        if (!value) {
            _burn(account,_tokenId2,1);
            _burn(account,id,1);
            if (getRarity(_tokenId2) == 0) {
                legendary -= 1;
            }
            if (getRarity(id) == 0) {
                legendary -= 1;
        ...
        _burn(account,_tokenId1,1);
        _burn(account,_tokenId2,1);
        if (getRarity(_tokenId2) == 0) {
            legendary -= 1;
        }
        if (getRarity(_tokenId1) == 0) {
            legendary -= 1;
        }
        return (value, int(id));
    }
    
    function upgradeStar(address account, uint256 _tokenId1, uint256 _tokenId2, uint16 choice) public virtual onlyPartner returns(bool){
        ItemAttr storage token1 = tokenIdToItemAttr[_tokenId1];
        ItemAttr storage token2 = tokenIdToItemAttr[_tokenId2];
        ...
        require(balanceOf(account,_tokenId1) == 1 && balanceOf(account,_tokenId2) == 1,"You must have this item");
        require(getStakeStatus(_tokenId1) == false && getStakeStatus(_tokenId2) == false, "one of token is staked");
        require(token1.star < token1.maxStar, "can not upgrade");
        ...
        if (!isSuccessfulAction(_tokenId1, factor)) {
            _burn(account,_tokenId2,1);
            return false;
        }
        ...
        _burn(account,_tokenId2,1);
        if (tokenRarity == 0) legendary -= 1;
        return true;
    }
    
    function giveBirth(address account, uint256 _tokenId, bytes memory data) public virtual onlyPartner returns(uint256){
        ItemAttr storage token = tokenIdToItemAttr[_tokenId];
        require(balanceOf(account,_tokenId) == 1,"You must have this item");
        ...
        if (legendary <= 100) {
            while ((strength.add(intelligence).add(agility) >= 265 || strength > 90 ||
                intelligence > 90 || agility > 90) && time < 3) {
                genes = _generateRandomDna(time);
                strength = genes.mod(100);
                intelligence = (genes.mod(10000)).div(100);
                agility = (genes.mod(1000000)).div(10000);
        ...
        ItemAttr memory attr = 
            ItemAttr(token.name,token.gameId,false,1,genes,strength,intelligence,agility,
            calculateMaxStar(strength.add(intelligence).add(agility)),
            strength.add(intelligence).add(agility));
        tokenIdToItemAttr[newItemId] = attr;
        setStakeStatus(newItemId, false);
    ...
    function _bid(uint256 _saleId, uint256 _bidAmount,uint16 count)internal returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            ceoAddress.transfer(cfoCut);
            cfoAddress.transfer(buyBackCut);
...
contract testNFT is testBase {
    ...
    function withdrawToken(IERC20 token) external onlyCEO {
        if (token.balanceOf(address(this)) > 0) {
            token.transfer(ceoAddress, token.balanceOf(address(this)));
        }
    }
