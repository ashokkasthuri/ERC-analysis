...
contract NFTZP is Base{
    ...
    function a1(uint256 b1, uint256 b2, uint256 b3, uint256 b4, uint256 b5) public onlyOwner {
        _constellationMap[b5][b1].probabilityStarting = b2;
        _constellationMap[b5][b1].probabilityEnd = b3;
        _constellationMap[b5][b1].LimitQuantity = b4; 
    }

   
   function a2(uint256[] calldata b1, uint256[] calldata b2, uint256[] calldata b3, uint256[] calldata b4, uint256[] calldata b5) public onlyOwner {
         for (uint256 i=0; i<b1.length; i++) {     
        _constellationMap[b5[i]][b1[i]].probabilityStarting = b2[i];
        _constellationMap[b5[i]][b1[i]].probabilityEnd = b3[i];
        _constellationMap[b5[i]][b1[i]].LimitQuantity = b4[i]; 
    }
     }
    ...
    function playerwinning(address playeraddress,uint256 winningNum,uint256 condition) private   returns(uint256)   {
            ...
            if( winningNum ==6 ){
                if( condition ==3){ 
                    PNDV.transfer(playeraddress, Convert(100));
                }
                else if( condition ==2){ 
                    NFT.safeBatchTransferFrom( address(USDTaddress),playeraddress,_asSingletonArray(9),_asSingletonArray(1),"0x00");
                }
                else{ 
                    PNDV.transfer(playeraddress, Convert(1));
                }
            }
            if( winningNum ==7 ){
                if( condition ==3){  
                    PNDV.transfer(playeraddress, Convert(1000));
                }
                else if( condition ==2){
                    PNDV.transfer(playeraddress, Convert(5000));
                }
                else{  
                    PNDV.transfer(playeraddress, Convert(10));
                }
            }
            if( winningNum ==8 ){
                if( condition ==3){  
                    PNDV.transfer(playeraddress, Convert(10000));
                }
                else if( condition ==2){
                    PNDV.transfer(playeraddress, Convert(100));
                }
                else{  
                    PNDV.transfer(playeraddress, Convert(200));
                }
            }
    ...
    function extract(uint256 i , uint256 condition) private   returns (uint256) {
