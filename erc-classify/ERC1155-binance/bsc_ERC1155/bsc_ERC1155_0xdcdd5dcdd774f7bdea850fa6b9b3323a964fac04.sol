...
contract testBase is testAccessControl,ERC1155{
    ...
    function isHero(uint256 id) external view returns (bool) {
        ItemAttr storage token = tokenIdToItemAttr[id];
        return token.itemType == 1;
    }
    ...
    uint16 itemName,uint16 gameId,bytes memory data, bool hardFlag)public virtual onlyPartner() returns(uint256){
        ...
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,1,true,1,genes,strength,intelligence,agility,
            100 + (strength.mul(85) + agility.mul(5) + intelligence.mul(10)).div(100),
            strength.add(intelligence).add(agility));
        setStakeStatus(newItemId, false);
        tokenIdToItemAttr[newItemId] = attr;
        creators[newItemId] = account;
        _tokenIds.increment();
    ...
    uint16 itemName,uint16 gameId,bytes memory data)public virtual onlyPartner() returns(uint256){
        ...
        ItemAttr memory attr = 
            ItemAttr(itemName,gameId,2,false,1,genes,strength,intelligence,agility,
            15 + (strength.mul(90) + agility.mul(5) + intelligence.mul(5)).div(100),
            strength.add(intelligence).add(agility));
        tokenIdToItemAttr[newItemId] = attr;
        setStakeStatus(newItemId, false);
        creators[newItemId] = account;
    ...
    function getClassType(uint256 _tokenId) external view returns (uint256) {
        ItemAttr storage token = tokenIdToItemAttr[_tokenId];
        uint256 typeNum = token.name % 3;
        if (token.itemType == 1) {
            if (typeNum == 0) {
                return 0;
            } else if (typeNum == 1) {
    ...
    function levelUpItem(address account, uint256 _tokenId1, uint256 _tokenId2, uint256 probability) public virtual onlyPartner returns(bool){
        ...
        tokenIdToItemAttr[newItemId] = 
            ...
            1,newGenes,strength,intelligence,agility,health,strength.add(intelligence).add(agility));
        setStakeStatus(newItemId, false);
    ...
    function giveBirth(address account, uint256 _tokenId, bytes memory data) public virtual onlyPartner returns(uint256){
        ...
        ItemAttr memory attr = 
            ItemAttr(token.name,token.gameId,1,false,1,genes,strength,intelligence,agility,
            (100 + (strength.mul(85) + agility.mul(5) + intelligence.mul(10)).div(100)), 
            strength.add(intelligence).add(agility));
        tokenIdToItemAttr[newItemId] = attr;
        setStakeStatus(newItemId, false);
    ...
    function _bid(uint256 _saleId, uint256 _bidAmount,uint16 count)internal returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            cooAddress.transfer(buyBackCut);
