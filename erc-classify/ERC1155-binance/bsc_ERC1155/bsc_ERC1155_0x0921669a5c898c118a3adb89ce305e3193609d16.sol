...
contract testBase is testAccessControl,ERC1155{
    ...
    function createNFT(address account, uint256 amount, uint16 itemType,string memory itemName,uint16 gameId,bool isSingle,bool isCopyright,bytes memory data)public virtual onlyPartner{
        ...
        uint256 health = 100 + (strength.mul(70) + agility.mul(20) + intelligence.mul(10)).div(100);
    ...
    function allLevelOne(uint256 _tokenId1, uint256 _tokenId2) public view returns(bool) {
    ...
    function levelUpItem(uint256 _tokenId1, uint256 _tokenId2) public returns(uint256 _rand,uint256 _genes){
        ItemAttr storage token1 = tokenIdToItemAttr[_tokenId1];
        ItemAttr storage token2 = tokenIdToItemAttr[_tokenId2];
        require(allLevelOne(_tokenId1, _tokenId2), "Cards should be all level 1");
    ...
    function upgradeStar(uint256 _tokenId1, uint256 _tokenId2, uint16 choice) public {
        ...
        token1.star += 1;
        if (choice == 1) {
            token1.strength.add(tempAttrPoints);
            token1.health += tempAttrPoints.mul(70).div(100); 
        } else if (choice == 2) {
            token1.intelligence.add(tempAttrPoints);
            token1.health += tempAttrPoints.mul(10).div(100); 
        } else if (choice == 3) {
            token1.agility.add(tempAttrPoints);
            token1.health += tempAttrPoints.mul(20).div(100); 
        }
