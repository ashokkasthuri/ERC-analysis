...
interface IProxy { 
    ...
    function _ylOwner() external view returns(address);
}
...
contract YLNFT1155 is ERC1155URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public default_sponsornft_qty;

    mapping(string => mapping(string => mapping(address => uint256))) private categoryAmount;
    mapping(string => mapping(string => mapping(address => uint256))) private categoryCount;
    ...
    event batchMinted1155( address indexed minter, uint256[] tokenIds, uint256[] amounts, string[] tokenUris, uint256 _type, uint256 mintedGas );
    ...
    function setCategoryAmount(
        ...
        categoryAmount[_sport][_cnft][_athlete] = _amount;
        return true;
    }

    function setDefaultSponsorNftQuantity(uint256 _amount) public onlyOwner returns(bool){
        default_sponsornft_qty = _amount;
        return true;
    }

    function deleteCategory(string memory _sport, string memory _cnft, address _athlete)
        public
        onlyOwner
        returns (bool)
    {
        delete categoryAmount[_sport][_cnft][_athlete];
        return true;
    }

    function getCategoryCount(string memory _sport, string memory _cnft, address _athlete)
        public
        view
        returns (uint256)
    {
        return categoryCount[_sport][_cnft][_athlete];
    }

    function getCategoryAmount(string memory _sport, string memory _cnft, address _athlete)
        public
        view
        returns (uint256)
    {
        return categoryAmount[_sport][_cnft][_athlete];
    }

    function getDefaultSponsorNftQuantity() public view returns(uint256){
        return default_sponsornft_qty;
    }
    ...
    function create1155Token(
        ...
        if(proxy.getRole(msg.sender) == 1 || proxy.getRole(msg.sender) == 2 || proxy.getRole(msg.sender) == 5 || proxy.getRole(msg.sender) == 6){
            require( proxy.isMintableAccount(msg.sender), "you can't mint YLT NFT, please contact the Admin");
            require(_amount <= default_sponsornft_qty, "Overflow! Amount of Sponsor NFT Default Quantity");
            require(_athlete != address(0), "You should select an athlete");
            require( categoryAmount[_sport][_cnft][_athlete] >= (categoryCount[_sport][_cnft][_athlete]) + _amount, " Overflow! Amount of NFT category" );
        }
        ...
        if(_athlete != address(0)){
            categoryCount[_sport][_cnft][_athlete] += _amount;
        }
    ...
    // batch mint
    function bundleCreate1155Token(        
        ...
        uint256 startGas = gasleft(); 
        require( proxy._ylOwner() == msg.sender || proxy.getRole(msg.sender) == 8, "you don't have permission");  
        require(getPauseContract() == false, "NFT1155 Contract was paused!");
        require((_tokenURIs.length == _sports.length) && (_tokenURIs.length == _amounts.length), "Input arrays length mismatch");
        ...
        for (uint256 i = 0; i < _tokenURIs.length; i++) {
            incrementTokenId();
            uint256 newTokenId = getCurrentTokenId();
            _mint(msg.sender, newTokenId, _amounts[i], "");
            _setURI(newTokenId, _tokenURIs[i]);
        ...
        uint256 gasUsed = startGas - gasleft(); 
        emit batchMinted1155(msg.sender, newTokenIds, _amounts, _tokenURIs, _type, gasUsed);
        return true;
    }
    ...
    function lazyCreateToken(
        ...
        setApprovalForAll(proxy.getYLVaultAddr(), true);
        setApprovalForAll(address(this), true);
        categoryCount[_sport][_cnft][_athlete] += 1;
        nftType[newTokenId] = _type;
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory values, string[] memory sports, string[] memory cnfts, address[] memory athlets) public virtual {
        require(
            account == _msgSender() || isApprovedForAll(account, _msgSender()),
            "ERC1155: caller is not token owner or approved"
        );
        require(ids.length == values.length && ids.length == sports.length && ids.length == cnfts.length && ids.length == athlets.length, "Input arrays length mismatch");

        _burnBatch(account, ids, values);
        for(uint i = 0; i < ids.length; i++)
        {   
            if(categoryCount[sports[i]][cnfts[i]][athlets[i]] >= values[i]){
                categoryCount[sports[i]][cnfts[i]][athlets[i]] -= values[i];
            } 
