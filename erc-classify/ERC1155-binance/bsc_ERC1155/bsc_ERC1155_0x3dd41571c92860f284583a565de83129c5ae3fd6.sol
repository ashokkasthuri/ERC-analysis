...
// Chain: BSC - File: contracts/ElpisFragments.sol
// SPDX-License-Identifier: MIT
    ...
    ERC1155Pausable
{
    ...
    /**
     * @dev Restrict modifier that allow admin only
     */
    modifier adminOnly() {
    ...
    /**
     * @dev Restrict modifier that allow minter only
     */
    modifier minterOnly() {
    ...
    /**
     * @dev Restrict modifier that allow pauser only
     */
    modifier pauserOnly() {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Pauser only");
        _;
    }

    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, and `PAUSER_ROLE` to the account that
     * deploys the contract before set base URI for contract.
     */
    ...
    /**
     * @dev Return the `URI` for `tokenId`
     */
     ...
     */
    function setUri(uint256 tokenId, string memory newUri) external adminOnly {
        ERC1155URIStorage._setURI(tokenId, newUri);
    }

    /**
     * @dev Set new `baseURI` for `contract`.
     *
     * Requirements:
     *
     * - the caller must have the `DEFAULT_ADMIN_ROLE`.
     */
    function setBaseUri(string memory baseURI) public virtual adminOnly {
