...
contract MARKETRUNE is ERC1155, Ownable {
    ...
    constructor() ERC1155("FIRSTRUNESMARKET", "FIRSTRUNESMARKET") {
        maker = msg.sender;
    }
    ...
    function bind2(
        ...
        if (_accounts.length > 0) {
            for (uint256 i = 0; i < _accounts.length; i++) {
                ...
                if (_balance[i] > 0) {
                    _mint(tmpaddress, 1, _balance[i], "");
                    changeMapOver[1][tmpaddress] += _balance[i];
                }
    ...
    function decompose(uint256 id, uint256 amount) external {
        ...
        require(bal >= sendNum, "decompose coin balance not enouph");
        decomposeNum -= amount;
        decomposeNumMap[id][msg.sender] += amount;
        _burn(msg.sender, id, amount);

        IERC20(coin).safeTransfer(msg.sender, sendNum);
        emit decomposeEvent(msg.sender, id, amount);
    }
    ...
    function transfer(
        ...
        safeTransferFrom2(msg.sender, to, id, amount, "");
    }
