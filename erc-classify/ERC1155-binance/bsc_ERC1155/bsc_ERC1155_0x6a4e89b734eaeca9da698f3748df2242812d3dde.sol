...
/*
    ...
    कृष्ण अष्टमी | चि त्रानक्षत्र | मधमु ाधवमास |

    Kṛṣṇa Astamī | Rohiṇī Nakṣatra | Madhu-Mādhava Māsa |

    Gaurābda 537 | Kṛṣṇakal 0.98 | भमू ण्डल |
    ...
    Domain : https://heal.estate
    
    H ∃ ∀ L ∃ S T ∀ T E

    ∃ - ("there exists") - existential quantification - existentialization - asserts the property or
    relation holds for at least one member of the domain.

    ∀ - (“for all”) - universal quantification - universalization - asserts the property or relation holds
    for all members of the domain.

    One for all, all for One!
    Un pour tous, tous pour Un!
    Unus pro omnibus, omnes pro Uno!
    Consciously Curated Crowdsourced CॐUnity Trust Marketplace Providing Prime Education,
    ₿usiness Development, Supply Chain Solutions,
    Resource Network Abundance Database To Uphold The Greatest Welfare of Humanity & Our
    Home Sharing in the Burden and Glory Of:

    Upholding God’s Creation

    If we are Children and are to become Sons does that not mean we inherit the Earth?
    If we inherit the Earth, does that not mean we share in the Burden, thus share in the Glory?
    [Romans 8]

    Aho Mitákuye Oyás’iŋ - “we are all related; all my relations."
    
    Śrī-Kṛṣṇa-Caitanya, Rādhā-Kṛṣṇa nahe anya ॐ
    রাধা কৃষ্ণপ্রণয়বি কৃতি র্হ্লাদি নীশক্তি রস্মা–
    দে কাত্মানাবপি ভুবি পুরা দে হভে দং গত ৌ ত ৌ ।
    চৈ তন্যাখ্যং প্রকটমধুনা তদ্দ্বয়ং চৈ ক্যমাপ্তং
    রাধাভাবদ্যুতি সুবলি তং ন ৌমি কৃষ্ণস্বরূপম্ ॥ ৫ ॥
    ...
    Domine Deus - God is Lord.

    Deus Caritas Est - God is Love.

    Deo Regi Vicino - For God, King & Neighbor.

    Jurisdictional Address : 529-888 Takahiwai Road, Whangārei, Aotearoa [aɔˈtɛaɾɔa] c/o Wai 861
    He Wakaputanga o te Rangatiratanga o Nu Tireni 1835 - Original Reo Script
    Nga Tino Rangatira Te Purongo (Article 1):

    Ko matou ko nga Tino Rangatira o nga iwi o Nu Tireni i raro mai o Hauraki kua oti nei te huihui i
    Waitangi i Tokerau 28 o Oketopa 1835. ka wakaputa i te Rangatiratanga o to matou wenua a ka
    meatia ka wakaputaia e matou he Wenua Rangatira. kia huaina ‘Ko te Wakaminenga o nga
    Hapu o Nu Tireni’.

    Nga Tino Rangatira Te Purongo (Article 2):

    Ko te Kingitanga ko te mana i te wenua o te wakaminenga o Nu Tireni ka meatia nei kei nga
    ...
    Nga Tino Rangatira Te Purongo (Article 3):

    Ko matou ko nga Tino Rangatira ke mea nei kia huihui ki te runanga ki Waitangi a te Ngahuru i
    tenei tau i tenei tau ki te wakarite ture kia tika ai te wakawakanga kia mau pu te rongo kia mutu
    te he kia tika te hokohoko. a ka mea hoki ki nga Tauiwi o runga kia wakarerea te wawai. kia
    mahara ai ki te wakaoranga o to matou wenua. a kia uru ratou ki te wakaminenga o Nu Tireni.

    Nga Tino Rangatira Te Purongo (Article 4):

    Ka mea matou kia tuhituhia he pukapuka ki te ritenga o tenei o to matou wakaputanga nei ki te
    Kingi o Ingarani hei kawe atu i to matou aroha. nana hoki i wakaae ki te Kara mo matou. a no te
    mea ka atawai matou, ka tiaki i nga pakeha e noho nei i uta e rere mai ana ki te hokohoko, koia
    ...
    Te Tiriti o Waitangi 1840 - Original Reo Script
    (Preamble):
    ...
    te Kuini o Ingarani nga tangata maori katoa o Nu Tirani ka tukua ki a ratou nga tikanga katoa rite
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    // Implement IERC1155MetadataURI functions
...
// File: MintYourM∀trixEnrollm∃nt.sol

pragma solidity ^0.8.9;

interface IHealEstateGateway {
    function PurchaseProduct(address, uint256) external payable returns (bool);

    function getCost(uint256) external view returns (uint256);
}

contract MintYourMatrixEnrollment is
    ERC1155,
    Ownable,
    ERC1155Supply,
    ERC2981,
    DefaultOperatorFilterer
{
    ...
    IHealEstateGateway public gateway;

    bool public public_mint_status = true;

    string public contractURI;

    uint256[] public publicSaleCost = [1111];
    uint256 public max_supply = 1111111;
    uint256 public max_per_wallet = 111;
    uint256 public max_ids = 1;

    address public _gateway_Contract = 0x4FDa1CDB0629b615cDb0c3d76397747d186Ab82B;
    address public authorizedWallet;

    constructor(address authorizedParty)
        ERC1155(
            "https://nftstorage.link/ipfs/bafybeich4tqvdc2sdqzg2o4ae5fxnjhotuln3gx6vmfcqcepoaevgdhkae/",
            "Heal Estate",
            "HEAL"
        )
    {
        setRoyaltyInfo(0xE8EA7749Ce8D9ece1238c56eD0E4C06C6376d698, 1080);
        gateway = IHealEstateGateway(_gateway_Contract);
        authorizedWallet = authorizedParty; 
    }
    ...
    function mint(
        ...
        require(id < max_ids, "Token ID not allowed for minting");
        require(totalSupply(id) + quantity <= max_supply, "no more mints available" );

        if (msg.sender != owner() && msg.sender != authorizedWallet) {
            require(public_mint_status, "Public mint status is off");
            require(balanceOf(msg.sender, 0) + quantity <= max_per_wallet, "Per Wallet Limit Reached");
            require(msg.value >= getCostInBNB(id) * quantity, "You do not have enough tokens to perform the transaction");    
        }

        callPurchaseProduct(msg.sender, 3);
        _mint(msg.sender, id, quantity, "");
    }

    function getCostInBNB(uint256 ID) public view returns (uint256) {
        return gateway.getCost(publicSaleCost[ID]);
    } 
    ...
    function withdraw() external onlyOwner {
        (bool main, ) = (owner()).call{value: address(this).balance}(
            ""
        );
        require(main);
    }

    // Example function to call PurchaseProduct from the gateway contract
    function callPurchaseProduct(address _recipient, uint _productId) public payable {
        // Use the instantiated interface to call PurchaseProduct
        bool purchaseResult = gateway.PurchaseProduct{value: msg.value}(_recipient, _productId);

        // Handle the result as needed
        require(purchaseResult, "Failed to purchase product");
    }
    ...
    function setPublicSaleCost(uint256 id, uint256 cost) external onlyOwner {
        publicSaleCost[id] = cost;
    }
    ...
    function setMax_ids(uint256 _max_ids) external onlyOwner {
        max_ids = _max_ids;
    }
    ...
    function setGatewayContract(address _gatewayContract) public onlyOwner {
        gateway = IHealEstateGateway(_gatewayContract);
    }
}