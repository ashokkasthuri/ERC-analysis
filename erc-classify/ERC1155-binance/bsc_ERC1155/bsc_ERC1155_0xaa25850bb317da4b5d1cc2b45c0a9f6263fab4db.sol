// Chain: BSC - File: contracts/OnekeyToken.sol
...
contract OnekeyToken is ERC1155, AccessControl, Pausable, ERC1155Burnable {
    ...
    uint256 public lockEndAt;

    // ========== EVENTS ==========
    event SetSupplies(uint256 pID, uint256 number);
    event SetLockEndAt(uint256 lockEndAt);

    // ========== CONSTRUCTOR ==========
    // solhint-disable-next-line
    constructor() ERC1155("https://onekey.so/api/item/{id}.json") {
    ...
    // ========== SETTERS ==========
    function setSupplies(uint256 _pID, uint256 _number) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        totalSupplies[_pID] = _number;
        emit SetSupplies(_pID, _number);
    }
    ...
    function setLockEndAt(uint256 _lockEndAt) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        lockEndAt = _lockEndAt;
        emit SetLockEndAt(_lockEndAt);
    }
    ...
    function mint(
        ...
        require(
            totalSupplies[_id] == 0 ||
                totalSupplies[_id] >= totalBalances[_id] + _amount,
            "OVERSELLING"
        );

        totalBalances[_id] += _amount;
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        require(block.number > lockEndAt, "LOCKED");
