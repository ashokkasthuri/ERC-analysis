/** 
 *  SourceUnit: d:\GitWork\FLKING\RaribleClone\Contracts\contracts\MultipleNFTManiaFixed.sol
*/
...
// Multiple NFTMania Fixed Price Marketplace contract
////// SPDX-License-Identifier-FLATTEN-SUPPRESS-WARNING: MIT 
pragma solidity ^0.8.0;
...
////import "./MultipleNFTMania.sol";
////import "./IBEP20.sol";

interface IMultipleNFTMania {
	function initialize(string memory _name, string memory _uri, address creator, bool bPublic) external;
...
contract MultipleNFTManiaFixed is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function initialize(address _feeAddress) external onlyOwner {
		...
		createCollection("Mania", "https://ipfshash.nftmania.app/ipfs/QmZEfbABBQHxLnw8cq78AWWoKZ8bkbduQUMxza67284Hdw", true);
	}
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(MultipleNFTMania).creationCode;
        ...
        IMultipleNFTMania(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function multipleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _amount, uint256 _price) public {
		...
		IMultipleNFTMania nft = IMultipleNFTMania(_collection);
	...
	function multipleDelist(uint256 _id) external {
		...
		IMultipleNFTMania(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId, pairs[_id].balance, "delist Marketplace");
		pairs[_id].balance = 0;
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external {
        require(pairs[_id].bValid, "invalid Pair id");
		...
		require(token_balance >= tokenAmount, "insufficient token balance");

		// transfer governance token to admin
		if(swapFee > 0) {
			require(governanceToken.transferFrom(msg.sender, feeAddress, tokenAmount.mul(swapFee).div(PERCENTS_DIVIDER)), "failed to transfer admin fee");
		}
		// transfer governance token to creator
		if(item.creatorFee > 0) {
			require(governanceToken.transferFrom(msg.sender, item.creator, tokenAmount.mul(item.creatorFee).div(PERCENTS_DIVIDER)), "failed to transfer creator fee");
		}
		// transfer governance token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(item.creatorFee);
		require(governanceToken.transferFrom(msg.sender, item.owner, tokenAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)), "failed to transfer to owner");

		// transfer NFT token to buyer
		IMultipleNFTMania(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, item.tokenId, _amount, "buy from Marketplace");
