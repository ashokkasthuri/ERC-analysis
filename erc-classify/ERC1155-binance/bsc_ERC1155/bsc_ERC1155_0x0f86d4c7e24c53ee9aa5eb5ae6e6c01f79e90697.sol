// Chain: BSC - File: contracts/LotteryNFT.sol
...
contract LotteryNFT is ERC1155Upgradeable, OwnableUpgradeable, Testable {
    // Libraries 
    // Safe math
    ...
    // Lottery ID => Ticket IDs
    mapping(uint256 => uint256[]) internal lotteryTickets_;
     ...
     * @param   _timer Only need to be set for local unit test, otherwise using zero address.
     */
    function initialize(
        string memory _uri,
        address _timer
    ...
    {
        __Ownable_init();
        __Testable_init(_timer);
        __ERC1155_init(_uri);
    }

    function setLottery(address _lotto) external onlyOwner() {
    ...
    function getLotteryTicketsPagination(
        uint256 _lotteryId,
        uint256 cursor, 
        uint256 size
    ) 
        ...
        returns(uint256[] memory, uint256) 
    {
        ...
        if (length > lotteryTickets_[_lotteryId].length - cursor) {
            length = lotteryTickets_[_lotteryId].length - cursor;
        }

        uint256[] memory values = new uint256[](length);
        for (uint256 i = 0; i < length; i++) {
            values[i] = lotteryTickets_[_lotteryId][cursor + i];
        }
        return (values, length);
    }

    function getLotteryTicketsLength(
        uint256 _lotteryId
    )
        external
        view
        returns (uint256) 
    {
        return lotteryTickets_[_lotteryId].length;    
    }
    ...
    function getUserTicketsWithNumber(
        uint256 _lotteryId,
        address _user
    ) 
        external 
        view 
        returns(uint256[] memory tickets, uint256[] memory numbers) 
    {
        tickets = userTickets_[_user][_lotteryId];
        uint8 size = ILottery(lotteryContract_).getNumberSize();
        uint256 numbersLength = tickets.length.mul(size);

        numbers = new uint256[](numbersLength);
        for ( uint256 i = 0; i < tickets.length; i ++ ) {
            uint256 ticketId = tickets[i];

            for ( uint8 k = 0; k < size; k ++ ) {
                uint256 tupleIndex = i.mul(size);
                numbers[tupleIndex + k] = ticketInfo_[ticketId].numbers[k];
            }
        }
    }

    function getLotteryTicketsWithNumber(
        uint256 _lotteryId,
        uint256 cursor, 
        uint256 size
    ) 
        ...
        returns(uint256[] memory tickets, uint256[] memory numbers) 
    {
        uint256[] memory allTickets = lotteryTickets_[_lotteryId];
        uint256 numberOfTickets = allTickets.length;
        
        uint256 readSize = numberOfTickets > cursor ? (numberOfTickets - cursor) : 0;
        if ( readSize == 0 ) {
            return (tickets, numbers);
        }
    
        tickets = new uint256[](readSize);
        uint8 size = ILottery(lotteryContract_).getNumberSize();
        uint256 numbersLength = tickets.length.mul(size);
        numbers = new uint256[](numbersLength);

        for ( uint256 i = cursor; i < numberOfTickets; i ++ ) {
            uint256 ticketId = allTickets[i];
            tickets[i - cursor] = ticketId;
    ...
    function getLotteryTickets(
        ...
        returns (uint256[] memory)
    {
        return lotteryTickets_[_lotteryId];
    }
        ...
        returns(uint256[] memory)
    {
        ...
        for (uint8 i = 0; i < _numberOfTickets; i++) {
            ...
            lotteryTickets_[_lotteryId].push(totalSupply_);
        }
...
// Chain: BSC - File: contracts/interfaces/ILottery.sol
//SPDX-License-Identifier: MIT
pragma solidity >= 0.6.0 < 0.8.0;
...
interface ILottery {
    ...
    function getNumberSize() external view returns(uint8);
...
// Chain: BSC - File: contracts/Testable.sol
// SPDX-License-Identifier: MIT
pragma solidity >= 0.6.0 < 0.8.0;
 ...
 */
abstract contract Testable is Initializable{
    // If the contract is being run on the test network, then `timerAddress` will be the 0x0 address.
    // Note: this variable should be set on construction and never modified.
    address public timerAddress;

    function __Testable_init(address _timerAddress) internal initializer {
...
// Chain: BSC - File: contracts/library/SafeMath16.sol
// SPDX-License-Identifier: MIT
pragma solidity >= 0.6.0 < 0.8.0;
...
// Chain: BSC - File: contracts/library/SafeMath8.sol
// SPDX-License-Identifier: MIT
pragma solidity >= 0.6.0 < 0.8.0;
...
// Chain: BSC - File: contracts/Timer.sol
// SPDX-License-Identifier: MIT
pragma solidity >= 0.6.0 < 0.8.0;
