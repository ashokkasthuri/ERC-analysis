// Chain: BSC - File: contracts/overdome1155.sol
...
contract Overdome is ERC1155, Ownable, Pausable {
    mapping(uint256 => string) private _tokenURIs;

    uint public price = 0.033 ether;
    uint public discountPercent = 10; // 10 = 10%
    uint public amountForDiscount = 10;

    // whitelist
    mapping(address => bool) public whitelisted;

    constructor() ERC1155("") Ownable(msg.sender) {
        _tokenURIs[0] = "https://bafkreigwycrzolkg4mll223hhwwscm224umzllhivnlcuuvt2e7ct52kgu.ipfs.nftstorage.link/";
    ...
    function setDiscountPercent(uint _percent) external onlyOwner {
        discountPercent = _percent;
    }

    function setAmountForDiscount(uint _amount) external onlyOwner {
        amountForDiscount = _amount;
    }

    function setURI(string calldata newURI, uint _id) external onlyOwner {
        _tokenURIs[_id] = newURI;
    }
    ...
    function getPrice(uint256 amount) public view returns (uint) {
        if (amount >= amountForDiscount) {
            return price * amount * (100 - discountPercent) / 100;
        } else {
            return price * amount;
        }
    }
    ...
    function mint(
        ...
        require(keccak256(abi.encodePacked(_tokenURIs[id])) != keccak256(abi.encodePacked("")), "URI not set");
        if (!whitelisted[msg.sender]) {
            require(msg.value == getPrice(amount), "Invalid price");
        }
