    ...
    ReentrancyGuardUpgradeable
{
    ...
    function payoutMarketingAndReferrals(uint256 level) internal {
        ...
        if (totalReferralPoints[level][levels[level].currentRaffle] > 0) {
            // payout referrals 5% of collection
            // referralEarnings = 5% of collection
        ...
        uint256 teamEarnings = ((levels[level].buyIn * totalSupply) * 22) / 100; //(22%)
    ...
    function payoutWinners(uint256 level) public {
        ...
        if (level < 4) {
            uint256 dripBought = swapToDrip((totalCollected * 73) / 100);
            ...
            uint256 dripBought = swapToDrip((totalCollected * 5) / 100);
            // send 5% to drip vault
            IERC20(dripToken).transfer(
                0xBFF8a1F9B5165B787a00659216D7313354D25472,
                dripBought
