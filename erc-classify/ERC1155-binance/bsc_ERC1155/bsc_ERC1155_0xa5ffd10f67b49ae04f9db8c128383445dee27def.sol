...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    struct CampaignInfo {
        ...
        mapping(uint256 => uint256) phasesToTime; // 1 => end timestamp, 2 => end timestamp
    ...
    function initialize(
        ...
        address _quoteErc20Address, // ZWZ token
        address _admin
    ...
    function updateCampaignTimePhase(
        uint256 _campaignId, 
        ...
        public onlyWhiteListUser 
    {
        ...
        require(_startEnds[1] > _startEnds[0],'Invalid start end time');
        CampaignInfo storage info = campaigns[_campaignId];
        info.start = _startEnds[0];
        info.end = _startEnds[1];
        for (uint256 j = 1; j < _phases.length; j++) {
            require(_phases[j] > info.start && _phases[j] <= info.end,"Invalid phase time");
    ...
    function getCurrentPhase(uint256 _campaignId) public view returns(uint256) {
        ...
        uint256 currPhase;
        for(uint256 i = 1; i <= campaign.maxPhase; i++) {
            if(block.timestamp >= campaign.start && block.timestamp <= campaign.phasesToTime[i]) {
                currPhase = campaign.phasesToTime[i];
            }
        }
        return currPhase;
    }
    ...
    function buyMysteryBox(
        ...
        require(campaign.usrToAmount[msg.sender] <= campaign.maxBuy, "Limited Quantity");
    ...
    function updateCampaignTime(uint256 campaignId, uint256 _start, uint256 _end) public onlyWhiteListUser {
        CampaignInfo storage campaign = campaigns[campaignId];
        require(_end > _start, "Invalid time");
        campaign.start = _start;
        campaign.end = _end;
        emit UpdateEndTime(_msgSender(), campaignId, _start, _end);
    }
