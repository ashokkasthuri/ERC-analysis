...
// Chain: BSC - File: @openzeppelin/contracts/token/common/ERC2981.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: contracts/BridgeNFT.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0; 
...
// Chain: BSC - File: contracts/BridgeNFTBatch.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0; 
...
// Chain: BSC - File: contracts/erc1155Collection.sol
// SPDX-License-Identifier:MIT
pragma solidity ^0.8.0;
...
contract ERC1155RoyaltiesCollection is
    ERC1155Supply,
    Ownable,
    BridgeNFTBatch,
    ERC2981,
    DefaultOperatorFilterer
{
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "Method not allowed");
        _;
    }

    modifier onlyBridge() {
        require(msg.sender == bridgeAddress, "Method not allowed");
        _;
    }
    ...
    function _burn(
        ...
        uint256 tokenId,
        uint256 qty
    ) internal virtual override onlyBridge{
        super._burn(from, tokenId, qty);
        _resetTokenRoyalty(tokenId);
    }

    function burnFor(address from, uint256 id) external override onlyBridge{
        _burn(from, id, 1);
    }
    ...
    function mintBatch(
        ...
        if (rawInfo.length != 0) {
            ...
            if (
                setRoyaltyInfo[0].receiver != address(0) &&
                setRoyaltyInfo.length == ids.length
            ) {
                for (uint256 i = 0; i < setRoyaltyInfo.length; i++) {
                    _setTokenRoyalty(
                        ids[i],
                        setRoyaltyInfo[i].receiver,
                        setRoyaltyInfo[i].feeNumerator
                    );
                }
            }
        }
    ...
    function updateBaseUri(string memory newBaseUri) external onlyAdmin {
        require(
            baseUriUpdatable == true,
            "Base URI not updatable at the moment"
        );
        _baseURI = newBaseUri;
        emit baseUriUpdated(newBaseUri);
    }

    function pauseURIUpdate() external onlyAdmin {
        require(baseUriUpdatable == true, "URI update already paused");
        baseUriUpdatable = false;
    }

    function unpauseURIupdate() external onlyAdmin {
        require(baseUriUpdatable == false, "URI update already unpaused");
        baseUriUpdatable = true;
    }
    ...
    {
        require(exists(_tokenID), "NON-Existent _tokenID");
    ...
    function baseURI() override external view returns (string memory)  {
        uint256 tokenId = getFirstTokenBaseURL();
        return uri(tokenId);
    }

    function getFirstTokenBaseURL() public view returns (uint256) {
        ...
        revert("No tokens exist");
    }
}

// Chain: BSC - File: operator-filter-registry/src/DefaultOperatorFilterer.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: operator-filter-registry/src/lib/Constants.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;
...
// Chain: BSC - File: operator-filter-registry/src/OperatorFilterer.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;
