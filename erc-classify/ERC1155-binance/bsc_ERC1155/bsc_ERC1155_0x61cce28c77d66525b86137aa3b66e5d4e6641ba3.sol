...
contract testBase is testAccessControl,ERC1155{
    ...
    uint16 itemName,uint16 gameId,bytes memory data)public virtual onlyPartner() returns(uint256){
        ...
        uint256 time = 1;
        if (legendary <= 100) {
            while ((strength.add(intelligence).add(agility) >= 240 || 
            (strength > 85 && intelligence > 85) || 
            (intelligence > 85 && agility > 85) || 
            (agility > 85 && strength >85)) && time < 3) {
    ...
    function levelUpItem(uint256 _tokenId1, uint256 _tokenId2) public virtual onlyPartner returns(bool){
        ...
        if (isSuccessfulAction(_tokenId1, 4)) {
        ...
        creators[newItemId] = _msgSender();
        _tokenIds.increment();
        if (getRarity(_tokenId2) == 0) legendary += 1;
        return true;
    }
    
    function upgradeStar(uint256 _tokenId1, uint256 _tokenId2, uint16 choice) public virtual onlyPartner returns(bool){
        ItemAttr storage token1 = tokenIdToItemAttr[_tokenId1];
        ItemAttr storage token2 = tokenIdToItemAttr[_tokenId2];
    ...
    function giveBirth(uint256 _tokenId, bytes memory data) public virtual onlyPartner returns(uint256){
        ItemAttr storage token = tokenIdToItemAttr[_tokenId];
        ...
        if (legendary <= 100) {
            while ((strength.add(intelligence).add(agility) >= 260 || strength > 92 ||
                intelligence > 92 || agility >92) && time < 3) {
                genes = _generateRandomDna(time);
                strength = genes.mod(100);
                intelligence = (genes.mod(10000)).div(100);
                agility = (genes.mod(1000000)).div(10000);
