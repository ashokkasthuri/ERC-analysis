// Chain: BSC - File: mock1155WithMarketMintLPWhitelist.sol
...
contract Origin is ERC1155, Ownable, Pausable, ERC1155Supply, ReentrancyGuard  {
    using ECDSA for bytes32;
    using SafeERC20 for IERC20;
    
    
    uint256[] public caps = [0, 600, 300, 99];
    address public uniswapRouter;
    address public og;
    address public usdt;
    address public uniswapPair;

    uint256 public whitelistGranted;

    uint256 private _whitelistCanMint;
    uint256 private _whitelistLPCount;
    uint256 private _whitelistThreshold;
    uint256 private _whitelistThresholdStep;
    uint256 private _whitelistThresholdInterval;
    uint256 private _whitelistTotal;
    address private _copyright;
    uint8 private _royalty;
    uint256 public sellingQuotaAmount;
    uint256 public sellingQuotaPrice;
    LockReceiptLP[] public _lockReceipts;
    
    mapping(address => uint256[10]) public invited;
    mapping(address => bool) public whitelist;
    mapping(address => uint256) public quota;
    mapping(address => uint256) public minted;
    mapping(uint256 => List[]) private onSaleLists;         //public better?
    ...
    constructor() ERC1155("http://localhost:3000/images/nft{id}.png") {

        whitelistGranted = 0;   
        _whitelistCanMint = 1;      //one can mint
        _whitelistLPCount = 6;      //need refer 6 people to be lp
        _whitelistThreshold = 100 * 10 ** 18;    //100u start
        _whitelistThresholdStep = 50 * 10 ** 18;    //50u step
        _whitelistThresholdInterval = 10;    // threshold increase every interval
        _whitelistTotal = 100;    // threshold increase every interval

        og = 0x70Be7F4Be6b51bC42d5CEC0d9C2ea019a2F53d67;
        uniswapRouter = OG(og).uniswapRouter();
        usdt = OG(og).usdt();
        uniswapPair = OG(og).uniswapPair();

        _royalty = 5;
        _copyright = msg.sender;

        sellingQuotaAmount = 0;
        sellingQuotaPrice = 1 * 10 ** 18;

        IERC20(og).approve(uniswapRouter, type(uint256).max);
        IERC20(usdt).approve(uniswapRouter, type(uint256).max);
    }

    function setWhitelistTotal(uint256 _wt) external onlyOwner {
        _whitelistTotal = _wt;
    }

    function setUniswapRouter(address _uniswapRouter) external onlyOwner {
        uniswapRouter = _uniswapRouter;
        IERC20(og).approve(_uniswapRouter, type(uint256).max);
        IERC20(usdt).approve(_uniswapRouter, type(uint256).max);
    ...
    function setCopyright(address newCopyright) public onlyOwner {
        _copyright = newCopyright;
    }

    function setRoyalty(uint8 newRoyalty) public onlyOwner {
        _royalty = newRoyalty;
    }
    ...
    function addLiquidityForWhitelist() public payable nonReentrant {
        require( whitelistGranted < _whitelistTotal, "No more whtielist");
        
        IERC20(usdt).safeTransferFrom(msg.sender, address(this), getCurrentThreshold());
        IERC20(og).safeTransferFrom(msg.sender, address(this), getCurrentThresholdOG());

        ( , , uint256 lp) = IUniswapV2Router02(uniswapRouter).addLiquidity(usdt, og, getCurrentThreshold(), getCurrentThresholdOG(), getCurrentThreshold() * 90 / 100, getCurrentThresholdOG() * 90 / 100, address(this), block.timestamp + 1 hours );
        
        //lock lp
        _lockReceipts.push(LockReceiptLP(msg.sender, lp, block.timestamp + 30 days, false));

        //add invited for inviter
        address[] memory inviters = OG(og).getInviter(msg.sender, 6);
        uint256 currentBatch = whitelistGranted / _whitelistThresholdInterval;

        
        invited[inviters[0]][currentBatch] += 1;

        if( invited[inviters[0]][currentBatch] >= _whitelistLPCount && !whitelist[inviters[0]] ){
            whitelistGranted += 1;
            whitelist[inviters[0]] = true;
        }

    }


    function whitelistMint( uint256 amount ) public payable nonReentrant {
    
        require( whitelist[msg.sender] , "No whitelist");
        require( minted[msg.sender] < _whitelistCanMint , "No quota");

    
        uint256[] memory availables = new uint256[](caps.length);
        uint256 totalAvailable = 0;
        uint256 pseudoTokenId = 0;

        for (uint i = 1; i < caps.length; i++) {
            availables[i] = totalSupply(i) < caps[i] ? caps[i] - totalSupply(i) : 0;
            totalAvailable += availables[i];
        }

        for (uint i = 0; i < amount; i++) { 
            pseudoTokenId = getShitPseudoRandomLimited(totalAvailable, i);

            for (uint j = 1; j < availables.length; j++){
                if( pseudoTokenId <= availables[j]){
    ...
    function dealLockReceipts(uint256 skip) public payable nonReentrant {

        for (uint i = skip; i < _lockReceipts.length; i++) {
            LockReceiptLP storage lr = _lockReceipts[i];

            if(!lr.done && lr.deadline < block.timestamp ){
                IERC20(uniswapPair).safeTransfer(lr.owner, lr.amount);
                lr.done = true;
            }
        }
    }
    ...
    function getCurrentThreshold() public view returns (uint256) {
        return _whitelistThreshold + (whitelistGranted / _whitelistThresholdInterval * _whitelistThresholdStep);
    }

    function getCurrentThresholdOG() public view returns (uint256) {
        (uint reserveA, uint reserveB, ) = IUniswapV2Pair(uniswapPair).getReserves();
        uint amountOGOptimal = IUniswapV2Router02(uniswapRouter).quote(getCurrentThreshold(), reserveA, reserveB);
        return amountOGOptimal;
    }
    ...
    function getShitPseudoRandomLimited(uint256 max, uint256 nonce) private view returns (uint256) {
        return 1 + uint256(keccak256(abi.encodePacked(block.timestamp , block.difficulty, nonce))) % max;
    }
    

    function sellQuota(uint256 amount, uint256 price) public onlyOwner
    {
        sellingQuotaAmount = amount;
        sellingQuotaPrice = price;
    }

    function buyQuota( uint256 amount ) public payable nonReentrant {
    
        require( sellingQuotaAmount >= amount , "No selling quota");
        require( msg.value >= amount * sellingQuotaPrice, "not enought eth");

        sellingQuotaAmount -= amount;
        quota[msg.sender] += amount;

    }

    function quotaMint( uint256 amount ) public payable nonReentrant {
    
        require( quota[msg.sender] >= amount , "No enough quota");
    ...
    function listItem(
        ...
        _preListItem(tokenId, msg.sender);
        _listItem(tokenId, price, deadline);
    }
    ...
    function buy(
        ...
        _buyItem(tokenId, listIndex);
    }

    function getLists(uint256 tokenId)
        public
        view
        returns (List[] memory)
    {
        uint256 length = onSaleLists[tokenId].length;
        List[] memory listed = new List[](length);
        for (uint256 index = 0; index < length; index++) {
            listed[index] = onSaleLists[tokenId][index];
        }
        return listed;
    }

    function _preListItem( uint256 tokenId, address owner) internal {

        //if list more than balance, cancel the first one to list new
        uint256 count = 0;
        uint256 first = type(uint256).max;
        uint256 length = onSaleLists[tokenId].length;
        List memory temp;
        for (uint256 index = 0; index < length; index++) {
            temp = onSaleLists[tokenId][index];
            if( !temp.sold &&  temp.deadline > block.timestamp && temp.owner == msg.sender ){
                count++;
                if( first == type(uint256).max ) first = index;
            } 
        }

        if( count >= balanceOf(owner, tokenId) ) delete (onSaleLists[tokenId][first]);
    ...
    function _listItem(
        ...
        onSaleLists[tokenId].push(List(price, msg.sender, deadline, false));
        emit ItemListed(msg.sender, tokenId, price);
    }

    function _unListItem(
        uint256 tokenId,
        uint256 listIndex
    ) internal {
        require(onSaleLists[tokenId][listIndex].owner == msg.sender, "not your list");
        // List storage list = onSaleLists[tokenId][listIndex];
        // list.deadline = 0;
        delete (onSaleLists[tokenId][listIndex]);
        emit ItemCanceled(tokenId, listIndex);
    }
    ...
    function _buyItem(
        ...
        List storage list = onSaleLists[tokenId][listIndex];
        uint256 tax = 0;
        require(msg.value >= list.price, "not enought eth");
        require(!list.sold, "already sold");
        require(list.deadline > block.timestamp, "expired");
        ...
        if(_royalty != 0){
            tax = list.price * _royalty / 100;
            (bool success, ) = (_copyright).call{value: tax}("");
        }

        (bool success, ) = (list.owner).call{value: list.price - tax}("");
        require(success, "Transfer failed.");
        list.sold = true;
        
        emit ItemBought(msg.sender, tokenId, list.price);
    }
...
// Chain: BSC - File: IUniswapV2Pair.sol
...
// Chain: BSC - File: IUniswapV2Router02.sol
...
// Chain: BSC - File: rewardsWithDividendWithBurnLP.sol
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.10;
...
// Chain: BSC - File: rewardsWithDividend.sol
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.10;
...
// Chain: BSC - File: @openzeppelin/contracts/token/ERC20/extensions/ERC20Burnable.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: IUniswapV2Router01.sol
pragma solidity >=0.6.2;
