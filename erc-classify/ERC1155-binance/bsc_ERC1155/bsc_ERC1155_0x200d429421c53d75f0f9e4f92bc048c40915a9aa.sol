// Chain: BSC - File: contracts/tokens/MetaFlokiRushNFT.sol
...
contract MetaFlokiRushNFT is NFTVillageERC1155 {
    constructor(string memory _uri) NFTVillageERC1155 ("MetaFlokiRushNFT", "MFlokiRNFT", _uri) {}
}

// Chain: BSC - File: contracts/tokens/NFTVillageERC1155.sol
// SPDX-License-Identifier: MIT
...
contract NFTVillageERC1155 is IHasSecondarySaleFees, Ownable, NFTVillageCardFeatures, ERC1155 {
    ...
    function getFeeRecipients(uint256 id) external override view returns (address payable[] memory) {
    ...
    function getFeeBps(uint256 id) external override view returns (uint[] memory) {
        Fee[] memory _fees = fees[id];
    ...
    function mint(address _to, uint256 _id, uint256 _amount, Fee[] memory _fees) external onlyOwner {
        _mint(_to, _id, _amount, "");
        _addSecondaryFee(_id, _fees);
    }

    function mintBatch(address _to, uint256[] memory _ids, uint256[] memory amounts, Fee[][] memory _fees) external onlyOwner {
        _mintBatch(_to, _ids, amounts, "");
        for (uint256 i = 0; i < _fees.length; i++) {
            _addSecondaryFee(_ids[i], _fees[i]);
        }
    }

    function _addSecondaryFee(uint256 _id, Fee[] memory _fees) internal {
    ...
    function addSecondaryFee(uint256 _id, Fee[] memory _fees) external onlyOwner  {
        _addSecondaryFee(_id, _fees);
    }
...
// Chain: BSC - File: contracts/exchange/IHasSecondarySaleFees.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
// Chain: BSC - File: contracts/tokens/NFTVillageCardFeatures.sol
// SPDX-License-Identifier: MIT
...
contract NFTVillageCardFeatures is  Ownable, INFTVillageCardFeatures {
    ...
    function getHarvestReliefCards() external override view returns(uint256[] memory) {
        return harvestReliefCards;
    }
    function getFeeDiscountCards() external override view returns(uint256[] memory) {
        return feeDiscountCards;
    }
    function getMultiplierCards() external override view returns(uint256[] memory) {
...
// Chain: BSC - File: contracts/interfaces/INFTVillageCardFeatures.sol
// SPDX-License-Identifier: MIT
...
abstract contract INFTVillageCardFeatures {
    function addHarvestReliefCard(uint256 _tokenId, uint256 _harvestRelief) virtual external;
    function addFeeDiscountCard(uint256 _tokenId, uint256 _feeDiscount) virtual external;
    function addMultiplierCard(uint256 _tokenId, uint256 _multiplier) virtual external;
    
    function removeHarvestReliefCard(uint256 _tokenId) virtual external;
    function removeFeeDiscountCard(uint256 _tokenId) virtual external;
    function removeMultiplierCard(uint256 _tokenId) virtual external;

    function getHarvestReliefCards() virtual external view returns(uint256[] memory);
    function getFeeDiscountCards() virtual external view returns(uint256[] memory);
    function getMultiplierCards() virtual external view returns(uint256[] memory);
