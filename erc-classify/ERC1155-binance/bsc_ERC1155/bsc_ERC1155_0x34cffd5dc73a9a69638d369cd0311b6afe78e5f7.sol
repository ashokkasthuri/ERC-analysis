...
contract BirdNFT is ERC1155, IBirdNFT, Ownable{
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(address => uint256) private nonces;
    mapping(uint256 => uint256) public tokenSupply;
    mapping(uint256 => uint8) public birdId;
    event TransferWithPermission(address from, address to, uint256 tokenId, uint256 nonce);
    ...
    modifier onlyDev() {
        require(
            ...
            "Permission Error: caller is not the dev"
        );
        _;
    }


    constructor(string memory NFT_URI, address _devAddress) ERC1155(NFT_URI) {
    ...
    function createSeveral1155NFT(
        ...
        for (uint i = 0; i < idsAmount; i++) {     
            birdId[newIdsArray[i]] = birdIds[i];
        }
        ...
        returns (uint256)
    {
     ...
     function permit(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(
                keccak256(
                    "Permit(address owner,address spender,uint256 tokenId,uint256 deadline)"
                ),
                owner,
                spender,
                tokenId,
    ...
    function transferWithPermission(
        ...
        emit TransferWithPermission(from, to, tokenId, nonce + 1);
        return true;
    }
    function approveAll1155NFT(
        address owner,
    ...
    function setDevAddress(address _newDevAddress) public onlyOwner {
        require(
