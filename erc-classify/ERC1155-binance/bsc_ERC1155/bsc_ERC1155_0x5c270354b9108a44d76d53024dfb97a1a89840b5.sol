// Chain: BSC - File: contracts/marketplace/Marketplace.sol
    ...
    AccessControlUpgradeable
{
    ...
    function initData(
        ...
        treasuryBox = payable(_treasuryBox);
        bcfAddress = _BCFAddress;
        busdAddress = _BUSDAddress;
        cycAddress = _CYCAddress;
    }
    ...
    address public treasuryBox;
    IERC20 public bcfAddress;
    IERC20 public cycAddress;
    ...
    mapping(uint16 => uint256) public boxPrice;
    ...
    function buyNFTBox(uint16 boxId, uint8 amountNFT) external {
        require(amountNFT > 0, "INVALID_AMOUNT");
        uint256 price = boxPrice[boxId];
        require(price > 0, "INVALID_BOX");
        uint256 totalPrice = price * amountNFT;
        busdAddress.transferFrom(_msgSender(), treasuryBox, totalPrice);
        emit BuyBox(_msgSender(), boxId, amountNFT, totalPrice);
    }

    function whitelistBox(uint256[] calldata _boxPrice) external onlyOwner {
        require(_boxPrice.length > 0, "BOX");
        for (uint16 i = 0; i < _boxPrice.length; i++) {
            if (_boxPrice[i] > 0) {
                boxPrice[i] = _boxPrice[i] * 1e18;
            }
        }
    }
