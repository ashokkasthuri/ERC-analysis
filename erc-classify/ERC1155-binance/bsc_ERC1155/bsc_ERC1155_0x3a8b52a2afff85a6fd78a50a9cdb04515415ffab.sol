...
contract Chest is Miner, ERC1155, Random{
    ...
    mapping (uint => SkillItem[]) private _skillProbability; //chestIndex => skillProbability
    ...
    constructor() public ERC1155("https://nftsbattle.com/metadata/chests/{id}.json") {}
    ...
    // Set skill probability
    function setSkillProbability (uint chestIndex, uint skillIndex, uint8 skillQuality, uint probability) external onlyOwner{
        _skillProbability[chestIndex].push(SkillItem(skillIndex, skillQuality, probability));
    }

    function skillProbability (uint chestIndex) view public returns(SkillItem[] memory) {
        return _skillProbability[chestIndex];
    }

    function initSkillProbability(uint chestIndex) external onlyOwner{
        SkillItem[] storage newArray;
        _skillProbability[chestIndex] = newArray;
    }
    ...
    function _openChest(uint chestIndex)  private{
        SkillItem[] memory newSkillProbability = skillProbability(chestIndex);
        uint probability = 0;
        uint allProbability = 0;
        // check probability
        for (uint256 j = 0; j < newSkillProbability.length; j++){
            allProbability = allProbability.add(newSkillProbability[j].probability);
        }
        uint randomNumber = _getRandomNumber() % allProbability;
        for (uint256 i = 0; i < newSkillProbability.length; i++){
            probability = probability.add(newSkillProbability[i].probability);
            if(probability >= randomNumber){
                _mintSkill(
                    msg.sender, 
                    newSkillProbability[i].skillQuality, 
                    newSkillProbability[i].skillIndex
                );
                break;
            }
        }
