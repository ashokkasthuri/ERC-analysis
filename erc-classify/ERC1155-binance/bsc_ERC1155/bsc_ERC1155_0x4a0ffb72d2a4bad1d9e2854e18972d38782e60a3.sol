// Chain: BSC - File: contracts/ClashFantasyDecks.sol
...
contract ClashFantasyDecks is ERC1155Upgradeable {
    ...
    function addCardToDeck(uint256 _tokenId, uint256 _cardId)
        public
    // limitCardDeck(msg.sender, _tokenId)
    {
        (
            address _cardOwner,
            uint256 _amount,
        ...
        require(
            _hasMana == 1 && _amount > 0,
            "addCardToDeck: Card Must have mana || Card Amount 0"
        );
        ...
        require(
            _cardLevel <= deck.deckLevel,
            "addCardToDeck: Cannot add a card higher level than the deck"
        );
    ...
    function withdrawCardDeck(uint256 _tokenId, uint256 _cardId)
        ...
        // existsCardRef(_tokenId, _cardId)
    {
        Decks storage deck = decksArray[msg.sender][userRefTokenId[msg.sender][_tokenId].index];
    ...
    //internal
    function randomLevel(uint256[] memory data) public view returns (uint256, uint256) {
        uint256 count = 0;
        ...
        uint256 purchasenumber = uint256(
            keccak256(
                abi.encodePacked(block.difficulty, block.timestamp, msg.sender, tokenIds.current())
            )
        ) % 1000;
