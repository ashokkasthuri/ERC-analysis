...
// Chain: BSC - File: contracts/AvatarNFT.sol
// SPDX-License-Identifier: MIT
...
contract AvatarNFT is ERC1155, Ownable {
    using Strings for uint256;  // Enable Strings.toString function for uint256

    uint256 public constant AVATAR_TYPE_1 = 0;
    uint256 public constant AVATAR_TYPE_2 = 1;
    uint256 public constant AVATAR_TYPE_3 = 2;

    uint256 public constant SUPPLY_PER_TYPE = 333;

    string public name = "AvatarNFT";
    string public symbol = "AVATAR";

    // Mapping to store the custom URIs for each avatar type
    mapping(uint256 => string) private _tokenURIs;

    constructor(address initialOwner, string memory baseUri) ERC1155(baseUri) Ownable(initialOwner) {
        // Mint 333 copies of each avatar type
        _mint(msg.sender, AVATAR_TYPE_1, SUPPLY_PER_TYPE, "");
        _mint(msg.sender, AVATAR_TYPE_2, SUPPLY_PER_TYPE, "");
        _mint(msg.sender, AVATAR_TYPE_3, SUPPLY_PER_TYPE, "");
    }

    // Set a custom URI for a specific avatar type
    function setTokenURI(uint256 tokenId, string memory newUri) public onlyOwner {
        _tokenURIs[tokenId] = newUri;
    }

    // Override the URI function to return the custom URI for each token type if set
    function uri(uint256 tokenId) override public view returns (string memory) {
        require(tokenId <= AVATAR_TYPE_3, "Invalid token ID");
