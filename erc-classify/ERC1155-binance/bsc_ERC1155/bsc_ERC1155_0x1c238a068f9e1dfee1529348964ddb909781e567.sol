...
contract AntMiner is AccessControl, ERC1155, ERC1155Burnable {
    using SafeERC20 for IERC20;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 constant BOX = 1;
    uint256 constant FRAGMENT = 2;

    uint256 constant WORKER = 3;
    uint256 constant SOLDIER = 4;
    uint256 constant MALE = 5;
    uint256 constant QUEEN = 6;
    ...
    constructor() ERC1155("https://antminer.world/api/token/{id}.json") {
    ...
    function setReceiveToken(address value) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _receiveToken = value;
    }
    ...
    function open() external  {
        ...
        if ((totalSupply(QUEEN) + totalSupply(MALE) + totalSupply(SOLDIER) + totalSupply(WORKER)) % 1000 == 0) {
    ...
    function purchase(uint256 amount, address inviter) external  {
        address account = _msgSender();
        if (_inviters[account] == address(0) && inviter != address(0)) {
        ...
        IERC20(_receiveToken).safeTransferFrom(account, address(this), payment);
    }

    function airdrop(address inviter) external {
        require(_airdropStatus, "The airdrop has ended");

        address account = _msgSender();
        require(_airdropped[account] == false, "Already received airdrop rewards");
