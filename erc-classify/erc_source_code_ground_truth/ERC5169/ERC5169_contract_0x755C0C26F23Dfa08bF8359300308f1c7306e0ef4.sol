// Details: https://earnmates.io/erc-o
...
import {ERC5169} from "./ERC5169.sol";
import {ERC404} from "./ERC404.sol";

contract ERCO is Ownable, ERC404, ERC5169 {
    using Strings for uint256;
    using EnumerableSet for EnumerableSet.UintSet;

    uint8 constant _decimals = 18;
    uint256 constant _mintSupply = 4096;

    event Set721TransferExempt(address txExempt);
    event Feed(uint256 tokenId, uint256 exp);
    event Invite(uint256 tokenId, uint256 invitedId);
    event AcceptPlayDate(uint256 tokenId, uint256 inviterId);
    event Breakup(uint256 tokenId);

    string private constant __NAME = "ERC-O";
    string private constant __SYM = "ERCO";
    string private _websiteUri = "https://earnmates.io";
    string private _contractUri = "https://earnmates.io/erco-metadata.json";
    string public baseURI = "https://earnmates.io/api/erco/";

    mapping(uint256 => uint256) public exp;
    mapping(uint256 => uint256) public latestFeed;
    uint256 public feedInterval = 1 hours;
    uint256 public expPerFeed = 100;
    uint256 public MAX_LEVEL = 10;
    mapping(uint256 => EnumerableSet.UintSet) private invitation;
    mapping(uint256 => uint256) public mates;
    mapping(uint256 => uint256) datingTime;
    uint256 public datingInterval = 1 days;
    uint256 public EXP_PER_LV = 700;
    ...
    function setExpPerFeed(uint256 newExpPerFeed) external onlyOwner {
        expPerFeed = newExpPerFeed;
    }

    function setFeedInterval(uint256 newFeedInterval) external onlyOwner {
        feedInterval = newFeedInterval;
    }

    function setDatingInterval(uint256 newDatingInterval) external onlyOwner {
        datingInterval = newDatingInterval;
    }

    function setExpPerLv(uint256 newExpPerLv) external onlyOwner {
        EXP_PER_LV = newExpPerLv;
    }
    ...
    function feed(uint256 tokenId) external onlyOwner {
        require(
            ownerOf(tokenId) != msg.sender,
            "Genius Puppeys: I'm not your pet!"
        );
        require(
            latestFeed[tokenId] + feedInterval < block.timestamp,
            "Genius Puppeys: I'm not hungry"
        );
        exp[tokenId] += expPerFeed;
        if (mates[tokenId] != 0) {
            exp[mates[tokenId]] += expPerFeed / 2;
            emit Feed(mates[tokenId], expPerFeed / 2);
        }
        latestFeed[tokenId] = block.timestamp;
        emit Feed(tokenId, expPerFeed);
    }

    function getLevel(uint256 id) public view returns (uint256) {
        uint256 _level = exp[id] / EXP_PER_LV;
        if (_level > MAX_LEVEL) {
            return MAX_LEVEL;
        }
        return _level;
    ...
    function inviteForPlaying(uint256 id, uint256 invitedId) external {
        require(
            ownerOf(id) == msg.sender,
            "Genius Puppeys: You are not the owner"
        );
        require(
            ownerOf(invitedId) != msg.sender,
            "Genius Puppeys: You can't invite yourself"
        );
        require(
            !invitation[invitedId].contains(id),
            "Genius Puppeys: You have invited this user"
        );
        require(
            getLevel(id) > 1 && getLevel(invitedId) > 1,
            "Genius Puppeys: You are not qualified to invite"
        );
        require(
            mates[invitedId] == 0 && mates[id] == 0,
            "Genius Puppeys: TokenId have a mate"
        );
        invitation[invitedId].add(id);
        emit Invite(id, invitedId);
    }

    function acceptPlayDate(uint256 id, uint256 inviterId) external {
        require(
            ownerOf(id) == msg.sender,
            "Genius Puppeys: You are not the owner"
        );
        require(
            invitation[id].contains(inviterId),
            "Genius Puppeys: You are not invited"
        );
        require(
            mates[inviterId] == 0 && mates[id] == 0,
            "Genius Puppeys: TokenId have a mate"
        );
        mates[inviterId] = id;
        mates[id] = inviterId;
        datingTime[id] = block.timestamp;
        datingTime[inviterId] = block.timestamp;

        delete invitation[id];
        delete invitation[inviterId];
        emit AcceptPlayDate(id, inviterId);
    }

    function breakup(uint256 id) external {
        require(
            ownerOf(id) == msg.sender,
            "Genius Puppeys: You are not the owner"
        );
        require(mates[id] != 0, "Genius Puppeys: You don't have a mate");
        require(
            datingTime[id] + datingInterval < block.timestamp,
            "Genius Puppeys: You can't break up now"
        );
        uint256 mateId = mates[id];
        mates[id] = 0;
        mates[mateId] = 0;
        datingTime[id] = 0;
        datingTime[mateId] = 0;
    ...
    function getInvitationCount(uint256 id) external view returns (uint256) {
        return invitation[id].length();
    }
