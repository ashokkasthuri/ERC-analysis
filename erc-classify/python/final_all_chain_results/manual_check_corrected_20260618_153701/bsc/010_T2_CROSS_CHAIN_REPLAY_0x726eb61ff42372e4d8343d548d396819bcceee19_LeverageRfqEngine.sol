...
// Address: 0x726eb61ff42372e4d8343d548d396819bcceee19
// Contract Name: LeverageRfqEngine



// ============================================================
// FILE: contracts/engines/LeverageRfqEngine.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
...
contract LeverageRfqEngine is ReentrancyGuardTransient, ILeverageFlashCallback {
    ...
    error LeverageReceiverMustBeUser();
    error LeverageUnsupportedKind();
    error LeveragePartiallyFillableUnsupported();
    error LeverageBalanceTypeUnsupported();
    error LeverageBuyAmountTooLow();
    error LeverageFeeCollectorZero();
    error LeverageOnlyLeverageEngine();
    ...
    /// @dev User signature for open (economic intent only).
    ///      - filler provides `buyToken`, credited to `user` via PM
    ///      - user borrows `sellToken` (amount = sellAmount + feeAmount)
    ///      - engine transfers `sellAmount` to filler and `feeAmount` to `feeCollector`
    /// @dev Fill routing (flash/non-flash, fillTarget/fillData) is chosen by the executor and is NOT signed.
    bytes32 internal constant OPEN_LEVERAGE_TYPEHASH = keccak256(
        "OpenLeverage(address user,address sellToken,address buyToken,uint256 sellAmount,uint256 buyAmount,uint32 validTo,bytes32 appData,uint256 feeAmount,uint256 nonce)"
    );

    /// @dev User signature for close (economic intent only).
    ///      - filler provides `buyToken` to repay debt for `user`
    ///      - engine withdraws `sellToken` collateral to filler to settle the trade
    ///      - any leftover `buyToken` (beyond what was used to repay) is deposited as collateral
    /// @dev Fill routing (flash/non-flash, fillTarget/fillData) is chosen by the executor and is NOT signed.
    bytes32 internal constant CLOSE_LEVERAGE_TYPEHASH = keccak256(
        "CloseLeverage(address user,address sellToken,address buyToken,uint256 sellAmount,uint256 buyAmount,uint32 validTo,bytes32 appData,uint256 feeAmount,uint256 nonce)"
    );

    /// @dev SessionCall.dataHash schemes (economic intent only; no routing data).
    bytes32 internal constant OPEN_LEVERAGE_SESSION_DATAHASH_TYPEHASH = keccak256(
        "OpenLeverageSession(address sellToken,address buyToken,uint256 sellAmount,uint256 buyAmount,uint32 validTo,bytes32 appData,uint256 feeAmount)"
    );

    bytes32 internal constant CLOSE_LEVERAGE_SESSION_DATAHASH_TYPEHASH = keccak256(
        "CloseLeverageSession(address sellToken,address buyToken,uint256 sellAmount,uint256 buyAmount,uint32 validTo,bytes32 appData,uint256 feeAmount)"
    );
    ...
    struct Order {
        address sellToken;
        address buyToken;
        address receiver; // address(0) => user
        uint256 sellAmount;
        uint256 buyAmount; // minimum
        uint32 validTo;
        bytes32 appData;
        uint256 feeAmount; // in sellToken units (extra borrowed and paid to feeCollector)
        OrderKind kind;
        bool partiallyFillable;
        uint8 sellTokenBalance; // 0 = ERC20
        uint8 buyTokenBalance; // 0 = ERC20
    }
    ...
    constructor(address pm_, address feeCollector_) {
        if (pm_ == address(0)) revert LeverageZeroAddress();
    ...
    function _onlyAdmin() internal view {
        if (msg.sender != pm.admin()) revert IPositionsManager.ftPositionManagerOnlyOwner();
    }
    ...
    /// @param user The account whose PM position is modified (signer).
    /// @param order CoW-like SELL order describing the borrow->sell->deposit settlement.
    /// @param sig User signature (EOA ECDSA or ERC-1271).
        ...
        nonReentrant
    {
        _openLeverage(
            ...
            sig,
            buyAmountIn,
            MetaPermit({usePermit: false, maxFee: 0, deadline: 0, sig: bytes("")})
        );
    }
    ...
    function openLeverage(
        ...
        nonReentrant
    {
        _openLeverage(user, order, sig, buyAmountIn, permit);
    }
        ...
        internal
    {
        ...
        bytes32 expectedHash = keccak256(
            abi.encode(
                OPEN_LEVERAGE_SESSION_DATAHASH_TYPEHASH,
                order.sellToken,
                order.buyToken,
                order.sellAmount,
                order.buyAmount,
        ...
        address user = _validateAndConsumeSession(
            order.sellToken, totalBorrow, expectedHash, sessionCall, delegateSignature
        );
        if (user == address(0)) revert LeverageZeroAddress();

        address receiver = _resolveReceiver(user, order.receiver);
        ...
        pm.borrowFlashAndDepositFor(
            ...
            abi.encode(
                FlashFill({
                    action: FlashAction.OPEN_FROM_FILLER,
                    user: user,
        ...
        internal
    {
        ...
        uint256 nonce = _useTypedNonce(user, OPEN_LEVERAGE_TYPEHASH);
        bytes32 structHash = keccak256(
            abi.encode(
                OPEN_LEVERAGE_TYPEHASH,
                user,
                order.sellToken,
                order.buyToken,
                order.sellAmount,
        ...
        if (!_isValidSig(user, digest, sig)) revert LeverageBadSignature();
    ...
    ///         `buyToken` to this engine so it can be deposited as collateral for `user`.
    ...
    function openLeverageFlash(
        ...
        nonReentrant
    {
        _openLeverageFlash(user, order, fillTarget, fillData, sig, permit);
    }
    ...
    function _openLeverageFlash(
        ...
        internal
    {
        if (user == address(0) || fillTarget == address(0)) revert LeverageZeroAddress();
        _validateOrder(order);
    ...
    function onLeverageFlash(bytes calldata data) external override {
        if (msg.sender != address(pm)) revert LeverageOnlyLeverageEngine();

        FlashFill memory f = abi.decode(data, (FlashFill));
        if (f.user == address(0) || f.fillTarget == address(0)) revert LeverageZeroAddress();

        Order memory order = f.order;
        if (order.kind != OrderKind.SELL) revert LeverageUnsupportedKind();
        if (order.partiallyFillable) revert LeveragePartiallyFillableUnsupported();
        if (order.sellTokenBalance != 0 || order.buyTokenBalance != 0) {
            revert LeverageBalanceTypeUnsupported();
        }
        if (order.sellAmount == 0 || order.buyAmount == 0) revert LeverageAmountZero();

        address receiver = order.receiver == address(0) ? f.user : order.receiver;
        if (receiver != f.user) revert LeverageReceiverMustBeUser();

        if (f.action == FlashAction.OPEN) {
            IERC20 buyToken = IERC20(order.buyToken);
            ...
            if (order.feeAmount > 0) {
                ...
                sellToken.safeTransfer(collector, order.feeAmount);
            }
        ...
        if (f.action == FlashAction.CLOSE) {
            IERC20 repayToken = IERC20(order.buyToken);
            uint256 repayThisBefore = repayToken.balanceOf(address(this));
            ...
            uint256 repayThisAfter = repayToken.balanceOf(address(this));
            uint256 deltaThis =
                repayThisAfter > repayThisBefore ? (repayThisAfter - repayThisBefore) : 0;
            if (deltaThis < order.buyAmount) revert LeverageBuyAmountTooLow();
            return;
        }

        if (f.action == FlashAction.OPEN_FROM_FILLER) {
            address filler = f.fillTarget;
            ...
            IERC20 sellToken = IERC20(order.sellToken);
            sellToken.safeTransfer(filler, order.sellAmount);
            if (order.feeAmount > 0) {
                address collector = feeCollector;
                if (collector == address(0)) revert LeverageFeeCollectorZero();
                sellToken.safeTransfer(collector, order.feeAmount);
            ...
            buyToken.safeTransferFrom(filler, address(this), buyAmountIn);
        ...
        if (f.action == FlashAction.CLOSE_FROM_FILLER) {
            address filler = f.fillTarget;
            uint256 buyAmountIn = abi.decode(f.fillData, (uint256));
            ...
            repayToken.safeTransferFrom(filler, address(this), buyAmountIn);
        ...
        revert LeverageBuyAmountTooLow();
    }

    /// @notice Fill a signed RFQ order to close (decrease leverage on) a position.
    /// @param user The account whose PM position is modified (signer).
    /// @param order CoW-like SELL order describing the withdraw->repay settlement.
    /// @param sig User signature (EOA ECDSA or ERC-1271).
    /// @param buyAmountIn Amount of buyToken provided by filler (must be >= order.buyAmount).
    ...
    function closeLeverage(
        ...
        nonReentrant
    {
        _closeLeverage(user, order, sig, buyAmountIn, permit);
    }
        ...
        internal
    {
        ...
        bytes32 expectedHash = keccak256(
            abi.encode(
                CLOSE_LEVERAGE_SESSION_DATAHASH_TYPEHASH,
                order.sellToken,
                order.buyToken,
                order.sellAmount,
                order.buyAmount,
        ...
        collateralAmount += sessionFee;
        address user = _validateAndConsumeSession(
            order.sellToken, collateralAmount, expectedHash, sessionCall, delegateSignature
        );
        if (user == address(0)) revert LeverageZeroAddress();
        ...
        pm.withdrawFlashAndRepayFor(
            ...
            abi.encode(
                FlashFill({
                    action: FlashAction.CLOSE_FROM_FILLER,
                    user: user,
                    fillTarget: msg.sender,
                    fillData: abi.encode(buyAmountIn),
                    order: order
        ...
        internal
    {
        ...
        uint256 nonce = _useTypedNonce(user, CLOSE_LEVERAGE_TYPEHASH);
        bytes32 structHash = keccak256(
            abi.encode(
                CLOSE_LEVERAGE_TYPEHASH,
                user,
                order.sellToken,
                order.buyToken,
                order.sellAmount,
    ...
    function closeLeverageFlash(
        ...
        nonReentrant
    {
        _closeLeverageFlash(user, order, fillTarget, fillData, sig, permit);
    }
        ...
        returns (address owner)
    {
        ...
        if (sessionCall.dataHash != expectedDataHash) {
            revert LeverageSessionDataHashMismatch(expectedDataHash, sessionCall.dataHash);
        }
    ...
    function _enforceRelayerFee(uint256 amount, uint16 maxFeeBps, uint256 feeAmount) internal pure {
        ...
        uint256 maxFee = Math.mulDiv(amount, maxFeeBps, BPS, Math.Rounding.Ceil);
    ...
    function _validateOrderAndBuyAmountIn(Order calldata order, uint256 buyAmountIn) internal view {
        _validateOrder(order);
    ...
    {
        resolved = receiver == address(0) ? user : receiver;
        if (resolved != user) revert LeverageReceiverMustBeUser();
    }
        ...
        internal
    {
        ...
        IERC20 feeToken = IERC20(borrowAsset);
        uint256 feeBalBefore = feeToken.balanceOf(address(this));
        ...
        uint256 feeBalAfter = feeToken.balanceOf(address(this));
        uint256 fee = feeBalAfter > feeBalBefore ? (feeBalAfter - feeBalBefore) : 0;
        if (fee != 0) feeToken.safeTransfer(msg.sender, fee);
    }
        ...
        returns (bool)
    {
        ...
        try IERC1271(signer).isValidSignature(digest, sig) returns (bytes4 res) {
...
// ============================================================
// FILE: contracts/interfaces/IPositionsManager.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
interface IPositionsManager is IPMViews {
    ...
    error ftPositionManagerRateLimitExceeded();
...
// ============================================================
// FILE: contracts/interfaces/IFlash.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
// ============================================================
// FILE: contracts/interfaces/ILeverageEngine.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
// ============================================================
// FILE: contracts/engines/MetaActions.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
// Copyright (c) 2025 Flying Tulip Inc. All rights reserved.
pragma solidity ^0.8.30;
...
contract MetaActions {
    ...
    error MetaTokenTransferMismatch();
    ...
    bytes32 private constant _APPROVE_ENGINE_TYPEHASH = keccak256(
        "ApproveEngine(address user,address engine,address asset,uint256 debitAllowance,address relayer,uint256 maxFee,uint256 nonce,uint256 deadline)"
    );
    bytes32 private constant _APPROVE_BORROW_DELEGATE_TYPEHASH = keccak256(
        "ApproveBorrowDelegate(address user,address delegate,address asset,uint256 borrowAllowance,address relayer,uint256 maxFee,uint256 nonce,uint256 deadline)"
    );
    ...
    constructor(address pm_) {
        if (pm_ == address(0)) revert MetaZeroAddress();
        pm = IPositionsManager(pm_);
    }
    ...
    function withdrawWithSig(
        ...
        external
    {
        _withdrawWithSigInternal(
            user, asset, amount, relayer, maxFee, deadline, abi.encodePacked(r, s, v)
        );
    }
    ...
    function withdrawWithSig(
        ...
        external
    {
        _withdrawWithSigInternal(user, asset, amount, relayer, maxFee, deadline, sig);
    }
    ...
    function borrowWithSig(
        ...
        external
    {
        _borrowWithSigInternal(user, asset, amount, relayer, maxFee, deadline, sig);
    }
    ...
    {
        _approveEngineWithSigInternal(
            user, engine, asset, debitAllowance, relayer, maxFee, deadline, sig
        );
    }
    ...
    {
        _approveBorrowWithSigInternal(
            user, delegate, asset, borrowAllowance, relayer, maxFee, deadline, sig
        );
    }
    ...
    function _depositWithSigInternal(
        ...
        internal
    {
        if (msg.sender != relayer) revert MetaRelayerMismatch();
        ...
        _verifyMetaSig(_DEPOSIT_TYPEHASH, user, asset, amount, relayer, maxFee, deadline, sig);
        ...
        if (token.balanceOf(address(this)) - balBefore != amount) {
            revert MetaTokenTransferMismatch();
        }
        ...
        internal
    {
        ...
        _verifyMetaSig(_WITHDRAW_TYPEHASH, user, asset, amount, relayer, maxFee, deadline, sig);
        ...
        internal
    {
        ...
        _verifyMetaSig(_BORROW_TYPEHASH, user, asset, amount, relayer, maxFee, deadline, sig);
        ...
        internal
    {
        ...
        _verifyMetaSig(_REPAY_TYPEHASH, user, asset, amount, relayer, maxFee, deadline, sig);
        ...
        internal
    {
        ...
        uint256 fee = _computeMetaFee(MetaAction.APPROVE_ENGINE, asset, debitAllowance);
        if (fee > maxFee) revert MetaFeeTooHigh();

        _verifyApproveEngineSig(user, engine, asset, debitAllowance, relayer, maxFee, deadline, sig);

        if (fee != 0) {
            pm.withdraw(user, asset, fee, relayer);
            emit MetaFeePaid(
                relayer, user, uint8(MetaAction.APPROVE_ENGINE), asset, debitAllowance, fee
            );
        }
        ...
        internal
    {
        ...
        uint256 fee = _computeMetaFee(MetaAction.APPROVE_BORROW_DELEGATE, asset, borrowAllowance);
        if (fee > maxFee) revert MetaFeeTooHigh();

        _verifyApproveBorrowDelegateSig(
            user, delegate, asset, borrowAllowance, relayer, maxFee, deadline, sig
        );
        ...
        if (fee != 0) {
            ...
            emit MetaFeePaid(
                relayer,
                user,
                uint8(MetaAction.APPROVE_BORROW_DELEGATE),
                asset,
                borrowAllowance,
                fee
            );
    ...
    function _verifyBorrowAndDepositViaSig(
        bytes32 typehash,
        address user,
        address borrowAsset,
        uint256 borrowAmount,
    ...
    function _verifyWithdrawAndRepayViaSig(
        bytes32 typehash,
        address user,
        address collateralAsset,
        uint256 collateralAmount,
...
// ============================================================
// FILE: contracts/engines/MetaSessionActions.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
// Copyright (c) 2025 Flying Tulip Inc. All rights reserved.
pragma solidity ^0.8.30;
...
contract MetaSessionActions {
    ...
    error MetaSessionTokenTransferMismatch();
    ...
    constructor(address pm_) {
        if (pm_ == address(0)) revert MetaSessionZeroAddress();
        pm = IPositionsManager(pm_);
    }
        ...
        internal
    {
        ...
        if (token.balanceOf(address(this)) - balBefore != amount) {
            revert MetaSessionTokenTransferMismatch();
        }
...
// ============================================================
// FILE: contracts/interfaces/ISessionManager.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
// ============================================================
// FILE: contracts/interfaces/IPMViews.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
// ============================================================
// FILE: contracts/interfaces/IConfigRegistry.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
...
// ============================================================
// FILE: contracts/interfaces/IAccountValuesLens.sol
// ============================================================
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.30;
