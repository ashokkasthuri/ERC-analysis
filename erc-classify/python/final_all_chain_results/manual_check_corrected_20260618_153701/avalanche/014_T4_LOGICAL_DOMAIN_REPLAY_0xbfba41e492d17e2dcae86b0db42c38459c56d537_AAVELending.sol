...
// Address: 0xbfba41e492d17e2dcae86b0db42c38459c56d537
// Contract Name: AAVELending
...
contract AAVELending is Draws, ReentrancyGuard, Pausable {
    ...
    function depositToLending(uint256 amount) private {
        (bool success, bytes memory returndata) = address(lendingBridge).delegatecall(abi.encodeWithSignature("deposit(address,address,uint256)", aaveLendingPool, address(depositToken), amount));
        if (success == false) {
    ...
    function withdrawFromLending(uint256 amount) private {
        (bool success, bytes memory returndata) = address(lendingBridge).delegatecall(abi.encodeWithSignature("withdraw(address,address,uint256)", aaveLendingPool, address(depositToken), amount));
        if (success == false) {
