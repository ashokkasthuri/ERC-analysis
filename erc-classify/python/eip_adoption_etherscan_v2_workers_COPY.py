...
def load_api_keys(env_path: Optional[str] = None, key_prefix: str = "ETHERSCAN_API_KEY") -> List[str]:
    if env_path:
    ...
    single = os.getenv(key_prefix)
    if single:
        keys.append(single.strip())

    # Fallback to Etherscan keys if chain-specific keys are missing.
    if not keys and key_prefix != "ETHERSCAN_API_KEY":
        i = 1
        while True:
    ...
    # Routescan may work without an API key depending on endpoint limits.
    if not keys and key_prefix == "ROUTESCAN_API_KEY":
        return [""]

    if not keys:
        raise ValueError(
            f"No API keys found. Set {key_prefix}1=... or ETHERSCAN_API_KEY1=... in .env"
        )
...
class EtherscanV2Client:
    def __init__(
        ...
        self.chainid_int = int(chainid)
        self.timeout = timeout
        self.provider = provider
        self.key_manager = APIKeyManager(api_keys, calls_per_second_per_key)

        if provider == "auto":
            if self.chainid_int == 43114:
                self.provider = "routescan"
            elif self.chainid_int in LEGACY_EXPLORER_URLS:
                self.provider = "legacy_explorer"
            else:
                self.provider = "etherscan_v2"

        if self.provider == "routescan":
            self.base_url = ROUTESCAN_URLS[self.chainid_int]
        elif self.provider == "legacy_explorer":
            self.base_url = LEGACY_EXPLORER_URLS[self.chainid_int]
        else:
            self.base_url = ETHERSCAN_V2_URL
    ...
    def get_json(self, params: Dict[str, Any], max_retries: int = 5) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            if self.provider == "etherscan_v2":
                p["chainid"] = self.chainid
                if key:
                    p["apikey"] = key
            else:
                # BscScan / PolygonScan / Routescan Etherscan-like mode.
                if key:
                    p["apikey"] = key

            try:
                r = requests.get(self.base_url, params=p, timeout=self.timeout)
                r.raise_for_status()
                data = r.json()
        ...
        raise RuntimeError(f"{self.provider} request failed after retries: {last_error}")
...
def main() -> None:
    ...
    parser.add_argument("--provider", choices=["auto", "etherscan_v2", "legacy_explorer", "routescan"], default="auto")
