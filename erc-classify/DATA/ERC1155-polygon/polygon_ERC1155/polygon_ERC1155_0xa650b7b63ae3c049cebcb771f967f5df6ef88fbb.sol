...
abstract contract AbsERC1155 is ERC1155 {
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        require(maxSupply[tokenId] != 0, "BaseERC1155: URI query for nonexistent token");
    ...
    function _create(
        ...
        require(maxSupply[tokenId] == 0, "BaseERC1155: token already created");
...
contract TreumERC1155 is AbsERC1155, Ownable {
    ...
    mapping(uint256 => TokenMintData) public tokenMintPrices;
    ...
    function createWithFixedPrice(
        ...
        if (initialSupply < maxSupply) {
            ...
            // Set price for minting via ERC20
            tokenMintPrices[tokenId] = TokenMintData({ erc20Address: mintPriceTokenAddress, mintPrice: mintPrice });
        }
    ...
    function mint(uint256 tokenId) external {
        ...
        TokenMintData storage tokenMintData = tokenMintPrices[tokenId];

        if (tokenMintData.mintPrice > 0) {
            // Receive ERC20 mintPrice amount
            IERC20(tokenMintData.erc20Address).safeTransferFrom(msg.sender, address(this), tokenMintData.mintPrice);
        }
