// Chain: POLYGON - File: contracts/ERC1155v2SaleUpgradeable.sol
...
import { ERC1155PausableUpgradeable } from "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC1155/extensions/ERC1155PausableUpgradeable.sol";
...
import { ERC1155URIStorageUpgradeable } from "./internal-upgradeable/ERC1155URIStorageUpgradeable.sol";
import { BaseUpgradeable } from "./internal-upgradeable/BaseUpgradeable.sol";
import { PaymenProcessingUpgradeableV2 } from "./internal-upgradeable/PaymenProcessingUpgradeableV2.sol";
import { MINTER_ROLE, OPERATOR_ROLE } from "./libraries/Constants.sol";

interface IERC1155v2Upgradeable {
    struct PresaleInfo {
    ...
    error IERC1155__InvalidPrice();
    error IERC1155__InvalidType();
    error IERC1155__ExceedsLimit();

    function buy(uint256 id_, uint256 quantity_, address paymentToken_, address recipient_, address referrer_) external payable;

    event PreSaleInfo(uint256 indexed id, address paymentToken, uint96 price, uint128 totalMinted, uint128 limit);
    event TokensPurchased(address indexed purchaser, uint256 id, uint256 quantity, address paymentToken);
}

contract ERC1155v2SaleUpgradeable is
    IERC1155v2Upgradeable,
    BaseUpgradeable,
    PaymenProcessingUpgradeableV2,
    ERC1155Upgradeable,
...
{
    mapping(uint256 => PresaleInfo) private _presaleInfo;
    ...
    function initialize(address roleManager_, string memory uri_, FeeInfo calldata client_, PresaleInfo[] calldata presaleInfo_) public initializer {
        __BaseUpgradeable_init(roleManager_);
        __ERC1155_init(uri_);
        __ERC1155Pausable_init();
        __ERC1155Burnable_init();
        __ERC1155Supply_init();
        __PaymenProcessingUpgradeableV2_init(client_, _msgSender(), 0);

        uint256 length = presaleInfo_.length;
        for (uint i = 0; i < length; i++) {
            _setPresale(i + 1, presaleInfo_[i]);
        }
    }

    function setPresale(uint256 id_, PresaleInfo calldata presaleInfo_) external onlyRole(OPERATOR_ROLE) {
        if (presaleInfo_.tokenPrice == 0) revert IERC1155__InvalidPrice();

        _setPresale(id_, presaleInfo_);
    }

    function _setPresale(uint256 id_, PresaleInfo calldata presaleInfo_) internal {
        _presaleInfo[id_] = presaleInfo_;

        emit PreSaleInfo(id_, presaleInfo_.paymentToken, presaleInfo_.tokenPrice, presaleInfo_.totalMinted, presaleInfo_.limit);
    }

    function buy(uint256 id_, uint256 quantity_, address paymentToken_, address recipient_, address referrer_) external payable {
        PresaleInfo storage presaleInfo = _presaleInfo[id_];

        uint256 total = presaleInfo.tokenPrice * quantity_;

        presaleInfo.totalMinted += uint128(quantity_);

        if (presaleInfo.totalMinted > presaleInfo.limit) revert IERC1155__ExceedsLimit();

        if (total == 0) revert IERC1155__InvalidType();

        _processPayment(presaleInfo.paymentToken, total, _msgSender(), referrer_);

        _mint(recipient_, id_, quantity_, "");
...
// Chain: POLYGON - File: contracts/interfaces/IAccessManagerUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/interfaces/IBaseUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/interfaces/IFeeRecipientUpgradeable.sol
// SPDX-License-Identifier: MIT

pragma solidity 0.8.21;

interface IFeeRecipientUpgradeable {
...
// Chain: POLYGON - File: contracts/internal-upgradeable/BaseUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/internal-upgradeable/BusinessAddressesUpgradeable.sol
// SPDX-License-Identifier: MIT
...
abstract contract BusinessAddressesUpgradeable is BaseUpgradeable {
    ...
    BitMapsUpgradeable.BitMap private _isAccepted;
    ...
    function __BusinessAddressUpgradeable_init_unchained(address[] calldata businessAddresses) internal onlyInitializing {
...
// Chain: POLYGON - File: contracts/internal-upgradeable/ERC1155URIStorageUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/internal-upgradeable/FeeRecipientUpgradeable.sol
// SPDX-License-Identifier: MIT

pragma solidity 0.8.21;

import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { IFeeRecipientUpgradeable } from "../interfaces/IFeeRecipientUpgradeable.sol";
import { OPERATOR_ROLE } from "../libraries/Constants.sol";

contract FeeRecipientUpgradeable is IFeeRecipientUpgradeable, BaseUpgradeable {
    ...
    function _updateFee(FeeInfo calldata feeInfo_) internal {
...
// Chain: POLYGON - File: contracts/internal-upgradeable/PaymenProcessingUpgradeableV2.sol
// SPDX-License-Identifier: MIT
...
import { FeeRecipientUpgradeable } from "./FeeRecipientUpgradeable.sol";
import { SafeTransferLib } from "../libraries/SafeTransfer.sol";
import { HUNDER_PERCENT_IN_BPS, OPERATOR_ROLE } from "../libraries/Constants.sol";

abstract contract PaymenProcessingUpgradeableV2 is BaseUpgradeable, BusinessAddressesUpgradeable, FeeRecipientUpgradeable {
    ...
    function __PaymenProcessingUpgradeableV2_init(FeeInfo calldata client_, address systemAddress_, uint96 affiliatePercentageInBps_) internal onlyInitializing {
        __PaymenProcessingUpgradeableV2_init_unchained(client_, systemAddress_, affiliatePercentageInBps_);
    }

    function __PaymenProcessingUpgradeableV2_init_unchained(FeeInfo calldata client_, address systemAddress_, uint96 affiliatePercentageInBps_) internal onlyInitializing {
        _updateFee(client_);
    ...
    function _processPayment(address paymentToken_, uint256 total_, address payer_, address affiliate_) internal returns (uint256 primarySaleAmount, uint256 affiliateAmount, uint256 systemAmount) {
        ...
        if (paymentToken_ == address(0)) {
            value = msg.value;
            (primarySaleAmount, affiliateAmount, systemAmount) = _calculateAmounts(value, discountTotal, total_, affiliate_);
            ...
            (primarySaleAmount, affiliateAmount, systemAmount) = _calculateAmounts(total_, discountTotal, total_, affiliate_);
...
// Chain: POLYGON - File: contracts/libraries/BitMapsUpgradeable.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;
...
// Chain: POLYGON - File: contracts/libraries/Helper.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/token/ERC1155/extensions/ERC1155BurnableUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/token/ERC1155/extensions/ERC1155PausableUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/token/ERC1155/extensions/ERC1155SupplyUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/utils/PausableUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts/contracts/proxy/ERC1967/ERC1967Utils.sol
// SPDX-License-Identifier: MIT
