...
// Chain: POLYGON - File: contracts/Airdrop.sol
// SPDX-License-Identifier: MIT
...
contract Airdrop is ERC1155, Ownable {
    ...
    mapping(uint256 => TokenMetadata) private idToMetadata;

    constructor() ERC1155("") {}

    modifier hasTokenBalance(address account, uint256 tokenId) {
        require(balanceOf(account, tokenId) == 0, "Address already has this token");
        _;
    }

    function createTokenModel(string memory newuri) public onlyOwner {
        idToMetadata[_tokenCounter] = TokenMetadata(
            _tokenCounter,
            newuri
        );

        emit TokenCreated(_tokenCounter, newuri);
        _tokenCounter += 1;
    }
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory data)
        public
        onlyOwner
        hasTokenBalance(account, id)
    {
        _mint(account, id, amount, data);
    }

    function updateTokenURI(uint256 tokenId, string memory newuri) public onlyOwner {
        idToMetadata[tokenId].uri = newuri;
    }

    function updateContractURI(string memory newuri)
        public
        onlyOwner
    {
        _contractURI = newuri;
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        return idToMetadata[tokenId].uri;
    }
