...
function mintBatch(
    ...
    require(to.length == itemIds.length, "Arrays must have the same length");

    uint256 batchSize = 1000; // Number of addresses to process per transaction
    uint256 totalItems = to.length;
    uint256 totalBatches = (totalItems + batchSize - 1) / batchSize;

    for (uint256 batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        uint256 start = batchIndex * batchSize;
        uint256 end = (batchIndex + 1) * batchSize;
        if (end > totalItems) {
            end = totalItems;
        }

        address[] memory batchTo = new address[](end - start);
        uint32[] memory batchItemIds = new uint32[](end - start);

        for (uint256 i = start; i < end; i++) {
            batchTo[i - start] = to[i];
            batchItemIds[i - start] = itemIds[i];
        }

        _mintBatch(msg.sender, batchTo, batchItemIds);
    }
}
...
function _mintBatch(
    ...
    for (uint256 i = 0; i < to.length; i++) {
        ...
        // Mint one NFT to the recipient
        emit TransferSingle(msg.sender, address(0), recipient, itemId, 1);

        // Update balance
        _balances[itemId][recipient]++;
    }
