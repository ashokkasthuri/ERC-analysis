...
interface IFameV2 {
    /* ========== CONSTANTS ========== */
    /**
        @notice Get the fame decimals constant.
        @dev    Get the fame decimals constant.
        @return The fame decimals constant
    */
    function FAME_DECIMALS() external view returns (uint256);

    /**
        @notice Get the milliseconds per week.
        @dev    Get the milliseconds per week.
        @return The milliseconds per week
    */
    function MS_PER_WEEK() external view returns (uint256);

    /**
        @notice Get the max value for the block time.
        @dev    Get the max value for the block time.
        @return The max value for the block time
    */
    function MAX_BLOCK_TIME() external view returns (uint256);

    /**
        @notice Get the max value for weights.
        @dev    Get the max value for weights.
        @return The max value for weights
    */
    function MAX_WEIGHT() external view returns (uint256);

    /**
        @notice Get the max value for user's shares..
        @dev    Get the max value for user's shares..
        @return The max value for user's shares.
    */
    function MAX_SHARE() external view returns (uint256);

    /**
        @notice Get the count of max decimals for weekly fame.
        @dev    Get the count of max decimals for weekly fame.
        @return The count of max decimals for weekly fame
    */
    function MAX_WEEKLY_FAME() external view returns (uint256);
    ...
    */
    ...
    /**
        ...
        @param  amount  The deposited amount of an Asset
    */
    function updateFameReward(uint256 assetId, uint256 amount) external;
    ...
    */
    function calculatedFameBalance(
        address userAddress
    ...
    */
    function calculatedFameBalanceById(
        uint256 assetId,
        address userAddress
    ) external view returns (uint256);
    ...
    */
    function getUserFamePerBlock(
        address userAddress
    ) external view returns (uint256 userFamePerBlock);
    ...
    */
    function getUserFamePerBlockById(
        uint256 assetId,
        address userAddress
    ) external view returns (uint256);

    /**
        @notice Get the calculatedFameBalance and getUserFamePerBlock as a tuple.
        @dev    Get the calculatedFameBalance and getUserFamePerBlock as a tuple.
        @param userAddress  Address of the user
        @return             The calculatedFameBalance and getUserFamePerBlock as a tuple
    */
    function getUserFameInfos(
        address userAddress
    ...
    */
    ...
    /**
        ...
        @param  withUpdate   True => Updates the rewards for all assets before setting new weights
    */
    function setBlockTime(uint256 milliseconds, bool withUpdate) external;
}
    ...
    CommonConstants
{
    ...
    /**
        ...
        @dev    Count of decimals for fame.
    */
    uint256 public constant FAME_DECIMALS = 1e38;
    /**
    ...
    */
    ...
    /**
        @notice The max value for the block time.
        @dev    This value can't be higher
                to prevent an overflow error in the update calculation.
    */
    uint256 public constant MAX_BLOCK_TIME = 21000;
    /**
        @notice The max value for weights.
        @dev    This value can't be higher
                to prevent an overflow error in the update calculation.
    */
    uint256 public constant MAX_WEIGHT = 1e6;
    /**
        @notice The max value for user's shares.
        @dev    This value can't be higher
                to prevent an overflow error in the update calculation.
    */
    uint256 public constant MAX_SHARE = 1e10 * 1e18; //1e28
    /**
        @notice Count of max decimals for weekly fame.
        @dev    This value can't be higher
                to prevent an overflow error in the update calculation.
    */
    uint256 public constant MAX_WEEKLY_FAME = 2 * 1e9 * FAME_DECIMALS; //2*1e47
    ...
    */
    ...
    modifier updateFameRewards(
        ...
        _updateFameRewards(contractAddress, tokenIds, amounts);
        _;
    }
    ...
    */
    ...
    function massUpdateFameRewards() external override isAuthorizedFameManager {
        _massUpdateFameRewards();
    }

    function updateFameReward(
        uint256 assetId,
        uint256 amount
    ) external override isAuthorizedFameManager {
        _updateFameReward(assetId, amount);
    }
    ...
    function setBlockTime(
        ...
        _setBlockTime(milliseconds, withUpdate);
    }
    ...
    function calculatedFameBalance(
        address userAddress
    ) public view override returns (uint256 fameBalance) {
    ...
    function getUserFamePerBlock(
        address userAddress
    ) public view override returns (uint256 userFamePerBlock) {
        for (uint256 i = 1; i < assets.length; i++) {
    ...
    function getUserFamePerBlockById(
        ...
        uint256 famePerAssetPerBlock = (famePerBlock * asset.weight) /
            _totalWeights;
        return (famePerAssetPerBlock * user.share) / stakedAssetBalance;
    }
    ...
    function getUserFameInfos(
        ...
        return (
            calculatedFameBalance(userAddress),
            getUserFamePerBlock(userAddress)
        );
    }

    function getUserData(
        address userAddress
    ) public view override returns (UserData[] memory) {
    ...
    function getFameVault(
        address userAddress
    ) public view override returns (uint256) {
        return _fameVault[userAddress];
    }

    function getFameSpent(
        address userAddress
    ) public view override returns (uint256) {
    ...
    function getAssetId(
        address contractAddress,
        uint256 tokenId
    ) public view override returns (uint256 assetId) {
    ...
    */
    function _stake(
        ...
        private
        nonReentrant
        updateFameRewards(contractAddress, tokenIds, amounts)
    {
    ...
    */
    ...
    /**
        @dev    Updates the fame reward for all existing assets based on the asset's weight and their balance.
                ...
                IF block gas limit is reached the Admin needs to manually call updateFameReward for all assetIds before changing any weights.
    */
    function _massUpdateFameRewards() private {
        for (uint256 i = 1; i < assets.length; i++) {
            _updateFameReward(i, 0);
        }
    }
    ...
    /**
        ...
        @param  amount  The deposited amount of an Asset
    */
    function _updateFameReward(uint256 assetId, uint256 amount) private {
        Asset storage asset = assets[assetId];
        if (asset.weight == 0) {
    ...
    */
    ...
    /**
        ...
        @param  user    The UserData object
    */
    function _attributePendingFame(
        Asset storage asset,
    ...
    /**
        @dev    Increases the balance of a user while increasing the share of his shareholder (the beneficiary).
                Reverts if the amount is 0.
                Reverts if the shareholder reaches a higher share amount than the max limit.
    ...
    */
    function _increaseBalance(
        ...
        require(shareholder.share <= MAX_SHARE, "Too many assets staked");
    }
    ...
    */
    function _setAssetWeightBatch(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                ...
                "Weights are required to be less or equal than max weight limit"
            );
    ...
    /** 
        @dev    Set a new value for the weekly reward.
        Reverts if value is smaller than 1e38 (as decimals).
        Reverts if value is bigger than 2 * 1e47 (as decimals).
    ...
    */
    function _setWeeklyReward(uint256 value, bool withUpdate) private {
        ...
        require(
            ...
            "Fame decimals are required to be between the min and max weekly fame"
        );
        weeklyReward = value;
        _updateFamePerBlock();
    ...
    /**
        ...
        @param  withUpdate  Updates all asset calculations before setting new weights. SHOULD be true unless the block gas limit is reached (requires manual update in multiple txs).
    */
    function _setBlockTime(uint256 milliseconds, bool withUpdate) private {
        if (withUpdate) {
            _massUpdateFameRewards();
        }

        require(
            milliseconds >= 1 && milliseconds <= MAX_BLOCK_TIME,
            "BlockTime should not be 0 or bigger than the max block time"
        );
