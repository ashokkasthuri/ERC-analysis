// Chain: POLYGON - File: /contracts/Seats.sol
// SPDX-License-Identifier: MIT
pragma solidity >=0.4.22 <0.9.0;

import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";  

contract TMS_Seat is ERC1155 {
    uint256 public constant SEATS = 0;
    address owner;
    
    constructor() ERC1155("https://themoonsociety.io/assets/images/NFT/Spacecraft/{id}.json") {
        owner = msg.sender;
        _mint(msg.sender, SEATS, 500, "");
    }
  ...
  function setBaseTokenURI(string memory _baseTokenURI) ownerOnly external{
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _beforeTokenTransfer(
        ...
        uint256 id = 0;//we only have 1 NFT
        
        if(users.length==0){
            users.push(to);

        }else{
            //check if the user holds an NFT
            for(uint256 i; i<users.length; i++){                
                require(amounts[ids[id]]==1 ,'1 NFT only!');
                require(_balances[ids[id]][to]==0,'Already holds 1 NFT!');
            }
            users.push(to);
        }
    }
