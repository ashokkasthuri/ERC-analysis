// Chain: POLYGON - File: src/Memories.sol
    ...
    UUPSUpgradeable
{
    ...
    address public constant GLRDT = 0x3e49E9C890Cd5B015A18ed76E7A4093f569f1A04;
    ...
    uint256 public constant COMMON_QUANTITY = type(uint256).max;
    uint256 public constant RARE_QUANTITY = 50;
    uint256 public constant FOIL_QUANTITY = 15;
    uint256 public constant LEGENDARY_QUANTITY = 5;

    uint256 public constant COMMON_PROB = 100;
    uint256 public constant RARE_PROB = 50;
    uint256 public constant FOIL_PROB = 20;
    uint256 public constant LEGENDARY_PROB = 5;
    ...
    //legendary rarity id range: [1-10000]
    //foil rarity id range: [10001, 20000]
    //rare rarity id range [20001, 30000]
    //common rarity id range [30001, 40000]

    //Ownable?
    constructor() ERC1155Upgradeable() OwnableUpgradeable() {
        currentEventId = 0;
    }
    ...
    function createEvent(
        ...
        _createCard(eventAddress, COMMON, commonUri);
        _createCard(eventAddress, RARE, rareUri);
        _createCard(eventAddress, FOIL, foilUri);
        _createCard(eventAddress, LEGENDARY, legendaryUri);

        emit CreateEvent(eventAddress, currentEventId);
    }
    ...
    function _createCard(
        ...
        uint256 cardId = getCardId(eventAddress, rarity);
        ...
        emit CreateCard(cardId, eventAddress, rarity, tokenUri);
    }

    function mint(address eventAddress) external {
        Event memory fetchedEvent = events[eventAddress];
        require(
            block.timestamp < fetchedEvent.endTime + 30 days,
            "MEM:DEADLINE_EXPIRED"
        );
        uint256 claimedTokenId = claims[msg.sender][eventAddress];
        require(claimedTokenId == 0, "MEM:ADDRESS_ALREADY_CLAIMED");
        uint256 xGETBalance = IERC20Upgradeable(GLRDT).balanceOf(msg.sender);

        require(
            fetchedEvent.minxGETAllowed < xGETBalance,
            "MEM:INSUFFICIENT_xGET_BALANCE"
        );

        uint256 randomNumber = _generateRandomNumber();

        uint256 legendaryId = getCardId(eventAddress, LEGENDARY);
        uint256 foilId = getCardId(eventAddress, FOIL);
        uint256 rareId = getCardId(eventAddress, RARE);
        uint256 commonId = getCardId(eventAddress, COMMON);

        uint256 claimedLegendary = cards[legendaryId].claimed;
        uint256 claimedFoil = cards[foilId].claimed;
        uint256 claimedRare = cards[rareId].claimed;

        if (randomNumber < LEGENDARY_PROB) {
            if (claimedLegendary < LEGENDARY_QUANTITY) {
                _mintCard(msg.sender, eventAddress, legendaryId);
            } else if (claimedFoil < FOIL_QUANTITY) {
                _mintCard(msg.sender, eventAddress, foilId);
            } else if (claimedRare < RARE_QUANTITY) {
                _mintCard(msg.sender, eventAddress, rareId);
            } else {
                _mintCard(msg.sender, eventAddress, commonId);
            }
        } else if (randomNumber < FOIL_PROB) {
            if (claimedFoil < FOIL_QUANTITY) {
                _mintCard(msg.sender, eventAddress, foilId);
            } else if (claimedRare < RARE_QUANTITY) {
                _mintCard(msg.sender, eventAddress, rareId);
            } else {
        ...
        } else if (randomNumber < RARE_PROB) {
            if (claimedRare < RARE_QUANTITY) {
                _mintCard(msg.sender, eventAddress, rareId);
            } else {
                _mintCard(msg.sender, eventAddress, commonId);
            }
    ...
    function _mintCard(
        ...
        cards[rarityId].claimed += 1;
        claims[sender][eventAddress] = rarityId;

        _mint(sender, rarityId, 1, "");

        emit Mint(sender, rarityId, 1, "");
    }
    ...
    function getCardId(
        ...
        uint256 cardId = events[eventAddress].id;
        if (rarity == 0) cardId = cardId + COMMON_ID_START;
        else if (rarity == 1) cardId = cardId + RARE_ID_START;
        else if (rarity == 2) cardId = cardId + FOIL_ID_START;
        else cardId = cardId + LEGENDARY_ID_START;

        return cardId;
    }

    function uri(uint256 _cardId) public view override returns (string memory) {
        return (uris[_cardId]);
    }
    ...
    function setCardUri(
        ...
        uint256 cardId = getCardId(_eventAddress, _rarity);
        uris[cardId] = _uri;

        emit SetCardUri(_uri, _eventAddress, cardId);
    }
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/IERC20Upgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/access/OwnableUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/security/ReentrancyGuardUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: src/interfaces/IMemories.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.19;

interface IMemories {
    event CreateEvent(address eventAddress, uint256 _eventId);
    ...
    event SetCardUri(string uri, address _eventAddress, uint256 _tokenId);
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/utils/ArraysUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/interfaces/draft-IERC6093Upgradeable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
// Chain: POLYGON - File: lib/openzeppelin-contracts-upgradeable/contracts/proxy/ERC1967/ERC1967UtilsUpgradeable.sol
// SPDX-License-Identifier: MIT
