...
// Chain: POLYGON - File: @openzeppelin/contracts-upgradeable/token/ERC1155/utils/ERC1155HolderUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @openzeppelin/contracts/utils/structs/EnumerableMap.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @openzeppelin/contracts/utils/structs/EnumerableSet.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @prb/math/src/Common.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/pcs/v2/collections/PolishCryptoStampV2.sol
...
contract PolishCryptoStampV2 is Initializable, ContextMixin, ERC1155Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155HolderUpgradeable, IAirdropSupplier, UUPSUpgradeable {
    // Add the library methods
     ...
     */
    event VariantAssigned(uint256 tokenId, uint8 variant);
    ...
    EnumerableMap.AddressToUintMap private eligibleWallets;
  ...
  /**
     ...
     * 
     * @param initialUri The base/initial token uri.
     ...
     */
    function initialize(string calldata initialUri, string memory contractUri, uint256 issueId, address repositoryContract) initializer public {
        ...
        __PolishCryptoStampV2_init_unchained(contractUri, ItemType.Stamp, issueId, repositoryContract);
    }

    function __PolishCryptoStampV2_init_unchained(string memory contractUri, ItemType itemType, uint256 issueId, address repositoryContract) internal onlyInitializing {
        uriSuffix = ".json";
    ...
    /**
     * @dev Allows to mint items' NFTs of a specific token to a number of pre-generated paper wallets.
     ...
     */
    function issueItems(uint256 issueId, uint8 tier, Depositary[] calldata depositaries) external onlyPrivileged {
        require(depositaries.length > 0, "Invalid call data");
        require(depositaries[0].offset > _itemsCounters[issueId][tier], "Invalid offset");

        for (uint256 i = 0; i < depositaries.length; i++) {
            require(!exists(depositaries[i].serial), "ID already minted");
            _mint(depositaries[i].wallet, depositaries[i].serial, 1, "");
        }

        unchecked {
            _itemsCounters[issueId][tier] += depositaries.length;
        }
    }
     ...
     * @dev Not implemented.
     */
    function dropItems(uint256, uint8, uint256, address) external nonReentrant onlyRole(CLAIMER_ROLE) returns (bool) {
        return false;
    }

    /**
     * @notice Items minting function for the on-chain purchases.
     * 
     * @param issueId The ID of the issue to mint.
     * @param tier The tier of the issue items to mint.
     ...
     */
    function mintItems(uint256 issueId, uint8 tier, uint256 amount, address recipient, uint80[2] calldata rounds, bool consentApproved) public payable virtual nonContract {
        require(consentApproved, "Terms not approved");

        (IssueInfo memory _issueInfo, ItemTier memory _itemTier) = collectionRepository.getItemInfo(allowedItemType, issueId, tier);
        PhaseInfo memory _phaseInfo = collectionRepository.getPhaseInfo(allowedItemType, issueId, tier, _issueInfo.issuePhase);
        if (_phaseInfo.isActive) {
            require((block.timestamp >= _phaseInfo.phaseStart) && (block.timestamp < _phaseInfo.phaseEnd), "Phase not active!");
        }

        require(supportedIssues[issueId] && _issueInfo.issueId > 0, "Invalid series");
        ...
        uint256 _mintAmount = _limitAllowance(_issueInfo.issuePhase, _itemTier, _phaseInfo, _recipient, amount);

        require(_itemTier.isMintable && ((_issueInfo.issuePhase == _mintingPhase) || _mintAmount > 0), "Sale not active!"); 
        require(_itemTier.maxSupply > _itemsCounters[_issueInfo.issueId][_itemTier.tier] + _mintAmount, "Not enough tokens");
        require(_itemsCollected[_sender][_itemTier.tier] + _mintAmount <= _itemTier.maxMints, "Limit claimed");

        uint256[] memory _ids = new uint256[](_mintAmount); 
        uint256[] memory _amounts = new uint256[](_mintAmount); 

        for (uint256 i = 0;  i < _mintAmount; i++) {
            uint256 _serialId = _itemsCounters[_issueInfo.issueId][_itemTier.tier] + _itemTier.offset + i + 1;
            uint256 _baseId = IssueHelper.combineTokenId(_issueInfo.issueId, _itemTier.tier);
            _ids[i] = IssueHelper.combineTokenId(_baseId, _serialId);

            require(!exists(_ids[i]), "ID already minted");

            _amounts[i] = 1;
        }

        if (!hasRole(MINTER_ROLE, _msgSender())) {
            (uint256 _tokenPrice,) = collectionRepository.getDerivedItemPrice(allowedItemType, _issueInfo.issueId, _itemTier.tier, rounds);
            // _tokenPrice = _tokenPrice - ((_tokenPrice * _phaseInfo.discount) / 100);

            _checkout(_issueInfo.beneficiary, (_tokenPrice * _mintAmount));
        }

        _mintBatch(recipient, _ids, _amounts, "");

        unchecked {
            _itemsCounters[_issueInfo.issueId][_itemTier.tier] += _mintAmount;  
            _itemsCollected[_sender][_itemTier.tier] += _mintAmount;         
        }

        emit ItemsDispatched(issueId, tier, _mintAmount, allowedItemType, _recipient);      
    }
    ...
    function setCollectionRepository(address repositoryContract) external onlyPrivileged {
        require(repositoryContract != address(0), "Invalid address");
        collectionRepository = ICollectionRepository(repositoryContract);
    }
     ...
     */
    function totalItemSupply(uint256 issueId, uint8 tier) public view returns (uint256) {
     ...
     */
    function collectedOf(address account, uint256, uint8 tier) public view virtual returns (uint256) {
        return _itemsCollected[account][tier];
    }

    /// @notice Allows owner to set a list of wallets to receive tokens.
    /// @dev This may need to be called many times to set the full list of wallets.
    function setEligibleWallets(address[] calldata wallets, uint256[] calldata amounts) external onlyPrivileged {
        require(wallets.length == amounts.length, "Invalid call data");

        for (uint256 i = 0; i < wallets.length; i++) {
            eligibleWallets.set(wallets[i], amounts[i]);
        }
    }

    /// @notice Allows owner to reset the eligible wallets list.
    function clearEligibleWallets() external onlyPrivileged {
        address[] memory wallets = eligibleWallets.keys();

        for (uint256 i = 0; i < wallets.length; i++) {
            eligibleWallets.remove(wallets[i]);
        }
    }

    /// @notice Allows to check wether given `wallet` is eligible.
    /// @dev returns the token ID if the `wallet` is eligible, `0` otherwise.
    function isWalletEligible(address wallet) external view returns (uint256) {
        return eligibleWallets.get(wallet);
    }
    ...
    function _resolveUri(uint256 tokenId) internal view returns (string memory) {
        (uint256 _baseId, ) = IssueHelper.extractTokenIds(tokenId);
        (uint256 _issueId, uint256 _tier) = IssueHelper.extractTokenIds(_baseId);

        (IssueInfo memory issue, ItemTier memory tier) = collectionRepository.getItemInfo(allowedItemType, _issueId, uint8(_tier));
        if (block.timestamp <= issue.issueTimestamp) {
            return super.uri(tokenId);
        } 

        bytes memory _bytesUri = bytes(tier.tierUri);

        if (_bytesUri.length > 0 && _bytesUri[_bytesUri.length - 1] == SLASH) {
            string memory _tokenUri = string(abi.encodePacked(tier.tierUri, Strings.toString(_tier)));
            if (tier.variantsCount > 1) {
                _tokenUri = string(abi.encodePacked(_tokenUri, "_", Strings.toString(_itemsVariants[uint8(_tier)][tokenId])));
            }
     ...
     */
    function _limitAllowance(IssuePhase phase, ItemTier memory itemTier, PhaseInfo memory, address wallet, uint256 amount) internal view returns (uint256 allowance) {
        if (phase == IssuePhase.Presale) {
            allowance = eligibleWallets.contains(wallet) ? (eligibleWallets.get(wallet) - _itemsCollected[wallet][itemTier.tier]) : 0;
        } else if (phase == IssuePhase.General) {
            allowance = eligibleWallets.contains(wallet) ? (eligibleWallets.get(wallet) - _itemsCollected[wallet][itemTier.tier]) : (itemTier.maxMints - _itemsCollected[wallet][itemTier.tier]);
        } else {
            allowance = amount;
        }
    ...
    function _checkPrivileged() internal view virtual {
        address _sender = _msgSender();
        require(hasRole(DEFAULT_ADMIN_ROLE, _sender) || hasRole(ADMIN_ROLE, _sender), "Caller is not privileged");
    }
    ...
    function onERC1155Received(
        ...
        revert("Transfer not yet allowed");
   ...
   */
    ...
    function _update(address from, address to, uint256[] memory ids, uint256[] memory values)
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; i++) {
                ...
                (uint256 _baseId, ) = IssueHelper.extractTokenIds(tokenId);
                (uint256 _issueId, uint256 _tier) = IssueHelper.extractTokenIds(_baseId);
                (, ItemTier memory _itemTier) = collectionRepository.getItemInfo(allowedItemType, _issueId, uint8(_tier));

                if (_itemTier.variantsCount > 1 && _itemsVariants[uint8(_tier)][tokenId] == 0) {
                    try collectionRepository.shuffleItemVariant(allowedItemType, _issueId, uint8(_tier), tokenId) returns (uint8 _variant) {
                        _itemsVariants[uint8(_tier)][tokenId] = _variant;

                        emit VariantAssigned(tokenId, _variant);
                    } catch {
                    }
                }
            }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155Upgradeable, AccessControlUpgradeable, ERC1155HolderUpgradeable)
        returns (bool)
    {
        return (ERC1155HolderUpgradeable.supportsInterface(interfaceId) || super.supportsInterface(interfaceId));
    }

}

// Chain: POLYGON - File: contracts/pcs/v2/common/CollectionModel.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.22;
...
// Chain: POLYGON - File: contracts/pcs/v2/interfaces/IAirdropSupplier.sol
...
// Chain: POLYGON - File: contracts/pcs/v2/interfaces/ICollectionRepository.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.22;
...
// Chain: POLYGON - File: contracts/pcs/v2/utils/IssueHelper.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.22;
