// Chain: POLYGON - File: @thirdweb-dev/contracts/base/ERC1155Base.sol
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/extension/Permissions.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/extension/PermissionsEnumerable.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/extension/interface/IPermissions.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: @thirdweb-dev/contracts/extension/interface/IPermissionsEnumerable.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// Chain: POLYGON - File: contracts/SoulboundLicense1155.sol
// SPDX-License-Identifier: MIT
...
contract SoulboundLicense1155 is ERC1155Base, PermissionsEnumerable {
  ...
  ) ERC1155Base(address(msg.sender), _name, _symbol, address(0), 0) {
  ...
  event TokenURIUpdated(uint256 indexed _tokenId, string _tokenURI);
  event FeeReceiverAddressUpdated(address indexed _address);
  event FeeCollected(
  ...
  function hasOperatorRole() internal view returns (bool) {
    return hasRole(OPERATOR_ROLE, msg.sender);
  }

  function hasMinterRole() internal view returns (bool) {
    return hasOperatorRole() || hasRole(MINTER_ROLE, msg.sender);
  }

  modifier onlyOperator() {
    require(hasOperatorRole(), 'SoulboundLicense: OPERATOR role is required');
    _;
  }

  modifier onlyMinter() {
    require(hasMinterRole(), 'SoulboundLicense: MINTER role is required');
    _;
  }
  ...
  function _canSetOperatorRestriction() internal view override returns (bool) {
    return hasOperatorRole();
  }
  ...
  function mintTo(
    ...
    require(
      balanceOf[_to][_tokenId] == 0 && _amount == 1,
      'SoulboundLicense: Account can only hold one license for each product'
    );
  ...
  function mintTo(
    ...
    mintTo(_to, _tokenId, _tokenURI, 1);
  }

  function mintTo(address _to, uint256 _tokenId) public {
    mintTo(_to, _tokenId, '', 1);
  }

  function mintToWithFee(
    address _to,
    uint256 _tokenId,
    address _feeToken,
    uint256 _feeAmount
    ...
    require(
      _feeToken == address(0) || _feeAmount > 0,
      'SoulboundLicense: Unacceptable fee token or amount'
    );
    if (_feeToken != address(0)) {
      IERC20(_feeToken).transferFrom(
        msg.sender,
        feeReceiverAddress,
        _feeAmount
      );
  ...
  function burn(address _owner, uint256 _tokenId) public onlyOperator {
    require(balanceOf[_owner][_tokenId] == 1, "SoulboundLicense: No license to burn");
    _burn(_owner, _tokenId, 1);
  }

  function setFeeReceiverAddress(address _address) public onlyOperator {
    feeReceiverAddress = _address;
    emit FeeReceiverAddressUpdated(_address);
  }

  /*//////////////////////////////////////////////////////////////
  UNSUPPORTED FEATURES region
  //////////////////////////////////////////////////////////////*/
  ...
  function batchMintTo(
    ...
    _amounts;
    _baseURI;
    require(false, 'SoulboundLicense: batchMintTo() is not supported');
  }
  ...
  function burnBatch(
    ...
    _tokenIds;
    _amounts;
    require(false, 'SoulboundLicense: burnBatch() is not supported');
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(
      ...
      'SoulboundLicense: License is not transferable'
    );
    super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
  }
}