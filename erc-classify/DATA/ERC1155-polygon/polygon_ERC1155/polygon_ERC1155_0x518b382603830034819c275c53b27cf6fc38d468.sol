// Chain: POLYGON - File: furniture.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";

contract VcityMeta_Furniture is ERC1155 {

    address public owner;

    constructor() ERC1155("https://vcity.app/furniture/{id}.json") {
        owner = msg.sender;
    }
    ...
    function mint(address to, uint256 id) public onlyOwner {
        require(balanceOf(to, id) == 0, "Token ID already minted");
        _mint(to, id, 1, "");
    }

    function batchMint(address[] memory to, uint256[] memory ids) public onlyOwner {
        require(to.length == ids.length, "Addresses and IDs length mismatch");
        for (uint256 i = 0; i < ids.length; i++) {
            require(balanceOf(to[i], ids[i]) == 0, "Token ID already minted");
            _mint(to[i], ids[i], 1, "");
        }
    }
}
