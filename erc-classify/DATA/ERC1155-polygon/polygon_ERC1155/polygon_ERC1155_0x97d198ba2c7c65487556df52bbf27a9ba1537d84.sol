// Chain: POLYGON - File: @a16z/contracts/licenses/CantBeEvil.sol
...
// Chain: POLYGON - File: @a16z/contracts/licenses/ICantBeEvil.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/AllsafeNft.sol
// SPDX-License-Identifier: MIT
...
contract AllsafeNft is ERC1155, ERC2981, Ownable, CantBeEvil(LicenseVersion.PERSONAL_NO_HATE) {
    // This constant denotes the default relative amount that is royalty contributed.
    // This is the fee numerator, the fee denominator is 10000, meaning it equates a permyriad (literally, per 10000).
    // 1 percent = 1/100, 1 promille = 1/1000, 1 permyriad = 1/10000
    // This currently equals: 1000/10000 = 10% royalty fees
    uint96 internal constant DEFAULT_PERMYRIAD = 1000;

    constructor(string memory baseUri, address feeReceiver) ERC1155(baseUri) {
        _setDefaultRoyalty(feeReceiver, DEFAULT_PERMYRIAD);
    }
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        return string.concat(super.uri(tokenId), "/", Strings.toString(tokenId), ".json");
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(CantBeEvil, ERC1155, ERC2981) returns (bool) {
        return CantBeEvil.supportsInterface(interfaceId) || ERC1155.supportsInterface(interfaceId) || ERC2981.supportsInterface(interfaceId);
    }
    ...
    function setRoyaltyReceiver(address feeReceiver)
    public
    onlyOwner
    {
        setRoyaltyReceiverAndAmount(feeReceiver, DEFAULT_PERMYRIAD);
    }

    // The second arguments denotes the permyriad (literally, per 10000) that should be contributed towards the royalty receiver.
    // 1 percent = 1/100, 1 promille = 1/1000, 1 permyriad = 1/10000
    function setRoyaltyReceiverAndAmount(address feeReceiver, uint96 permyriad)
    public
    onlyOwner
    {
        _setDefaultRoyalty(feeReceiver, permyriad);
    }
}