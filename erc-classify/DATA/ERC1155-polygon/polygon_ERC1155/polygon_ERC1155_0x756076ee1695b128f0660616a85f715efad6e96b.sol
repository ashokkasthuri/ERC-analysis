...
// File: contracts/GalaxyLords.sol
...
contract GalaxyLords is ERC1155, Ownable {
    ...
    uint256 public constant MAX_GALAXY_LORDS = 10000;
    ...
    constructor() 
        ERC1155(
            "ipfs://QmPpZ3BgQ5zyYkV6yTriVCEV3uzFHNdVhPh7DmzagvVSsA/metadata/{id}.json"
        ) {}
    ...
    function _internalMint(address to, uint256 count) private {
        ...
        require(tokenId.add(count) <= MAX_GALAXY_LORDS, "Maximum Galaxy Lords supply exceeded");
    ...
    function mint(uint256 count) public payable {
        ...
        require(msg.value >= price.mul(count), "Insufficient count");
    ...
    function mintPresale(uint256 count) public payable {
        ...
        require(count <= reservedAmount, "Can't mint more than reserved presale amount");
        require(msg.value >= price.mul(count), "Insufficient amount");

        presaleList[msg.sender] = reservedAmount.sub(count);

        _internalMint(msg.sender, count);
    }

    function giveAway(address destination, uint256 count) public onlyOwner {
        _internalMint(destination, count);
    }
