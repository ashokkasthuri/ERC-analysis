...
// Chain: POLYGON - File: src/AdminToken.sol
// SPDX-License-Identifier: MIT
...
//  declares the contract AdminToken which inherits from the Initializable and ERC1155Upgradeable contracts.
//  Initializable is used to create upgradeable contracts, and ERC1155Upgradeable is an upgradeable version of the ERC1155 token standard

contract AdminToken is Upgradeable, Initializable, ERC1155Upgradeable {
    uint256 public constant ADMIN_TOKEN = 0;
    mapping(address => bool) public isAdmin;

    function initialize() external initializer {
        __ERC1155_init("");
        _mint(msg.sender, ADMIN_TOKEN, 1, "");
        isAdmin[msg.sender] = true;
    }

    function grantAdmin(address to) external returns (bool) {
        require(balanceOf(msg.sender, ADMIN_TOKEN) > 0, "Only admin can grant admin privileges");
        require(balanceOf(to, ADMIN_TOKEN) == 0, "User already has an admin token");
        _mint(to, ADMIN_TOKEN, 1, "");
        isAdmin[to] = true;
        return true;
    }

    function revokeAdmin(address from) external returns (bool) {
        require(balanceOf(msg.sender, ADMIN_TOKEN) > 0, "Only admin can revoke admin privileges");
        _burn(from, ADMIN_TOKEN, balanceOf(from, ADMIN_TOKEN));
        isAdmin[from] = false;
        return true;
    }

    function _authorizeUpgrade(address) internal view override {
        require(isAdmin[msg.sender], "AdminToken: caller is not an admin");
    }
}