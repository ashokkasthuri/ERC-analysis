...
// Chain: POLYGON - File: contracts/Blueprints.sol
//SPDX-License-Identifier: MIT
...
@dev    Implementation of Crystale Blueprints Non Fungible Tokens using ERC1155.
*/
contract CrystaleBlueprints is ERC1155, ERC2981, AccessControl {
    ...
    event MintedTokenInfo(address user, uint256 tokenId, uint256 amount);
    event MintedBatchTokenInfo(address user, uint256[] tokenIds, uint256[] amounts);
    ...
    constructor(
        ...
        _setDefaultRoyalty(royaltyAddress_, 500);
    }

    /**
     * @dev Allows the admin to set the royalty address of the smart contract. 
     */
    function setRoyaltyAddress(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        royaltyAddress = newAddress;
    }

    /**
     * @dev set the royalty for the tokens
     * @param fee the royalty fee
     */
    function setRoyalty(uint96 fee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setDefaultRoyalty(royaltyAddress, fee);
    }

    /**
     *@dev sets the maximum amount of token ids
     */
    function setMaxTokenIds(uint256 newMaxTokenIds) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxTokenIds = newMaxTokenIds;
    }

    /**
     * @dev freezes the setURI function
     */
     ...
     */
    function setOwner(address previousOwner, address newOwner) external onlyRole(DEFAULT_ADMIN_ROLE) {
        owner = newOwner;
    ...
    /**
     * @dev mints a new token to a given user
     */
    function mint(address user, uint256 tokenId, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(balanceOf(user, tokenId) == 0, "User has this token already");
        require(tokenId <= maxTokenIds, "Max supply reached"); 
        _mint(user, tokenId, amount, '');
        emit MintedTokenInfo(user, tokenId, amount);
    }

    /**
     * @dev mints a new token to a given user
     */
    function batchMint(address user, uint256[] memory tokenIds, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            require(balanceOf(user, tokenIds[i]) == 0, "User has this token already");
            require(tokenIds[i] <= maxTokenIds, "Max supply reached");  
        }
        _mintBatch(user, tokenIds, amounts, '');
        totalSupply += amounts.length;
        emit MintedBatchTokenInfo(user, tokenIds, amounts);
    }
