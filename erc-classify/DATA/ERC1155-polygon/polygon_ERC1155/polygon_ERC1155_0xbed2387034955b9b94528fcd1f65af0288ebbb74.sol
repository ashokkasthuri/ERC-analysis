...
contract Migrater is ERC1155Holder {
    address private owner; // contract address

    struct NFT{
        string desc;
        uint256 price;
        uint256 lowerBound; // lowerBound in ERC721, id in ERC1155
        uint256 upperBound;
        uint256 balance;
        address addr;
    ...
    function createNFT(
        ...
        uint256 balance, 
        address addr,
        uint nftType) public onlyOwner{
            NFTs[nid] = NFT(desc, price, lowerBound, upperBound, balance, addr, nftType);
    }
    ...
    function getNFT(uint256 nid) public view returns (string memory, uint256, uint256, uint256, uint256, address, uint) {
        NFT memory nft = NFTs[nid];
        return (nft.desc, nft.price, nft.lowerBound, nft.upperBound, nft.balance, nft.addr, nft.nftType);
    }
    ...
    function createNFTGroup(string memory groupName, string memory platform, uint nftType, uint256[] memory nids) public onlyOwner {
        for (uint256 i = 0; i < nids.length; i++) {
            require(NFTs[nids[i]].addr != address(0), "nft not found");
    ...
    function transferNFTGroup(address to, string memory groupName, string memory fromPlatform, string memory toPlatform) public {
        checkNFTEnough(msg.sender, groupName, fromPlatform);
        checkNFTEnough(address(this), groupName, toPlatform);
        transferNFTGroup(msg.sender, address(this), groupName, fromPlatform);
        transferNFTGroup(address(this), to, groupName, toPlatform);
    }

    function checkNFTEnough(address from, string memory groupName, string memory platform) internal {
        if(from == address(this)) {
            ...
            if (NFTGroups[groupName][platform].nftType == 0) {
                ...
                for (uint256 i = 0; i < NFTGroups[groupName][platform].NFTList.length; i++) {
                    NFTs[NFTGroups[groupName][platform].NFTList[i]].balance++;
                    NFT memory nft = NFTs[NFTGroups[groupName][platform].NFTList[i]];
                    IERC1155 nftContract = IERC1155(nft.addr);
    ...
    function transferNFTGroup(address from, address to, string memory groupName, string memory platform) internal {
        if (NFTGroups[groupName][platform].nftType == 0) {
            for (uint256 i = 0; i < NFTGroups[groupName][platform].NFTList.length; i++) {
                NFT memory nft = NFTs[NFTGroups[groupName][platform].NFTList[i]];
                IERC721Enumerable nftContract = IERC721Enumerable(nft.addr);
