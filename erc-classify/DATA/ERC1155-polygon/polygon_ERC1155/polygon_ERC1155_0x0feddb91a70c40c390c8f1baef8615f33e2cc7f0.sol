...
// Chain: POLYGON - File: @openzeppelin/contracts-upgradeable/utils/introspection/ERC165StorageUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/pearl/interface/IPearl.sol
...
interface IPearl is IERC1155Upgradeable {
...
// Chain: POLYGON - File: contracts/pearl/PearlV4.sol
...
import "./interface/IPearl.sol";


contract PearlV4 is IPearl, ERC1155Upgradeable, AccessControlUpgradeable, OwnableUpgradeable, ERC165StorageUpgradeable, IERC2981Upgradeable {

    bytes32 public constant MINT_ROLE = keccak256("MINT");
    bytes32 public constant URI_ROLE = keccak256("URI");
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public constant BURN_FORWARDER_ROLE = keccak256("BURN_FORWARDER_ROLE");
    ...
    function initialize(string memory uri_) external initializer {
        __Ownable_init_unchained(); //ensures msg sender is set correctly
        __ERC1155_init(uri_);
    ...
    function addMintRole(
        address _to
    ) public onlyRole(ADMIN_ROLE) {
        _setupRole(MINT_ROLE, _to);
    }

    function removeMintRole(
        address _from
    ) public onlyRole(ADMIN_ROLE) {
        _revokeRole(MINT_ROLE, _from);
    }
    ...
    function burnForward(
        uint256 id,
        uint256 amount
    ) external override onlyRole(BURN_FORWARDER_ROLE) {
    ...
    function burnDirect(
        ...
        uint256 id,
        uint256 amount
    ) external onlyRole(BURN_FORWARDER_ROLE) {
    ...
    function mintBatchMultiple(
        ...
        require(tos.length == ids.length && tos.length == amounts.length, "address list, id list, and amount list must be equal");
        for (uint i = 0; i < tos.length; i++) {
