...
// Chain: POLYGON - File: contracts/NFT.sol
// SPDX-License-Identifier: MIT
...
contract NFT is ERC1155Burnable, ERC2981, Ownable {
    using Counters for Counters.Counter;
    string private _baseTokenURI = "https://sound-desert-upload-prod.s3.ap-northeast-1.amazonaws.com/artist/metame/nft/web3_crossroads";
    mapping(uint256 => uint256) public totalSupply;

    mapping(uint256 => bool) public locked; // true: lock, false: unlock
    mapping(address => bool) public allowedTransferAddresses;

    constructor() ERC1155("") {
        _setDefaultRoyalty(address(0x2557B21F84e60308DF9525c4F73fA3D21C4f9A14), 1000);
    }
    ...
    function mint(uint256 _tokenId, address _target, uint256 _amount) public onlyOwner {
        _mint(_target, _tokenId, _amount, "");
        totalSupply[_tokenId] += _amount;
    }

    // Lock
    function setLocked(uint256 _tokenId, bool _state) external onlyOwner {
        locked[_tokenId] = _state;
    }

    function setAllowedTransferAddress(address _target, bool _allowed) public onlyOwner {
        allowedTransferAddresses[_target] = _allowed;
    }
    ...
    function burn(
        ...
        require(balanceOf(_targetAddress, _tokenId) >= _amount, "User does not own enough of the token");

        totalSupply[_tokenId] -= _amount;
        super.burn(_targetAddress, _tokenId, _amount);
    }
    ...
    function burnBatch(
        ...
        for (uint256 i = 0; i < _tokenIds.length;) {
            ...
            require(balanceOf(_targetAddress, tokenId) >= amount, "User does not own enough of the token");
        ...
        super.burnBatch(_targetAddress, _tokenIds, _amounts);
    }
    ...
    function safeTransferFrom(
        ...
        if (!allowedTransferAddresses[msg.sender]) {
    ...
    function safeBatchTransferFrom(
        ...
        if (!allowedTransferAddresses[msg.sender]) {
            for (uint i = 0; i < ids.length; i++) {
                require(!locked[ids[i]], "One of the tokens is locked");
            }
        }
