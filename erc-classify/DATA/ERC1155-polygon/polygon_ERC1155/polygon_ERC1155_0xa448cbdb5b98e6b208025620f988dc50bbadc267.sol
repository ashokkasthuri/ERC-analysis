// Chain: POLYGON - File: @chainlink/contracts/src/v0.8/interfaces/VRFCoordinatorV2Interface.sol
...
// Chain: POLYGON - File: @chainlink/contracts/src/v0.8/VRFConsumerBaseV2.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/access/OwnablePermissions.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/erc1155c/ERC1155C.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/interfaces/ICreatorToken.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/interfaces/ICreatorTokenTransferValidator.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/interfaces/IEOARegistry.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/interfaces/ITransferSecurityRegistry.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/interfaces/ITransferValidator.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/programmable-royalties/BasicRoyalties.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/token/erc1155/ERC1155OpenZeppelin.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/utils/CreatorTokenBase.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/utils/TransferPolicy.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @limitbreak/creator-token-contracts/contracts/utils/TransferValidation.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: POLYGON - File: @openzeppelin/contracts/interfaces/IERC165.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/interfaces/IValeriaBoosterPacks.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

interface IValeriaBoosterPacks {
    event ItemDrops(address indexed to, uint256[] ids, uint256[] amounts);

    function moderatorMint(
        address account,
        uint256 id,
        uint256 amount,
        bytes memory data
    ...
    function mintItem(address to, uint256 typeId, uint256 amount) external;
...
// Chain: POLYGON - File: contracts/ValeriaLandBeforeTheWar.sol
// SPDX-License-Identifier: MIT
...
import {IValeriaBoosterPacks} from "./interfaces/IValeriaBoosterPacks.sol";
...
/// @title ValeriaLandBeforeTheWar
/// @notice The Valeria Land Before The War collection.
///         Burn a Booster pack for a distribution of valerians/champions
///         Allows the option for external contracts to burn/mint
/// @author @ValeriaStudios
contract ValeriaLandBeforeTheWar is
    AccessControl,
    ERC2771Context,
    ERC1155C,
    BasicRoyalties,
    VRFConsumerBaseV2,
    IValeriaBoosterPacks
{
    // VRF V2
    ...
    event RandomnessRequest(uint256 indexed requestId, uint256 indexed packId);
    ...
    error IsPaused();
    error MaxPacksPerTransaction();
    error InvalidPackConfiguration();

    IValeriaBoosterPacks boosterPacksContract;

    /// @dev Whether pack openings are live or not
    bool public isPaused = true;

    /// @dev Use VRF configuration for openings
    bool public useVRF = false;

    /// @notice Nonce counter for pseudo requests
    uint256 private requestNonce;

    /// @dev Max packs you can open per transaction
    uint256 public maxPacksPerTx = 20;

    /// @dev VRF callback limit
    uint32 public vrfGasLimit = 500000;

    /// @dev requestId => drop config at time of request
    mapping(uint256 => PackOpenRequest) private requestIdToRequestConfig;

    /// @dev packId => opening config at time of request
    mapping(uint256 => PackConfiguration) public configurations;

    constructor(
        address _boosterPacksContractAddress,
        ...
        BasicRoyalties(royaltyReceiver_, royaltyFeeNumerator_)
    {
        ...
        boosterPacksContract = IValeriaBoosterPacks(
            _boosterPacksContractAddress
        );
    }

    function name() external pure returns (string memory) {
        return "Valeria: Land Before The War";
    }
    ...
    /**
     * @notice Opens a set of packs for a particular token id
     * @dev Emits a pack opening outcome
     * @param packId - The token id of a particular pack
     * @param amount - The amount of packs to open
     */
    function open(uint256 packId, uint256 amount) external payable {
        if (isPaused) revert IsPaused();
        if (amount > maxPacksPerTx) revert MaxPacksPerTransaction();

        PackConfiguration storage config = configurations[packId];

        if (config.packId != packId) revert InvalidPackConfiguration();

        address requester = _msgSender();

        boosterPacksContract.burnItem(requester, packId, amount);

        uint256 requestId;

        // Process opening sequence thru vrf or pseudorandom
        if (useVRF) {
            ...
            _processRandomnessRequest(requestId, packId, amount, requester);
            emit RandomnessRequest(requestId, packId);
        } else {
            requestNonce++; // Bump internal request nonce for request id usage

            _processFulfillment(
                requestNonce,
                configurations[packId],
    ...
    function _processFulfillment(
        uint256 requestId,
        PackConfiguration memory config,
        uint256 randomness,
        uint256 amount,
        address requester
    ) internal {
        uint256 mintQuantity = config.quantity * amount;
        uint256[] memory ids = new uint256[](mintQuantity);
        uint256 seed;
        uint256 indexChosen;
        uint256 i = 0;

        for (; i < mintQuantity; i++) {
        ...
        _mintOpenedItems(requester, ids);
     ...
     * @param to - the address to mint items too
     * @param tokenIds - the token ids to mint
     */
    function _mintOpenedItems(address to, uint256[] memory tokenIds) internal {
    ...
    function _handlePackOpeningRequest(
        uint256 requestId,
        uint256 packId,
        uint256 amount,
        address requester
        ...
        requestIdToRequestConfig[requestId] = PackOpenRequest({
            requester: requester,
            packId: packId,
            amount: amount
        });
    ...
    /// @dev Handle minting items related to a pack opening request
    function _handlePackOpeningFulfillment(
        uint256 requestId,
        uint256 randomness
    ) internal {
        PackOpenRequest storage requestConfig = requestIdToRequestConfig[
            requestId
        ];
        _processFulfillment(
            requestId,
            configurations[requestConfig.packId],
            randomness,
            requestConfig.amount,
    ...
    function _processRandomnessRequest(
        ...
        _handlePackOpeningRequest(requestId, packId, amount, requester);
    }
    ...
    function _processRandomnessFulfillment(
        ...
        _handlePackOpeningFulfillment(requestId, randomness);
    }
    ...
    /// @dev if not using VRF, "randomness"
    ...
    /**
     * @notice Returns the config information of specific set of pack token ids
     * @param packIds token ids to check against
     * @return bytes[]
     */
    function configurationOf(
        uint256[] memory packIds
    ) external view returns (bytes[] memory) {
        bytes[] memory configs = new bytes[](packIds.length);
        PackConfiguration storage config;
        uint256 tokenId;

        for (uint256 i; i < packIds.length; i++) {
            tokenId = packIds[i];
            config = configurations[tokenId];
    ...
    function setUseVRF(bool _useVRF) external onlyRole(MODERATOR_ROLE) {
        useVRF = _useVRF;
    }

    /**
     * @notice Sets the booster packs contract
     * @param _boosterPacksContractAddress - The address of the packs contract
     */
    function setBoosterPackContract(
        address _boosterPacksContractAddress
    ...
    /**
     * @notice Sets the booster packs contract
     * @param _maxPacksPerTx - The max amount of packs to open
     */
    function setMaxPacksPerTransaction(
        uint256 _maxPacksPerTx
    ) external onlyRole(MODERATOR_ROLE) {
        maxPacksPerTx = _maxPacksPerTx;
    }

    /**
     * @notice Sets the vrf gas callback limit
     * @param _vrfGasLimit - A gas limit
     */
    function setVRFCallbackLimit(
        uint32 _vrfGasLimit
    ) external onlyRole(MODERATOR_ROLE) {
    ...
    /**
     * @notice Sets a booster packs configuration by pack token id
     * @param packId - The pack id
     * @param config - The pack configuration
     */
    function setBoosterPackConfiguration(
        uint256 packId,
        PackConfiguration calldata config
    ) external onlyRole(MODERATOR_ROLE) {
        configurations[packId] = config;
    }

    /**
     * @notice Sets pack configuration
     * @param config - The config object representing the pack
     */
    function setPackConfiguration(
        PackConfiguration calldata config
    ) public onlyRole(MODERATOR_ROLE) {
        require(config.quantity > 0, "Mint quantity should be greater than 0");
        require(config.itemIds.length > 0, "Must have at least 1 item");
        require(
            config.probabilities.length == config.aliases.length,
            "Invalid config"
        );
        configurations[config.packId] = config;
    }
    ...
    /// @dev Allows a moderator to batch mint items
    function moderatorMintBatch(
        address to,
        uint256[] calldata ids,
        uint256[] calldata amounts,
    ...
    /// @dev Allows a moderator to airdrop items to multiple addresses
    function moderatorAirdrop(
        ...
        for (uint256 i; i < tos.length; i++) {
            _mintBatch(tos[i], ids[i], amounts[i], data);
        }
    }
