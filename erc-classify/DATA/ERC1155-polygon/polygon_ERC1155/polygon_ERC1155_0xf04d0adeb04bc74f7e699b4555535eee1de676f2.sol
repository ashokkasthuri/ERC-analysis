// Chain: POLYGON - File: contracts/storefront/StorefrontNFT.sol
// contracts/storefront/StorefrontNFT.sol
...
contract StorefrontNFT is ERC1155, IRamperInterface1155, IERC2981Royalties {
    ...
    uint256 constant public ROYALTY_BPS = 1000;
    address public royaltyWallet;

    constructor(string memory uri, uint256 _price, address _royaltyWallet) ERC1155(uri) {
      // Token indexes start at 1
      for(uint i=1; i<10; i++){
        tokenIdToInfo[i] = TokenInfo(
          1, // Quantity is always 1
          _price, // Price is set by user
          true
        );
      }
      // Set creator's wallet for royalties
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _value) override external view returns (address _receiver, uint256 _royaltyAmount) {
        _receiver = royaltyWallet;
        _royaltyAmount = _value * ROYALTY_BPS / 10000;
    }
}
...
// Chain: POLYGON - File: contracts/storefront/IRamperInterface1155.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/storefront/IERC2981Royalties.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
