...
// Chain: POLYGON - File: contracts/implementations/TreasureHunt.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

// L           A     BBBBB    RRRR    Y     Y   SSSS
// L          A A    B    B   R   R    Y   Y   S
// L         AAAAA   BBBBB    RRRR       Y      SSS
// L        A     A  B    B   R  R       Y         S
// LLLLL   A       A B    B   R   R      Y     SSSS
...
contract TreasureHunt is ERC1155, ERC1155Burnable, Ownable {
  using Strings for uint256;

  // Token ID for each letter in LABRYS
  uint256 public constant L = 1;
  uint256 public constant A = 2;
  uint256 public constant B = 3;
  uint256 public constant R = 4;
  uint256 public constant Y = 5;
  uint256 public constant S = 6;

  // Magic 7th token that lets user claim a prize
  uint256 public constant LABRYS = 7;

  /// @dev thrown when array lengths don't match in bulkMint
  error MismatchedArrayLength();

  /// @dev thrown when someone is being greedy
  error DontBeGreedy();

  /// @dev event emitted when someone has indeed gotem
  event GotEm(address account);

  /// @dev where the metadata will live
  string baseUri;

  constructor(string memory _baseUri) ERC1155("") Ownable() {
    baseUri = _baseUri;
  }
  ...
  // Contract owner can mint more if run out of initial supply
  function mint(
    address account,
    uint256 id,
    uint256 amount,
  ...
  // Override transfer functions to only be callable by owner
  function safeTransferFrom(
    address from,
    address to,
    uint256 id,
