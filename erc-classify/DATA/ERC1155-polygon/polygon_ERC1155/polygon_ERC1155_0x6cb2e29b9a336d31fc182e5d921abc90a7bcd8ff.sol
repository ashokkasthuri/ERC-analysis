 ...
 */
contract ERC1155tucasa24tokendaoInternal is Ownable, AccessControl, ERC1155Supply {
    ...
    modifier onlyMinter() {
        ...
        require(
            canIMint(),
            "ERC1155tucasa24tokendao: must have minter role to mint"
        );
        _;
    }
...
/**
 * @title ERC1155tucasa24tokendao
 * ERC-721 Marketplace with tokens and royalties support
 */
contract ERC1155Marketplace is ERC1155tucasa24tokendaoInternal, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC1155tucasa24tokendaoInternal(_owner, uri, name, symbol, _anyoneCanMint)
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "tucasa24tokendao: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "tucasa24tokendao: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "tucasa24tokendao: the user cannot buy his own offer");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "tucasa24tokendao: only the original seller can cancel his sales");
    ...
    function bid(uint256 tokenId, uint256 qty, uint256 price) external nonReentrant {
        ...
        require(qty>0, "tucasa24tokendao: qty has to be positive");
        require(price>0, "tucasa24tokendao: price has to be positive");
        require(qty <= totalSupply(tokenId), "tucasa24tokendao: not enough items for sale");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "tucasa24tokendao: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "tucasa24tokendao: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "tucasa24tokendao: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "tucasa24tokendao: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "tucasa24tokendao: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract ERC1155tucasa24tokendao is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "ERC1155tucasa24tokendao-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "tucasa24tokendao: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "tucasa24tokendao: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "tucasa24tokendao: Wrong NFT ID");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "tucasa24tokendao: wrong signature");
    }
