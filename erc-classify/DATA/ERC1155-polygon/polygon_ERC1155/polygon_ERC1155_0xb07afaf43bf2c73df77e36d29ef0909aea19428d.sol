...
// Chain: POLYGON - File: contracts/IRamperInterface1155.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/Mintiero.sol
// SPDX-License-Identifier: MIT
...
contract Mintiero is ERC1155, Ownable, IRamperInterface1155 {
    ...
    function mint(address _userWallet, uint256 _quantity, uint256 _tokenId) override external payable {
        uint256 tokenPriceInUSD = tokenPrices[_tokenId];
        uint256 tokenPriceInMatic = getTokenPriceInMaticWithDiscount(tokenPriceInUSD);
    ...
    function mintWeb(address _userWallet, uint256 _quantity, uint256 _tokenId) external payable {
        uint256 tokenPriceInUSD = tokenPrices[_tokenId];
    ...
    function priceWeb(uint256 _tokenId) external view returns (uint256) {
        uint256 tokenPriceInUSD = tokenPrices[_tokenId];
        uint256 tokenPriceInMatic = getTokenPriceInMatic(tokenPriceInUSD);
        return tokenPriceInMatic;
    }
    ...
    function getTokenPriceInMaticWithDiscount(uint256 priceInUSD) public view returns (uint256) {
        uint256 startGas = gasleft();

        uint256 _price = getLatestPrice();
        require(_price > 0, "Price feed error");

        uint256 feePercentage = 390;
        uint256 feeAmount = (priceInUSD * feePercentage) / 10000;

        uint256 priceMinusFee = priceInUSD - feeAmount;
        uint256 priceMinusFeeAndFiftyCents = priceMinusFee - (5 * 10 ** 17);

        uint256 priceInMatic = (priceMinusFeeAndFiftyCents * 10 ** 18) * 10 ** 8 / _price;

        uint256 serviceFee = (1 ether * 10 ** 8) / (_price * 2);
        uint256 gasUsed = startGas - gasleft();

        uint256 priceInMaticMinusGas = priceInMatic - gasUsed - serviceFee;

        return priceInMaticMinusGas;
    }
}