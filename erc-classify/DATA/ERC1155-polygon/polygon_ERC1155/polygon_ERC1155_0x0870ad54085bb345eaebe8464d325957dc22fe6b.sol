...
// Chain: POLYGON - File: contracts/OxiZen.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
contract OxiZen is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, ERC1155PausableUpgradeable, ERC1155SupplyUpgradeable {
    using Arrays for uint256[];
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    mapping (uint256 id => string) public _metadataHash;
    // Contract name
    string public constant name = "Green GAYA by OXI-ZEN";
    // Contract symbol
    string public constant symbol = "GAYA";
    ...
    function initialize(address initialAuthority) initializer public {
        ...
        _grantRole(ADMIN_ROLE, initialAuthority);
    }

    function uri(uint256 tokenId) public view override returns (string memory) {
        require(exists(tokenId), "ERC1155Metadata: URI query for nonexistent token");

        string memory _tokenMetadataHash = _metadataHash[tokenId];

        string memory baseURI = super.uri(tokenId);

        // If there is no base URI, return the token URI.

        if(bytes(_tokenMetadataHash).length > 0) {
            // If there is a baseURI but no tokenURI, follow https://eips.ethereum.org/EIPS/eip-1155#metadata
            if (bytes(baseURI).length == 0) {
                return string(abi.encodePacked("ipfs://", _tokenMetadataHash));
            }
            
            return string(abi.encodePacked(baseURI, _tokenMetadataHash));
        }
    ...
    function mint(address account, uint256 id, uint256 amount, string memory metadataHash)
        public
        onlyRole(ADMIN_ROLE)
    {   
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, string[] memory metadataHashList)
        public
        onlyRole(ADMIN_ROLE)
    {
        _mintBatch(to, ids, amounts, "");
        _setMetadataHashBatch(ids, metadataHashList);
    }
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory values) public virtual {
        ...
        _removeMetaHashBatch(account, ids);
        
    }

    function _setMetadataHash(uint256 id, string memory metadataHash) internal virtual {
        uint256[] memory ids = new uint256[](1);
        ids[0] = id;
        string[] memory values = new string[](1);
        values[0] = metadataHash;
        _setMetadataHashBatch(ids, values);
    }

    function _setMetadataHashBatch(uint256[] memory ids, string[] memory metadataHashList) internal virtual {
        require(ids.length == metadataHashList.length, "ERC1155: Invalid array length");
        for (uint256 i = 0; i < ids.length; ++i) {
            _metadataHash[ids.unsafeMemoryAccess(i)] = metadataHashList[i];
        }
    }

    function _removeMetaHash(address account, uint256 id) internal virtual {
        uint256[] memory ids = new uint256[](1);
        ids[0] = id;
        _removeMetaHashBatch(account, ids);
    }

    function _removeMetaHashBatch(address owner, uint256[] memory ids) internal virtual {
        for (uint256 i = 0; i < ids.length; ++i) {
            if(
                bytes(_metadataHash[ids.unsafeMemoryAccess(i)]).length != 0 &&
                balanceOf(owner, ids.unsafeMemoryAccess(i)) == 0
            ){
                delete _metadataHash[ids.unsafeMemoryAccess(i)];
            }
        }
    }
