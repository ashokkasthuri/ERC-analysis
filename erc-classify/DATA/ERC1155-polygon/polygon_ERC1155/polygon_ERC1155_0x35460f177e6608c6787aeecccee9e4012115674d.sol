...
contract MintERC1155 is ERC1155, Ownable, ERC1155Burnable {
    ...
    string public constant name = "Spinblade First Edition Box";
    string public constant symbol = "SBCBOX";

    mapping(uint256=> string) private _uris;
    string baseURI;
    bool public paused;

    address public TreasuryWallet = 0x94eC5b4905C2BBF8eabBFf9B230360098B5ae5b4;//our secret multi-sig wallet and we will save all of money here until presale is ended
    ...
    mapping(uint256 => uint256) public mintTypes;  // id => percent(100 = 1%);
    ...
    function mint(address to, uint256 amount, uint256 tokenId) external payable {
        ...
        require(
            ...
            block.timestamp <
            idToDrop[totalDrop].publicStartTime + idToDrop[totalDrop].publicDuration,
            "Not time to mint"
        );
        require(
        ...
        (bool sent,) = TreasuryWallet.call{value: value}("");
    ...
    function mintTool(uint256 amount, uint256 passId, uint256 tokenId) external payable {
        require(!paused, "mint is paused");
        ...
        require(
            token1155.balanceOf(user, passId) > 0,
            "You don't have this token"
        );

        mintPassContract.burn(msg.sender, passId, amount);

        if(mintTypes[passId] != 0) {
            // token.transferFrom(
                // msg.sender,
                // address(this),
                // (cost * amount * mintTypes[passId]) / 10000
            // );
            // address(this).send((cost * amount * mintTypes[passId]) / 10000);
            uint256 value = msg.value;
            
            (bool success,) = TreasuryWallet.call{value : value}("");
    ...
    function setTreasuryWallet(address _newMultiSig) public onlyOwner{
        TreasuryWallet = _newMultiSig;
    } 
    ...
    function showItems(uint256 number) external view returns (Item memory) {
        require(items[number].id == number);
        return items[number];
    }
    ...
    function setMintTypes(uint256 _passId, uint256 _percent) external onlyOwner {
        mintTypes[_passId] = _percent;
    }
