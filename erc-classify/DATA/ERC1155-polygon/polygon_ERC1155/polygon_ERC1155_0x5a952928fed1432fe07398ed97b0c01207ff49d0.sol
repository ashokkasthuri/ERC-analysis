...
// File contracts/PurrErc1155Bridge.sol
...
contract PurrErc1155Bridge is ERC1155Holder, EIP712, Pausable, AccessControl {
    ...
    constructor(address _nft) EIP712("Purr1155Bridge", "1") {
        nft = IERC1155(_nft);
    ...
    function transferTokenToOffchain(uint256[] calldata tokenIds)
        external
        whenNotPaused
    {
        uint256[] memory amounts = _getAmountInArray(tokenIds.length);
        ...
        nonce++;
        emit TransferTokenToOffchain(msg.sender, nonce, tokenIds);
    }
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "transfer(address to,uint256[] tokenIds,uint256 nonce,uint256 deadline)"
                        ),
                        to,
    ...
    function _getAmountInArray(uint256 _length)
        internal
        pure
        returns (uint256[] memory)
    {
    ...
    {
        require(
            _trustedValidator != address(0),
            "trusted validator address cannot be zero"
        );
        require(
