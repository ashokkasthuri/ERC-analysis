// Chain: POLYGON - File: contracts/StacheCO2RetiredNFT.sol
...
contract StacheCO2RetiredNFT is ERC1155Supply, Admin {
    string public name = "StacheCO2 Retired";
    string baseUrl;
    mapping(uint256 => string) public idToName; //id to name mapping
    mapping(uint256 => uint256) public idToPrice; //id to name mapping
    mapping(uint256 => bool) public idExist; //id whether exist mapping
    mapping(uint256 => uint256) public idToCap; //id to cap mapping
    ...
    */
    constructor(string memory _uri, string[] memory _names, uint256[] memory _ids, uint256[] memory _prices, uint256[] memory _caps) ERC1155(_uri) Admin(msg.sender) {
        baseUrl=_uri;
        setURI(_uri);
        for (uint id = 0; id < _ids.length; id++) {
            require(!idExist[_ids[id]],"id should not exist");
            idToName[_ids[id]] = _names[id];
            idToPrice[_ids[id]] = _prices[id];
            idToCap[_ids[id]] = _caps[id];
            idExist[_ids[id]] = true;
        }
    }

    /*
    add mapping to new tokens
    */
    function createNewTokenCategories(uint256[] memory _ids, uint256[] memory _prices,string[] memory _names, uint256[] memory _caps) public onlyAdmin {
        for (uint id = 0; id < _ids.length; id++) {
            require(!idExist[_ids[id]],"id should not exist");
            idToName[_ids[id]] = _names[id];
            idToPrice[_ids[id]] = _prices[id];
    ...
    /*
        overwrite price and name of tokenId
    */
    function overwriteTokensOfId(uint256 _id, uint256 _price,string memory _name, uint256 _cap) public onlyAdmin {
        idToName[_id] = _name;
    ...
    function getCap(uint256 _id) public view returns(uint256) {
        return idToCap[_id];
    }

    function tokenIdExist(uint256 _id) public view returns(bool) {
        return idExist[_id];
    }
    ...
    */
    function mintBatch(address to, uint256[] memory _ids, uint256[] memory _amounts) public onlyAdmin
    {
        for (uint id = 0; id < _ids.length; id++) {
            uint256 totalSupply=totalSupply(id);
            require(totalSupply+_amounts[id]<=idToCap[id],"exceeds cap");
        }
    ...
    function mint(address _to, uint256 _id, uint256 _amount) public onlyAdmin {
        ...
        require(totalSupply+_amount<=idToCap[_id],"exceeds cap");
        _mint(_to, _id, _amount, '');
    }

    function burnBatch(address to, uint256[] memory _ids, uint256[] memory _amounts) public onlyAdmin
    {
        _burnBatch(to, _ids, _amounts);
    }

    function burn(address _to, uint256 _id, uint256 _amount) public onlyAdmin {
...
// Chain: POLYGON - File: contracts/Admin.sol
// SPDX-License-Identifier: MIT
