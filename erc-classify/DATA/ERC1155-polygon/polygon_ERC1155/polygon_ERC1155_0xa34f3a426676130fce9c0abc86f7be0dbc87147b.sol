...
// Chain: POLYGON - File: contracts/TornadoNFT.sol
...
contract TornadoNFT is ERC1155, Ownable {
    ...
    function addNewTokenMetadata(
        string[] memory metadataURI
    ) public onlyOwner {
        for (uint256 i = 0; i < metadataURI.length; i++) {
            currMaxId++;
            tokenIdToMetadata[currMaxId] = metadataURI[i];
        }
    }
    ...
    function mint(
        ...
        // require(block.timestamp - lastMinted[account] >= lockPeriod, "Can only claim once every 30 days");
        // lastMinted[account] = block.timestamp;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 0 && tokenIds[i] <= currMaxId, "Token ID invalid");
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        require(tokenId <= currMaxId, "Token ID invalid");
