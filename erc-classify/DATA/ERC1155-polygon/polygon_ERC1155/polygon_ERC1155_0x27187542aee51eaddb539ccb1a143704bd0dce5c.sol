...
// Chain: POLYGON - File: contracts/Trade.sol
...
contract Trade is AccessControl {
    event GenericFeeUpdated(uint256 fee);
    event EpicFeeUpdated(uint256 fee);
    event LegendaryFeeUpdated(uint256 fee);
    ...
    IERC1155 public immutable tokenAddr;
    address public owner;
    address public signer;
    uint256 public genericFee;
    uint256 public epicFee;
    uint256 public legendaryFee;
    ...
    constructor(
        ...
        genericFee = _generic * 1e18;
        epicFee = _epic * 1e18;
        legendaryFee = _legendary * 1e18;
    ...
    function setPrice(
        ...
        if (nftType == traitType.Generic) {
            genericFee = fee;
            emit GenericFeeUpdated(genericFee);
        } else if (nftType == traitType.Epic) {
            epicFee = fee;
            emit EpicFeeUpdated(genericFee);
        } else if (nftType == traitType.Legendary) {
            legendaryFee = fee;
            emit LegendaryFeeUpdated(genericFee);
        } else {
            revert("invalid nftType");
        }
        return true;
    }
    ...
    function tradeAsset(
        ...
        require((order.ids.length == order.supply.length), "Invalid length");
        require(!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        require(
            msg.value >= getFee(order.nftType, order.supply),
            "Paid invalid amount"
        );
        ...
        tokenAddr.safeBatchTransferFrom(
            owner,
            buyer,
            order.ids,
            order.supply,
    ...
    function recoverMATIC(
        ...
        require(sent, "Failed to recover MATIC");
    }
    ...
    function getFee(
        ...
        if (nftType == traitType.Generic) {
            price = sum * genericFee;
            return price;
        } else if (nftType == traitType.Epic) {
            price = sum * epicFee;
            return price;
        } else if (nftType == traitType.Legendary) {
            price = sum * legendaryFee;
            return price;
        } else {
            revert("invalid nftType");
        }
