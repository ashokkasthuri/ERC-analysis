...
// Chain: POLYGON - File: contracts/goonzOriginals/ERC1155MaxSupply.sol
// SPDX-License-Identifier: MIT
 ...
 * clearly identified.
 */
abstract contract ERC1155MaxSupply is ERC1155Burnable {
    struct Supply {
        uint80 totalSupply;
        uint80 maxSupply;
        uint80 numberBurned;
    ...
    function _setMaxSupply(
        ...
        require(supply > _supplyInfo[id].totalSupply, "ERC1155: Max supply cannot be lower than total supply");
...
// Chain: POLYGON - File: contracts/goonzOriginals/GoonzOriginals.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
import { ERC1155 } from "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
import { ERC1155MaxSupply } from "./ERC1155MaxSupply.sol";
import { SafeMath } from "@openzeppelin/contracts/utils/math/SafeMath.sol";
import { Counters } from "@openzeppelin/contracts/utils/Counters.sol";
import { Strings } from "@openzeppelin/contracts/utils/Strings.sol";
import { AccessControl } from "@openzeppelin/contracts/access/AccessControl.sol";

contract GoonzOriginals is ERC1155MaxSupply, IERC2981, Ownable, AccessControl {
    ...
    ) ERC1155MaxSupply(_name, _symbol, _uri) {
    ...
    function mint(address[] memory _recipients, uint256 _id) external onlyRole(MINTER_ROLE) {
        for (uint256 i = 0; i < _recipients.length; i++) {
            _mint(_recipients[i], _id, 1, "");
        }
    }
    ...
    function retire(uint256 id) public onlyRole(MINTER_ROLE) {
        super._retire(id);
    }
    ...
    function setRoyaltyAddress(address _royaltyAddress) external onlyRole(MINTER_ROLE) {
        royaltyAddress = _royaltyAddress;
    }

    function setRoyaltyBasisPoints(uint256 _royaltyBasisPoints) external onlyRole(MINTER_ROLE) {
