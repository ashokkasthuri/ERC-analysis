...
// Chain: POLYGON - File: contracts/DragonNft.sol
    ...
    ERC1155Supply
{
     ...
     * @param uri - uri to use for metadata. for example: "https://blue-dragon.io/api/web3/metadata/collection-name/{id}.json"
     */
    ...
    function mint(
        ...
        uint256 amount,
        bytes memory data
    ) public override(IDragonNft) onlyRole(MINTER_ROLE) {
        require(totalSupply(id) == 0, "cannot mint more of this token");
        require(amount == 1, "cannot mint more than one at a time");
    ...
    function mintBatch(
        ...
        require(
            ...
            "ids and amounts are not of the same length"
        );
        for (uint i = 0; i < ids.length; i++) {
            if (totalSupply(ids[i]) > 0) {
                revert(
                    string.concat(
                        "cannot mint more of token id ",
                ...
                revert(
                    string.concat(
                        "cannot mint more than 1 of token id ",
                        Strings.toString(ids[i])
                    )
                );
            }
