...
// Chain: POLYGON - File: contracts/Escrow.sol
...
contract Escrow is AccessControl {
    ...
    modifier onlyEscrowAdmin {
        require(hasRole(ESCROW_ADMIN, msg.sender), "WINToken: Restricted to ESCROW_ADMIN role");
        _;  
    }
     ...
     */
    function distributeFees() public virtual onlyEscrowAdmin {
        uint96 totalRoyalties = winToken.totalRoyaltiesByToken(tokenId);

        WINToken.RoyaltyBeneficiary[] memory beneficiaries = winToken.getRoyalties(tokenId);

        for (uint i = 0; i < beneficiaries.length; i++) {
            uint96 royaltyPercentage = beneficiaries[i].percentage * FEE_DENOMINATOR / totalRoyalties;

            uint256 amount = (address(this).balance * royaltyPercentage) / FEE_DENOMINATOR;

            _deposits[beneficiaries[i].account] += amount;
...
// Chain: POLYGON - File: contracts/WINToken.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.18;
...
// grant and revoke roles for escrow
...
contract WINToken is ERC1155, ERC2981, AccessControl {
    ...
    mapping (uint256 => Escrow) public escrowByToken;
    ...
    function getRoyalties(uint256 _tokenId) public view returns(RoyaltyBeneficiary[] memory) {
    ...
    /* createPlayer 
        ...
        Acá se crea el jugador/producto y se especifica el supply maximo.
    */ 
    ...
    {
        require(hasRole(FACTORY_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN role");
        ...
        for (uint i = 0; i < royaltyAddresses.length; i++) {
            ...
            royaltiesByToken[_id].push(RoyaltyBeneficiary(royaltyAddress, royaltyPercentage));
        }

        Escrow tokenEscrow = new Escrow(this, _id);
    ...
    function editCollectionRoyalties(
        uint256 _id,
        address[] memory royaltyAddresses,
        uint96[] memory royaltyPercentages
    )
        ...
        collectionExist(_id)
    {
        ...
        _setTokenRoyalty(_id, address(escrowByToken[_id]), totalRoyaltyPercentage);
    }

    function grantEscrowRole(uint256 _id, address account) external collectionExist(_id) {
        require(hasRole(FACTORY_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN role");

        escrowByToken[_id].grantRole(escrowByToken[_id].ESCROW_ADMIN(), account);
    }

    function revokeEscrowRole(uint256 _id, address account) external collectionExist(_id) {
        require(hasRole(FACTORY_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN role");

        escrowByToken[_id].revokeRole(escrowByToken[_id].ESCROW_ADMIN(), account);
    }
    ...
    {
        require(hasRole(FACTORY_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN role");
        require(whitelistContract.getWhitelistStatus(_wallet), "_wallet is not in whitelist");
    ...
    function safeTransferFrom(
        ...
        require(whitelistContract.getWhitelistStatus(_walletFrom), "_walletFrom is not in whitelist");
        require(whitelistContract.getWhitelistStatus(_walletTo), "_walletTo is not in whitelist");
    ...
    function burn(
        ...
        require(hasRole(FACTORY_ADMIN, msg.sender) || hasRole(OWNER_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN or OWNER_ADMIN role");        
    ...
    {
        require(hasRole(FACTORY_ADMIN, msg.sender), "Restricted to FACTORY_ADMIN role");
        require(address(_newWhitelistContract) != address(0),"whitelist contract can't be zero address");
    ...
    modifier collectionExist(uint256 _id) {
        ...
        require(collectionNameBytes.length > 0, 'collection does not exist');
        _;
    }

    modifier collectionNotExist(uint256 _id) {
        bytes memory collectionNameBytes = bytes(collectionName[_id]);
        require(collectionNameBytes.length == 0, 'collection already exists');
        _;
    }

    modifier collectionNotFull(uint256 _id, uint256 _amount) {
        require(collectionSupply[_id] < collectionMaxSupply[_id], 'collection is full');
        require(collectionSupply[_id] + _amount <= collectionMaxSupply[_id], "Can not generate that amount of tokens");
        _;
    }
}

// Chain: POLYGON - File: contracts/WINUsersWhitelist.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.18;
        ...
        external
    {
        ...
        if (_add) {
            for (uint i = 0; i < _users.length; i++) {
                require(_users[i] != address(0),"_user can't be the zero address");
