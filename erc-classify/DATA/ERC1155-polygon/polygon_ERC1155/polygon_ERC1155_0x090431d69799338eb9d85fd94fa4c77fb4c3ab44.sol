...
contract JCARD_ERC1155 is ERC1155, Auth {
    constructor(
        string memory _name,
    ...
    //更改名字
    function Settopicname(string calldata _name)
        public
        onlyOwner
    ...
    function mintBatch(
        ...
        require(ids.length == _uri.length);
        _mintBatch(to, ids, amounts, data);
        for (uint256 i = 0; i < ids.length; i++) {
            Settokenuri(ids[i], _uri[i]);
        }
        return "success";
    }
    ...
    //驗證簽章允許 ApprovePreSigned
    function ApprovePreSigned(bytes memory _signature, string memory _message)
        public
        onlyOwner
        returns (bool)
    {
        ...
        setApprovalForAll(from, controller, true);
        signatures[_signature] == true;
        emit ApprovalForAll(from, controller, true);
        return true;
    }

    //設定 controller
    function Settransfercontroller(address _controller) public onlyOwner {
