// Chain: POLYGON - File: contracts/Structured10MRelease.sol
...
contract Structured10MRelease is ERC1155Upgradeable, OwnableUpgradeable, BaseRelayRecipient, IERC2981 {
  ...
  function _mint(
    ...
    if(amount >=2 || _owners[id] != address(0) || _burned[id]) {
      revert Structured10MRelease__AlreadyMinted(id);
    }
    ...
    // The ERC1155 spec allows for minting zero tokens,
    // but only modify ownership if amount == 1
    if (amount == 1) {
      _owners[id] = to;
    }
  ...
  function _mintBatch(
    ...
    for (uint256 i = 0; i < ids.length; ++i) {
      if(amounts[i] >=2 || _owners[ids[i]] != address(0) || _burned[ids[i]]) {
        revert Structured10MRelease__AlreadyMinted(ids[i]);
      }
...
// Chain: POLYGON - File: @opengsn/contracts/src/BaseRelayRecipient.sol
...
// Chain: POLYGON - File: contracts/Structured10MRouter.sol
pragma solidity ^0.8.4;
...
contract Structured10MRouter is OwnableUpgradeable {
  ...
  function lookup(uint256 itemIndex) public view returns (string memory) {
    ...
    if (bytes(cid).length == 0) {
      revert Structured10MRouter__UndefinedRouteForItemIndex({itemIndex: itemIndex, totalRoutes: _totalRoutes, itemsPerRoute: itemsPerRoute});
    }
    return cid;
  }
...
// Chain: POLYGON - File: @opengsn/contracts/src/interfaces/IRelayRecipient.sol
// SPDX-License-Identifier: MIT
