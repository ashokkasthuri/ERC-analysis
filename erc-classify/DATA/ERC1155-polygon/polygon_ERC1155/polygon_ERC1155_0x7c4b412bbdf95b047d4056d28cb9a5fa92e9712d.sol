...
// Chain: POLYGON - File: @openzeppelin/contracts/utils/ReentrancyGuard.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/AscionContextUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: POLYGON - File: contracts/AscionSpaceships.sol
// SPDX-License-Identifier: MIT
//            _____  _____ _____ ____  _   _
//     /\    / ____|/ ____|_   _/ __ \| \ | |
//    /  \  | (___ | |      | || |  | |  \| |
...
import "./UniqueCollection.sol"; // Import the base contract

contract AscionSpaceships is UniqueCollection {

    // Constructor to initialize Collection
    constructor(address _owner, address _forwarder)
    UniqueCollection("Ascion: Spaceships", "ASC_SPACESHIPS", "https://meta.ascion.space/spaceships/" , "https://meta.ascion.space/collections/spaceships.json", _owner, _forwarder) {
    }
}


// Chain: POLYGON - File: contracts/IUniqueCollection.sol
// SPDX-License-Identifier: MIT
//            _____  _____ _____ ____  _   _
//     /\    / ____|/ ____|_   _/ __ \| \ | |
//    /  \  | (___ | |      | || |  | |  \| |
...
interface IUniqueCollection is IERC1155  {
  ...
  function mintToAddress(address _toAddress, uint256 _itemId) external;
  function mintBatchToAddress(address _toAddress, uint256[] calldata _itemIds) external;
...
// Chain: POLYGON - File: contracts/Roles.sol
// SPDX-License-Identifier: MIT
//            _____  _____ _____ ____  _   _
//     /\    / ____|/ ____|_   _/ __ \| \ | |
//    /  \  | (___ | |      | || |  | |  \| |
...
// Chain: POLYGON - File: contracts/UniqueCollection.sol
// SPDX-License-Identifier: MIT
//            _____  _____ _____ ____  _   _
//     /\    / ____|/ ____|_   _/ __ \| \ | |
//    /  \  | (___ | |      | || |  | |  \| |
...
import "./IUniqueCollection.sol";
import "./AscionContextUpgradeable.sol";
import "./Roles.sol";

contract UniqueCollection is IUniqueCollection, ERC1155, AscionContextUpgradeable, Roles, AccessControl, Ownable, ReentrancyGuard {
  ...
  // Mapping to check if an item exists (itemId => bool)
  mapping(uint256 => bool) public itemExists;

  // Mapping to track the supply of each item (itemId => supply)
  mapping(uint256 => uint256) public itemSupplies;

  // Mapping to handle transfer timelocks for items (itemId => timestamp)
  mapping(uint256 => uint256) public itemTransferTimelocks;

  // Mapping to store URIs for each item (itemId => URI)
   ...
   * @param _baseUri Used as the URI for all token types by relying on ID substitution, e.g. https://meta.ascion.space/items/
   * @param _collectionMetaUri The URI for the contract metadata
   * @param _initialOwner The address of the initial owner
   * @param _forwarder The address of the trusted forwarder for meta-transactions
   */
  constructor(string memory _collectionName, string memory _collectionSymbol, string memory _baseUri, string memory _collectionMetaUri, address _initialOwner, address _forwarder)
  ERC1155(_baseUri)
  AscionContextUpgradeable(_forwarder)
  Ownable(_initialOwner)
  {
    _name = _collectionName;
    _symbol = _collectionSymbol;
    _contractURI = _collectionMetaUri;
    itemBaseURI = _baseUri;
  ...
  /**
   * @dev Sets the base URI for all items
   * @param _itemBaseURI The base URI to be set
   */
  function setItemBaseURI(string memory _itemBaseURI) external {
  ...
  /**
   * @dev Returns the URI for a specific item
   * @param _itemId The ID of the item
   * @return The URI for the item
   */
  function uri(uint256 _itemId) public view override returns (string memory) {
    ...
    return string(abi.encodePacked(itemBaseURI, _itemId.toString(), ".json"));
  }

  /**
   * @dev Sets the URI for a specific item
   * @param _itemId The ID of the item
  ...
  /**
   * @dev Checks if an item is transferable based on the transfer timelock
   * @param _itemId The ID of the item
   * @return True if the item is transferable, otherwise false
   */
  ...
  /**
   * @dev Sets a transfer timelock for a specific item
   * @param _itemId The ID of the item
   * @param _unlockTimestamp The timestamp when the item becomes transferable
   */
  ...
  /**
   *  @dev Mints an item to a specific address unless item already exists
   * @param _toAddress The address to mint the item to
   * @param _itemId The ID of the item to be minted
   */
  function mintToAddress(address _toAddress, uint256 _itemId) external canMint {
    require(!itemExists[_itemId], "Item already minted");
    _mint(_toAddress, _itemId, 1, "");
    setItemIdExists(_itemId);
  }

  /**
   * @dev Mints multiple items to a specific address. Each item can only be minted once.
   * @param _toAddress The address to mint the items to
   * @param _itemIds The IDs of the items to be minted
   */
  function mintBatchToAddress(address _toAddress, uint256[] calldata _itemIds) external nonReentrant canMint  {
    for (uint256 i = 0; i < _itemIds.length; i++) {
      require(!itemExists[_itemIds[i]], "Item already minted");
    }
    uint256[] memory quantities = new uint256[](_itemIds.length);
    for (uint256 i = 0; i < _itemIds.length; i++) {
      quantities[i] = 1;
      setItemIdExists(_itemIds[i]);
    }
    _mintBatch(_toAddress, _itemIds, quantities, "");
  }

  /**
   * @dev Burns an item from a specific address
   * @param _fromAddress The address to burn the item from
   * @param _itemId The ID of the item to be burned
   * @param _quantity The quantity of the item to be burned
   */
  function burnFromAddress(address _fromAddress, uint256 _itemId, uint256 _quantity) external nonReentrant canBurn(_fromAddress) {
    _burn(_fromAddress, _itemId, _quantity);
  }

  /**
   * @dev Burns multiple items from a specific address
   * @param _fromAddress The address to burn the items from
   * @param _itemIds The IDs of the items to be burned
   * @param _quantities The quantities of the items to be burned
   */
  function burnBatchFromAddress(address _fromAddress, uint256[] calldata _itemIds, uint256[] calldata _quantities) external nonReentrant canBurn(_fromAddress) {
    _burnBatch(_fromAddress, _itemIds, _quantities);
  }

  /**
   * @dev Returns the total number of item IDs
   * @return The total number of item IDs
   */
  function totalItemIds() external view returns (uint256) {
  ...
  /**
   * @dev Returns all item IDs
   * @return An array of all item IDs
   */
  function allItemIds() external view returns (uint256[] memory) {
    return itemIds;
  }
  ...
  /**
   * @dev Returns the supplies of all items
   * @return A 2D array where each element contains the item ID and its supply
   */
  function allItemSupplies() external view returns (uint256[][] memory) {
  ...
  /**
   * @dev Returns the URIs of all items
   * @return An array of all item URIs
   */
  function allItemURIs() external view returns (string[] memory) {
  ...
  /**
   * @dev Returns the balances of all items for a specific address
   * @param _address The address to check the balances for
   * @return A 2D array where each element contains the item ID and its balance
   */
  function balanceOfAll(address _address) external view returns (uint256[][] memory) {
  ...
  /**
   * @dev Paginates item IDs
   * @param _itemIdsStartIndexInclusive The start index for pagination
   * @param _limit The maximum number of items to return
   * @return An array of paginated item IDs
   */
  ...
  /**
   * @dev Paginates item URIs
   * @param _itemIdsStartIndexInclusive The start index for pagination
   * @param _limit The maximum number of items to return
   * @return An array of paginated item URIs
   */
  ...
  /**
   * @dev Upgrades the trusted forwarder address
  ...
  /**
   * @dev Returns the message sender, supporting meta-transactions
  ...
  /**
   * @dev Returns the message data, supporting meta-transactions
   ...
   * @param ids The IDs of the items being transferred
   * @param values The amounts of the items being transferred
   */
  function _update(
    address from,
    address to,
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      if (from == address(0)) { // Minting
        ...
        require(itemSupplies[id] >= value, "ERC1155: burn amount exceeds itemSupply");
        itemSupplies[id] = 0;
      }
    }
  }
  ...
  /**
   * @dev Transfers ownership and updates roles accordingly
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, IERC165, AccessControl) returns (bool) {
    return interfaceId == type(IUniqueCollection).interfaceId || super.supportsInterface(interfaceId);
  }

  /**
   * @dev Marks an item ID as existing
   * @param _id The ID of the item to mark as existing
   */
  function setItemIdExists(uint256 _id) internal virtual {
    if (!itemExists[_id]) {
  ...
  /**
   * @dev Modifier to check if the caller can mint items
   */
  modifier canMint {
  ...
  /**
   * @dev Modifier to check if the caller can burn items
   * @param _fromAddress The address to burn the items from
   */
