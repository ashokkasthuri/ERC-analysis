...
// File contracts/SozUkrajina.sol
...
contract SozUkrajina is ERC1155, Ownable {
  using Strings for uint256;
    
  string public name;
  string public symbol;

  mapping(uint => uint) private tokenSupply; // Supply per token
  ...
  /**
   * @dev Return URI for tokenId.
   */
  function uri(uint _id) public override view returns (string memory) {
    require(tokenSupply[_id] > 0, 'URI query for nonexistent token.');
