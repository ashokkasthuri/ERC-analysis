...
// Chain: POLYGON - File: contracts/lottery/LyncLotteryNFT.sol
...
contract LyncLotteryNFT is ERC1155, ERC1155Supply, Ownable {
    using Counters for Counters.Counter;
    using ECDSA for bytes32;

    Counters.Counter private _tokenIdCounter;
    string private constURI;
    string public name;
    ...
    constructor(string memory _constURI, address _authorizedSigner) ERC1155(_constURI) {
        name = "LYNC Lottery NFT";
        symbol = "LyncLotteryNFT";
    ...
    function safeMint(address to, bytes calldata signature, uint256 _nonce) public {
        bytes32 hash = getHash(_nonce);
        require(!executed[hash], "Hash has already been used!");
        require(verifySignature(signature, _nonce), "Invalid signature of authorized signer!");
        executed[hash] = true;
    ...
    function verifySignature(bytes calldata _sig, uint256 _nonce) public view returns (bool isValid) {
        ...
        isValid = (authorizedSigner == ethSignedHash.recover(_sig));
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(from == address(0), "This is a Soulbound token. It cannot be transferred.");
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }
}