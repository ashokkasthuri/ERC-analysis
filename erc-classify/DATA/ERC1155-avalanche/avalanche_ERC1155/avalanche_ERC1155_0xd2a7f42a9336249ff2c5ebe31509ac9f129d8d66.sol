// Chain: AVALANCHE - File: contracts/Node.sol
...
contract NodeERC1155 is ERC1155Upgradeable, INodeERC1155, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function mintTo(address[] calldata to, uint256[] calldata _nodeType, uint256[] calldata mintDate, uint256[] calldata claimDate, string[] calldata _uri) public onlyOwner{
        for (uint256 i = 0; i < to.length; i++) {
            _mint(to[i], _nodeType[i], 1, "");
            
            uint256 _id = _getNextTokenID();
            _incrementTokenID();
            nodeState[_id].purchaser = to[i];
            nodeState[_id].nodeType = _nodeType[i];
            
            if (mintDate[i] == 0) {
                nodeState[_id].purchasedAt = block.timestamp;
            } else {
                nodeState[_id].purchasedAt = mintDate[i];
            }

            if (claimDate[i] == 0) {
                nodeState[_id].snowballAt = block.timestamp;
            } else {
                nodeState[_id].snowballAt = claimDate[i];
            }

            nodeState[_id].uri = _uri[i];

            if (bytes(_uri[i]).length > 0) {
                emit URI(_uri[i], _id);
            }

            ownedNodes[to[i]].push(_id);
            collection[_nodeType[i]].currentSupply ++;
        }
    }

    function deleteTo(address from, uint256 _nodeType, uint256 amount) public onlyOwner{
        _safeTransferFrom(from, 0x000000000000000000000000000000000000dEaD, _nodeType, amount, "0x0");

        uint256 count;
        for (uint256 i = 0; i < ownedNodes[from].length; i++) {
            if (nodeState[ownedNodes[from][i]].nodeType == _nodeType) {

                /* Claim Part */

                nodeState[ownedNodes[from][i]].purchaser = 0x000000000000000000000000000000000000dEaD;
    ...
    function safeBatchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public virtual Blacklist override {
        return;
    }
...
// Chain: AVALANCHE - File: @openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// Chain: AVALANCHE - File: contracts/interfaces/INodeERC1155.sol
...
// Chain: AVALANCHE - File: contracts/interfaces/IJoePair.sol
...
// Chain: AVALANCHE - File: contracts/interfaces/ICorkToken.sol
// SPDX-License-Identifier: MIT
...
// Chain: AVALANCHE - File: contracts/interfaces/ISwapCork.sol
// SPDX-License-Identifier: MIT LICENSE
