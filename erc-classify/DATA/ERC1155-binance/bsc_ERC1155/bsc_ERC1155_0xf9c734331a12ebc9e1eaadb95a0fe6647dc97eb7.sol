...
// File: contracts\MultipleOpensaleNft.sol

// Multiple Opensale token

pragma solidity ^0.8.0;



contract MultipleOpensaleNft is ERC1155, AccessControl {
    ...
    constructor() ERC1155("MultipleOpensaleNft") {
        factory = msg.sender;
    }
...
// File: contracts\MultipleOpensaleNftFixed.sol

// Multiple Opensale Fixed Price Marketplace contract
...
interface IMultipleOpensaleNft {
    function initialize(
        string memory _name,
        string memory _uri,
        address creator,
...
contract MultipleOpensaleNftFixed is Ownable, ERC1155Holder {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    uint256 public constant PERCENTS_DIVIDER = 100;
    uint256 public constant swapFee = 2;
    ...
    function createCollection(
        ...
        bytes memory bytecode = type(MultipleOpensaleNft).creationCode;
        ...
        IMultipleOpensaleNft(collection).initialize(
            _name,
            _uri,
            msg.sender,
            bPublic
    ...
    function multipleList(
        ...
        IMultipleOpensaleNft nft = IMultipleOpensaleNft(_collection);
    ...
    function multipleDelist(uint256 _id) external {
        ...
        IMultipleOpensaleNft(pairs[_id].collection).safeTransferFrom(
            address(this),
            pairs[_id].owner,
            pairs[_id].tokenId,
            pairs[_id].balance,
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external payable {
        ...
        uint256 createrAmount = tokenAmount.mul(item.creatorFee).div(
            PERCENTS_DIVIDER
        );
