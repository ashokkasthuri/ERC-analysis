// Chain: BSC - File: contracts/Marketplace.sol
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function buyItems(
        ...
        require(_amount <= listing.amount, "Exceed number of items");

        uint256 priceInOne = listing.price.div(listing.amount);
        uint256 finalPrice = priceInOne.mul(_amount);
        if (serviceFee > 0) {
            require(
                lfwToken.transferFrom(
                    msg.sender,
                    payable(treasury),
                    finalPrice -
                        (finalPrice * (100 - serviceFee)).div(100)
                ) == true,
                "Transfer failed"
            );
        }

        uint256 price = (finalPrice * (100 - serviceFee)).div(100);
        require(
