// Chain: BSC - File: contracts/KOTHNFT.sol
...
contract KOTHNFT is ERC1155, AccessControl {
    ...
    uint256 public constant king = 0;
    uint256 public constant inheritor = 1;
    uint256 public constant reclaimer = 2;

    constructor(address kothAddress, address _admin)
        public
        ERC1155("https://koth-nft.web.app/{id}.json")
    {
        koth = IERC20(kothAddress);
    ...
    function mintToken(uint256 tokenId) public {
        require(
            ...
            "Caller has already claimed this token"
        );
        require(1 <= tokenId && tokenId <= 23, "Invalid token ID");
        require(
            koth.balanceOf(msg.sender) >= thresholds[tokenId],
            "User is not holding enough KOTH"
        );
