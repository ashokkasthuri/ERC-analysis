...
abstract contract EquinoxUpgradeable is ERC1967Upgrade {
    ...
    function _initializer(bytes calldata data) internal virtual;
}

contract EquinoxMarketplaceUpgradeableProxy is TransparentUpgradeableProxy {
    constructor(
        address logic,
        address admin
...
contract EquinoxNftUpgradeableProxy is TransparentUpgradeableProxy {
    constructor(
        address logic,
        address admin
    ) TransparentUpgradeableProxy(logic, admin, "") {}
...
// File: contracts/equinox-nft.sol
...
contract EquinoxTokenMemoryLayout {
    ...
    bytes32 public constant TOKEN_PROPOSER_ROLE =
        keccak256("TOKEN_PROPOSER_ROLE");
    ...
    bytes32 public constant GIFTED_TOKEN_ADMIN =
        keccak256("GIFTED_TOKEN_ADMIN");
    mapping(uint256 => mapping(address => GraylistEntry)) transferGraylist;
}

abstract contract TransferBlacklistToken is ERC1155, EquinoxTokenMemoryLayout {
    event TransferBlacklisted(uint256 indexed tokenId, bool blacklisted);
    event TransferGraylisted(
    ...
    function transferAllowed(
        ...
        if (transferBlacklist[id]) return false;
        return graylistTransferAllowed(id, from, to);
    }

    function graylistTransferAllowed(
        uint256 id,
        address from,
        address to
    ) public view returns (bool) {
        if (!transferGraylist[id][from].graylisted) return true;
        for (
            uint i = 0;
            i < transferGraylist[id][from].allowTransferTo.length;
            i++
        ) {
    ...
    function _addTransferGraylistAllowedDestination(
        uint256 id,
        address owner,
        address allowTo
    ) internal {
...
abstract contract ClaimableToken is ERC1155, EquinoxTokenMemoryLayout {
    event Claimed(
    ...
    function claimToken(uint256 tokenId, uint256 amount) public virtual {
...
contract EquinoxTokens is
    EquinoxTokenMemoryLayout,
    ClaimableToken,
    TransferBlacklistToken,
    AccessControlEnumerable,
    EquinoxUpgradeable
{
    ...
    constructor()
        ERC1155(
            "https://ipfs.moralis.io:2053/ipfs/QmZzpJnFkZs4QqU73EpR2VG48ZoTpfUnsPyJCTNPDgQ4tV/metadata/{id}.json"
        )
    {
        initialized = true;
    }
    ...
    function allowTransferForUser(uint256 id, address from) public {
        require(
            hasRole(GIFTED_TOKEN_ADMIN, msg.sender) ||
                graylistTransferAllowed(id, from, msg.sender),
            "You cannot cancel the graylist"
        );
        _setTransferGraylist(id, from, false);
    }

    function graylistTransferForUser(
        uint256 id,
        address from
    ) public onlyRole(GIFTED_TOKEN_ADMIN) {
        _setTransferGraylist(id, from, true);
    }

    function addGraylistAllowedDestination(
        uint256 id,
        address owner,
        address allowTo
    ) public onlyRole(GIFTED_TOKEN_ADMIN) {
    ...
    function proposeToken(uint256 id) public onlyRole(TOKEN_PROPOSER_ROLE) {
        ...
        INotifyOwnerChange(didRegistry).changeOwner(
            address(this),
            id,
            _msgSender()
        );
    ...
    function cancelProposal(uint256 id) public {
        require(
            proposedTokens[id] != address(0) && !tokens[id].created,
            "Proposal not found"
        );
        require(
            hasRole(ADMIN_ROLE, _msgSender()) ||
                proposedTokens[id] == _msgSender(),
            "you cannot change this proposal"
        );
    ...
    function addToken(
        ...
        if (proposedTokens[id] != address(0)) proposedTokens[id] = address(0);
    ...
    function safeTransferFrom(
        ...
        uint256 _currentBalance = balanceOf(from, id);

        require(_currentBalance >= amount, "No enough Nfts to transfer");
        if ((_currentBalance - amount) == 0) {
            require(
                transferAllowed(id, from, to) ||
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            bytes32 MINTER_ROLE = keccak256(
                abi.encodePacked("MINTER_ROLE", ids[i])
            );
            require(
                transferAllowed(ids[i], from, to) ||
                    hasRole(MINTER_ROLE, _msgSender()),
                "Transfer is not allowed"
            );
        }
    ...
    function mintBatch(
        ...
        require(
            ...
            "ERC1155: destinations and amounts length mismatch"
        );
        require(tokens[id].created, "Token does not exist");
    ...
    function getSupplyAndBalanceOfBatch(
        uint256[] memory ids,
        address account
    )
        public
        view
        returns (uint256[] memory supplies, uint256[] memory balances)
    {
