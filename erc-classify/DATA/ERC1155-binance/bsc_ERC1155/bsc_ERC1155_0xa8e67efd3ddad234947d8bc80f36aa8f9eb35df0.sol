// Chain: BSC - File: contracts/ArtistLootBoxNft.sol
...
contract ArtistMoonBoxNft is Context, Ownable {
    ...
    event Bet(uint256 indexed bet, uint256 amount, address indexed account, uint256 moonboxID, uint256 nonce,address artist );
    ...
    function submitBet(uint256 moonboxID, uint256 price,address payable artist, uint256 bets,uint256 betLimit,uint8 v, bytes32 r, bytes32 s)  external payable {
        ...
        string memory hash = keccak256(abi.encode(moonboxID, price,betLimit)).toString();
        ...
        emit Bet(totalBets, bets, _msgSender(), moonboxID, 0,artist);
