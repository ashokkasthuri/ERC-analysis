...
// Chain: BSC - File: contracts/token/BabyERC1155.sol
// SPDX-License-Identifier: MIT
...
contract BabyERC1155 is ERC1155, Ownable {
    ...
    mapping(uint => uint) public babyValue;
    ...
    function mintBatch(address[] memory tos, uint256 id, uint256[] memory amounts, bytes memory data, uint _babyValue) external onlyOwner {
        require(tos.length == amounts.length, "illegal amounts length");
        for (uint i = 0; i < tos.length; i ++) {
            _mint(tos[i], id, amounts[i], data);
        }
        if (babyValue[id] == 0) {
            babyValue[id] = _babyValue;
        }
    }
}