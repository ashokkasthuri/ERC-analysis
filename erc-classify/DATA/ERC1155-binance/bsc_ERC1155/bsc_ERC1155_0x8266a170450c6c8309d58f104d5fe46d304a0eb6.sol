// Chain: BSC - File: /D/code/kaka-truffle/contracts/interface/Idodo.sol
...
// Chain: BSC - File: /D/code/kaka-truffle/contracts/interface/Ikaka1155.sol
// SPDX-License-Identifier: GPL-3.0
pragma solidity ^0.8.4;
...
// Chain: BSC - File: /D/code/kaka-truffle/contracts/interface/Ikaka721.sol
// SPDX-License-Identifier: GPL-3.0
pragma solidity ^0.8.4;
...
// Chain: BSC - File: /D/code/kaka-truffle/contracts/kaka/kaka_upgrade.sol
...
contract KAKACardUpgrade is Ownable, ERC721Holder, ERC1155Holder {
    ...
    IKTN public newKTN;
    IDODO public dodo;
    mapping(uint => uint) public mickeyMap;
    ...
    function setMickeyMap(uint oldCardId, uint tokenId) public onlyOwner {
        mickeyMap[oldCardId] = tokenId;
    }

    function setAddr(address dodo_, address oldKTN_, address oldKTA_, address newKTN_, address newKTA_) public onlyOwner{
        dodo = IDODO(dodo_);
        oldKTN = IKTN(oldKTN_);
        oldKTA = IKTA(oldKTA_);
        newKTN = IKTN(newKTN_);
    ...
    function swap721(uint[] calldata ids_) public onlyOpen returns (bool) {
        for (uint i = 0; i < ids_.length; ++i) {
            ...
            if (cardId > 1000000 && cardId < 1000100) {
                swapMickey(cardId, tokenId);
                continue;
            }
    ...
    function swapMickey(uint cardId_, uint tokenId_) internal {
        uint tokenIdNew = mickeyMap[cardId_];
        require(tokenIdNew != 0, "target token nonexist");
        uint cardIdNew = newKTA.cardIdMap(tokenIdNew);
        require(cardIdNew == 40001, "not this one");

        oldKTA.safeTransferFrom(_msgSender(), address(this), tokenId_);
        newKTA.safeTransferFrom(address(this), _msgSender(), tokenIdNew);
        emit Swap721(_msgSender(), tokenId_, tokenIdNew);
    }
