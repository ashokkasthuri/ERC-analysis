...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    function getCurrentPhase(uint256 _campaignId) public view returns(uint256) {
        CampaignInfo storage info = campaigns[_campaignId];
        if (block.timestamp >= info.phasesToTime[1] && block.timestamp <= info.phasesToTime[2]) {
            return 1;
        } else if (block.timestamp >= info.phasesToTime[3] && block.timestamp <= info.phasesToTime[4]) {
            return 2;
        } else {
            return 0;
        }
    ...
    function getCampaignPhaseTime(uint256 _campaignId) public view returns(uint256, uint256, uint256, uint256, uint256) {
        CampaignInfo storage info = campaigns[_campaignId];
        return (
            info.phasesToTime[1],
            info.phasesToTime[2],
