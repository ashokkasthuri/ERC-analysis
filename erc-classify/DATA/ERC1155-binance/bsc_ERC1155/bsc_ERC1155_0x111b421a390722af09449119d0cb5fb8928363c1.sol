// Chain: BSC - File: prod-contracts/FarmLand.sol
    ...
    UUPSUpgradeable
{
    ...
    function stackApe(
        ...
        require(
            apesInstance.isApeOnSale(apeNftId) == false,
            "Ape is on sale"
        );
        require(
        ...
        require(
            apesInstance.getTypeIdByNftId(apeNftId) ==
        ...
        public
    {
        ...
        uint256 rewards = currentFarmAccumulatorByApe(apeNftId);
        apeFarmingInitTimestamp[apeNftId] = block.timestamp;
        if (rewards > 0) {
        ...
        returns (uint256)
    {
        ...
        if (buildingOwnerByTypeBuildings[owner][genericType].length <= 0) {
            ...
            if (apeFarmingInitTimestamp[apeId] < changeRewardsDate) {
              uint256 apeOldDelta = changeRewardsDate -
                apeFarmingInitTimestamp[apeId];
              uint256 apeNewDelta = currentTimestamp -
                changeRewardsDate;
              totalRewards += (apeNewDelta * apeTypeBaseRewards[genericType]) +
                (apeOldDelta * apeTypeBaseRewards[genericType] * 2);
            } else {
...
interface IApes {
  ...
  function isApeOnSale (uint256 nftID) external view returns(bool);
}
...
// Chain: BSC - File: prod-contracts/IAPapyrus.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
interface IAPapyrus {
