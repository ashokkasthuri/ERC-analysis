// Chain: BSC - File: /D/code/kaka-truffle/contracts/kaka/troland_assets.sol
...
contract TrolandAssets is Ownable, ERC1155Holder {
    using SafeMath for uint;
    KTNContract public KTN;

    constructor (address KTN_) {
    ...
    modifier onlyBanker () {
        require(_msgSender() == owner() || _msgSender() == banker, "not banker's calling");
        _;
    }
    function setBanker(address newBanker_) public onlyOwner returns (bool) {
        banker = newBanker_;

        return true;
    }

    function setColdWallet(address newWallet_) public onlyOwner returns (bool) {
