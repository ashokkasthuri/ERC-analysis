// Chain: BSC - File: prod-contracts/ApesLand.sol
  ...
  UUPSUpgradeable
{
  ...
  */
  ...
  function adminUnStackApe(uint256 genericType, address owner, uint nftIdCheck) public onlyOwner {
    //require(addressToBuildingsType[owner][genericType].length == 0, "make claim instead");
    // IApes apesInstance = IApes(apeContract);
    //require(addressToApesType[owner][genericType].length > 0, "not apes of this type");
    uint256 nftId = addressToApesType[owner][genericType][addressToApesType[owner][genericType].length];
    require(nftId == nftIdCheck, "Ape NFT Id not match");
    addressToApesType[owner][genericType].pop();
    // require(apesAtAddress[nftId] == sender, "not the stacker"); => in case the previous owner transfer it outside game
    apesAtAddress[nftId] = address(0x000); 
    // apesInstance.transferFrom(address(this), sender, nftId); => in case the previous owner transfer it outside game
  }
  /*
...
// Chain: BSC - File: prod-contracts/IApeBuilding.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: prod-contracts/IApes.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
interface IApes {
...
// Chain: BSC - File: prod-contracts/IAGold.sol
