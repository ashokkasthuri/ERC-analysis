...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    mapping(uint256 => mapping(address => uint256)) public userVRs;  //campaignId => address => bought
...
//        require(_mysteryBoxIds.length == 4, 'Not Enough Mystery Box For Special Box');
    ...
    function buyMysteryBox(
        ...
        require(_quantity <= maxBuyBox, "Limited Quantity");
    ...
    function buySpecialBox(uint256 _campaignId, bytes calldata sig) external payable whenNotPaused {
        ...
        require(campaign.active == true, 'Inactive Campaign!');
        require(campaign.boughtSpecialBoxes[_msgSender()].add(1) <= maxBuySpecialBox, "Limited Buy");
    ...
    function getBoughtSpecialBoxes(address buyer, uint256 _campaignId) public view returns (uint256) {
        CampaignInfo storage campaign = campaigns[_campaignId];
        return campaign.boughtSpecialBoxes[buyer];
    }
