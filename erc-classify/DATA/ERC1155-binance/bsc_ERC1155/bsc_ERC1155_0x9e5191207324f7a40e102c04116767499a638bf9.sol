...
contract PMOMembership_V1_1 is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, ERC1155SupplyUpgradeable {
    ...
    function initialize(string memory _baseUri, string memory _name, string memory _symbol) initializer public {
        __ERC1155_init("");
        __AccessControl_init();

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(ADMIN_ROLE, msg.sender);
        _mint(msg.sender, 1, 500, "");   // PINK DAISY
        _mint(msg.sender, 2, 100, "");   // MINT DAISY
    ...
    function safeMultiTransferFrom(address from, address[] memory to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) 
        public
        onlyRole(ADMIN_ROLE)
    {
        uint len = to.length;
        require(len > 0, "array length zero");
        require(len == ids.length, "array length mismatch");
        require(len == amounts.length, "array length mismatch");

        for(uint i=0; i<len; i++) {
            _safeTransferFrom(from, to[i], ids[i], amounts[i], data);
        }
    }

    function safeMultiWithdrawFrom(address[] memory from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) 
        public
        onlyRole(ADMIN_ROLE)
    {
        uint len = from.length;
        require(len > 0, "array length zero");
        require(len == ids.length, "array length mismatch");
        require(len == amounts.length, "array length mismatch");

        for(uint i=0; i<len; i++) {
            _safeTransferFrom(from[i], to, ids[i], amounts[i], data);
        }
    }
...
contract PMOMembership_V2 is PMOMembership_V1_1, PausableUpgradeable {
    ...
    function initializeV2() reinitializer(2) public onlyRole(ADMIN_ROLE) {
        __Pausable_init();
    }
