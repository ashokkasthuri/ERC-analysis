// Chain: BSC - File: contracts/HexoNFT.sol
...
contract HexoNFT is ERC1155, Ownable {
    address internal hexoContract_;
    ...
    mapping(uint256 => uint8[7]) internal betDicesCounter_;

    modifier onlyHexo() {
        require(msg.sender == hexoContract_, "Only Hexo can mint");
        _;
    }

    constructor(string memory _uri, address _hexo) ERC1155(_uri) {
        // Only Hexo contract will be able to mint new tokens
        hexoContract_ = _hexo;
    }
     ...
     * @return  Bet: The bet of ticket
     */
    function getTicketBets(uint256 _ticketID)
        external
        view
        returns (Bet memory)
    {
        return ticketInfo_[_ticketID].bets;
    }
    ...
    {
        return userTickets_[_user][_roundID];
    }
    ...
    function getUserTicketsPagination(
        ...
        uint256 length = size;
        if (length > userTickets_[_user][_roundID].length - cursor) {
            length = userTickets_[_user][_roundID].length - cursor;
        }
        uint256[] memory values = new uint256[](length);
        for (uint256 i = 0; i < length; i++) {
            values[i] = userTickets_[_user][_roundID][cursor + i];
        }
        return (values, cursor + length);
    }

    function getBetDicesCounter(uint256 _roundID)
        external
        view
        returns (uint8[7] memory)
    {
        return betDicesCounter_[_roundID];
    }

    function setBetDicesCounter(uint256 _roundID, uint8[] calldata _betDices)
        external
        onlyHexo()
    {
        betDicesCounter_[_roundID][1] += _betDices[1];
        betDicesCounter_[_roundID][2] += _betDices[2];
        betDicesCounter_[_roundID][3] += _betDices[3];
        betDicesCounter_[_roundID][4] += _betDices[4];
        betDicesCounter_[_roundID][5] += _betDices[5];
        betDicesCounter_[_roundID][6] += _betDices[6];
    }

    function mintTicket(
        address _to,
        uint256 _roundID,
        uint8[] calldata _betDices,
        uint256[] calldata _amounts,
        uint8 _diceAmount
    ) external onlyHexo() {
        Bet[] memory bets = new Bet[](_amounts.length);
        for (uint16 i = 0; i < _amounts.length; i++) {
            uint16 start = i * _diceAmount;
            uint16 end = start + _diceAmount;
            bets[i] = Bet({
                betDices: _betDices[start:end],
                amount: _amounts[i]
            });
        }
        ...
        for (uint16 i = 0; i < bets.length; i++) {
            ...
            userTickets_[_to][_roundID].push(tokenIds[i]);
        }
    ...
    function claimTicket(uint256 _ticketID, uint256 _roundID)
        external
        onlyHexo()
        returns (bool)
    {
        ...
        require(
            ticketInfo_[_ticketID].hexoRoundID == _roundID,
