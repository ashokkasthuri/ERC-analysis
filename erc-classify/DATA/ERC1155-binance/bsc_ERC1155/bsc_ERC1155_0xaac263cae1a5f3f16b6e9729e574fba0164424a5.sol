...
// Chain: BSC - File: contracts/RaffleMaster.sol
// SPDX-License-Identifier: MIT
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // map level -> raffle -> address -> entries
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) public entriesByLevelAddress;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public totalReferralPoints;
    mapping(address => bool) public isReferrer;
    address[] public referrerArray;
    ...
    bool public paused;
    mapping(address => mapping(uint256 => mapping(uint256 => uint256))) public totalNftsBoughtByCurrentRaffleLevel;
    ...
    function initialize(
        ...
        numWords = 30; // testing with 3 prod = 60 generate 60 random numbers

        totalSupply = 250; // TEST , PROD = 500
    ...
    function deposit(
        ...
        require(level == 4 || upline != address(0), "Upline not set");
        require(team.length == teamCuts.length, "Team cuts not set");

        // set Level variable
        Level memory _level = levels[level];

        require(totalNftsBoughtByCurrentRaffleLevel[msg.sender][level][_level.currentRaffle] + mintAmount <= 25, "Max 25 per raffle");
        ...
        totalNftsBoughtByCurrentRaffleLevel[msg.sender][level][_level.currentRaffle] += mintAmount;
        entriesByAddress[msg.sender] += mintAmount;
        entriesByLevelAddress[level][_level.currentRaffle][msg.sender] += mintAmount;
        ...
        if (entriesByLevelAddress[level][_level.currentRaffle][msg.sender] <= 0) {
    ...
    function drawRaffle(uint256 level) public {
        ...
        uint256 requestId = s_requestLevels[level];

        if (requestId > 0) {
            require(
                s_requests[requestId].fulfilled,
    ...
    function payoutMarketingAndReferrals(uint256 level) internal {
        ...
        require(
            totalSupply > 0,
            "Invalid totalSupply: totalSupply should be greater than 0"
        );
        require(
            levels[level].buyIn > 0,
            "Invalid level buyIn: buyIn should be greater than 0"
        );

        if (totalReferralPoints[level][levels[level].currentRaffle] > 0) {
            // payout referrals 4% of collection
            ...
            for (uint256 i = 0; i < referrerArray.length; i++) {
                ...
                uint256 referrerPoints = referralPoints[level][
                    levels[level].currentRaffle
                ][referrer];
                uint256 referrerCut = (referralEarnings * referrerPoints) /
                    totalReferralPoints[level][levels[level].currentRaffle];
                totalUSDCEarnings[referrer] += referrerCut;
                totalReferralEarnings[referrer] += referrerCut;
                totalReferralEarningsByLevel[referrer][level] += referrerCut;
                totalUSDCEarningsByLevel[referrer][level] += referrerCut;
            }
        }

        // includes marketing and team and drip vault
        uint256 teamEarnings = ((levels[level].buyIn * totalSupply) * 12) / 100; //(12%)

        for (uint256 i = 0; i < team.length; i++) {
            totalUSDCEarnings[team[i]] += (teamEarnings * teamCuts[i]) / 100;
            totalUSDCEarningsByLevel[team[i]][level] +=
                (teamEarnings * teamCuts[i]) /
                100;
        }
    }
    ...
    function payoutWinners(uint256 level) public {
        ...
        require(!s_requests[requestId].paid, "Previous drawing already paid");
        ...
        for (uint256 i = 0; i < _randomWords.length; i++) {
            uint256 winnerIndex = (_randomWords[i] % totalSupply);
            //select winner using random number and rafle entry
            address winner = raffleEntries[level][_level.currentRaffle][
                winnerIndex
            ];
        ...
        // call swapToDrip with 84% of totalCollected
        if (level < 4) {
            uint256 dripBought = swapToDrip((totalCollected * 84) / 100);
            // send 5% to drip vault
            IERC20(dripToken).transfer(
            ...
            uint256 dripBought = swapToDrip((totalCollected * 4) / 100);
            // send 5% to drip vault
            IERC20(dripToken).transfer(
                0xBFF8a1F9B5165B787a00659216D7313354D25472,
                dripBought
        ...
        s_requests[requestId].paid = true;
    ...
    function airdropUsdcWinnings(
        ...
        for (uint256 i = 0; i < totalWinners; i++) {
            ...
            if (i < bottom) {
                totalUSDCEarnings[winner] += firstShare / bottom;
                totalUSDCEarningsByLevel[winner][_level] += firstShare / bottom;
            } else {
                totalUSDCEarnings[winner] += shares[(i - (bottom))];
                totalUSDCEarningsByLevel[winner][_level] += shares[
                    (i - (bottom))
                ];
            }
        }
    ...
    function airdropWinnings(
        ...
        uint256 bottom = 25;
        // Calculate percentage allocations
        uint256 firstShare = (_drip * 2) / 170;
        uint256[] memory shares = new uint256[](5);
        shares[0] = (_drip * 1) / 17;
        shares[1] = (_drip * 2) / 17;
        shares[2] = (_drip * 3) / 17;
        ...
        for (uint256 i = 0; i < totalWinners; i++) {
            address winner = raffleWinners[_level][_raffleId][i];
            if (i < bottom) {
                faucet.airdrop(winner, firstShare / bottom);
                totalTokenEarnings[winner] += firstShare / bottom;
                totalTokenEarningsByLevel[winner][_level] +=
                    firstShare /
                    bottom;
            } else {
                // top winners
                faucet.airdrop(winner, shares[(i - (bottom))]);
                totalTokenEarnings[winner] += shares[(i - (bottom))];
                totalTokenEarningsByLevel[winner][_level] += shares[
                    (i - (bottom ))
                ];
            }
        }
    ...
    function recycleEntries(uint256 _level) internal {
        Level memory level = levels[_level];

        // Recycle entries
        for (uint256 i = 0; i < entrantsByLevelArray[_level].length; i++) {
            // get previous raffle entry count
            uint256 entriesForLevel = entriesByLevelAddress[_level][level.currentRaffle - 1][
                entrantsByLevelArray[_level][i]
            ];

            if (entriesForLevel >= 10) {
                // Reduce entries by a factor of 10, rounding down any decimals
                entriesByLevelAddress[_level][level.currentRaffle][entrantsByLevelArray[_level][i]] =
                    entriesForLevel /
                    10;
            } else {
                // Set entries to 0 if less than 10
                entriesByLevelAddress[_level][level.currentRaffle][
                    entrantsByLevelArray[_level][i]
                ] = 0;
            }
            ...
            for (
                ...
                raffleEntries[_level][level.currentRaffle].push(
                    entrantsByLevelArray[_level][i]
                );
            }
        }
    ...
    function getWinners(uint256 level, uint256 raffleId) public view returns (address[] memory) {
        // loop raffleWinners by level and raffleId and return all winners
        return raffleWinners[level][raffleId];
    }
    ...
    // function setLevelRequestId(
    //     uint256 _level,
    //     uint256 _requestId
    // ) public onlyOwner {
    //     s_requestLevels[_level] = _requestId;
    // }

    // function setFufilled(uint256 _requestId) public onlyOwner {
    //     s_requests[_requestId].fulfilled = true;
    // }
