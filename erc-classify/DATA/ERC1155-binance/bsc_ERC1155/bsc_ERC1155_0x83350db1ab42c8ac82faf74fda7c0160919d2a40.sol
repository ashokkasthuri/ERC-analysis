// Chain: BSC - File: contracts/NFTLootbox.sol
...
contract MoonBoxNFT is Context, Ownable, ReentrancyGuard {
    ...
    event Bet(uint256 indexed bet, uint256 amount, address indexed account, uint256 moonboxID, uint256 seed, uint256 poolAmount);
    event UpdateLootbox(uint256 indexed time, uint256 price);
    event ClaimReward(address nftAsset,uint256 indexed seed,uint256 nftAmount,address indexed account);
    event redeemNFT(address indexed user,uint256 indexed betid,uint256 time);

    address public constant moonshotToken=0xd27D3F7f329D93d897612E413F207A4dbe8bF799;
    uint256 public moonboxPrice;
    ...
    uint256 public totalNFTForSale = 1000;
    uint256 public maxPerBet = 1;
    ...
    uint256 public prizePoolPercentage=1000;

    function submitBet(uint256 moonboxID, uint256 seed, uint256 bets) external payable nonReentrant {
        require(moonboxPrice > 0, "Invalid Moonbox");
        require(bets > 0, "Must place bets");
        require(bets<=maxPerBet,"Invalid no of Bets");
        require(totalNFTForSale>=totalSold.add(bets),"All NFTs Sold");

        require(msg.value==(moonboxPrice.mul(bets)),"Invalid Amount");
        require(IERC20(moonshotToken).balanceOf(_msgSender())>=moonshootLimit[moonboxID],"Need more moonshot");
        totalBets = totalBets.add(1);
        totalSold = totalSold.add(bets);
        claimedBet[totalBets] = _msgSender();
        claimedBetAmount[totalBets] = bets;
        uint256 cost = moonboxPrice.mul(bets);
        uint256 fee = cost.sub(cost.mul(prizePoolPercentage).div(10000));
        feeAddress.transfer(fee);
        emit Bet(totalBets, bets, _msgSender(), moonboxID, seed, cost.mul(prizePoolPercentage).div(10000));
    }

    function redeemBulk( address nftAsset, uint256[] calldata id, uint256[] calldata nftAmount, uint256 bet, uint8 v, bytes32 r, bytes32 s) external {
        require(claimedBet[bet] == _msgSender(), "Invalid bet");
        string memory hash = keccak256(abi.encode(nftAsset, id, nftAmount, bet)).toString();
        ...
        emit redeemNFT(_msgSender(),bet,block.timestamp);
    }

    function claimReward( uint256 rewardAmount, address nftAsset, uint256 id, uint256 nftAmount,uint256 seed, uint8 v, bytes32 r, bytes32 s) external {
        string memory hash = keccak256(abi.encode(rewardAmount,nftAsset, id, nftAmount, seed,_msgSender())).toString();
        address signer = hash.recover(v, r, s);
        require(signer == authAddress, "Invalid signature");
        require(!claimedReward[seed],"Allready claimed");
        claimedReward[seed] = true;
        _msgSender().transfer(rewardAmount);
        emit ClaimReward(nftAsset,seed,nftAmount,_msgSender());
    }
    ...
    function updateLootbox(uint256 price) external onlyOwner {
        moonboxPrice = price;
        emit UpdateLootbox(block.timestamp, price);
    }
    ...
    function setTotalNFTForSale(uint256 _totalNFTForSale) external onlyOwner {
        totalNFTForSale = _totalNFTForSale;
    }

    function setMaxPerBet(uint256 _maxPerBet) external onlyOwner {
        maxPerBet = _maxPerBet;
    }
