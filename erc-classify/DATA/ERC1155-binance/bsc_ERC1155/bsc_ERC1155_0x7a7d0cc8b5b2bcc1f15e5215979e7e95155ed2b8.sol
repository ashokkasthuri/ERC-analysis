...
function mintBatch2(
    ...
    contract Base {
        ...
        Erc20Token   public PNDV    = Erc20Token(0x55d398326f99059fF775485246999027B3197955);
        Erc20Token   public XMB    = Erc20Token(0x55d398326f99059fF775485246999027B3197955);
        address public _owner;
        address USDTaddress = 0xdAc31eE6cae87B54A83057985963F49a64f08165; 
         modifier onlyOwner() {
...
contract BlindBox is Base{
    ...
    function synthesisMars(uint256 idType) public  {
        ...
        NFT.safeBatchTransferFrom( msg.sender,USDTaddress,IDarray,Quantityarray,"0x00");
        registry(msg.sender);
    ...
    function synthesisVenus() public    {
    ...
    if(block.timestamp.sub(_playerMap[id].Xtime)>oneday&&_playerMap[id].rewardXMB > 0){
        settleStaticXMB();
    }
        _playerMap[id].rewardXMB = _playerMap[id].rewardXMB.add(50000000000000000000000); 
       _playerMap[id].Xtime = block.timestamp;
    }
...
function settleStaticXMB() public      {
        uint256 id = _playerAddrMap[msg.sender];
        InvestInfo memory investList = _playerMap[id];
        uint256 staticaAmount = 0;
        uint256 lsrewardXMB = investList.rewardXMB;
        uint256 daynum = block.timestamp.sub(investList.Xtime);
        require(daynum > oneday, " time field" ); 
        require(investList.rewardXMB > 0, " Amount field" ); 
        for (uint256 i = 0; i < daynum.div(oneday); i++) {
            uint256 dayd = lsrewardXMB.mul(3).div(1000);
            staticaAmount = staticaAmount.add(dayd);    
            lsrewardXMB = lsrewardXMB.sub(dayd);
        }
        require(staticaAmount > 0, " this field" ); 
        XMB.transfer(address(msg.sender), staticaAmount);
       _playerMap[id].rewardXMB = lsrewardXMB;
       _playerMap[id].Xtime = block.timestamp;
    }
