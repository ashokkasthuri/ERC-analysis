...
// Chain: BSC - File: contracts/ExSportsAirDrop.sol
// SPDX-License-Identifier: MIT
...
contract ExSportsAirDrop is ERC1155, AccessControl {
    ...
    function _addUri(uint256 tokenId, string memory _baseUri) internal {
        baseUri[tokenId] = _baseUri;
    }

    //setting Unique Uri for each Token
    function updateUri(uint256 tokenId_, string memory baseUri_)
        external
        onlyRole(ADMIN_ROLE)
    {
        _addUri(tokenId_, baseUri_);
    }
    ...
    function mint(
        ...
        require(_totalSupply[id] == 0);
        _mint(account, id, amount, data);
        _addUri(id, baseUri_);
        _totalSupply[id] += amount;
        _addressBalance[account].add(id);
        _totalTokenIds.add(id);
    }
    ...
    */
    ...
    function allTokenIdInAddress(address owner)
        public
        view
        returns (uint256[] memory) {
    ...
    */
    ...
    function allTokenIds() public view returns (uint256[] memory) {
        uint256 balanceTokens = _totalTokenIds.length();
        uint256[] memory ids = new uint256[](balanceTokens);
        for (uint256 index = 0; index < balanceTokens; ++index) {
