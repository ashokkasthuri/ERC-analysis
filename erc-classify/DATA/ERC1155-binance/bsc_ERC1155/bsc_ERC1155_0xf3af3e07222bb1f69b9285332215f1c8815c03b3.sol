...
// Chain: BSC - File: contracts/Ticket.sol
//SPDX-License-Identifier: MIT
...
contract Ticket is ERC1155, Ownable {
    ...
    function indexTicket(
        ...
        uint256[sizeOfIndex_ + 1] memory indexes = generateNumberIndexKey(_maxValidRange, _numbers);
    ...
    function generateNumberIndexKey(uint16 _maxValidRange, uint16[] memory numbers)
        public
        pure
        returns (uint256[sizeOfIndex_ + 1] memory result)
    {
        uint16 power = numbers[numbers.length - 1];
        uint256 len = numbers.length - 1;

        for (uint256 i = 0; i < len; i++) {
            result[0] += numbers[i] * 10**(_maxValidRange - numbers[i] + 1);
        }

        uint256 counter = 1;
        for (uint256 i = 0; i < len; i++) {
            uint256 base = result[0] - numbers[i] * 10**(_maxValidRange - numbers[i] + 1);
            result[counter++] = base + 1;
        }

        result[sizeOfIndex_] = result[0] * 10 + power;
    }
    ...
    function countMatch(
        ...
        uint256[sizeOfIndex_ + 1] memory keys = generateNumberIndexKey(_maxValidRange, _winningNumbers);
        for (uint256 i = 0; i < keys.length - 1; i++) {
            uint256 missing = keys[i] % sizeOfLottery_; // (0, 1, 2, 3)
            results[missing + 1] += ticketHashes_[_lotteryId][keys[i]];
        }

        uint256 jackpotKey = keys[keys.length - 1];
        results[0] = ticketHashes_[_lotteryId][jackpotKey];

        // Ticket with 5 numbers matched is included when count tickets with 4 numbers matched
        // and so on. Therefore we remove the duplications
        // We hardcoded here to reduce the unnecessary computation
        results[1] = results[1] - results[0];
        results[2] = results[2] - results[0] * 4 - results[1] * 4;
        // results[3] = results[3] - results[0] * 6 - results[1] * 6 - results[2] * 3;
    }
}
