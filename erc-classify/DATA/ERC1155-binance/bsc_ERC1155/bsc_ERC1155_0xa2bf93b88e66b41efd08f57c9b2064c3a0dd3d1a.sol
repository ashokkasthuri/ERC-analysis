...
// Chain: BSC - File: contracts/ethregistrar/IBaseRegistrar.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: contracts/registry/PID.sol
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.4;
...
// Chain: BSC - File: contracts/reverseRegistrar/IReverseRegistrar.sol
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.4;
...
// Chain: BSC - File: contracts/reverseRegistrar/ReverseClaimer.sol
//SPDX-License-Identifier: MIT
pragma solidity >=0.8.17 <0.9.0;

import {PID} from "../registry/PID.sol";
...
contract ReverseClaimer {
    ...
    constructor(PID pid, address claimant) {
        IReverseRegistrar reverseRegistrar = IReverseRegistrar(
            pid.owner(ADDR_REVERSE_NODE)
        );
...
// Chain: BSC - File: contracts/utils/BytesUtils.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// Chain: BSC - File: contracts/utils/ERC20Recoverable.sol
//SPDX-License-Identifier: MIT
pragma solidity >=0.8.17 <0.9.0;
...
// Chain: BSC - File: contracts/wrapper/Controllable.sol
//SPDX-License-Identifier: MIT
...
// Chain: BSC - File: contracts/wrapper/ERC1155Fuse.sol
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
...
// Chain: BSC - File: contracts/wrapper/IMetadataService.sol
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
...
// Chain: BSC - File: contracts/wrapper/INameWrapper.sol
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
...
interface INameWrapper is IERC1155 {
    ...
    function pid() external view returns (PID);
...
// Chain: BSC - File: contracts/wrapper/INameWrapperUpgrade.sol
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
...
// Chain: BSC - File: contracts/wrapper/NameWrapper.sol
//SPDX-License-Identifier: MIT
pragma solidity ~0.8.17;
    ...
    ReverseClaimer
{
    ...
    bytes32 private constant PI_NODE =
        0x468c5118fb1f721217539457c3b71b9e541c76888ff07d5bdd31a07d6cd6fdce;
    bytes32 private constant PI_LABELHASH =
        0x419836c8408874ba655788993e485ecb986fa97e7bead3d63fa047b127e3c998;
    ...
    ) ReverseClaimer(_pid, msg.sender) {
        pid = _pid;
    ...
    function setUpgradeContract(
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            pid.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            pid.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(PI_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
        ...
        // transfer the pid record back to the new owner (this contract)
     ...
     * @param resolver The resolver address to set on the PID registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
    ...
    function renew(
        ...
        bytes32 node = _makeNode(PI_NODE, bytes32(tokenId));
    ...
    function wrap(
        ...
        if (parentNode == PI_NODE) {
            revert IncompatibleParent();
        }

        address owner = pid.owner(node);

        if (owner != msg.sender && !pid.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }

        if (resolver != address(0)) {
            pid.setResolver(node, resolver);
        }

        pid.setOwner(node, address(this));
    ...
    function unwrapETH2LD(
        ...
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(PI_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(PI_NODE, labelhash), controller);
    ...
    function setChildFuses(
        ...
        if (owner == address(0) || pid.owner(node) != address(this)) {
            revert NameIsNotWrapped();
        }
        // max expiry is set to the expiry of the parent
    ...
    function setSubnodeOwner(
        ...
        if (!_isWrapped(node)) {
            pid.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /**
     * @notice Sets records for the name in the PID Registry
    ...
    {
        pid.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
    ...
    function _wrapETH2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(PI_NODE, labelhash);
