...
// Chain: BSC - File: contracts/core/SafeOwnable.sol
// SPDX-License-Identifier: MIT
...
// Chain: BSC - File: contracts/token/PandoraERC1155.sol
// SPDX-License-Identifier: MIT
...
contract PandoraERC1155 is ERC1155, SafeOwnable {
    ...
    modifier onlyMinterOrCreator(uint256 id) {
        require(minters[msg.sender] || msg.sender == creators[id], "only minter or creator can do this");
        _;
    }
    ...
    function disableTokenTransfer(uint _id) external onlyMinterOrCreator(_id) {
        enableTransfer[_id] = false;
    }

    function enableTokenTransfer(uint _id) external onlyMinterOrCreator(_id) {
    ...
    function updateUri(uint256 _id, string memory _uri) external onlyMinterOrCreator(_id) {
    ...
    function createBatchDefault(uint256[] memory _types, uint256[] memory _values, bool _enableTransfer) external onlyMinter returns (uint256[] memory tokenIds) {
    ...
    function mint(address to, uint256 _id, uint256 _quantity, bytes memory _data) public onlyMinterOrCreator(_id) {
        uint256 tokenId = _id;
    ...
    function burn(address _account, uint256 _id, uint256 _amount) external onlyMinterOrCreator(_id) {
        _burn(_account, _id, _amount);
    }

    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) external {
        for (uint i = 0; i < ids.length; i ++) {
            require(minters[msg.sender] || msg.sender == creators[ids[i]], "only owner or creator can do this");
        }
