...
/** 
 *  SourceUnit: d:\GitWork\CourtlandWorking\TheMetaStry\contracts\contracts\TheMetaStry.sol
*/
...
contract TheMetaStry is ERC1155, Ownable {
	...
	uint256 public feeAdmin = 100000000;	
    ...
    event ItemCreated(uint256 id, address creator, string uri, uint256 supply);

    constructor (string memory _name,
        string memory _uri,
        bool bPublic,
        address _governanceToken, 
		address _adminAddress) ERC1155(_uri) {
    ...
    function setFeeToken(address _governanceToken) external onlyOwner {		
        governanceToken = IBEP20(_governanceToken);				
    }
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 supply) public returns (uint256){
        require( isPublic, "Collection is not published");

        uint256 token_balance = governanceToken.balanceOf(msg.sender);
		require(token_balance >= feeAdmin, "insufficient token balance");
        require(governanceToken.transferFrom(msg.sender, adminAddress, feeAdmin), "failed to transfer Admin fee");
        ...
        Items[currentID] = Item(currentID, msg.sender, _tokenURI, supply);
        emit ItemCreated(currentID, msg.sender, _tokenURI, supply);
        return currentID;
    }
