...
interface ISggcdNFT {
    ...
    error ProvenanceHashCannotBeSetTwice();

    event BatchMetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId);
    //    event RoyaltyInfoUpdated(uint256 tokenId, address receiver, uint256 bps);

    function SGGC_NFT() external view returns (ISggcNFT);

    function FEE_NUMBERATOR() external view returns (uint96);
    ...
    function mintSGGCD(uint256 _sggc_tokenId, address _to, uint256 _quantity) external;
...
contract SggcdNFT is ISggcdNFT, ERC1155Supply, ERC2981, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, address _sggc_nft_address) ERC1155('') {
        name = _name;
        symbol = _symbol;
        SGGC_NFT = ISggcNFT(_sggc_nft_address);
    }

    function _setTokenIds(uint256 _sggc_tokenId) private {
        for (uint256 i = 0; i < all_token_ids.length; i++) {
            if (all_token_ids[i] == _sggc_tokenId) {
                return;
            }
        }
        all_token_ids.push(_sggc_tokenId);
    }
    ...
    * @param _new_contractURI The new contract URI.
    ...
    function setBaseURI(string calldata _new_base_URI) external override onlyOwner {
        ...
        if (this.totalSupply() != 0) {
            emit BatchMetadataUpdate(1, this.totalSupply() - 1);
        }
    }
