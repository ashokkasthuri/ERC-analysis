...
// File: contracts/dopClaims/DopNFT.sol
...
/// @title DopNFT contract
/// @notice Implements the minting of Dop NFTs
/// @dev The dop nft contract allows you to mint dop nft
contract DopNFT is ERC1155, Ownable {
    using Strings for uint256;

    /// @notice Thrown when caller is other than dop claims contract
    error OnlyDopClaims();
    ...
    /// @notice The address of dop claims contract
    address public dopClaims;

    /// @notice Ensures that only claims contract can call the function
    modifier OnlyClaims() {
        if (dopClaims != msg.sender) {
            revert OnlyDopClaims();
        }
        _;
    }

    /// @dev Emitted when dop claims contract is updated
    event DopClaimsUpdated(address indexed prevAddress, address indexed newAddress);
    ...
    /// @notice Changes dop claims contract to a new address
    /// @param newDopClaims The address of the new claims contract
    function setDopClaims(address newDopClaims) external onlyOwner {
        if (newDopClaims == address(0)) {
            revert ZeroAddress();
        }
        address oldWallet = dopClaims;
        if (oldWallet == newDopClaims) {
            revert IdenticalValue();
        }

        emit DopClaimsUpdated({ prevAddress: oldWallet, newAddress: newDopClaims });
        dopClaims = newDopClaims;
    }
}
