    ...
    RevokableDefaultOperatorFiltererUpgradeable
{
    ...
    function contractPublicSaleTransfer(
        ...
        if (!isContractSaleActive) revert DNA__NotReadyYet();
    ...
    function setBatchASZTokenUsed(
        address[] memory _addressList,
        uint256[] memory _tokenIdList,
        bool[] memory _isUsedList
    ) public onlyOwner {
        if (
            _addressList.length != _tokenIdList.length ||
            _addressList.length != _isUsedList.length
        ) revert DNA__MismatchedArrayLengths();
        uint256 maxCount = _addressList.length;
        for (uint256 i = 0; i < maxCount; ) {
            setASZTokenUsed(_addressList[i], _tokenIdList[i], _isUsedList[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function setASZTokenUsed(
        address _contractAddress,
        uint256 _tokenId,
        bool _isUsed
    ) public onlyOwner {
        ASZTokenUsed[_contractAddress][_tokenId] = _isUsed;
    }
...
// File: contracts/IDNAErrorCodes.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;
