// File: contracts/ClaimContractV5.sol
    ...
    AccessControlUpgradeable
{
    ...
    function claimERC1155(
        ...
        uint256 _collection = getCollectionIndex(_collectionName);
        ...
        blocklistPerWallet[_collection][_msgSender()] = true;
        emit ClaimedERC1155(
            toAddress,
            claimableTokenIds,
            claimableQuantities,
    ...
    function claimPerVoxoERC1155(
        ...
        require(
            !hasPriorClaim(
                getCollectionIndex(_collectionName),
                qualifyingTokensIds
            ),
        ...
        if ((qualifyingTokensIds.length > 0) && (qualifyingTokensIds[0] != 0)) {
            require(
                voxoOwnership(
                    qualifyingTokensIds,
                    tokenHoldings(toAddress, VOXODEUS_CONTRACT_ADDRESS)
                ),
        ...
        bytes32 message = createClaimMessageERC1155(
            ...
            hashArray(claimableTokenIds),
            hashArray(claimableQuantities),
            hashArray(qualifyingTokensIds),
            tokenAddress,
            collection[getCollectionIndex(_collectionName)]
        );
        address signer = validateMessageSignature(message, signature);
    ...
    function claimERC721(
        ...
        uint256 collectionName = getCollectionIndex(_collectionName);
        require(tokenAddress.isContract(), "VoxoDeus: Invalid Token Address");
        require(
            block.timestamp < deadline,
            "VoxoDeus: Expired Claim Permission"
     ...
     * @param _collection Index of the collection the VoxoDeus tokenIds qualify the claimant for
    ...
    function hasPriorClaim(
        ...
        for (uint256 i = 0; i < _voxoTokenIds.length; i++) {
            if (blocklistPerVoxo[_collection][_voxoTokenIds[i]]) {
                return true;
            }
        }
        return false;
     ...
     */
    function hashArray(uint256[] memory _array)
        public
        pure
     ...
     */
    function getCollectionIndex(string memory _collectionName)
        internal
        view
        returns (uint256)
    {
...
// File: contracts/lib/draft-EIP712Upgradeable.sol
// SPDX-License-Identifier: MIT
