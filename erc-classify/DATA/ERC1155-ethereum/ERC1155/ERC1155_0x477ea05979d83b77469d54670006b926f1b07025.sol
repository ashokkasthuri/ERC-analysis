...
// File: contracts/access/AdminPool.sol
//SPDX-License-Identifier: MIT
...
// File: contracts/interfaces/IUzuSharedCollection.sol
//SPDX-License-Identifier: Unlicense
pragma solidity ^0.8.17;
...
// File: contracts/library/Domain.sol
//SPDX-License-Identifier: Unlicense
pragma solidity ^0.8.17;
...
// File: contracts/nft/UzuSharedCollection.sol
//SPDX-License-Identifier: Unlicense
pragma solidity ^0.8.17;
  ...
  UUPSUpgradeable
{
   ...
   */
  mapping(INftTransferProxy => bool) internal _trustedNftTransferProxy;
  ...
  function _checkMintDataAndVerifySignaturesForMint(
    ...
    require(
      !_processedMinting[mintData.mintId],
      "UzuSharedCollection: mintId is already used"
    );
    require(
  ...
  /**
   * @dev [Internal] Mint, set it URI and store the creator
   *
  ...
  /**
   * @dev Update tokenURI of an token, exposing `ERC1155URIStorageUpgradeable._setURI()`
   *
  ...
  function approveByNftCreatorAndTransfer(
    ...
    if (!isNftTransferableForNonCreator(nftId)) {
  ...
  /**
   * @dev See {IUzuSharedCollection.isNftTransferableForNonCreator}
   */
  function isNftTransferableForNonCreator(
    uint256 nftId
  ) public view virtual override returns (bool) {
    return _isNftTransferable[nftId];
  }
  ...
  function _beforeTokenTransfer(
    ...
    //     - Transfer for one of NFTs is not approved yet by the NFT Creator.
    //        (isNftTransferableForNonCreator(nftId) != true)
    ...
    if (!fromTrustedProxy) {
      for (uint256 idx = 0; idx < ids.length; idx++) {
        ...
        // Allow Case 1: When minting, this is needed to allow minting as _beforeTokenTransfer is called when minting as well
        // Allow Case 2: When creator want to transfer the NFT
        if (
          _creatorOf[nftId] == address(0) || _creatorOf[nftId] == _msgSender()
        ) {
          continue;
        }

        // require if Transfer for one of NFTs is approved by the NFT Creator.
        require(
          isNftTransferableForNonCreator(nftId),
