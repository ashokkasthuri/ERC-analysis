// File: contracts/sneakyPlayers.sol
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    //token minted status 
    // if minted returns true
    // if not minted returns false
    ...
    //minted status function returns mintedStatus
        function mintedStatus(uint id) public view returns(bool){
        require(id > 0 && id <= 300, "Token supply limit is 300");
        return minted[id];
    }
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0 ; i < ids.length; i++) {
            ...
            require (mintedStatus(ids[i]) == false, "Token id doesn't exist");  //Token id exists check
            require (ids[i] == mintedCount + i + 1, "Token id not in order");   //Token id order check
            require (amounts[i] == 1, "Nft can be minted only once");   //Token id quantity check, should not be more than 1
            _balances[ids[i]][to] += amounts[i];
            minted[i] = true;
        }

        // updating the totak Nfts minted count
        mintedCount += ids.length;  
