// File: contracts/tenktf/popcorn.sol
  ...
  ReentrancyGuardUpgradeable
{
  ...
  address internal registryContractAddress;
  bool public isTransfersLocked;
  ...
  view
  virtual
  override(ERC2981Upgradeable, AccessControlEnumerableUpgradeable, ERC1155Upgradeable)
  returns (bool)
  {
    return
  ...
  function setIsTransfersLocked(bool _isTransfersLocked)
  external
  onlyRole(DEFAULT_ADMIN_ROLE)
  {
    isTransfersLocked = _isTransfersLocked;
  }

  function setIsBurnEnabled(bool _isBurnEnabled)
  external
  onlyRole(DEFAULT_ADMIN_ROLE)
  {
  ...
  function getRegistryContractAddress()
  external
  view
  returns (address)
  {
  ...
  function setRegistryContractAddress(address contractAddress)
  external
  onlyRole(DEFAULT_ADMIN_ROLE)
  {
    registryContractAddress = contractAddress;
  }
  ...
  returns (bool)
  {
    IRegistry registry = IRegistry(registryContractAddress);
  ...
  override
  {
    if (_isValidAgainstRegistry(msg.sender) == false) revert IRegistry.NotAllowed();
    if (isTransfersLocked && to != address(0)) {
  ...
  onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    if(walletsLength != _ids.length || walletsLength != _amounts.length) {
      revert MismatchedArrayLength();
    }
    
    for (uint256 i = 0; i < walletsLength;) {
      if(_ids[i].length != _amounts[i].length) revert MismatchedArrayLength();
      _mintBatch(_wallets[i], _ids[i], _amounts[i], "");
  ...
  {  
    ...
    for (uint256 i = 0; i < walletsLength;) {
      if(_ids[i].length != _amounts[i].length) revert MismatchedArrayLength();
      _burnBatch(_wallets[i], _ids[i], _amounts[i]);
  ...
  nonReentrant
  {    
    if (!isBurnEnabled) revert NotAllowedToBurn();
  ...
  function setDefaultRoyalties(uint96 feeNumerator) 
  external 
  onlyRole(DEFAULT_ADMIN_ROLE) 
  {
