// File: contracts/deployed/FearWolfShow.sol
...
contract FearNft1155 is ERC1155, AccessControl, OpenSeaCompatible, NativeMetaTransaction, ContextMixin
{
    ...
    constructor(string memory name, string memory ticker, string memory domainName, uint64 _mintingChainId, uint256 _maxId) ERC1155(name, ticker, domainName) {
    ...
    */
    function initialMint(address account, uint256 id, uint256 amount, bytes calldata data) external onlyRole(MINTER_ROLE) {
        ...
        require(id <= maxId || maxId == 0, "Id overflow");
    ...
    */
    function setGlobalMintingLimit(uint256 limit) external onlyRole(ADMIN_ROLE) {
        ...
        for (uint i = 0; i <= maxId; i++) {
            require(mintingLimits[i] == 0, "Minting limit for this id is already set.");
...
{
    constructor() FearNft1155("FEAR Wolf Show", "FEARWOLFSHOW", "https://fearwolf.com", 1, 160) {
    }
}
