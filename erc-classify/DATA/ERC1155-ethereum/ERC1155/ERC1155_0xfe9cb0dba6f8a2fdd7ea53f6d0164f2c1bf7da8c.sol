// File: contracts/CommodityTokens.sol
...
// my own interface to get price data from another simple contract that can be updated when/if there are any changes to chainlink addresses in the future
    interface PriceOracles {
        ...
        function Oil_ETHprice() external view returns (uint256);
...
// ID: Gold 0, Silver 1, Oil 2, BTC 3

contract CommodityTokens is ERC1155, Ownable, ERC1155Supply {

    string public name = "Commodity Tokens"; // name displayed on OpenSea
    string internal constant baseURI = "https://ipfs.io/ipfs/bafybeigao2oaago33u7ka7kju6av62bsmdhnv7nlyfuhcxenuoy4kizfay/";
    IERC20 public EQT;
    PriceOracles public Commodities;
    error Wrong_id();
    error Wrong_amount();
    ...
    function setOracleAddress(PriceOracles _addr) external onlyOwner {Commodities = _addr;}
    function forwardStuckEQT () external {EQT.transfer(address(EQT), EQT.balanceOf(address(this)));}
    function mintTokensBurnEQT(uint256 id, uint256 amount_of_ERC1155) external payable {
    ...
    function mintEQTburnTokens(uint256 id, uint256 amount_of_ERC1155) external payable {
        uint256 EQTamount = calculateEQTamount(id, amount_of_ERC1155);
    ...
    function calculateEQTamount(uint256 id, uint256 amount_of_ERC1155) internal view returns(uint256) {
        ...
        uint256 EQTamount;
        if (id == 0) {EQTamount = 10 ** 21 * amount_of_ERC1155 * Commodities.Gold_ETHprice();}          // 1 token = 1/1000 of 1 ounce of gold
        if (id == 1) {EQTamount = 5 * 10 ** 22 * amount_of_ERC1155 * Commodities.Silver_ETHprice();}    // 1 token = 1/20 of 1 ounce of silver
        if (id == 2) {EQTamount = 10 ** 22 * amount_of_ERC1155 * Commodities.Oil_ETHprice();}           // 1 token = 1/100 of 1 barrel of WTI crude oil
        if (id == 3) {EQTamount = 10 ** 19 * amount_of_ERC1155 * Commodities.BTC_ETHprice();}           // 1 token = 1/100000 of 1 BTC
