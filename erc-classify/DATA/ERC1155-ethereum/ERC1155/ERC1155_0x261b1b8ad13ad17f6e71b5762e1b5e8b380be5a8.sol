    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    mapping(uint256 => uint256) public maxMintable;
    ...
    function mint(
        ...
        require(mintable[id], "This token is not mintable.");
        require(
            balanceOf(to, id) + amount <= maxMintable[id],
            "Exceeds max mintable tokens."
        );
        require(
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(mintable[ids[i]], "This token is not mintable.");
            require(
                balanceOf(to, ids[i]) + amounts[i] <= maxMintable[ids[i]],
                "Exceeds max mintable tokens."
            );
    ...
    function setMintable(uint256 id, bool _mintable) public onlyOwner {
        mintable[id] = _mintable;
    }

    function setMaxMintable(uint256 id, uint256 _maxMintable) public onlyOwner {
        maxMintable[id] = _maxMintable;
    }
    ...
    function setMintableBatch(
        uint256[] memory ids,
        bool[] memory _mintables
    ) public onlyOwner {
        require(ids.length == _mintables.length, "Mismatch arrays length.");
        for (uint i = 0; i < ids.length; i++) {
            mintable[ids[i]] = _mintables[i];
        }
    }

    function setMaxMintableBatch(
        uint256[] memory ids,
        uint256[] memory _maxMintables
    ) public onlyOwner {
        require(ids.length == _maxMintables.length, "Mismatch arrays length.");
        for (uint i = 0; i < ids.length; i++) {
            maxMintable[ids[i]] = _maxMintables[i];
        }
    }
