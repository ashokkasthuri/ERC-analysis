// File: src/core/shares/HashnoteVaultShare.sol
...
contract HashnoteVaultShare is ERC1155, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @dev descriptor for an erc1155 share
    ...
    constructor(address _registrar, address _whitelist, address _descriptor) {
    ...
    function getBalanceOf(address _owner, address _vault) external view returns (uint256) {
        return balanceOf[_owner][_vaultToTokenId(_vault)];
    }

    function transferFromWithVault(address _from, address _to, address _vault, uint256 _amount, bytes calldata _data) public {
        ERC1155.safeTransferFrom(_from, _to, _vaultToTokenId(_vault), _amount, _data);
    }

    function transferVaultOnly(address _from, address _to, uint256 _amount, bytes calldata _data) public {
        if (!whitelist.isVault(msg.sender)) revert Unauthorized();
    ...
    function transferRegistrarOnly(address _from, address _to, address _vault, uint256 _amount, bytes calldata _data) public {
        if (msg.sender != registrar) revert Unauthorized();

        uint256 tokenId = _vaultToTokenId(_vault);

        if (!isApprovedForAll[_from][registrar]) isApprovedForAll[_from][registrar] = true;
...
// File: src/interfaces/IVaultShare.sol
// SPDX-License-Identifier: BUSL-1.1
pragma solidity ^0.8.0;
...
// File: src/interfaces/IVaultShareDescriptor.sol
// SPDX-License-Identifier: BUSL-1.1
pragma solidity ^0.8.0;
...
// File: src/interfaces/IWhitelistManager.sol
// SPDX-License-Identifier: BUSL-1.1
pragma solidity ^0.8.0;
