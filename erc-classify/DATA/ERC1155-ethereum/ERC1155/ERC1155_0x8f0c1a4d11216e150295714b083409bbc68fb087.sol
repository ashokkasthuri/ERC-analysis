    ...
    RevokableDefaultOperatorFilterer
{
    ...
    string public constant name = "GHOSTYBOTZ";
    string public constant symbol = "GBOTZ";
    ...
    uint256 public PUBLIC_MINT_PRICE = 0.042 ether;
    bool public PUBLIC_MINT_OPEN = false;
    address public paymentRecipient =
        0x47c6FF76574aC28869aADDa7c276D0CfB0193fC7;

    address internal MASTER_SIGNER;
    mapping(bytes => bool) internal processedSigs;
    ...
    bytes32 constant MA_TYPEHASH =
        keccak256(
            "Master_Approval(address addy,uint256 qty,uint256 price,uint256 id,uint256 tstamp,uint256 validity,bool unique_sig)"
        );
    ...
    constructor() ERC1155G("overwritten URI") Ownable() {
        ...
        _setupRole(MINTER_ROLE, 0x47c6FF76574aC28869aADDa7c276D0CfB0193fC7);
        _setupRole(PAUSER_ROLE, 0x47c6FF76574aC28869aADDa7c276D0CfB0193fC7);
        _setDefaultRoyalty(0x47c6FF76574aC28869aADDa7c276D0CfB0193fC7, 700);
    ...
    function updateNonReselableTokens(
        uint256[] calldata tokenIds,
        bool[] calldata non_reselable
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            tokenIds.length == non_reselable.length,
            "Nb addys doesn't match nb bools."
        );
    ...
    function ghostyMint(
        ...
        require(!tokensLockedMinting[id], "This token is locked for minting.");
    ...
    function ghostyMintBatch(
        ...
        for (uint256 ii; ii < ids.length; ++ii) {
            require(
                !tokensLockedMinting[ids[ii]],
                "This token is locked for minting."
            );
        }
        _mintBatch(to, ids, amounts, data);
    }

    function publicMint(uint256 qty, uint256 id) public payable virtual {
        require(!tokensLockedMinting[id], "This token is locked for minting.");
    ...
    function signedMint(
        ...
        if (m_a.unique_sig) {
            require(!processedSigs[_master_sig], "Sig already used!");
            processedSigs[_master_sig] = true;
        }
        require(
            m_a.tstamp + m_a.validity > block.timestamp,
...
// File: contracts/ERC1155PausableG.sol
// SPDX-License-Identifier: MIT
 ...
 */
abstract contract ERC1155PausableG is ERC1155G, Pausable {
    /**
...
// File: contracts/ERC1155BurnableG.sol
// SPDX-License-Identifier: MIT
 ...
 * _Available since v3.1._
 */
abstract contract ERC1155BurnableG is ERC1155G {
 ...
 * _Available since v3.1._
 */
contract ERC1155G is Context, ERC165, ERC2981, IERC1155, IERC1155MetadataURI {
    ...
    function safeBatchTransferFrom(
        ...
        if (from != _msgSender()) {
            ...
            for (uint256 i; i < ids.length; ++i) {
                require(
                    !non_resalable_tokens[ids[i]],
                    "One or more tokens are non-reselable."
                );
            }
        }
    ...
    function _updateBlackListedApprovals(
        address addy,
        bool blacklisted
    ) internal virtual {
    ...
    function _updateNonReselableTokens(
        uint256 tokenId,
        bool non_reselable
    ) internal virtual {
