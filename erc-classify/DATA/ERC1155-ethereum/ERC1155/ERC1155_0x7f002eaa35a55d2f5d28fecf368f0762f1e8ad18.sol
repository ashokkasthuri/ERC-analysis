...
// File: contracts/ShibaDogeArmy.sol
...
contract ShibaDogeArmy_Base is
    ...
    PausableUpgradeable
{
    ...
    error BatchEFTLengthMismatch(
        uint256 recipientLength,
        uint256 idsLength,
    ...
    function freeMint(
        ...
        require(
            totalSupply(freeMintID) < freeMintMaxAmount,
            "Max mint amount already reached"
        );
        require(
            !freeMintClaimed[freeMintID][msg.sender],
    ...
    function mintNFT(
        ...
        require(
            DogeArmy.ownerOf(dogeParentID) == msg.sender,
            "Sender not Owner of Doge"
        );
        require(
            ShibaArmy.ownerOf(shibaParentID) == msg.sender,
            "Sender not Owner of Shiba"
        );
    ...
    function equipEFT(
        ...
        require(
            NFTEquipmentLockTime[nftId] < block.timestamp,
            "Equipment is locked"
        );
    ...
    function unequipEFT(
        ...
        require(
            nftTraitsArray[nftId][eftslot] == eftId,
            "Requested EFT is not equipped"
        );
    ...
    function lockNFTEquipmentForSale(
        ...
        require(
            ...
            "Must be set for a later time than now"
        ); // must be set for a later time than now
        require(
            lockTime > NFTEquipmentLockTime[tokenID],
            "Must be set for a later time than previously set"
        ); // must be set for a later time than previously set
    ...
    // crate mappings deprecated, do not use, do not delete

    mapping(uint256 => bool) crateInitialized;
    mapping(uint256 => uint256[]) crateItems;
    mapping(uint256 => uint256[]) crateItemSupplies;
    mapping(uint256 => uint256) sealedCrateCount;
    mapping(uint256 => bool) cratePaused;

    // crate id => user address => depositedBlockNumber;
    mapping(uint256 => mapping(address => uint256)) crateAddressDepositBlock;

    modifier onlyManager(address addy) {
        require(addy != address(0), "Manager not set");
        require(msg.sender == addy || msg.sender == owner(), "Caller not the manager/owner");
        _;
    }
...
contract ShibaDogeArmy_V1_B is
    ShibaDogeArmy_Base,
    ERC2981Upgradeable,
    DefaultOperatorFiltererUpgradeable
{
    address public ArmoryManager;
    address public WarzoneManager;

    mapping(uint256 => bool) public nftLocked;
    
    event UnlockedToken(uint256 id);
    event LockedToken(uint256 id);

    error TokenIsLockedError(uint256 tokenId);
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 index = 0; index < ids.length; index++) {
            if (nftLocked[ids[index]]) {
                revert TokenIsLockedError(ids[index]);
            }
        }
    }

    function updateWarzoneManager(
        address newManagerAddress
    ) external onlyOwner {
        WarzoneManager = newManagerAddress;
    }

    function lockNft(
        uint256 id
    ) external onlyManager(WarzoneManager) {
        require(id <= 10000, "Only NFTs can be locked");
        require(nftLocked[id] == false);
        nftLocked[id] = true;
    ...
    function unlockNft(
        ...
        require(id <= 10000, "Only NFTs can be locked");
        require(nftLocked[id] == true);
        nftLocked[id] = false;

        emit UnlockedToken(id);
    }
    ...
    function equipEFT(
        ...
        if (nftLocked[nftId]) {
            revert TokenIsLockedError(nftId);
        }
        super.equipEFT(nftId, eftId, data);
    }
    ...
    function unequipEFT(
        ...
        super.unequipEFT(nftId, eftId, data);
    }

        // SUPPLY CRATE CODE

    address public CrateOpener;

    function setCrateOpener(address addy) external onlyOwner {
        CrateOpener = addy;
    }

    function depositLockedCrateFromManager(address holder, uint256 crateId) external onlyManager(CrateOpener) {
        _burn(holder, crateId, 1);
    }
    ...
    function crateContentMint(
        ...
        address receiver,
        bytes calldata data
    ) external onlyManager(CrateOpener) {
        _mintEFT(receiver, eftID, amount, data);
    }
}
