...
// File: contracts/access/IOperatableUpgradeable.sol
...
// File: contracts/access/OperatableUpgradeable.sol
// SPDX-License-Identifier: GPL-3.0-or-later
...
import {IOperatableUpgradeable} from "./IOperatableUpgradeable.sol";
...
abstract contract OperatableUpgradeable is Initializable, OwnableUpgradeable, IOperatableUpgradeable {
    ...
    function __Operatable_init(address initialOwner) internal onlyInitializing {
        ...
        __Operatable_init_unchained(initialOwner);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __Operatable_init_unchained(address initialOwner) internal onlyInitializing {
    ...
    function setOperator(address operator, bool isOperator) external onlyOwner {
        _setOperator(operator, isOperator);
    }

    function _setOperator(address operator, bool isOperator) internal {
...
// File: contracts/rewards/ggQuest/LoreCards.sol
// SPDX-License-Identifier: GPL-3.0-or-later
...
contract LoreCards is Initializable, ERC1155URIStorageUpgradeable, OperatableUpgradeable {
    error InvalidArrayLength(uint256 recepientsLength, uint256 idsLength, uint256 amountsLength);
    ...
    function mintBatchTo(address to, uint256[] calldata ids, uint256[] calldata amounts) public onlyOperator {
    ...
    function mintBatch(
        ...
        if (recipients.length != ids.length || ids.length != amounts.length)
            revert InvalidArrayLength(recipients.length, ids.length, amounts.length);
    ...
    function setBaseURI(string memory newUri) public onlyOperator {
        _setBaseURI(newUri);
        _setURI(newUri);
    }

    function setURI(uint256 id, string memory newUri) public onlyOperator {
        _setURI(id, newUri);
    }

    // NOTE: Teporary solution to burn cards for testnet
    function burn(address account, uint256 id, uint256 amount) public onlyOperator {
