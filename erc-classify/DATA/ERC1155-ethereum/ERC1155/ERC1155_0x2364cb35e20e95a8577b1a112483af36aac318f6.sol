    ...
    UUPSUpgradeable
{
    ...
    // Token Type IDs
    uint256 public constant CHAPTER_1_SEALED = 1;
    uint256 public constant CHAPTER_1_UNSEALED = 2;
    uint256 public constant CHAPTER_2_SEALED = 3;
    uint256 public constant CHAPTER_2_UNSEALED = 4;
    uint256 public constant CHAPTER_3_SEALED = 5;
    uint256 public constant CHAPTER_3_UNSEALED = 6;
    uint256 public constant CHAPTER_4_SEALED = 7;
    uint256 public constant CHAPTER_4_UNSEALED = 8;
    uint256 public constant FULL_VOLUME_SEALED = 9;
    uint256 public constant FULL_VOLUME_UNSEALED = 10;
    ...
    // For allowlist
    address[] public allowlistSigners;

    // Allowlist signature tracker
    ...
    mapping(uint256 => TimedToken) public timedTokens;

    // Events
    event MintPriceChanged(uint256 newPrice);
    event MintStartDateChanged(uint256 newMintStartDate);
    event MintEndDateChanged(uint256 newMintEndDate);
    event TimedTokenStartDateChanged(uint256 tokenId, uint256 newStartDate);

    // Custom errors for minting
    error IncorrectPaymentAmount(uint256 expected, uint256 actual);
    error AllowlistAlreadyAccessed();
    error MintingNotStartedYet();
    error MintingEnded();
    error MintLimitExceeded();

    // Custom errors for redeeming
    error RedeemNotAvailableYet();
    error TokenTypeMustBeSealed();
    error TokenTypeMustBeUnsealed();
    error NotEnoughTokens();
    error IncompleteChapterTokens();
    ...
    function initialize(
        ...
        TimedToken[] memory timedTokens_,
        uint256 price_,
        uint256 mintStartDate_,
        uint256 mintEndDate_,
        uint256 mintLimit_,
        uint96 secondaryRoyaltyPercentage_,
        address[] memory accounts_,
        uint256[] memory paymentSplit_,
        address[] memory allowlistSigners_
        ...
        _grantRole(OWNER_ROLE, author);
        _grantRole(PAUSER_ROLE, author);
        _grantRole(PAUSER_ROLE, admin);
        _grantRole(TOKEN_SETTER_ROLE, author);
        _grantRole(TOKEN_SETTER_ROLE, admin);
        _grantRole(PRICE_SETTER_ROLE, author);
        _grantRole(PRICE_SETTER_ROLE, admin);
        _grantRole(ALLOWLIST_ROLE, author);
        _grantRole(ALLOWLIST_ROLE, admin);
        _grantRole(UPGRADER_ROLE, author);
        _grantRole(UPGRADER_ROLE, admin);

        allowlistSigners = allowlistSigners_;

        // Set the secondary royalty percentage per ERC2981
        _setDefaultRoyalty(getRoleMember(OWNER_ROLE, 0), secondaryRoyaltyPercentage_);

        // iterate through the timed tokens array and set them
        for (uint256 i = 0; i < timedTokens_.length; i++) {
            setTimedTokenURI(i + 1, timedTokens_[i].tokenTypeURI);
            if (timedTokens_[i].startDate > 0) {
                setTimedTokenStartDate(i + 1, timedTokens_[i].startDate);
            }
        }
    ...
    function setTimedTokenStartDate(uint256 tokenTypeId, uint256 startDate)
        public
        onlyRole(TOKEN_SETTER_ROLE)
    {
        timedTokens[tokenTypeId].startDate = startDate;
        emit TimedTokenStartDateChanged(tokenTypeId, startDate);
    }

    function setTimedTokenURI(uint256 tokenTypeId, string memory tokenTypeURI)
        public
        onlyRole(TOKEN_SETTER_ROLE)
    {
        timedTokens[tokenTypeId].tokenTypeURI = tokenTypeURI;
        emit URI(tokenTypeURI, tokenTypeId);
    }

    function setMintPrice(uint256 newPrice) external onlyRole(PRICE_SETTER_ROLE) {
        price = newPrice;
        emit MintPriceChanged(newPrice);
    }

    function setMintStartDate(uint256 newMintStartDate) external onlyRole(TOKEN_SETTER_ROLE) {
        mintStartDate = newMintStartDate;
        emit MintStartDateChanged(newMintStartDate);
    }

    function setMintEndDate(uint256 newMintEndDate) external onlyRole(TOKEN_SETTER_ROLE) {
        mintEndDate = newMintEndDate;
        emit MintEndDateChanged(newMintEndDate);
    }

    /**
     * @dev Mints a chapter 1 token
     * @param to The address to mint the token to
     * @param quantity The number of tokens to mint
     * @param allowlistSignature optional signature to check against the allowlist
     *
     */
    function mintChapter1(
        address to,
        uint256 quantity,
        bytes calldata allowlistSignature
    ) external payable {
        if (block.timestamp < mintStartDate) {
        ...
        uint256 freeMints = 0;
        if (allowlistSignature.length > 0) {
            freeMints = _checkFreeMints(msg.sender, allowlistSignature);
        }
        if (msg.value != price * (quantity - freeMints)) {
            revert IncorrectPaymentAmount(price * (quantity - freeMints), msg.value);
        }

        _mint(to, CHAPTER_1_SEALED, quantity, "");
        if (freeMints > 0) {
            signatureUsed[allowlistSignature] = true;
        }
    }

    function unsealTokens(uint256 originalTokenTypeId, uint256 quantity) external {
        if (!(originalTokenTypeId % 2 == 1)) {
            revert TokenTypeMustBeSealed();
        }
        if (balanceOf(msg.sender, originalTokenTypeId) < quantity) {
            revert NotEnoughTokens();
        }

        _burn(msg.sender, originalTokenTypeId, quantity);
        _mint(msg.sender, originalTokenTypeId + 1, quantity, "");
    }

    function redeemNextChapter(uint256 originalTokenTypeId, uint256 quantity) external {
        if (originalTokenTypeId > CHAPTER_3_UNSEALED) {
            revert InvalidTokenTypeId();
        }
        if (originalTokenTypeId % 2 == 1) {
            revert TokenTypeMustBeUnsealed();
        }
        if (!availableToRedeem(originalTokenTypeId + 1)) {
            revert RedeemNotAvailableYet();
        }
        if (balanceOf(msg.sender, originalTokenTypeId) < (2 * quantity)) {
            revert NotEnoughTokens();
        }

        _burn(msg.sender, originalTokenTypeId, (2 * quantity));
        _mint(msg.sender, originalTokenTypeId + 1, quantity, "");
    }

    function redeemFullVolume() external {
        if (!availableToRedeem(FULL_VOLUME_SEALED)) {
            revert RedeemNotAvailableYet();
        }

        // solidity won't let you create an array literal and just pass it in as the argument
        // because the array literal is fixed size while the argument is dynamically sized
        // so we have to go through this rigamarole
        address[] memory addresses = new address[](4);
        for (uint256 i = 0; i < 4; i++) addresses[i] = msg.sender;

        uint256[] memory tokenTypeIds = new uint256[](4);
        tokenTypeIds[0] = CHAPTER_1_UNSEALED;
        tokenTypeIds[1] = CHAPTER_2_UNSEALED;
        tokenTypeIds[2] = CHAPTER_3_UNSEALED;
        tokenTypeIds[3] = CHAPTER_4_UNSEALED;

        uint256[] memory chapterBalances = balanceOfBatch(addresses, tokenTypeIds);

        for (uint256 i = 0; i < 4; i++) {
            if (chapterBalances[i] < 1) revert IncompleteChapterTokens();
        }

        uint256[] memory quantities = new uint256[](4);
        for (uint256 i = 0; i < 4; i++) quantities[i] = 1;

        _burnBatch(msg.sender, tokenTypeIds, quantities);
        _mint(msg.sender, FULL_VOLUME_SEALED, 1, "");
    }

    /**
     * @dev See {IERC1155MetadataURI-uri}.
     *      Returns the unique URI for a given token type.
     */
    function uri(uint256 tokenTypeId) public view override returns (string memory) {
        return timedTokens[tokenTypeId].tokenTypeURI;
    }

    /**
     * @dev Returns true if chapter 1 is available to mint
     */
    function availableToMint() public view returns (bool) {
        return block.timestamp >= mintStartDate && block.timestamp <= mintEndDate;
    }

    /**
     * @dev Returns true if the token type is available to redeem, meaning the timed
     *      token start date has passed
     */
    function availableToRedeem(uint256 tokenTypeId) public view returns (bool) {
        return block.timestamp >= timedTokens[tokenTypeId].startDate;
    }
     ...
     *      Includes a modifier to insure that that the operator is allowed by the OperatorFilterRegistry.
     */
     ...
     *
     *      This role also has the ability to revoke the OpenSea operator filter via a call to
     ...
     *      This is where Pausable hooks into mints, transfers and burns.
     */
     ...
     *      Only holders of the UPGRADER_ROLE can upgrade.
     */
    function _authorizeUpgrade(address newImplementation) internal override onlyRole(UPGRADER_ROLE) {}

    /**
     * @dev Set the signer for the spearmint allowlist.
     */
    function setAllowlistSigners(address[] memory allowlistSigners_) external onlyRole(ALLOWLIST_ROLE) {
        allowlistSigners = allowlistSigners_;
    }

    /**
     * @dev returns the number of free mints the address is entitled to based on the signature
     */
    function _checkFreeMints(address _addr, bytes calldata _signature) internal view returns (uint256) {
        uint256 freeMints = 0;
        if (signatureUsed[_signature]) {
            revert AllowlistAlreadyAccessed();
        }
        bytes32 digest = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encode(_addr)))
        );
        address allowlistSigner = digest.recover(_signature);
        if (allowlistSigner == allowlistSigners[0]) {
            freeMints = 1;
        } else if (allowlistSigner == allowlistSigners[1]) {
            freeMints = 2;
        }
        return freeMints;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155Upgradeable, ERC2981Upgradeable, AccessControlEnumerableUpgradeable)
        returns (bool)
    {
