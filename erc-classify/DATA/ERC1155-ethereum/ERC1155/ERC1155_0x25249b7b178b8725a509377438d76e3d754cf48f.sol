...
contract BattlePass is ERC1155Upgradeable {
    using ECDSAUpgradeable for bytes32;

    BattlePassStorage public battlePassStorage;
    ...
    function mintSignature(
        ...
        uint256 newTotalMintedSupply = battlePassStorage.mintedSupply(tokenId) +
            amount;
        if (battlePassStorage.valRewardLevelMinted(valTokenId, rewardLevel))
        ...
        battlePassStorage.setValRewardLevelMinted(valTokenId, rewardLevel);
    ...
    /**
     * @notice Allows users to batch mint tokens by providing a valid signature.
     * @param tokenIds The list of tokenIds to be minted.
     * @param valTokenIds The list of Valhalla tokenIds linked to this mint.
     * @param rewardLevels A 2D array where the reward level list indexes correspond to the index of the valTokenIds list.
     * @param signature The signature provided by the signer to validate the minting.
     * @param vault The protected wallet being requested from the delegated wallet. Use zero addr if not using a delegated wallet.
     */
    function batchMintSignature(
        uint256[] memory tokenIds,
        uint256[] memory valTokenIds,
        uint256[][] memory rewardLevels,
        bytes memory signature,
        address vault
    ) external {
        if (valTokenIds.length != rewardLevels.length)
            revert InvalidBatchMintInput();
        ...
        for (uint i = 0; i < valTokenIds.length; i++) {
            uint256 valTokenId = valTokenIds[i];
            for (uint j = 0; j < rewardLevels[i].length; j++) {
                if (
                    battlePassStorage.valRewardLevelMinted(
                        valTokenId,
                        rewardLevels[i][j]
                    )
                ) revert RewardAlreadyMinted();

                battlePassStorage.setValRewardLevelMinted(
                    valTokenId,
                    rewardLevels[i][j]
                );
            }
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            uint256 tokenId = i + 1;
            uint256 newMintedSupply = battlePassStorage.mintedSupply(tokenId) +
                tokenIds[i];
            battlePassStorage.setMintedSupply(tokenId, newMintedSupply);
            _mint(requester, tokenId, tokenIds[i], "");
        }
...
// File: contracts/BattlePassStorage.sol
// SPDX-License-Identifier: MIT
...
contract BattlePassStorage is Initializable, OwnableUpgradeable {
    ...
    // Mapping from Valhalla tokenId to a mapping from reward level to whether it has been minted
    mapping(uint256 => mapping(uint256 => bool)) _valRewardLevelMinted;
    ...
    /**
     * @notice Returns whether a token has been minted given the Valhalla tokenId and the reward level.
     * @param valTokenId The Valhalla token ID.
     * @param rewardLevel The reward level of the mint.
     */
    function valRewardLevelMinted(
        uint256 valTokenId,
        uint256 rewardLevel
    ) external view returns (bool) {
        return _valRewardLevelMinted[valTokenId][rewardLevel];
    }
    ...
    /**
     * @notice Sets the reward as already minted given the Valhalla tokenId and the reward level.
     * @param valTokenId The Valhalla token ID.
     * @param rewardLevel The reward level of the mint.
     */
    function setValRewardLevelMinted(
        uint256 valTokenId,
        uint256 rewardLevel
    ) external onlyBattlePass {
        _valRewardLevelMinted[valTokenId][rewardLevel] = true;
    }
