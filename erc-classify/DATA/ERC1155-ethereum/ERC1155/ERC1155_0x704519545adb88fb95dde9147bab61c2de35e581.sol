// File: contracts/loop2-fizz/Loop2FizzNFT.sol
...
contract Loop2FizzNFT is Ownable, ERC1155URIStorage {
  ...
  mapping(address => mapping(uint256 => uint256)) private purchaseCounterPerId;
  ...
  function buy(uint256[] memory tokenIds, uint256[] memory tokenQuantities) external payable {
    ...
    for (uint32 index = 0; index < tokenIds.length; index++) {
      ...
      require(purchaseCounterPerId[msg.sender][tokenIds[index]] + tokenQuantities[index] <= maxPurchasePerId, "Sorry, there are not enough token left to be minted on this id.");
    }
    ...
    for (uint32 index = 0; index < tokenIds.length; index++) {
      ...
      purchaseCounterPerId[msg.sender][tokenIds[index]] = purchaseCounterPerId[msg.sender][tokenIds[index]] + tokenQuantities[index];
    }
    // drain message
  ...
  function adminMint(uint32 tokenId, uint32 tokenQuantity, address to) external onlyOwner {
    ...
    require(purchaseCounterPerId[to][tokenId] + tokenQuantity <= maxPurchasePerId, "Sorry, there are not enough token left to be minted on this id.");
