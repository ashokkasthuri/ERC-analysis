...
contract EventNFT is ERC1155, Ownable, IEventNFT {
    ...
    function mintEvent(
        ...
        uint256 newEventId = _tokenIdTracker.current();
    ...
    function addTokenEventToAIBOLT(
        address aiboltContract,
        uint16 aiboltId,
        uint256 eventId
    ) public {
        // Ensure the caller owns the EventNFT
        require(
            balanceOf(msg.sender, eventId) > 0,
            "Caller must own the EventNFT"
        );

        // Here you'll want to interface with the AIBOLT contract and call its private 'addTokenEvent' method
        // This will depend on how you've implemented the AIBOLT contract
        AIBOLT(aiboltContract).addTokenEvent(aiboltId, eventId, msg.sender);

        // Burn the EventNFT after the event has been added
        _burn(msg.sender, eventId, 1);
    }
}


// File: /contracts/OrbitProxy.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
