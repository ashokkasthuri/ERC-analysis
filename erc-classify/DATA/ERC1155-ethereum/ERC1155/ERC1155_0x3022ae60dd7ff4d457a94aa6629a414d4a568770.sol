// File: src/partners/InBetweeners.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;

import { ERC1155SeaDrop } from "../ERC1155SeaDrop.sol";

contract InBetweeners is ERC1155SeaDrop {
    address private _burnAddress;

    error IncorrectBurnAddress(address expected, address got);
    ...
    function burn(address from, uint256 id, uint256 amount) external override {
        if (msg.sender != _burnAddress) {
            revert IncorrectBurnAddress(_burnAddress, msg.sender);
        }
        // Burn the token.
