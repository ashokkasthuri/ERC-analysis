...
contract NFTCC is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, IERC1155Receiver, ReentrancyGuard {
    ...
    function buyNFT(address to, uint256 tokenId, uint256 quantity, bytes memory data) external nonReentrant {
        ...
        require(balanceOf(address(this), tokenId) >= quantity, "Invalid request");

        uint256 totalAmount = tokenInfo[tokenId].price * quantity;
        require(usdtToken.allowance(msg.sender, address(this)) >= totalAmount && usdtToken.balanceOf(msg.sender) >= totalAmount, "Invalid balance");
    ...
    function getWithdrawUSDT() external view returns (uint256) {
        return withdrawableUSDT;
    }
    ...
    // Add a new method to get all the information of Token
    function getTokenInfo(uint256 tokenId) external view returns (uint256 price, uint8 tokenType, uint8 status, string memory uri) {
        TokenInfo storage info = tokenInfo[tokenId];
        return (info.price, info.tokenType, info.status, info.uri);
    }
