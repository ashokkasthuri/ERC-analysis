    ...
    PermissionsEnumerable
{
    ...
    function __ERC1155LazyMint_init(
        string memory _name,
    ...
    function claim(
        ...
        verifyClaim(_msgSender(), _tokenId, _quantity); // Add your claim verification logic by overriding this function.
...
contract ERC1155 is IERC1155, IERC1155Metadata, ERC2771ContextUpgradeable {
    /*//////////////////////////////////////////////////////////////
                        State variables
    //////////////////////////////////////////////////////////////*/
    ...
    function safeTransferFrom(
        ...
        require(from == _msgSender() || isApprovedForAll[from][_msgSender()], "!OWNER_OR_APPROVED");
...
// File: @thirdweb-dev/contracts/extension/interface/IClaimableERC1155.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
// File: @thirdweb-dev/contracts/openzeppelin-presets/security/ReentrancyGuard.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/BurnToRedeem1155.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;

import "@thirdweb-dev/contracts/base/ERC1155LazyMint.sol";

contract BurnToRedeem1155 is ERC1155LazyMint {
    /*///////////////////////////////////////////////////////////////
                        Contract identifiers
    //////////////////////////////////////////////////////////////*/
    
    function contractType() external pure returns (bytes32) {
        return bytes32("BurnToRedeem1155");
    }
    ...
    ERC1155LazyMint private accessKeyContract;
    mapping(uint256 => AccessKeyPair) private accessKeyPairs;
    uint256 private accessKeyPairCount = 0;
    ...
    function initialize(
        ...
        __ERC1155LazyMint_init(_name, _symbol, _royaltyRecipient, _royaltyBps);
    ...
    function setAccessKeyTokens(
        address _accessKeyContract,
        AccessKeyPair[] calldata _accessKeyPairs
    ) external virtual onlyRole(MINTER_ROLE) {
        accessKeyContract = ERC1155LazyMint(_accessKeyContract);
        
        accessKeyPairCount = _accessKeyPairs.length;
        for (uint256 i = 0; i < accessKeyPairCount; i++) {
            accessKeyPairs[i] = _accessKeyPairs[i];
        }
    }
    
    function resetAccessKeyTokens() external virtual onlyRole(MINTER_ROLE) {
        accessKeyPairCount = 0;
    }
    
    /// @notice 
    function currentAccessKeyTokens() public view virtual returns (address, AccessKeyPair[] memory) {
        require(accessKeyPairCount > 0, "Rejected: No tokens has been set yet.");
        
        AccessKeyPair[] memory pairs = new AccessKeyPair[](accessKeyPairCount);
        for (uint256 i = 0; i < accessKeyPairCount; i++) {
            pairs[i] = accessKeyPairs[i];
        }
        
        return (address(accessKeyContract), pairs);
    }
    ...
    function verifyClaim(
        ...
        for (uint256 i = 0; i < accessKeyPairCount; i++) {
            AccessKeyPair memory pair = accessKeyPairs[i];
            require(
                accessKeyContract.balanceOf(_claimer, pair.tokenId) >= _quantity * pair.quantity,
                "Rejected: Claimer does not have enough access keys"
            );
        }
    }
    ...
    function _transferTokensOnClaim(
        ...
        for (uint256 i = 0; i < accessKeyPairCount; ++i) {
            AccessKeyPair memory pair = accessKeyPairs[i];
            accessKeyContract.burn(
                _receiver,
                pair.tokenId,
                _quantity * pair.quantity
            );
        }
