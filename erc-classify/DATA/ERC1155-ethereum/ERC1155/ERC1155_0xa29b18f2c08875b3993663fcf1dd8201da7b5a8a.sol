// File: contracts/IdiotMedicine.sol
...
contract IdiotMedicine is ERC1155, ERC2981, Ownable {
    ...
    mapping(uint256 => bool) private idiotGreenClaimed;
    ...
    function claimGreen() external {
        ...
        uint256 amount = 0;
        uint256[] memory tokens = idiotContract.tokensOfOwner(msg.sender);
        for (uint256 i = 0; i < tokens.length; ) {
            uint256 id = tokens[i];
            if (!idiotGreenClaimed[id]) {
                amount++;
                idiotGreenClaimed[id] = true;
            }
            unchecked {
                i++;
            }
