...
/// @title Gala Film
/// @notice The proxy contract for Gala Film
/// @author Smith Tanny

contract GalaFilm is
    Initializable,
    CommonConstants,    
    IERC1155Upgradeable,
    ...
    OperatorFiltererUpgradeable
{
    ...
    function initialize(string calldata _client, address _minter, address _upgrader) public initializer notZeroAddress(_minter) notZeroAddress(_upgrader) {
    ...
    function mintNonFungible(uint256[] calldata _ids, address[] calldata _to, bytes calldata _data) external whenNotPaused onlyRole(MINTER_ROLE) {
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            emit TransferSingle(_msgSender(), address(0x0), distributeTo, _ids[i], 1);

            broadcastURIEvent(_ids[i]);

            if (distributeTo.isContract()) {
                _doSafeTransferAcceptanceCheck(_msgSender(), address(0x0), distributeTo, _ids[i], 1, _data);
            }
        }
    }
    ...
    function mintFungible(
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            emit TransferSingle(_msgSender(), address(0x0), _to[i], _id, _quantities[i]);

            if (_to[i].isContract()) {
                _doSafeTransferAcceptanceCheck(_msgSender(), address(0x0), _to[i], _id, _quantities[i], _data);
            }
        }
    }
    ...
    function safeTransferFrom(
        ...
        require(_from == _msgSender() || operatorApproval[_from][_msgSender()], "Need operator approval for 3rd party transfers");
    ...
    function burn(address _from, uint256[] calldata _ids, uint256[] calldata _values) external whenNotPaused {
        require(_from == _msgSender() || operatorApproval[_from][_msgSender()], "Need operator approval for 3rd party burn");
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            emit TransferSingle(_msgSender(), _from, address(0x0), _ids[i], _values[i]);
        }
    }
    ...
    function broadcastURIEvent(uint256 _tokenId) public onlyRole(MINTER_ROLE) {
    ...
    function transferAdmin(address _newOwner) public {
        require(_newOwner != _msgSender(), "new owner must be different than the current owner");
