...
error CannotRemintNft(uint256 id);

/// @custom:security-contact security@artory.com
contract Artory is
    Initializable,
    ERC1155Upgradeable,
    ...
    UUPSUpgradeable
{
    // Note to future engineers: upgradeable contracts need to maintain variable order, so new variables must be added at the end.
    // Read more here: https://docs.openzeppelin.com/upgrades-plugins/1.x/writing-upgradeable#modifying-your-contracts
    ...
    function initialize() public initializer {
        __ERC1155_init("https://algorand-connector.artory.com/certificates/%7Bid%7D/metadata/erc-1155");
    ...
    function ownedIds(address owner) public view returns (uint256[] memory) {
        uint256[] memory owned = new uint256[](_allTokens.length);
        uint256 ownedCount = 0;
        for (uint256 i = 0; i < _allTokens.length; i++) {
            if (balanceOf(owner, _allTokens[i]) > 0) {
                owned[ownedCount] = _allTokens[i];
                ownedCount++;
            }
        }
        uint256[] memory ownedTrimmed = new uint256[](ownedCount);
        for (uint256 i = 0; i < ownedCount; i++) {
            ownedTrimmed[i] = owned[i];
        }
        return ownedTrimmed;
    }
