...
// File: contracts/ReaderTokens1155.sol
// SPDX-License-Identifier: MIT
  ...
  UpdatableOperatorFiltererUpgradeable
{
  ...
  /**
   * @notice Claims Age of Valor 01 Follow the Rabbit Edition
   * @dev Only one claim per address and only claimable while mint is enabled
   */
  function mintAgeOfValorFTR() external nonReentrant {
    if (!s_tokenIdToMintEnabled[AGE_OF_VALOR_01_FTR_ID]) {
      revert ReaderTokens__NotEnabled();
    }
    if (s_tokenIdToAddressClaimed[AGE_OF_VALOR_01_FTR_ID][msg.sender]) {
      revert ReaderTokens__AlreadyClaimed();
    }
    s_tokenIdToAddressClaimed[AGE_OF_VALOR_01_FTR_ID][msg.sender] = true;
    _mint(msg.sender, AGE_OF_VALOR_01_FTR_ID, 1, "");
    emit MintedToken(AGE_OF_VALOR_01_FTR_ID, 1, msg.sender);
  }
...
// File: operator-filter-registry/src/upgradeable/UpdatableOperatorFiltererUpgradeable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;
