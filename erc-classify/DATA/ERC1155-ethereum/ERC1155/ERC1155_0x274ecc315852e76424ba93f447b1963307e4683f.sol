// File: contracts/manifold/creator-core/ERC1155CreatorImplementation.sol
...
import "../../manifold/libraries-solidity/access/UserControl.sol";
    ...
    ERC1155CreatorCore
{
    ...
    function initialize(
	    ...
	    bool _UserRestrictions, 
	    address[] calldata _allowedUsers
    ) public initializer {
        __ERC1155_init(_name, _symbol);
        __Ownable_init();
        _initializeUserRestriction(_UserRestrictions, _allowedUsers);
    }
    ...
    function mintExtensionNew(
        ...
        uint256[] calldata amounts,
        string[] calldata uris
    ) public virtual override nonReentrant returns (uint256[] memory tokenIds) {
        requireExtension();
    ...
    function tokenExtension(
        uint256 tokenId
    ) public view virtual override returns (address extension) {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        if(isUserRestricted()) {
        require(isUser(operator), "AllowedUser: operator is not an allowed User");
        }
...
// File: contracts/manifold/creator-core/token/ERC1155/ERC1155Upgradeable.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/core/ERC1155CreatorCore.sol
// SPDX-License-Identifier: MIT
...
abstract contract ERC1155CreatorCore is CreatorCore, IERC1155CreatorCore {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(CreatorCore, IERC165) returns (bool) {
        return
     ...
     */
    function _setApproveTransferExtension(
        address extension,
        bool enabled
    ) internal override {
        if (
        ...
        if (extension != address(0) && _extensionApproveTransfers[extension]) {
            require(
                IERC1155CreatorExtensionApproveTransfer(extension)
                    .approveTransfer(msg.sender, from, to, tokenIds, amounts),
                "Extension approval failure"
            );
        } else if (_approveTransferBase != address(0)) {
            require(
                IERC1155CreatorExtensionApproveTransfer(_approveTransferBase)
                    .approveTransfer(msg.sender, from, to, tokenIds, amounts),
                "Extension approval failure"
            );
        }
...
// File: contracts/manifold/libraries-solidity/access/UserControl.sol
// SPDX-License-Identifier: MIT
...
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "./IUserControl.sol";
import "./IAdminControl.sol";

abstract contract UserControl is IUserControl, ERC165 {
    using EnumerableSet for EnumerableSet.AddressSet;

    // Track registered users
    EnumerableSet.AddressSet private _users;
    bool private _userRestricted;
    ...
    {
        return
            interfaceId == type(IUserControl).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {IAdminControl-getUsers}.
     */
    function getUsers()
        external
        view
    ...
    /**
     * Set third party transfer restrictions
     */
     function _initializeUserRestriction(bool restricted, address[] calldata allowedUsers )internal  {
         _userRestricted = restricted;
         if(_userRestricted) {
        for (uint256 i = 0; i < allowedUsers.length; i++) {
            _users.add(allowedUsers[i]);
        }
         }
    }
    /**
     * @dev See {IAdminControl-approveUser}.
     */
    function approveUser(address user) external override {
        require(
    ...
    /**
     * @dev See {IAdminControl-revokeUser}.
     */
    ...
    function setUserRestriction(bool restricted ) external override {
        require(
            IAdminControl(address(this)).isAdmin(msg.sender),
            "AdminControl: Must be owner or admin"
        );
    ...
    /**
     * @dev See {IAdminControl-isUser}.
     */
    function isUser(address user) public view override returns (bool) {
        return _users.contains(user);
    }

    /**
     * @dev See {IAdminControl-isUserRestricted}.
     */
    function isUserRestricted() public view override returns (bool) {
        return _userRestricted;
    }   
}


// File: contracts/manifold/libraries-solidity/access/AdminControlUpgradeable.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/libraries-solidity/access/IAdminControl.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/libraries-solidity/access/IUserControl.sol
// SPDX-License-Identifier: MIT
 ...
 * @dev Interface for admin control
 */
interface IUserControl is IERC165 {

    event UserApproved(address indexed account, address indexed sender);
    event UserRevoked(address indexed account, address indexed sender);

    /**
     * @dev gets address of all allowed users
     */
    function getUsers() external view returns (address[] memory);

    /**
     * @dev add an user.  Can only be called by contract owner.
     */
    function approveUser(address user) external;

    /**
     * @dev remove an user.  Can only be called by contract owner.
     */
     ...
     * @dev remove an user.  Can only be called by contract owner.
     */
    function setUserRestriction(bool restricted ) external;

    /**
     * @dev checks whether or not given address is an allowed user
     * Returns True if they are
     */
    function isUser(address user) external view returns (bool);

    /**
     * @dev checks whether the contract has user restrictions
     * Returns True if they are
     */
    function isUserRestricted() external view returns (bool);
}
...
// File: contracts/manifold/creator-core/token/ERC1155/ERC1155Core.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/core/CreatorCore.sol
// SPDX-License-Identifier: MIT
...
abstract contract CreatorCore is ReentrancyGuard, ICreatorCore, ERC165 {
     ...
     */
    function _setBaseTokenURIExtension(
        string calldata uri,
        bool identical
    ) internal {
     ...
     */
    function _setTokenURIExtension(
        uint256 tokenId,
        string calldata uri
    ) internal {
    ...
    function _setTokenURI(uint256 tokenId, string calldata uri) internal {
        require(
            tokenId > 0 &&
                tokenId <= _tokenCount &&
                _tokenExtension(tokenId) == address(0),
            "Invalid token"
        );
        _tokenURIs[tokenId] = uri;
        emit MetadataUpdate(tokenId); 
    ...
    function _tokenURI(uint256 tokenId) internal view returns (string memory) {
        ...
        if (bytes(_extensionBaseURI[extension]).length != 0) {
            ...
            if (!_extensionBaseURIIdentical[address(0)])
            {
            return
                string(
                    abi.encodePacked(
                        _extensionBaseURI[address(0)],
                        tokenId.toString()
                    )
                );
            }else{
                return _extensionBaseURI[address(0)];
            }
        }
    }
        ...
        if (royalties.length == 0) {
            ...
            if (extension != address(0)) {
                if (
                    ERC165Checker.supportsInterface(
                        extension,
                        type(ICreatorExtensionRoyalties).interfaceId
                    )
                ) {
                    (receivers, bps) = ICreatorExtensionRoyalties(extension)
                        .getRoyalties(address(this), tokenId);
...
// File: contracts/manifold/creator-core/core/IERC1155CreatorCore.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/permissions/ERC1155/IERC1155CreatorMintPermissions.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/extensions/ERC1155/IERC1155CreatorExtensionBurnable.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/extensions/ERC1155/IERC1155CreatorExtensionApproveTransfer.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/core/ICreatorCore.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/extensions/ICreatorExtensionRoyalties.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/manifold/creator-core/extensions/ICreatorExtensionTokenURI.sol
// SPDX-License-Identifier: MIT
