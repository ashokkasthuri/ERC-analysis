...
// $$  __$$\                                                     $$ |          $$  __$$\            $$ |
// $$ |  $$ |$$\   $$\  $$$$$$\  $$$$$$$\   $$$$$$\  $$\   $$\ $$$$$$\         $$ |  $$ | $$$$$$\ $$$$$$\    $$$$$$$\
// $$$$$$$\ |$$ |  $$ |$$  __$$\ $$  __$$\ $$  __$$\ $$ |  $$ |\_$$  _|        $$$$$$$\ |$$  __$$\\_$$  _|  $$  _____|
// $$  __$$\ $$ |  $$ |$$ |  \__|$$ |  $$ |$$ /  $$ |$$ |  $$ |  $$ |          $$  __$$\ $$ /  $$ | $$ |    \$$$$$$\
// $$ |  $$ |$$ |  $$ |$$ |      $$ |  $$ |$$ |  $$ |$$ |  $$ |  $$ |$$\       $$ |  $$ |$$ |  $$ | $$ |$$\  \____$$\
// $$$$$$$  |\$$$$$$  |$$ |      $$ |  $$ |\$$$$$$  |\$$$$$$  |  \$$$$  |      $$$$$$$  |\$$$$$$  | \$$$$  |$$$$$$$  |
// \_______/  \______/ \__|      \__|  \__| \______/  \______/    \____/       \_______/  \______/   \____/ \_______/

/**
 * @title Burnout Bots NFT
 * @author @Xirynx
 * @notice NFT minting contract with whitelist and public phase as well as burn to mint. 10,000 max supply.
 */
contract BurnoutBotsNFT is
    ERC721A("Burnout Bots", "BBOTS"),
    Ownable,
    DefaultOperatorFilterer
{
    ...
    uint8 public constant MAX_PER_WALLET_PUBLIC = 30;
    ...
    /**
     * @notice Sets parts contract from which to burn tokens.
     * @dev Requirements:
     *	Caller must be contract owner.
     *	`_address` must not be an empty string.
     * @param _address Parts contract address.
     */
    function setPartsContract(address _address) external onlyOwner {
        if (address(0) == _address) revert InvalidAddress();
        partsContract = IERC1155(_address);
    }
    ...
    function publicMint(uint8 amount) external payable {
        ...
        _setAux(
            msg.sender,
            uint64((auxData & ((1 << 32) - 1)) | (numMinted << 32))
        );
        _mint(msg.sender, amount);
    }

    /**
     * @notice Burns 1 of each part NFT and mints 1 token to caller's address.
     * @dev Requirements:
     *	Caller must be an externally owned account.
     *	`phase` must not equal NONE.
     *	Total supply must be less than or equal to `MAX_SUPPLY` after mint.
     */
    function partsMint() external {
        if (tx.origin != msg.sender) revert CallerNotOrigin();
        if (phase == Phase.NONE) revert IncorrectPhase();
        ...
        _amounts[2] = 1;
        _amounts[3] = 1;
        partsContract.safeBatchTransferFrom(
            msg.sender,
            address(0xdead),
            _ids,
            _amounts,
    ...
    /**
     * @notice Overridden to return 1 as starting token.
     * @return uint256 1.
     */
