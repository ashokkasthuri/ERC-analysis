...
contract DSCOLL is Context, AccessControlEnumerable, ERC1155BurnableX, ERC1155PausableX {
    using Strings for uint256;
    event Withdrawn(address indexed payee, uint256 weiAmount);
    string public constant name = "Disco Studios Collectors Pass";
    ...
    uint256[] public tokensLockedMinting;
    address public paymentRecipient = 0xA94F799A34887582987eC8C050f080e252B70A21;
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, and `PAUSER_ROLE` to the account that
     * deploys the contract.
     */
    constructor() ERC1155X("overwritten URI") {
    ...
    function lockTokenMinting(
       uint256 tokenId
        ...
        for (uint256 i; i < tokensLockedMinting.length; ++i) {
            if (tokenId == tokensLockedMinting[i]) return;
        }
        tokensLockedMinting.push(tokenId);
    }
    ...
    function mint(
        ...
        bool tokens_locked_mint = false;
        for (uint256 i; i < tokensLockedMinting.length; ++i) {
            if (id == tokensLockedMinting[i]) {
                tokens_locked_mint = true;
                break;
            }
        }
        require(!tokens_locked_mint, "This token is locked for minting. No more of this one can be minted.");
    ...
    function mintBatch(
        ...
        for (uint256 i; i < tokensLockedMinting.length; ++i) {
            for (uint256 ii; ii < ids.length; ++ii) {
                if (ids[ii] == tokensLockedMinting[i]) {
                    tokens_locked_mint = true;
                    break;
                }
            }
            if (tokens_locked_mint == true) break;
        }
        require(!tokens_locked_mint, "Some tokens are locked for minting and cannot be minted anymore.");
...
// File: contracts/ERC1155PausableX.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/ERC1155BurnableX.sol
// SPDX-License-Identifier: MIT
