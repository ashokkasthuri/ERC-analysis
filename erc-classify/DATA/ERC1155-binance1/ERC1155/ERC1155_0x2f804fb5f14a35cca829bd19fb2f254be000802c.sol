// File: contracts/validators/KSZapValidator.sol
...
contract KSZapValidator is IKSZapValidator, KSRescue {
  uint8 constant ZAP_OUT_TYPE = 100;
  ...
  function prepareValidationData(
    ...
    if (_dexType == uint8(DexType.UniswapV2)) {
      return _getUniswapV2ValidationData(_zapInfo);
    }
    if (_dexType == uint8(DexType.AlgebraV19) || _dexType == uint8(DexType.AlgebraV19DirFee)) {
      return _getAlgebraValidationData(_zapInfo);
    }
    if (_dexType == ZAP_OUT_TYPE) {
      return _getZapOutValidationData(_zapInfo);
    }
    return new bytes(0);
  }
  ...
  function validateData(
    ...
    if (_dexType == uint8(DexType.UniswapV2)) {
      return _validateUniswapV2Result(_extraData, _initialData);
    }
    if (_dexType == uint8(DexType.AlgebraV19) || _dexType == uint8(DexType.AlgebraV19DirFee)) {
      return _validateAlgebraResult(_extraData, _initialData);
    }
    if (_dexType == ZAP_OUT_TYPE) {
      return _validateZapOutResult(_extraData, _initialData);
    }
    return true;
  }
  ...
  /// @notice Generate initial data for validation for Algebra
  ///   2 cases: minting a new position or increase liquidity
  ///   - minting a new position:
  ///     + posID in zapInfo should be 0, then replaced with the curren totalSupply
  ///     + isNewPosition is true
  ...
  ///     + isNewPosition is false
  ///     + initialLiquidity is the current position liquidity, fetched from Position Manager
  function _getAlgebraValidationData(bytes calldata zapInfo) internal view returns (bytes memory) {
    ElasticValidationData memory data;
    data.initialData = abi.decode(zapInfo, (ElasticZapData));
    if (data.initialData.posID == 0) {
      // minting new position, temporary store the total supply here
      data.initialData.posID = IAlgebraV19NFT(data.initialData.posManager).totalSupply();
      data.isNewPosition = true;
      data.initialLiquidity = 0;
    } else {
      data.isNewPosition = false;
      (,,,,,, data.initialLiquidity,,,,) =
        IAlgebraV19NFT(data.initialData.posManager).positions(data.initialData.posID);
    }
    return abi.encode(data);
  }

  /// @notice Generate initial data for validation for Uniswap v2
  ///  in order to validate, we need to get the initial token balance of the recipient
  /// @param zapInfo contains info of zap out
  ///   should be (token address, recipient address)
  function _getZapOutValidationData(bytes calldata zapInfo) internal view returns (bytes memory) {
    (address token, address recipient) = abi.decode(zapInfo, (address, address));
    uint256 initialBalance =
      token == ETH_ADDRESS ? recipient.balance : IERC20(token).balanceOf(recipient);
    return abi.encode(ZapOutValidationData(token, recipient, initialBalance));
  }
  /// @notice Generate initial data for validation for Uniswap v2
  ///  in order to validate, we need to get the initial LP balance of the recipient
  /// @param zapInfo contains info of zap with Uniswap v2
  ///   should be (pool_address, recipient_address)

  function _getUniswapV2ValidationData(bytes calldata zapInfo) internal view returns (bytes memory) {
    UniswapV2ValidationData memory data;
    data.initialData = abi.decode(zapInfo, (UniswapV2ZapData));
    data.initialLiquidity =
      uint128(IERC20(data.initialData.pool).balanceOf(data.initialData.recipient));
    return abi.encode(data);
  }
  ...
  function _validateAlgebraResult(
    ...
    ElasticValidationData memory data = abi.decode(_initialData, (ElasticValidationData));
    IAlgebraV19NFT posManager = IAlgebraV19NFT(data.initialData.posManager);
    if (data.isNewPosition) {
      // minting a new position, need to validate many data
      // Calculate the posID and replace, it should be the last index
      data.initialData.posID = posManager.tokenByIndex(data.initialData.posID);
  ...
  /// @notice Validate result for zapping into Uniswap v2
  ///   - _extraData is the minLiquidity (for validation)
  ///   - to validate, fetch the current LP balance of the recipient
  ///     then compares with the initialLiquidity, make sure the increment is expected (>= minLiquidity)
  /// @param _extraData just the minLiquidity value, uint256
  /// @param _initialData contains initial data before zap, including initialLiquidity
  function _validateUniswapV2Result(
    bytes calldata _extraData,
    bytes calldata _initialData
  ) internal view returns (bool) {
    UniswapV2ValidationData memory data = abi.decode(_initialData, (UniswapV2ValidationData));
    // getting new lp balance, make sure it should be increased
    uint256 lpBalanceAfter = IERC20(data.initialData.pool).balanceOf(data.initialData.recipient);
    if (lpBalanceAfter < data.initialLiquidity) return false;
    // validate increment in liquidity with min expectation
    uint256 minLiquidity = uint256(abi.decode(_extraData, (uint256)));
    require(minLiquidity > 0, 'zero min_liquidity');
    return (lpBalanceAfter - data.initialLiquidity) >= minLiquidity;
  }

  function _validateZapOutResult(
    bytes calldata _extraData,
    bytes calldata _initialData
  ) internal view returns (bool) {
    ZapOutValidationData memory data = abi.decode(_initialData, (ZapOutValidationData));
    uint256 minAmountOut = uint256(abi.decode(_extraData, (uint256)));
    uint256 currentBalance = data.token == ETH_ADDRESS
      ? data.recipient.balance
      : IERC20(data.token).balanceOf(data.recipient);
...
// File: contracts/interfaces/zap/validators/IKSZapValidator.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.9;
...
interface IKSZapValidator is IZapValidator {
  ...
  /// @notice Only need pool address and recipient to get data
  struct UniswapV2ZapData {
    address pool;
    address recipient;
  }

  /// @notice Return Uniswap V2 Zap Data, and initial liquidity of the recipient
  struct UniswapV2ValidationData {
    UniswapV2ZapData initialData;
    uint256 initialLiquidity;
  }

  /// @notice Validation data for zap out
...
// File: contracts/interfaces/ks_elastic/IBasePositionManager.sol
