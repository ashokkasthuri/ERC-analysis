...
/// @notice Allows callers to approve the BestDex V3 position manager from this contract,
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IBestDexV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IBestDexV3Pool pool) {
        pool = IBestDexV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IBestDexV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IBestDexV3Pool(pool).slot0();
            }
...
interface IImmutableState {
    /// @return Returns the address of the BestDex V2 factory
    function factoryV2() external view returns (address);

    /// @return Returns the address of BestDex V3 NFT position manager
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via BestDex V2
...
import '../v3-core/contracts/interfaces/callback/IBestDexV3SwapCallback.sol';

/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via BestDex V3
interface IV3SwapRouter is IBestDexV3SwapCallback {
...
// File: contracts/libraries/BestDexV2Library.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
library BestDexV2Library {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBestDexV2Pair(pairFor(factory, tokenA, tokenB)).getReserves();
...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given BestDex V3 pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) =
            IBestDexV3Pool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of BestDex V3 pool that we want to observe
    /// @return secondsAgo The number of seconds ago of the oldest observation stored for the pool
    function getOldestObservationSecondsAgo(address pool) internal view returns (uint32 secondsAgo) {
        (, , uint16 observationIndex, uint16 observationCardinality, , , ) = IBestDexV3Pool(pool).slot0();
        require(observationCardinality > 0, 'NI');

        (uint32 observationTimestamp, , , bool initialized) =
            IBestDexV3Pool(pool).observations((observationIndex + 1) % observationCardinality);

        // The next index might not be initialized if the cardinality is in the process of increasing
        // In this case the oldest observation is always in index 0
        if (!initialized) {
            (observationTimestamp, , , ) = IBestDexV3Pool(pool).observations(0);
        }
    ...
    /// @param pool Address of BestDex V3 pool
    /// @return The tick that the pool was in at the start of the current block
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        (, int24 tick, uint16 observationIndex, uint16 observationCardinality, , , ) = IBestDexV3Pool(pool).slot0();
        ...
        (uint32 observationTimestamp, int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128, ) =
            IBestDexV3Pool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, IBestDexV3Pool(pool).liquidity());
        }
        ...
        ) = IBestDexV3Pool(pool).observations(prevIndex);
...
/// @title BestDex V2 and V3 Swap Router
...
// File: contracts/v2-core/contracts/interfaces/IBestDexV2Pair.sol
...
/// @title BestDex V2 Swap Router
/// @notice Router for stateless execution of swaps against BestDex V2
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IBestDexV2Pair pair = IBestDexV2Pair(BestDexV2Library.pairFor(factoryV2, input, output));
            uint256 amountInput;
            ...
            // scope to avoid stack too deep errors
            {
                ...
                amountOutput = BestDexV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) =
                input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
            address to = i < path.length - 2 ? BestDexV2Library.pairFor(factoryV2, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        pay(
            ...
            BestDexV2Library.pairFor(factoryV2, path[0], path[1]),
            amountIn
        );
    ...
    function swapTokensForExactTokens(
        ...
        amountIn = BestDexV2Library.getAmountsIn(factoryV2, amountOut, path)[0];
        require(amountIn <= amountInMax, 'Too much requested');

        pay(path[0], msg.sender, BestDexV2Library.pairFor(factoryV2, path[0], path[1]), amountIn);
...
// File: contracts/v3-core/contracts/interfaces/callback/IBestDexV3SwapCallback.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/IBestDexV3Pool.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolActions.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolDerivedState.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolEvents.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolImmutables.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolOwnerActions.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/interfaces/pool/IBestDexV3PoolState.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/libraries/LowGasSafeMath.sol
// SPDX-License-Identifier: GPL-2.0-or-later
...
// File: contracts/v3-core/contracts/libraries/SafeCast.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: contracts/v3-core/contracts/libraries/TickMath.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0 <0.8.0;
...
/// @title BestDex V3 Swap Router
/// @notice Router for stateless execution of swaps against BestDex V3
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IBestDexV3Pool) {
        return IBestDexV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    /// @inheritdoc IBestDexV3SwapCallback
    function BestDexV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
