...
contract Forwarder is Ownable, Pausable, EIP712 {
    ...
    function transferEther(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            if (balance > 0) payable(recipients[i]).transfer(values[i]);
        }
    }
    ...
    /* for withdraw admin to user  1st step function*/
    ///@dev function for approval of multi token to send from admin wallet to all user wallet
    function multiApprovalbyAdmin(
        address[] memory tokenContracts,
        uint256 _values
    ) external {
        for (uint256 i = 0; i < tokenContracts.length; i++)
            IERC20(tokenContracts[i]).approve(address(this), _values);
    }
