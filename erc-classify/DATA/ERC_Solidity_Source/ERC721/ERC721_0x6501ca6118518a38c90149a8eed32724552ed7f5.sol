...
// File: contracts/BybitVIPExclusivePerks.sol
// SPDX-License-Identifier: MIT
...
contract BybitVIPExclusivePerks is Initializable, ERC721Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    event Mint(address indexed vip, uint256 tokenid);
    event Burn(address indexed vip, uint256 tokenid);

    function initialize(string memory name_, string memory symbol_, string memory baseUrl_, address minter_) initializer public {
    ...
    function mintBatch(address[] memory vips) external onlyMinter whenNotPaused {
        uint256 len = vips.length;
        require(len > 0);
    ...
    function mint(address vip) external onlyMinter whenNotPaused {
        _mint(vip);
    }
