// File: /contracts/PerseaSimpleCollectionSeq.sol
...
contract Splitter {
    ...
    uint256 public _totalpercentages;
    mapping(address => uint256) private _percentages;

    constructor(address[] memory payees, uint256[] memory percentages_) {
        require(payees.length == percentages_.length, "PaymentSplitter: payees and percentages length mismatch");
        require(payees.length > 0, "PaymentSplitter: no payees");
        for (uint256 i = 0; i < payees.length; i++) {
            _addPayee(payees[i], percentages_[i]);
        }
        require(_totalpercentages == base, "PaymentSplitter: The total percentage must be equal to 10000");
    }

    function _addPayee(address account, uint256 percentages_) internal {
        require(account != address(0), "PaymentSplitter: account is the zero address");
        require(percentages_ >= 100, "PaymentSplitter: percentages are 0");
        require(percentages_ <= base, "PaymentSplitter: percentages are a 10000 at most");
        require(_percentages[account] == 0, "PaymentSplitter: account already has percentages");
        _payees.push(account);
        _percentages[account] = percentages_;
        _totalpercentages = _totalpercentages + percentages_;
    }

    function getShareAmount(address account, uint256 value) public view returns(uint256) {
        require(_percentages[account] > 100, "PaymentSplitter: Doest not have percentages");
        return (value * _percentages[account]) / base;
    }

    function getPercentage(address account) public view returns(uint256) {
        require(_percentages[account] > 100, "PaymentSplitter: Doest not have percentages");
        return _percentages[account];
    }

    function getPayess() public view returns(address[] memory) {
        return _payees;
    }
}
