...
// File: contracts/NokkoCollection.sol
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;
...
contract nokko is ERC721URIStorage, Ownable, ContractMetadata {
    ...
    uint public totalSupply; // Total number of NFTs in the collection
    uint public unrevealedSupply; // Number of unrevealed NFTs

    string private revealURl = "https://bafybeigw5aud3ras5drvhnnxmxaforqrcpu26gy7qyqag2daahq3otzpgq.gateway.ipfscdn.io/notRevealed.json";
    ...
    constructor() ERC721("nokko", "NNFT") {
     ...
     function mintNFT(address to, string memory uri, uint256 nftId, bool isUnrevealed) public payable returns(NFTMetadata memory) {
        totalSupply++;
       
        uint256 nftTokenId = _tokenIds.current();
        
        super._mint(to, nftTokenId);
        ...
        if (isUnrevealed) {
            NFTMetadata memory metadata = NFTMetadata(nftId, uri, false);
            _nftMetadata[nftTokenId] = metadata;
        } else {
            NFTMetadata memory metadata = NFTMetadata(nftId, uri, true);
            _nftMetadata[nftTokenId] = metadata;
        }
    ...
    function mintNFTBatch(address to, BatchNFT[] memory nfts) public payable returns (BatchNFT[] memory) {
        ...
        for (uint256 i = 0; i < nfts.length; i++) {
            ...
            super._mint(to, nftTokenId);
            _setTokenURI(nftTokenId, nft.uri);
            _tokenIds.increment();
    ...
    function revealBatchNFT(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            // require(!_nftMetadata[tokenId].revealed, "Token already revealed");
 ...
 // New function to send payment to the contract address
