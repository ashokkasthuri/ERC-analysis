...
/* 
  ...
  _____ ______ _____   _____ ______ ___  __          
 |_   _|  ____|  __ \ / ____|____  |__ \/_ |   /\    
   | | | |__  | |__) | |        / /   ) || |  /  \   
   | | |  __| |  _  /| |       / /   / / | | / /\ \  
  _| |_| |____| | \ \| |____  / /   / /_ | |/ ____ \ 
 |_____|______|_|  \_\\_____|/_/   |____||_/_/    \_\
...
contract CaliopeProject is ERC721A, ERC2981, OperatorFilterer, Ownable {
    ...
    uint256 public projectSupply = 900;
    uint256 public limitPerAccount = 900;
    uint256 public saleAmount = 900;
    ...
    constructor(address _royaltyReceiver, uint96 _royaltyFraction)
        ERC721A("ORFEO Cultures_Calliope", "CALI")
    {
        setHiddenMetadataUri(
            "https://storage.googleapis.com/calliope_oc/"
        );
    ...
    function publicMint(uint256 quantity) external payable {
        ...
        require(balanceOf(msg.sender) + quantity <= limitPerAccount, "Exceed Max Amount Per Wallet!");

        require(totalSupply() + quantity <= saleAmount,"Not Enough Tokens Left!");

        require(msg.value >= (mintPrice * quantity), "Not Enough Ether Sent");

        require(totalSupply() + quantity <= projectSupply,"Not Enough Tokens");
    ...
    /* WHITELiST MINT ACTIONS */
 
     function whiteListMint(uint256 quantity) external payable {
  
        require(!paused, "The contract is paused!");

        require(whiteListOpen, "WhiteList Sales are Closed!");

        require(whiteList[msg.sender], "You are not a WhiteList Member!");

        require(balanceOf(msg.sender) + quantity <= limitPerAccount, "Exceed Max Amount Per Wallet!");

        require(totalSupply() + quantity <= saleAmount,"Not Enough Tokens Left For WhiteList Sales!");
    ...
    /* OGList MINT ACTIONS */
     ...
     function ogListMint(uint256 quantity) external payable {
  
        require(!paused, "The Contract is Paused!");

        require(ogListOpen, "OGlist Mint is Closed!");

        require(ogList[msg.sender], "You are not a OGList Member!"); 
    ...
    /* CHANGE LIMIT PER ACCOUNT */
    
    function setLimitPerAccount(uint256 _maxsupply) public onlyOwner {
        limitPerAccount = _maxsupply;
    }
    ...
    /* CONTROL THE SALE AMOUNT  */

    function setSaleAmount(uint256 _limitsale) public onlyOwner{
     saleAmount = _limitsale;
    }
    ...
    /* CHANGE PROJECT MAX SUPPLY  */

    function setProjectSupply(uint256 _maxmint) public onlyOwner {
        projectSupply = _maxmint;
    }
    ...
    /* CONTROL PUBLIC MINT ENABLED  */

   function PublicMintEnabled(bool _state) public onlyOwner {
    ...
    /* CONTROL WHILELIST MINT ENABLED  */

   function WhiteListMintEnabled(bool _state) public onlyOwner {
    ...
    /* CONTROL OGLIST MINT ENABLED  */

    function OGListMintEnabled(bool _state) public onlyOwner {
        ogListOpen = _state;
    }
    ...
    /* POPULATE OGLIST WALLETS  */

    function addOGList(address[] calldata addresses) external onlyOwner {
        for(uint256 i = 0; i < addresses.length; i++){
    ...
    /* REMOVE OGLIST WALLETS  */

    function removeOGList(address[] calldata addresses) external onlyOwner {
        for(uint256 i = 0; i < addresses.length; i++){
            ogList[addresses[i]] = true;
        }
    }
   ...
   function setMintCost(uint256 _mintprice) public onlyOwner{
    ...
    /* CONTROL TOKEN URL HIDDEN OR REVEALED  */
...
// =============================================================
//               NEVER DELETE WITHDRAWL FUNCTION
// =============================================================


    /* WITHDRAWL TO OWNERS WALLET */
