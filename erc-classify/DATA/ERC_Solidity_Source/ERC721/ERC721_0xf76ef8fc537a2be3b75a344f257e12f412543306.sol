...
// File: contracts/AquarelleAccessPass.sol
...
contract AquarelleAccessPass is ERC721URIStorage, IERC721Enumerable, ERC721Enumerable, ERC2981, PaymentSplitter, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address contractAddress;
    address syntContractAddress;
    uint public constant mintPrice = 0.03 ether; // 0.03 ETH
    uint public constant maxSupply = 800; // total number of mintable tokens
    uint public constant userLimit = 20; // max number of NFTs per wallet

    constructor(address marketplaceAddress, 
                address deployedSyntContractAddress,
                address[] memory _payees,
                uint256[] memory _shares) ERC721("Aquarelle Access Pass", "AQGP") PaymentSplitter(_payees, _shares) {
        contractAddress = marketplaceAddress;
        syntContractAddress = deployedSyntContractAddress;
    }
    
    function contractURI() public view returns (string memory) {
        return "https://api.stage.adiem.com/synerative/aquarelleaccess";
    }
    ...
    function createToken(string memory IPFStokenURI) public payable returns (uint) {
        if (checkIfAddressOwnsAnyNFT(syntContractAddress, msg.sender) != true){
            require(msg.value >= mintPrice, "Mint price is 0.03 ETH."); 
        }
