// File: contracts/v4/platform/memberships/LiveArtXCard.sol
    ...
    ERC2981Upgradeable
{
     ...
     * Note: If tokenId is 0, then user is not an xCard holder
     */
    function isXCardHolder(uint24 tokenId) public view returns (bool) {
        if (tokenId != 0) {
            address xCardOwner = ownerOf(tokenId);
            if (xCardOwner != tx.origin) {
                revert CustomErrors.NotXCardOwner();
            }
            return true;
        }
        return false;
...
// File: contracts/v4/platform/memberships/IV1Memberships.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
