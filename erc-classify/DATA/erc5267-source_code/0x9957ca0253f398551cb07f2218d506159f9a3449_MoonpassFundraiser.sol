...
// Source: 0x9957ca0253f398551cb07f2218d506159f9a3449
// Contract Name: MoonpassFundraiser
// Generated on: 2026-05-14 11:56:21


// ============================================================================
// FILE: src/finance/MoonpassFundraiser.sol
// ============================================================================
...
import { IMoonpassFeeCollector } from "../interfaces/finance/IMoonpassFeeCollector.sol";
import { IMoonpassFundraiser, FundraiseInitializer, InvestmentRequest, RefundRequest, ClaimFundsRequest, FundraiseStatus, ClaimRefundRequest, SetFundraiseInfoRequest } from "../interfaces/finance/IMoonpassFundraiser.sol";
import { MoonpassContract } from "../MoonpassContract.sol";
import { MoonpassRoles } from "../auth/MoonpassRoles.sol";
import { MoonpassFeeCollector } from "./MoonpassFeeCollector.sol";

contract MoonpassFundraiser is
  IMoonpassFundraiser,
  NoncesUpgradeable,
  MoonpassFeeCollector,
  MoonpassContract
{

  bytes32 internal constant FUNDRAISE_INITIALIZER_TYPEHASH = 0xaa7766a5eb94bc8577a452c269f8139be836c96522374332c22ebd16235a5ded;
  bytes32 internal constant COMMIT_TYPEHASH = 0x72b24495b7b3be616b5bd4b31a8f76717ab7a2ac5583b7987ee3ca1cdecedeb9;
  bytes32 internal constant CANCEL_AND_REFUND_ALL_TYPEHASH = 0xed61989c1f85fea1eb0d0ff6e3b155375bb0d0cb955492de5cd07df1eeba3ca4;
  bytes32 internal constant INVESTMENT_TYPEHASH = 0x49ef9ca3ac355471bbb4f0fc8634076262a0cca7d2d40c4728c01933ddafa4d3;
  bytes32 internal constant REFUND_TYPEHASH = 0xa6541878aeac52c06758b659a6026920b94665a0f7191c54ef3a2fb629ef83ca;
  bytes32 internal constant CLAIM_FUNDS_TYPEHASH = 0xc97bdcd160ebc2ed9d48a35e8b88724ba24b86f85e062fb4953353e9900dfec1;
  bytes32 internal constant CLAIM_REFUND_TYPEHASH = 0xdb2761ba897a736ba4a8d729a53c9c90f398f61355b00288768b48546dd86a75;
  bytes32 internal constant SET_FUNDRAISE_INFO_TYPEHASH = 0x809d99a5b131260283352f7b74aeec4dcb9a2858151a69b6f462731582572edf;
  ...
  struct MoonpassFundraiserStorage {
    bytes32 _description;
    uint256 _totalRaise;
    uint256 _tokenPrice;
    uint256 _minInvestment;
    uint256 _maxInvestment;
    uint256 _investmentStart;
    uint256 _investmentEnd;
    
    uint256 _investmentsCount;
    uint256 _investmentsTotal;

    uint256 _refundsCount;
    uint256 _refundsTotal;
    uint256 _refundsClaimed;
  ...
  bytes32 private constant MoonpassFundraiserStorageLocation = 0xdca850f6e30dd1db2c0a8d6ff41fbc3aed5465f126202f26180ce17172b812ef;

  function _getMoonpassFundraiserStorage()
    internal
    pure 
    returns (MoonpassFundraiserStorage storage $) 
  {
    assembly {
      $.slot := MoonpassFundraiserStorageLocation
    }
  }

  function initialize(
    FundraiseInitializer calldata data, 
  ...
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
  ...
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    supply = $._totalRaise / $._tokenPrice;
  }
    ...
    )
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    total = $._totalRaise;
    totalInvestments = $._investmentsTotal;
  }

  function fundraiseInfo() 
    external 
    view 
    returns(
      string memory name,
    ...
    )
  {
    ...
    investmentStart = $._investmentStart;
  ...
  {
    (uint256 minInvestment,uint256 maxInvestment) = investmentLimit();
    (,,,uint256 actual) = investmentOf(owner);
    max = maxInvestment < actual ? 0 : maxInvestment - actual;
    min = minInvestment > actual ? minInvestment - actual : actual > minInvestment ? 1 : minInvestment;
  }
  ...
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    count = $._investmentsCount;
    total = $._investmentsTotal;
    actual = total - $._refundsTotal;
  }
  ...
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    total = $._investments[owner].total;
    refunded = $._committed == 2 ? total : $._investments[owner].refunded;
    claimed = $._investments[owner].claimed;
    actual = total - refunded;
  }

  function fundraiseStatus() 
    public 
    view 
    returns(
    ...
    )
  {
    ...
    if(committed == 0) {
      return FundraiseStatus.DRAFT;
    }
    if(committed == 2) {
      return FundraiseStatus.CANCELLED;
    }
    uint256 ts = block.timestamp;
    if(ts < $._investmentStart) {
      return FundraiseStatus.UPCOMING;
    }
    if(ts < $._investmentEnd) {
      return FundraiseStatus.OPEN;
    }
    return FundraiseStatus.ENDED;
  }

  function invest(
    InvestmentRequest calldata request, 
    ...
    whenNotPaused
  {
    ...
    bytes memory data = abi.encode(
      ...
      owner,
      amount,
      _encodeFee(request.feeCollector, msg.value),
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.FUNDRAISER_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _collectFee(request.feeCollector);

    if(fundraiseStatus() != FundraiseStatus.OPEN) {
    ...
    (,,uint256 raise) = investments();

    if(raise + amount > $._totalRaise) {
      revert InvalidRequestError();
    }

    (,,,uint256 current) = investmentOf(owner);
    if(current + amount > $._maxInvestment) {
      revert InvalidRequestError();
    }
    ...
    $._investments[owner].total += amount;
    $._investmentsCount++;
    $._investmentsTotal += amount;

    emit FundraiseInvestment(owner, amount);
  }
    ...
    external
  {
    bytes memory data = abi.encode(
      REFUND_TYPEHASH,
      keccak256(abi.encodePacked(request.owners)),
  ...
  function refund(
    ...
    external
    onlyRole(MoonpassRoles.FUNDRAISER_MANAGER_ROLE)
  {
    _refund(request);
  }
    ...
    internal
  {
    ...
    uint256 len = request.owners.length;
    if(len != request.amounts.length) {
      revert InvalidRequestError();
    }
    ...
    for(uint256 i = 0; i < len; i++) {
      owner = request.owners[i];
      amount = request.amounts[i];
      if(amount == 0) {
        revert InvalidRequestError();
      }
      balance = $._investments[owner].total - $._investments[owner].refunded;
      if(balance < amount) {
        revert InvalidRequestError();
      }
      $._investments[owner].refunded += amount;
      total += amount;

      emit FundraiseRefund(owner, amount);
    }
  ...
  function cancelAndRefundAll(
    RequestSignature calldata signature
  ) 
    external
  {
    bytes memory data = abi.encode(
      CANCEL_AND_REFUND_ALL_TYPEHASH,
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.FUNDRAISER_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
  ...
  function _cancelAndRefundAll() 
    internal
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    $._committed = 2;
    $._refundsCount = $._investmentsCount;
    $._refundsTotal = $._investmentsTotal;

    emit FundraiseCancel();
  }

  function claimFunds(
    ClaimFundsRequest calldata request,
    RequestSignature calldata signature
  ) 
    external
    payable
    ...
    internal
  {
    ...
    token.transfer(request.to, amount);
    
    emit FundraiseFundsClaim(request.to, amount);
  }

  function claimRefund(
    ClaimRefundRequest calldata request,
    RequestSignature calldata signature
  )
    external
    payable
    whenNotPaused
  {
    ...
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    address owner = request.owner;
    (,uint256 refunded, uint256 claimed,) = investmentOf(owner);
    uint256 claimable = refunded - claimed;

    if(claimable == 0) {
      revert InvalidRequestError();
    }

    $._investments[owner].claimed = refunded;
    $._refundsClaimed += claimable;

    IERC20 token = IERC20($._paymentToken);
    token.transfer(owner, claimable);

    emit FundraiseRefundClaim(owner, claimable);
  }
    ...
    internal
  {
    ...
    emit FundraiseCommit();
  }

  function setFundraiseInfo(
    SetFundraiseInfoRequest calldata request, 
    RequestSignature calldata signature
  ) 
    external
  {
    bytes memory data = abi.encode(
      SET_FUNDRAISE_INFO_TYPEHASH,
      keccak256(abi.encode(
        FUNDRAISE_INITIALIZER_TYPEHASH,
        keccak256(bytes(request.data.name)),
        request.data.description,
        request.data.tokenPrice,
        request.data.totalRaise,
        request.data.paymentToken,
        request.data.minInvestment,
        request.data.maxInvestment,
        request.data.investmentStart,
        request.data.investmentEnd
      )),
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.FUNDRAISER_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _setFundraiseInfo(request.data);
  }
  ...
  function _setFundraiseInfo(
    FundraiseInitializer calldata data
  ) 
    internal
  {
    MoonpassFundraiserStorage storage $ = _getMoonpassFundraiserStorage();
    if(fundraiseStatus() != FundraiseStatus.DRAFT
      && $._paymentToken != data.paymentToken
      && $._investmentsCount > 0) {
      revert InvalidRequestError();
    }
    $._name = data.name;
    $._description = data.description;
    $._tokenPrice = data.tokenPrice;
    $._totalRaise = data.totalRaise;
    $._paymentToken = data.paymentToken;
    $._minInvestment = data.minInvestment;
    $._maxInvestment = data.maxInvestment;
    $._investmentStart = data.investmentStart;
    $._investmentEnd = data.investmentEnd;

    emit FundraiseChange();
  }
...
// ============================================================================
// FILE: src/interfaces/IMoonpassContract.sol
// ============================================================================
...
// ============================================================================
// FILE: src/interfaces/finance/IMoonpassFeeCollector.sol
// ============================================================================
...
interface IMoonpassFeeCollector
{
  error FeeCollectionError();
}

// ============================================================================
// FILE: src/interfaces/finance/IMoonpassFundraiser.sol
// ============================================================================
...
// ============================================================================
// FILE: src/auth/MoonpassRoles.sol
// ============================================================================
...
// ============================================================================
// FILE: src/finance/MoonpassFeeCollector.sol
// ============================================================================
...
contract MoonpassFeeCollector is
  IMoonpassFeeCollector
{

  bytes32 public constant FEE_TYPEHASH = 0xb83f1df0f7931c310ae226324b8e56c0c7c00a520f77c023f0c05ee7c2ed4b86;

  function _collectFee(address to)
    internal
  {
    _collectFee(to, msg.value);
  }

  function _collectFee(address to, uint256 amount)
    internal
  {
