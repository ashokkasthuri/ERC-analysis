...
// Source: 0xfe927363e3e13559c4363b2b867513c4ac6ee82e
// Contract Name: ATestingTokenA
// Generated on: 2026-05-14 12:04:16
...
// ============================================================================
// FILE: contracts/ATestingTokenA.sol
// ============================================================================
...
/// @title ATestingTokenA
/// @dev This contract implements the ATestingTokenA, an ERC20 token with additional features.
contract ATestingTokenA is ERC20Capped, ERC20Permit, Pausable, Ownable2Step, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

    /// @notice Creates a new ATestingTokenA contract.
    constructor()
    ERC20("ATestingTokenA", "AAAX")
        // we can tweak this to change the initial owner (instead of the contract deployer account)
    Ownable(msg.sender)
    ERC20Permit("ATestingTokenA")
    ...
    /// @notice Pauses all token and ownership transfers.
    /// @dev This function can only be called by the owner.
    function pause() public onlyRole(DEFAULT_ADMIN_ROLE) {
        _pause();
    }

    /// @notice Unpauses all token and ownership transfers.
    /// @dev This function can only be called by the owner.
    ...
    /// @notice Updates the balance of `from` and `to` and checks if the total supply exceeds the cap.
    ...
    /// @dev Renouncing ownership will leave the contract without an owner, thereby removing any functionality that is only available to the owner.
    ...
    function burn(uint256 value) public onlyRole(BURNER_ROLE) whenNotPaused {
    ...
    /// @param value The amount of tokens to burn.
    function burnFrom(address account, uint256 value) public onlyRole(BURNER_ROLE) whenNotPaused {
        _spendAllowance(account, _msgSender(), value);
        _burn(account, value);
    }

    /// @notice Moves `value` tokens from `from` to `to` using the allowance mechanism. `value` is then deducted from the caller's allowance.
    /// @param from The account to transfer the tokens from.
    /// @param to The account to transfer the tokens to.
    /// @param value The amount of tokens to transfer.
    ...
    function transferOwnership(address newOwner) public virtual override(Ownable2Step) onlyOwner whenNotPaused {
        super.transferOwnership(newOwner);
    }

    /// @notice Accepts ownership of the contract.
    /// @dev Can only be called by the new owner.
    function acceptOwnership() public virtual override(Ownable2Step) whenNotPaused {
