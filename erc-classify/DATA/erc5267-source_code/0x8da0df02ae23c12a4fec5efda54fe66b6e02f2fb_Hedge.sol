...
// Source: 0x8da0df02ae23c12a4fec5efda54fe66b6e02f2fb
// Contract Name: Hedge
// Generated on: 2026-05-14 12:01:38
...
// ============================================================================
// FILE: contracts/Token/Hedge.sol
// ============================================================================
...
// Define the Hedge contract, which inherits various OpenZeppelin contracts
contract Hedge is
    ERC20Upgradeable,
    ...
    UUPSUpgradeable
{
    address public tokenMarketPlace;

    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
    bytes32 public constant SUBADMIN_ROLE = keccak256("SUB_ADMIN");
    ...
    // Modifier to restrict access to only the Token Marketplace contract
    modifier onlyTokenMarketPlace() {
        require(
            address(0) != tokenMarketPlace,
            "Address must be a valid address"
        );
        if (
            msg.sender != tokenMarketPlace &&
            !hasRole(SUBADMIN_ROLE, msg.sender)
        ) {
            revert("Only the TokenMarketPlace contract and admin is allowed");
        }
        _;
    }

    /**
     * @dev Creates a snapshot of the token's state.
     */
    function snapshot() external onlyRole(SUBADMIN_ROLE) {
        _snapshot();
    }
     ...
     */
    function pause() external onlyRole(SUBADMIN_ROLE) {
        _pause();
    }
     ...
     */
    function unpause() external onlyRole(SUBADMIN_ROLE) {
        _unpause();
    }

    /**
     * @dev Updates the Token Marketplace contract address.
     *
     * @param _tokenMarketPlace The new Token Marketplace contract address.
     */
    function updateTokenMarketPlace(
        address _tokenMarketPlace
    ) external onlyRole(SUBADMIN_ROLE) {
        require(address(0) != _tokenMarketPlace);
        tokenMarketPlace = _tokenMarketPlace;
    }

    /**
     * @dev Mints new tokens (only accessible by the Token Marketplace contract).
     *
     * @param to The recipient address for the minted tokens.
     * @param amount The amount of tokens to mint.
     */
    function mint(address to, uint256 amount) external onlyTokenMarketPlace {
        _mint(to, amount);
    }

    // Override of the _beforeTokenTransfer function to include pausing functionality
