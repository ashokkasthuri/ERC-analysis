...
// Source: 0x4828909cc149142a4263c015678bd51faf062efe
// Contract Name: GimbutisTokenGold
// Generated on: 2026-05-14 11:57:34
...
// ============================================================================
// FILE: contracts/GimbutisTokenGold.sol
// ============================================================================
    ...
    ERC20PermitUpgradeable
{
    ...
    // how many tokens = 1 gold
    uint256 public goldRatio;
    ...
    function initialize(
        ...
        for (uint i = 0; i < __gap.length; i++) {
            __gap[i] = 0;
        }

        __Ownable_init();
        __ERC20_init("GimbutisToken", "GAUG");
        __ERC20Permit_init("GimbutisToken");
        emit Initialized(msg.sender, block.number);
    }

    modifier onlyActive() {
        require(toggleStatus, "GAUG: contract is not available right now ");
        _;
    }
    ...
    modifier onlyASWM() {
        require(msg.sender == aswmAddress, "GAUG: only aswm");
        _;
    }
    ...
    function setGoldRatio(uint256 _goldRatio) external onlyAdmin {
        goldRatio = _goldRatio;
        require(_goldRatio >= 100, "GAUG: SiverRatio less than 100");
        emit SetGoldRatio(_goldRatio);
    }
    ...
    function setSellPercentage(uint256 _sellPercentage) external onlyAdmin {
        sellPercentage = _sellPercentage;
    }

    function getSellPercentage() public view returns (uint256) {
        return sellPercentage;
    }

    function setBuyPercentage(uint256 _buyPercentage) external onlyAdmin {
        buyPercentage = _buyPercentage;
    }

    function getBuyPercentage() public view returns (uint256) {
     ...
     */ //XXX, uint256 expectedGAUG
    function buy(address token, uint256 amount) external onlyActive {
        require(
            token != address(this),
            "GAUG: Buy GAUG for GAUG is prohibited"
        );
        require(amount > 0, "GAUG: buy amount is null");
        uint256 _userBalance = IERC20Upgradeable(token).balanceOf(msg.sender);
        require(_userBalance >= amount, "GAUG: Invalid user balance for buy");

        if (token == usdcAddress || token == usdtAddress) {
            _buyTokenWithUsd(token, msg.sender, amount);
        } else {
            address _pairAddress = IUniswapV2Factory(factoryAddress).getPair(
                token,
                usdcAddress
            );
            require(
                _pairAddress != address(0),
                "GAUG: USDC pair with this token not exist"
            );
    ...
    /**
     * @notice Function to purchase tokens using ERC-20 permit.
     * @param token ERC20 token contract address to purchase tokens from.
     * @param amount The amount of tokens to purchase.
     * @param deadline UNIX timestamp after which the permit will expire.
     * @param v Component 'v' of the ECDSA signature, which allows recovering the correct signature.
     * @param r Component 'r' of the ECDSA signature, representing the x-coordinate of the curve point.
     * @param s Component 's' of the ECDSA signature, representing the y-coordinate of the curve point.
     */
    ...
    function addReserve(address _holder, uint256 _amount) external onlyASWM {
        require(
            reserves[_holder] == 0,
            "GAUG: already have redeem with this address"
        );
        require(
            super.balanceOf(_holder) >= _amount && minReserveAmount < _amount,
            "GAUG: Invalid amount for redeem"
        );
    ...
    function releaseReserve(address _holder) external onlyASWM {
        require(reserves[_holder] != 0, "GAUG: Redeem is null");
    ...
    function getTokenPrice() public view returns (uint256 price) {
        ...
        (, int256 _price, , , ) = aggregator.latestRoundData(); //an ounce of gold with decimals
        require(_price > 0, "GAUG: Aggregator price less or equal 0");
        price =
            (uint256(_price) *
                (10 ** (18 - aggregator.decimals())) *
                goldRatio) /
            100;
    }

    /**
     * @notice Functon to buy gaug tokens with usdc/usdt
     * @param usdToken USDC/USDT contract address
     * @param buyer sender address
     * @param amount amount
     */
    function _buyTokenWithUsd(
        address usdToken,
        address buyer,
        uint256 amount
    ) private {
        ...
        uint256 adjustedAmount = tokenPrice +
            ((tokenPrice * getBuyPercentage()) / 100);
        uint256 _gaugAmount = (amount * 1e30) / adjustedAmount;

        require(
            _gaugAmount >= minBuyAmount,
            "GAUG: Too small amount to buy minimum GAUG"
        );
        require(
            super.balanceOf(address(this)) >= _gaugAmount,
            "GAUG: Not enough GAUG on balance"
        );
        if (usdToken == usdcAddress) {
            IERC20Upgradeable(usdToken).safeTransferFrom(
                buyer,
                address(this),
                amount
            );
        ...
        _transfer(address(this), buyer, _gaugAmount);
    }

    /**
     * @notice Functon to buy gaug tokens with ERC20
    ...
    function _buyToken(
        ...
        uint256 _gaugUsdPrice = getTokenPrice(); // 18 decimals
        uint256 adjustedPrice = _gaugUsdPrice +
            ((_gaugUsdPrice * getBuyPercentage()) / 100);
        uint256 _erc20UsdPrice = _getERC20Price(_pairAddress, _token, amount);
        uint256 _gaugAmount = (_erc20UsdPrice * 1e18) / adjustedPrice;

        require(
            _gaugAmount >= minBuyAmount,
            "GAUG: Too small GAUG amount for buy"
        );
        require(
            super.balanceOf(address(this)) >= _gaugAmount,
            "GAUG: Not enough GAUG tokens on contract balance"
        );
        ...
        IUniswapRouterV2(routerAddress)
        ...
        _transfer(address(this), _sender, _gaugAmount);
    }
    ...
    function _getERC20Price(
        ...
        (uint reserve0, uint reserve1, ) = IUniswapV2Pair(_pairAddress)
            .getReserves();
        uint256 usdcDecimalsPlus = 10 **
            (18 - IERC20MetadataUpgradeable(usdcAddress).decimals());
        uint256 erc20DecimalsPlus = 10 **
            (18 - IERC20MetadataUpgradeable(_token).decimals());
        if (IUniswapV2Pair(_pairAddress).token0() == usdcAddress) {
            return
                (amount * reserve0 * usdcDecimalsPlus) /
                (reserve1 * erc20DecimalsPlus);
        } else {
            return
                (amount * reserve1 * usdcDecimalsPlus) /
                (reserve0 * erc20DecimalsPlus);
        }
    }
     ...
     * @param _gaugAmount amount of GAXG tokens
     */
    function sell(uint256 _gaugAmount, address token) external onlyActive {
        require(
            super.balanceOf(msg.sender) >= _gaugAmount,
            "GAUG: Too small user balance"
        );
        require(
            token == usdcAddress || token == usdtAddress,
            "GAUG: Sell for USDC or USDT only"
        );
        ...
        uint256 adjustedPrice = uint256(_price) +
            ((uint256(_price) * getSellPercentage()) / 100);

        uint256 _usdAmount = (_gaugAmount * adjustedPrice) /
            10 ** (aggregator.decimals() + 12);

        require(
            IERC20Upgradeable(token).balanceOf(address(this)) >= _usdAmount,
            "GAUG: Too small contract balance for sell"
        );
        ...
        _transfer(msg.sender, address(this), _gaugAmount);
    }
    ...
    function claim(address token, uint256 amount) external onlyAdmin {
        require(
            ...
            "GAUG: Too small contract balance to claim"
        );
