...
// Source: 0xe8bd11a4748411b9d84c52efa8fef399a44d0225
// Contract Name: MoonpassVault
// Generated on: 2026-05-14 11:54:57


// ============================================================================
// FILE: src/finance/MoonpassVault.sol
// ============================================================================
...
import { IMoonpassVault, NativeWithdrawRequest, TokenWithdrawRequest } from "../interfaces/finance/IMoonpassVault.sol";
import { MoonpassContract } from "../MoonpassContract.sol";
import { MoonpassRoles } from "../auth/MoonpassRoles.sol";

contract MoonpassVault is
  IMoonpassVault,
  NoncesUpgradeable,
  MoonpassContract
{
  ...
  bytes32 internal constant NATIVE_WITHDRAW_TYPEHASH = keccak256("NativeWithdrawRequest(address to,uint256 amount,uint256 value,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant TOKEN_WITHDRAW_TYPEHASH = keccak256("TokenWithdrawRequest(address token,address to,uint256 amount,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  function withdraw(
    NativeWithdrawRequest calldata request,
    ...
    whenNotPaused
  {
    bytes memory data = abi.encode(
      NATIVE_WITHDRAW_TYPEHASH,
      request.to,
      request.amount,
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.VAULT_TRUSTEE_ROLE, _signerFrom(data, signature));

    _useCheckedNonce(_msgSender(), signature.nonce);

    _withdraw(TokenLibrary.NATIVE, request.to, request.amount);
  }

  function withdraw(
    TokenWithdrawRequest calldata request,
    RequestSignature calldata signature
  ) 
    external
    whenNotPaused
  {
    ...
    _withdraw(Token.wrap(request.token), request.to, request.amount);
  }
    ...
    internal
  {
    ...
    emit VaultWithdraw(Token.unwrap(token), _msgSender(), to, amount);
  }
}
...
interface IMoonpassVault
{
  ...
  function withdraw(NativeWithdrawRequest calldata request, RequestSignature calldata signature) external;

  function withdraw(TokenWithdrawRequest calldata request, RequestSignature calldata signature) external;

  event VaultDeposit(address sender, uint256 amount);

  event VaultWithdraw(address indexed token, address account, address indexed to, uint256 amount);
}

struct NativeWithdrawRequest {
  address to;
  uint256 amount;
}
...
library MoonpassRoles {
  ...
  bytes32 public constant AUTH_MANAGER_ROLE = keccak256("AUTH_MASTER_ROLE");
