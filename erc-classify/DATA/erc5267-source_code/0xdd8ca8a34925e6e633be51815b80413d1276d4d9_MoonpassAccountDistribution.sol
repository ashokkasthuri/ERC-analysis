...
// Source: 0xdd8ca8a34925e6e633be51815b80413d1276d4d9
// Contract Name: MoonpassAccountDistribution
// Generated on: 2026-05-14 11:56:15


// ============================================================================
// FILE: src/token-distribution/account/MoonpassAccountDistribution.sol
// ============================================================================
...
import { IMoonpassAccountDistribution, AddAccountAllocationsRequest, RemoveAccountAllocationsRequest } from "../../interfaces/token-distribution/account/IMoonpassAccountDistribution.sol";
...
contract MoonpassAccountDistribution is 
  IMoonpassAccountDistribution,
  MoonpassTokenDistribution
{
  using TokenLibrary for Token;
  
  bytes32 internal constant RESET_ALLOCATIONS_TYPEHASH = keccak256("ResetAccountAllocationsRequest(RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant ADD_ALLOCATIONS_TYPEHASH = keccak256("AddAccountAllocationsRequest(uint256[] tokenIds,uint256[] deposits,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant REMOVE_ALLOCATIONS_TYPEHASH = keccak256("RemoveAccountAllocationsRequest(uint256[] tokenIds,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  struct MoonpassAccountDistributionStorage {
    uint256 _decimalsGap;
    address[] _allAccounts;
  }

  bytes32 private constant MoonpassAccountDistributionStorageLocation = 0xa495bd9add43d5026a955f7efd18c685ac70f093f7320b8c2ee6a928ffa95ee8;

  function _getMoonpassAccountDistributionStorage()
    internal
    pure 
    returns (MoonpassAccountDistributionStorage storage $) 
  {
    assembly {
      $.slot := MoonpassAccountDistributionStorageLocation
    }
  }
  ...
  function initialize(
    ...
    public
  {
    __TokenDistribution_init(data, moonpass);
  }
  ...
  {
    uint256 allocation = _accountToAllocation(account);
    balanceAmount = _balanceOf(allocation);
    depositAmount = _resolveDeposit(allocation);
    releasedAmount = _releasedAt(allocation, block.timestamp);
    withdrawableAmount = _withdrawableAt(allocation, block.timestamp);
    withdrawnAmount = _resolveWithdrawn(allocation);
  }
  ...
  {
    amount = _resolveDeposit(_accountToAllocation(account));
  }
  ...
  {
    amount = _balanceOf(_accountToAllocation(account));
  }
  ...
  {
    amount = _resolveWithdrawn(_accountToAllocation(account));
  }
  ...
  {
    amount = _withdrawableAt(_accountToAllocation(account), block.timestamp);
  }
  ...
  {
    amount = _withdrawableAt(_accountToAllocation(account), timestamp);
  }
  ...
  {
    amount = _releasedAt(_accountToAllocation(account), block.timestamp);
  }
  ...
  {
    amount = _releasedAt(_accountToAllocation(account), timestamp);
  }
  ...
  function withdraw(
    ...
    whenNotPaused
    whenCommitted
    onlyRole(MoonpassRoles.ACCOUNT_DISTRIBUTION_WITHDRAW_ROLE)
  {
    _withdraw(account, to, amount);
  }
  ...
  function withdraw(
    ...
    onlyRole(MoonpassRoles.ACCOUNT_DISTRIBUTION_WITHDRAW_ROLE)
  {
    _withdraw(account, to, _withdrawableAt(_accountToAllocation(account), block.timestamp));
  }

  function addAllocations(
    AddAccountAllocationsRequest calldata request,
    ...
    whenNotCommitted
  {
    bytes memory data = abi.encode(
      ADD_ALLOCATIONS_TYPEHASH,
      keccak256(abi.encodePacked(request.accounts)),
    ...
    _addAllocations(request.accounts, request.deposits);
  }
  ...
  {
    _addAllocations(accounts, deposits);
  }

  function removeAllocations(
    RemoveAccountAllocationsRequest calldata request,
    RequestSignature calldata signature
  )
    external
    whenNotCommitted
  {
    ...
    _removeAllocations(request.accounts);
  }
  ...
  function _onTokenChange(Token newToken)
    ...
    override
  {
    MoonpassAccountDistributionStorage storage $ = _getMoonpassAccountDistributionStorage();
    ...
    private
  {
    ...
    uint256 allocation = _accountToAllocation(account);
    uint256 availableAmount = _withdrawableAt(allocation, block.timestamp);
    if(availableAmount < amount) {
      revert InsufficientFundsError(account, amount, availableAmount);
    }
    MoonpassTokenDistributionStorage storage $$ = _getMoonpassTokenDistributionStorage();
    $$._totalWithdrawals++;
    $$._totalWithdrawalsValue += amount;
    $$._withdrawals[allocation] += amount;

    $$._token.transfer(to, amount);

    emit AccountAllocationWithdraw(account, to, amount);
  }
    ...
    internal
  {
    ...
    for(uint256 i = 0; i < accounts.length; i++) {
      deposit = (deposits[i / DEPOSIT_SLOTS] >> (i % DEPOSIT_SLOTS * DEPOSIT_BITS) & DEPOSIT_MASK) * $._decimalsGap;
      ...
      $$._allocations[_accountToAllocation(account)] = deposit;

      total++;
      amount += deposit;
      $._allAccounts.push(account);

      emit AccountAllocationCreate(account, deposit);
    }

    $$._totalDeposits += total;
    $$._totalDepositsValue += amount;
  }

  function _accountToAllocation(
    address account
  )
    internal
    pure
  ...
  {
    allocation = uint256(uint160(account));
  }
    ...
    internal
  {
    ...
    for(uint256 i = 0; i < accounts.length; i++) {
      ...
      allocation = _accountToAllocation(account);
      deposit = _resolveDeposit(allocation);
      if(deposit == 0) continue;

      $$._allocations[allocation] = 0;

      total++;
      amount += deposit;

      emit AccountAllocationRemove(account);
    }
    ...
    internal
  {
    ...
    for(uint i = 0; i < $._allAccounts.length; i++) {
      $$._allocations[_accountToAllocation($._allAccounts[i])] = 0;
    }
...
// ============================================================================
// FILE: src/token-distribution/MoonpassTokenDistribution.sol
// ============================================================================
