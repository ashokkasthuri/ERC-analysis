...
// Source: 0x7a569bff9f87b526b39331ca870516c1d93c0fda
// Contract Name: Mullet
// Generated on: 2026-05-14 11:59:50
...
contract LPAdapter is ILPAdapter, Context, Ownable {
  using SafeERC20 for IERC20;
  ...
  uint256 immutable LP_COLLECT_FREQ;
  uint256 immutable LP_REBALANCE_FREQ;
  uint8 immutable LP_REBALANCE_PERC;

  IWETH immutable WETH;
  INonfungiblePositionManager immutable LP_MANAGER;
  IUniswapV3Factory immutable V3_FACTORY;
  IV3SwapRouter immutable ROUTER;
  ...
  constructor(
    ...
    address _v3Factory,
    address _swapRouter,
    uint256 _collectFrequency,
    uint256 _lpRebalFreq,
    uint8 _lpRebalPerc
  ) Ownable(_msgSender()) {
    ADMIN = _admin;
    WETH = IWETH(_weth);
    LP_MANAGER = INonfungiblePositionManager(_lpManager);
    V3_FACTORY = IUniswapV3Factory(_v3Factory);
    ROUTER = IV3SwapRouter(_swapRouter);
    LP_COLLECT_FREQ = _collectFrequency;
    LP_REBALANCE_FREQ = _lpRebalFreq;
    LP_REBALANCE_PERC = _lpRebalPerc;
    lastCollect = block.timestamp;
    lastRebalance = block.timestamp;
  }

  function setLpId(uint256 _tokenId) external onlyOwner {
    require(lpPositionId == 0, 'F');
    _launchedAt = block.timestamp;
    lpPositionId = _tokenId;
    (, , address _t0, address _t1, uint24 _f, , , , , , , ) = LP_MANAGER
      .positions(lpPositionId);
    _token0 = _t0;
    _token1 = _t1;
    _fee = _f;
    _liquidityPool = V3_FACTORY.getPool(_token0, _token1, _fee);
    LP_MANAGER.transferFrom(_msgSender(), address(this), _tokenId);
  }

  function removePositionAfterInactivity() external {
    // can only ever withdraw after 30 days of no fee collection activity
    // this would only happen if the token died which is pretty much
    // impossible because mullets are about to rule the fucking world
    require(block.timestamp > lastCollect + 30 days, 'I');
    uint256 _tokenId = lpPositionId;
    lpPositionId = 0;
    LP_MANAGER.transferFrom(address(this), ADMIN, _tokenId);
  }
  ...
  function donateLiquidity(
    ...
    require(lpPositionId > 0, 'ILP');
    require(_token == _token0 || _token == _token1, 'P');
    require(msg.value > 0 && _amountTokens > 0, 'DA');
    uint256 _balBeforeTokens = IERC20(_token).balanceOf(address(this));
    uint256 _balBeforeETH = address(this).balance - msg.value;
    uint256 _balBeforeWETH = WETH.balanceOf(address(this));
    WETH.deposit{ value: msg.value }();
    IERC20(_token).safeTransferFrom(_msgSender(), address(this), _amountTokens);
    uint256 _amountAdd0 = _token0 == _token ? _amountTokens : msg.value;
    uint256 _amountAdd1 = _token0 == _token ? msg.value : _amountTokens;
    uint256 _amount0Min = _token0 == _token ? _amountTokensMin : _amountWethMin;
    uint256 _amount1Min = _token0 == _token ? _amountWethMin : _amountTokensMin;
    IERC20(_token0).safeIncreaseAllowance(address(LP_MANAGER), _amountAdd0);
    IERC20(_token1).safeIncreaseAllowance(address(LP_MANAGER), _amountAdd1);
    LP_MANAGER.increaseLiquidity(
    ...
    TransferHelper.safeApprove(_token0, address(LP_MANAGER), 0);
    TransferHelper.safeApprove(_token1, address(LP_MANAGER), 0);
    uint256 _balFinalTokens = IERC20(_token).balanceOf(address(this));
    if (_balFinalTokens > _balBeforeTokens) {
      IERC20(_token).safeTransfer(
        _msgSender(),
        _balFinalTokens - _balBeforeTokens
      );
    }
    uint256 _balFinalWETH = WETH.balanceOf(address(this));
    if (_balFinalWETH > _balBeforeWETH) {
      WETH.withdraw(_balFinalWETH - _balBeforeWETH);
      (bool _s, ) = payable(_msgSender()).call{
        value: _balFinalWETH - _balBeforeWETH
      }('');
      require(_s, 'SE');
      require(address(this).balance >= _balBeforeETH, 'VE');
    }
  }

  function _checkAndProcessLPFees() internal {
    if (lpPositionId == 0) {
      return;
    }
    if (block.timestamp < lastCollect + LP_COLLECT_FREQ) {
      return;
    }
    (, , , , , , bool _unlocked) = IUniswapV3Pool(_liquidityPool).slot0();
    if (!_unlocked) {
      return;
    }
    (uint256 _amount0, uint256 _amount1) = _collect();
    if (_amount0 == 0 && _amount1 == 0) {
      return;
    }
    address _token = _token0 == address(WETH) ? _token1 : _token0;
    uint256 _amountToken = _token0 == address(WETH) ? _amount1 : _amount0;
    if (_amountToken > 0) {
      _swap(_token, address(WETH), _amountToken, 0, address(this));
    }
    uint256 _wethBal = WETH.balanceOf(address(this));
    if (_wethBal > 0) {
      WETH.withdraw(_wethBal);
    }
    uint256 _adminETH = address(this).balance / 10; // 10%
    if (_adminETH > 0) {
      (bool _s, ) = payable(ADMIN).call{ value: _adminETH }('');
      require(_s);
    }
  }

  function _checkAndRebalanceLp() internal {
    if (lpPositionId == 0) {
      return;
    }
    if (block.timestamp < lastRebalance + LP_REBALANCE_FREQ) {
      return;
    }
    if (lastRebalance > lastCollect || block.timestamp == lastCollect) {
      return;
    }
    (, , , , , , bool _unlocked) = IUniswapV3Pool(_liquidityPool).slot0();
    if (!_unlocked) {
    ...
    _withdrawBuyAndBurnFromLpPosition();
  }

  function _collect() internal returns (uint256 _amount0, uint256 _amount1) {
    lastCollect = block.timestamp;
    (_amount0, _amount1) = LP_MANAGER.collect(
  ...
  function _withdrawBuyAndBurnFromLpPosition() internal {
    lastRebalance = block.timestamp;
    (, , , , , , , uint128 _liquidity, , , , ) = LP_MANAGER.positions(
      lpPositionId
    );
    LP_MANAGER.decreaseLiquidity(
      INonfungiblePositionManager.DecreaseLiquidityParams({
        tokenId: lpPositionId,
        liquidity: (_liquidity * LP_REBALANCE_PERC) / 100,
        amount0Min: 0,
        amount1Min: 0,
        deadline: block.timestamp
      })
    ...
    (uint256 _amount0, uint256 _amount1) = _collect();
    address _token = _token0 == address(WETH) ? _token1 : _token0;
    uint256 _amountWETH = _token0 == address(WETH) ? _amount0 : _amount1;
    if (_amountWETH > 0) {
      _swap(address(WETH), _token, _amountWETH, 0, address(this));
    }
    uint256 _tokensToBurn = IERC20(_token).balanceOf(address(this));
    if (_tokensToBurn > 0) {
      IBurnable(_token).burn(_tokensToBurn);
    }
  }
...
// ============================================================================
// FILE: contracts/Mullet.sol
// ============================================================================

// Project Mullet ($MULLET)
// Business in the front, party in the back
// https://projectmullet.com
...
import './GameTheoryRewards.sol';
import './LPAdapter.sol';

contract Mullet is IBurnable, OFT, ERC20Permit, LPAdapter {
  string constant NAME = 'Project Mullet';
  string constant SYMBOL = 'MULLET';
  string constant DESCRIPTION = 'Business in the front, party in the back';

  uint256 constant LAUNCH_COOLDOWN = 3 minutes;
  uint256 constant FIRST_CLAIM_DELAY = 1 days;
  uint256 constant MINIMUM_LOCKUP_TIME = 3 days;
  uint256 constant LP_COLLECT_FREQUENCY = 60 minutes;
  uint256 constant LP_REBALANCE_FREQUENCY = 1 days;
  uint8 constant LP_REBALANCE_PERCENTAGE = 3; // %

  GameTheoryRewards public immutable GAME_THEORY_REWARDS;

  mapping(address => bool) _excludeRewards;
  ...
  constructor(
    ...
    OFT(NAME, SYMBOL, _layerZeroEndpoint, _oftAdm)
    ERC20Permit(NAME)
    LPAdapter(
      _oftAdm,
      _weth,
      ...
      _v3Factory,
      _swapRouter,
      LP_COLLECT_FREQUENCY,
      LP_REBALANCE_FREQUENCY,
      LP_REBALANCE_PERCENTAGE
    )
  {
    GAME_THEORY_REWARDS = new GameTheoryRewards(
      FIRST_CLAIM_DELAY,
      MINIMUM_LOCKUP_TIME
    );
    ...
    _excludeRewards[address(0xdead)] = true;
    _excludeRewards[_v3Pair(_v3Factory, address(this), _weth, 100)] = true;
    _excludeRewards[_v3Pair(_v3Factory, address(this), _weth, 500)] = true;
    _excludeRewards[_v3Pair(_v3Factory, address(this), _weth, 3000)] = true;
    _excludeRewards[_v3Pair(_v3Factory, address(this), _weth, 10000)] = true;
  ...
  function _update(
    ...
    _checkAndProcessLPFees();
    _checkAndRebalanceLp();
    if (address(this).balance > 0) {
      GAME_THEORY_REWARDS.depositRewards{ value: address(this).balance }();
    }
    uint256 _fee = _checkAndAddTempSniperFee(_from, _amount);
    if (_fee > 0) {
      super._update(_from, ADMIN, _fee);
      _afterTokenTransfer(_from, ADMIN, _fee);
    }
    super._update(_from, _to, _amount - _fee);
    _afterTokenTransfer(_from, _to, _amount - _fee);
  }

  function _checkAndAddTempSniperFee(
    address _from,
    uint256 _amount
    ...
    if (
      lpPositionId > 0 &&
      _amount > 0 &&
      _from == _liquidityPool &&
      block.timestamp < _launchedAt + LAUNCH_COOLDOWN
    ) {
      _fee = _amount / 5; // 20%
      _fee = _fee == 0 ? _amount : _fee;
    }
  }
  ...
  function _v3Pair(
    ...
    return PoolAddress.computeAddress(_factory, _key);
  }
  ...
  function _afterTokenTransfer(
    ...
    if (!_excludeRewards[_from]) {
      require(GAME_THEORY_REWARDS.isUnlocked(_from), 'LK');
      GAME_THEORY_REWARDS.update(_from, _amount, true);
    }
    if (!_excludeRewards[_to]) {
      GAME_THEORY_REWARDS.update(_to, _amount, false);
    }
  }
}
