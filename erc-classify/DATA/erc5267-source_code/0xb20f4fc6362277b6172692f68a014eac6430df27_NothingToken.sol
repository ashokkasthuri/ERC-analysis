...
// File: contracts/NothingToken.sol
...
contract NothingToken is ERC20, ERC20Permit {
    ...
    error ERC20RedemptionTokenVerifySign(
        address sender,
        uint256 airdrop_id,
        uint256 _value,
        address to
    ...
    constructor(address airdrop, address fluidity ,address _pledge ,uint256 airdrop_value , uint256 fluidity_value)
        ERC20("Cheif Nothing Officer", "Nothing", 10000000000)
        ERC20Permit("Cheif Nothing Officer")
    {
        airdrop_addr = airdrop;
        fluidity_addr = fluidity;
        pledge_addr = _pledge;
        _update(address(0), airdrop_addr,  airdrop_value * 10 ** decimals());
        _update(address(0), fluidity_addr, fluidity_value * 10 ** decimals());
    }
    ...
    function airdrop_token(
        ...
        require(address_all.length > 0 && address_all.length == values.length , "param different lengths");
    ...
    function claim_token(
        ...
        require(_airdrops[airdrop_id] == false , "airdrop_id used");
    ...
    function claim_eth(
        ...
        require(_airdrops_eth[airdrop_eth_id] == false , "airdrop_id used");

        uint256 fromBalance = balanceEth();
        if (fromBalance < value) {
            revert ERC20EthTransferInsufficientBalance(address(this), fromBalance, value,msg.sender);
        }
    ...
    function redemption(
        ...
        require(_redemption[redemption_id] == false, "airdrop_id used");
        ...
        if (signer != pledge_addr) {
            revert ERC20RedemptionTokenVerifySign(pledge_addr,redemption_id,value,msg.sender);
        }
        ...
        emit ERC20RedemptionToken(pledge_addr,redemption_id, value, msg.sender);
