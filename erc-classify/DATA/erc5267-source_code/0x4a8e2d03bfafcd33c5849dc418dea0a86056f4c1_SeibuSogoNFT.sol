...
// Source: 0x4a8e2d03bfafcd33c5849dc418dea0a86056f4c1
// Contract Name: SeibuSogoNFT
...
// ============================================================================
// FILE: contracts/SeibuSogoNFT.sol
// ============================================================================
...
contract SeibuSogoNFT is Initializable, ERC721URIStorageUpgradeable, Ownable2StepUpgradeable,
         EIP712Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable{
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter internal _tokenAmount; // start from 1
    ...
    event AdminDropEvent(address indexed _to, uint256 indexed _tokenId, string _uri);

    function initialize() initializer public {
        __ERC721_init("NFT PRODUCED by SEIBU SOGO", "SEIBU SOGO");
        ...
        m_quantity = 100;   // max quantity
        m_royalty = 1000;   // 10%
        firstSaleFee = 800; // 0.08 = 8%
        totalSupply = 10000;
        units = 10000;
        decimal = 10 ** 18;
    ...
    function mintToken(NFTVoucher calldata voucher) external payable  nonReentrant{
        ...
        require(signer == nftSigner, "Signature invalid or unauthorized");
    ...
    function adminDrop(address[] calldata _to, uint256[] calldata _tokenId, string[] calldata _uri) external {
        require(msg.sender == admin, "Only admin can call this function");
        require(_to.length == _tokenId.length, "Invalid length");
        require(_to.length == _uri.length, "Invalid length");

        for (uint256 i = 0; i < _to.length; i++) {
            _safeMint(_to[i], _tokenId[i]);
            _setTokenURI(_tokenId[i], _uri[i]);

            emit AdminDropEvent(_to[i], _tokenId[i], _uri[i]);
        }
    }
    ...
    function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
        ...
        keccak256("NFTVoucher(address creator,address recipient,uint256 tokenId,uint256 royalty,uint256 price,uint256 nonce,string uri)"),
        voucher.creator,
        voucher.recipient,
        voucher.tokenId,
        voucher.royalty,
