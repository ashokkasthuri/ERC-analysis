...
// Source: 0x56abf7334e2b4c2ec925a59cb2994a1a7632ecc2
// Contract Name: StUSR
// Generated on: 2026-05-14 12:04:40
...
// ============================================================================
// FILE: contracts/interfaces/IStUSR.sol
// ============================================================================
...
/**
 * @title StUSR Contract
 * @dev StUSR is an ERC20-like token with elastic (rebasing) balance.
 */
contract StUSR is IStUSR, ERC20PermitUpgradeable {
    using Math for uint256;
    using SafeERC20 for IERC20Metadata;

    IERC20Metadata public usr;
    ...
    function initialize(
        ...
        usr = IERC20Metadata(_usrAddress);
        if (usr.decimals() != 18) revert InvalidUnderlyingTokenDecimals();
    }

    /**
     * @dev Retrieves the total supply of StUSR tokens, denominated in USR.
     */
    function totalSupply() public view override(IStUSR, ERC20Upgradeable) returns (uint256 usrAmount) {
        return _totalUSR();
    }

    /**
     * @dev Retrieves the balance of StUSR tokens for a specific account, denominated in USR.
     */
    function balanceOf(address _account) public view override returns (uint256 usrAmount) {
        return convertToUSR(sharesOf(_account));
    }

    /**
     * @dev Overrides the internal spend allowance function to handle USR amount.
     */
    function _spendAllowance(address _owner, address _spender, uint256 _shares) internal override {
        uint256 usrAmount = convertToUSR(_shares);
        super._spendAllowance(_owner, _spender, usrAmount);
    }

    function transfer(address _to, uint256 _usrAmount) public override returns (bool isSuccess) {
        uint256 shares = convertToShares(_usrAmount);
        if (super.transfer(_to, shares)) {
            emit TransferUnderlying(msg.sender, _to, _usrAmount);
            return true;
        } else {
            return false;
        }
    ...
    function transferShares(address _to, uint256 _shares) external override returns (bool isSuccess) {
        uint256 usrAmount = convertToUSR(_shares);
        if (super.transfer(_to, _shares)) {
            emit TransferUnderlying(msg.sender, _to, usrAmount);
            return true;
        } else {
            return false;
        }
    ...
    function transferFrom(
        ...
        uint256 shares = convertToShares(_usrAmount);
        if (super.transferFrom(_from, _to, shares)) {
            emit TransferUnderlying(_from, _to, _usrAmount);
            return true;
        } else {
            return false;
        }
    ...
    function transferSharesFrom(
        ...
        uint256 usrAmount = convertToUSR(_shares);
        if (super.transferFrom(_from, _to, _shares)) {
            emit TransferUnderlying(_from, _to, usrAmount);
            return true;
        } else {
            return false;
        }
    ...
    /*
    * @dev Retrieves the total number of StUSR shares.
    */
    ...
    /*
    * @dev Retrieves the number of StUSR shares owned by a specific account.
    */
    function sharesOf(address _account) public view returns (uint256 shares) {
        return super.balanceOf(_account);
    }

    function convertToShares(uint256 _usrAmount) public view returns (uint256 shares) {
        return _convertToShares(_usrAmount, Math.Rounding.Floor);
    }

    function convertToUSR(uint256 _shares) public view returns (uint256 usrAmount) {
        return _convertToUSR(_shares, Math.Rounding.Floor);
    }
    ...
    function _convertToShares(uint256 _usrAmount, Math.Rounding rounding) internal view returns (uint256 shares) {
        return _usrAmount.mulDiv(totalShares() + 1, _totalUSR() + 1, rounding);
    }

    function _convertToUSR(uint256 _shares, Math.Rounding rounding) internal view returns (uint256 usrAmount) {
        return _shares.mulDiv(_totalUSR() + 1, totalShares() + 1, rounding);
    }

    function _totalUSR() internal view returns (uint256 usrAmount) {
        return usr.balanceOf(address(this));
    }
}
