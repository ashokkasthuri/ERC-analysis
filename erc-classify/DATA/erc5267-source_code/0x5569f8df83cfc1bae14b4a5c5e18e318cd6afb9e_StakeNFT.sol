...
// Source: 0x5569f8df83cfc1bae14b4a5c5e18e318cd6afb9e
// Contract Name: StakeNFT
// Generated on: 2026-05-14 12:06:10
    ...
    OwnableUpgradeable
{
    ...
    /// @dev Constant for max slippage percentage.
    uint256 public constant MAX_SLIPPAGE = 2500;
    /// @dev Decimal point constant used for calcualtion fees percentage.
    uint256 public constant DECIMAL_POINT = 10000;
    /// @dev Treasury tax percentage.
    ...
    /// @dev Slippage percentage that will be used at the time of selling token in tax.
    uint256 public slippage;

    /// @dev V2 Router address.
    ...
    /// @dev Treasury addreess to receive tax.
    address public treasury;

    /// @notice Pair path used for selling token in tax.
    address[] public pairPathToken;

    /// @dev Fallen NFT address
    address public fallenNFT;
    address public fallenNFTSale;
    /// @dev ADAO sale address
    address public adaoNFTSale;

    /**
     * @dev Events
     */
    event Deposit(uint256 indexed amount, uint256[] tokenIds);
    event DepositFromSale(uint256 indexed tokenID, address userAddress);
    event ADAONFTSaleUpdated(address indexed oldNFT, address indexed newNFT);
    event Withdraw(uint256 indexed amount, uint256[] tokenIds);
    event ClaimReward(uint256 indexed amount);
    event RouterUpdated(address indexed oldRouter, address indexed newRouter);
    event FallenNFTUpdated(address indexed oldNFT, address indexed newNFT);
    event FallenNFTSaleUpdated(
        address indexed fallenNFTSale,
        address indexed _nftSale
    );

    event TreasuryTaxUpdated(
        uint256 indexed oldTreasuryTax,
        uint256 indexed newTreasuryTax
    );
    ...
    function initialize(
        ...
        __ERC20_init_unchained("sADAO", "sADAO");
        ERC20PermitUpgradeable.__ERC20Permit_init("sADAO");
        REWARD_TOKEN = _rewardToken;
        ERC721_TOKEN = _erc721Token;
    ...
    function claimRewards() public {
        uint256 rewardBalance = IERC20Upgradeable(REWARD_TOKEN).balanceOf(
            address(this)
        );
        if (ERC20_TOKEN == REWARD_TOKEN) {
            rewardBalance = rewardBalance.sub(totalStaked);
        }
        ...
        IERC20Upgradeable(REWARD_TOKEN).transfer(msg.sender, userReward);
        lastRewardBalance = IERC20Upgradeable(REWARD_TOKEN).balanceOf(
            address(this)
        );

        if (ERC20_TOKEN == REWARD_TOKEN) {
            lastRewardBalance = lastRewardBalance.sub(totalStaked);
        }
    ...
    function claimableRewards(
        ...
        uint256 totalTokenSupply = totalStaked;
        ...
        if (totalTokenSupply != 0) {
            ...
            accTokenPerShare = accPerShare.add(
                _totalReward.mul(10000).div(totalTokenSupply)
            );
        }
        return
     ...
     *
     * @param tokenId. Token Ids to deposit.
     * @param userAddress User address for stake
     */
    function depositFromSaleContract(
        uint256 tokenId,
        address userAddress
    ) external {
        require(
            msg.sender == adaoNFTSale,
            "ADAO Sale: Sender must be ADAO sale contract"
        );
        claimRewards();
        _deposits[userAddress].add(tokenId);
        _deposit(userAddress);
        _mint(userAddress, 1 * 10 ** decimals());
        emit DepositFromSale(tokenId, userAddress);
    }

    function _deposit(address userAddress) internal {
        uint256 userTokenBalance = IERC20Upgradeable(ERC20_TOKEN).balanceOf(
            address(userAddress)
        );
        if (userTokenBalance > 0) {
            IERC20Upgradeable(ERC20_TOKEN).transferFrom(
                userAddress,
                address(this),
                userTokenBalance
            );
        }
        totalStaked = totalStaked.add(userTokenBalance);
        userAmount[userAddress] = userAmount[userAddress].add(userTokenBalance);
        rewardDebt[userAddress] = userAmount[userAddress].mul(accPerShare).div(
            10000
        );
    }
    ...
    function withdraw() external {
        ...
        uint256[] memory _tokenIds = new uint256[](depositSet.length());
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                _deposits[msg.sender].contains(depositSet.at(0)),
                "withdraw: Token not deposited"
            );
            _tokenIds[i] = depositSet.at(0);
            IERC721ADAONFT(ERC721_TOKEN).burn(depositSet.at(0));

            _deposits[msg.sender].remove(depositSet.at(0));
        }
        totalStaked = totalStaked.sub(userAmount[msg.sender]);
        _swapAndTransfer(userAmount[msg.sender]);

        uint256 _tokenId = IERC721FallenNFT(fallenNFT).safeMint(fallenNFTSale);
        IFallenNFTSale(fallenNFTSale).updateFallenNftIds(_tokenId);

        userAmount[msg.sender] = 0;
        rewardDebt[msg.sender] = userAmount[msg.sender].mul(accPerShare).div(
            10000
        );
        _burn(msg.sender, _tokenIds.length * 10 ** decimals());
        emit Withdraw(depositSet.length(), _tokenIds);
    }

    function _swapAndTransfer(uint256 _amount) private {
        uint256 _treasuryAmount = (_amount * treasuryTax) / DECIMAL_POINT;
        uint256 _transferAmount = _amount - _treasuryAmount;
        uint _tokenAmount = IUniswapV2Router02(router).getAmountsOut(
            _treasuryAmount,
            pairPathToken
        )[1];
        IERC20Upgradeable(ERC20_TOKEN).transfer(msg.sender, _transferAmount);
        IERC20Upgradeable(ERC20_TOKEN).approve(router, _treasuryAmount);
    ...
    /**
     * @notice Update the Fallen NFT contract address. Use v2 router address only.
     * @param _nft New Fallen NFT address
     */
    function updateFallenNFTAddress(address _nft) external onlyOwner {
        require(address(0) != _nft, "Zero address");
        emit FallenNFTUpdated(fallenNFT, _nft);

        fallenNFT = _nft;
    }

    /**
     * @notice Update the Fallen NFT Sale contract address.
     * @param _nftSale New Fallen NFT Sale address
     */
    function updateFallenNFTSaleAddress(address _nftSale) external onlyOwner {
        require(address(0) != _nftSale, "Zero address");
        emit FallenNFTSaleUpdated(fallenNFTSale, _nftSale);

        fallenNFTSale = _nftSale;
    }

    /**
     * @notice Update the ADAO NFT Sale contract address.
     * @param _nftSale New ADAO NFT Sale address
     */
    function updateADAONFTSaleAddress(address _nftSale) external onlyOwner {
        require(address(0) != _nftSale, "Zero address");
        emit ADAONFTSaleUpdated(adaoNFTSale, _nftSale);

        adaoNFTSale = _nftSale;
    }
    ...
    function updateSlippage(uint256 _slippage) external onlyOwner {
        require(
            ...
            "Cannot set Slippage more above max value."
        );
    ...
    function updatePairPathToken(address[] memory _path) external onlyOwner {
        require(_path.length > 1, "Pair should contain at least 2 addresses");
        pairPathToken = _path;
    ...
    function updateTreasuryTax(uint256 _treasuryTax) external onlyOwner {
        ...
        require(_treasuryTax <= 5000, "Invalid treasury tax");
...
/**
 * @notice Represents NFT Smart Contract interface
 */
contract IERC721FallenNFT {
    /**
...
/**
 * @notice Represents NFT Smart sale Contract interface
 */
contract IFallenNFTSale {
    /**
     * @dev ERC-721 Fallen Sale INTERFACE
     */
    function updateFallenNftIds(uint256 tokenId) external {}
}
