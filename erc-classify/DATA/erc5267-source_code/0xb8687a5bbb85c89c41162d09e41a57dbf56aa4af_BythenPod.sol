...
// Source: 0xb8687a5bbb85c89c41162d09e41a57dbf56aa4af
// Contract Name: BythenPod
// Generated on: 2026-05-14 12:01:01
...
contract BythenPod is ERC721Enumerable, ERC721URIStorage, ERC721Pausable, AccessControlEnumerable, EIP712 {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    uint256 private _nextTokenId;
    address private _primarySaleRecipient;
    bytes32 private constant SIGN_MINT_TYPEHASH =
        keccak256("MintRequest(address to,string uri,uint256 price,uint256 validityStartTimestamp,uint256 validityEndTimestamp,bytes32 uid)");
    ...
    event TokensMintedWithSignature(address indexed signer, address indexed mintedTo, uint256 indexed tokenIdMinted, MintRequest mintRequest);
    event TokensMinted(address indexed mintedTo, uint256 indexed tokenIdMinted);
    event TokensBurned(uint256 indexed tokenIdBurned);

    constructor(string memory name, string memory symbol, address admin, address primarySaleRecipient_, string memory collectionURI_)
        ERC721(name, symbol)
        EIP712(name, "1.0.0")
    {
        _collectionURI = collectionURI_;
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
        _setupPrimarySaleRecipient(primarySaleRecipient_);
    }
    ...
    function mint(address to, string memory uri, bytes32 uid) public onlyRole(MINTER_ROLE) {
        _checkUID(uid);
    ...
    function setCollectionURI(string memory newCollectionURI) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _checkAddress(address addr) internal view {
        ...
        if (size > 0) revert InvalidRecipientAddress();
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable) returns (bool) {
    ...
    function _processRequest(MintRequest calldata req, bytes calldata signature) internal returns (address signer) {
        if (bytes(req.uri).length == 0) {
            revert InvalidSignRequest("Invalid uri");
        }
