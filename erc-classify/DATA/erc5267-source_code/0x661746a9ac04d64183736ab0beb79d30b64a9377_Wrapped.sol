...
// Source: 0x661746a9ac04d64183736ab0beb79d30b64a9377
// Contract Name: Wrapped
// Generated on: 2026-05-14 11:59:04


// ============================================================================
// FILE: src/Wrapped.sol
// ============================================================================

/**
 * @title Earnmates, the DN404 token standard and Volatility Farming
 * @author admin@earnmates.io
 * website: https://twitter.com/earnmates
 * telegram: https://t.me/earnmates
 * docs: https://docs.earnmates.io
 */
...
import {DecentralizedIndex} from "./DecentralizedIndex.sol";
import {IUniswapV2Factory} from "./interfaces/IUniswapV2Factory.sol";
import {IUniswapV2Router02} from "./interfaces/IUniswapV2Router02.sol";
import {IStakingPoolToken} from "./interfaces/IStakingPoolToken.sol";
import {ITokenRewards} from "./interfaces/ITokenRewards.sol";

contract Wrapped is DecentralizedIndex {
    ...
    constructor(
        ...
        address _v2Router
    ) DecentralizedIndex(_name, _symbol, _fees, _lpRewardsToken, _v2Router) {
    ...
    function bond(uint256 _amount) external lock noSwapOrFee {
        bool _firstIn = _isFirstIn();
        _transferFromAndValidate(IERC20(indexToken), _msgSender(), _amount);

        uint256 _tokensMinted = _amount;
        uint256 _feeTokens = _firstIn ? 0 : (_tokensMinted * fees.bond) / DEN;

        _mint(_msgSender(), _tokensMinted - _feeTokens);

        if (_feeTokens > 0) {
            address rewardAddress = IStakingPoolToken(lpStakingPool)
                .poolRewards();
            IERC20(indexToken).transfer(rewardAddress, _feeTokens);
            ITokenRewards(rewardAddress).depositRewardsFromIndexFund(
                _feeTokens
            );
        }
        emit Bond(_msgSender(), indexToken, _amount, _tokensMinted);
    }

    function debond(uint256 _amount) external lock noSwapOrFee {
        _transfer(_msgSender(), address(this), _amount);
        uint256 _fee = (_amount * fees.debond) / DEN;
        _burn(address(this), _amount - _fee);
        IERC20(indexToken).transfer(_msgSender(), _amount - _fee);
...
// ============================================================================
// FILE: src/DecentralizedIndex.sol
// ============================================================================
...
import {StakingPoolToken} from "./StakingPoolToken.sol";
import {IStakingPoolToken} from "./interfaces/IStakingPoolToken.sol";
    ...
    ERC20Permit
{
    ...
    address public immutable V2_POOL;
    ...
    function _processPreSwapFeesAndSwap() internal {
        ...
        uint256 _min = totalSupply() / 200; // 0.5%

        if (_passesSwapDelay && _bal >= _min) {
            _swapping = true;
    ...
    function addLiquidityV2AndStake(
        uint256 _amountTokens,
        uint256 _ethAmount
    ) external payable lock noSwapOrFee {
        super._transfer(_msgSender(), address(this), _amountTokens);
        _approve(address(this), V2_ROUTER, _amountTokens);

        (
            uint256 amountToken,
            uint256 amountETH,
            uint256 liquidity
        ) = IUniswapV2Router02(V2_ROUTER).addLiquidityETH{value: _ethAmount}(
                address(this),
                _amountTokens,
                0,
                0,
        ...
        IStakingPoolToken(lpStakingPool).stakeInternal(_msgSender(), liquidity);
        if (amountToken < _amountTokens) {
            super._transfer(
                address(this),
                _msgSender(),
                _amountTokens - amountToken
            );
        }
        if (amountETH < _ethAmount) {
            (bool _sent, ) = payable(_msgSender()).call{
                value: _ethAmount - amountETH
            }("");
            require(_sent, "SENT");
        }

        emit AddLiquidity(_msgSender(), _amountTokens, _ethAmount);
    }

    function unstakeAndRemoveLiquidityV2() external lock noSwapOrFee {
        uint256 _amountLiquidity = IERC20(lpStakingPool).balanceOf(
            _msgSender()
        );

        IStakingPoolToken(lpStakingPool).unstakeInternal(
            _msgSender(),
            _amountLiquidity
        );
        IERC20(IStakingPoolToken(lpStakingPool).stakingToken()).approve(
            V2_ROUTER,
            _amountLiquidity
        );
        ...
        emit RemoveLiquidity(_msgSender(), _amountLiquidity);
    }
}
...
// ============================================================================
// FILE: src/interfaces/IStakingPoolToken.sol
// ============================================================================
...
interface IStakingPoolToken {
    ...
    function stakeInternal(address user, uint256 amount) external;

    function unstakeInternal(address user, uint256 amount) external;
}


// ============================================================================
// FILE: src/interfaces/ITokenRewards.sol
// ============================================================================
...
interface ITokenRewards {
    ...
    function depositFromETH(uint256 amount) external payable;
    ...
    function depositRewardsFromIndexFund(uint256 amount) external;
}
...
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IStakingPoolToken} from "./interfaces/IStakingPoolToken.sol";
import {TokenRewards, ITokenRewards} from "./TokenRewards.sol";
...
contract StakingPoolToken is IStakingPoolToken, ERC20 {
    ...
    constructor(
        ...
        poolRewards = address(new TokenRewards(indexFund, address(this), _rewardsToken, _weth));
    }

    function stakeInternal(address _user, uint256 _amount) external {
        require(_msgSender() == indexFund, "FORBIDDEN");
        _mint(_user, _amount);
    ...
    function unstakeInternal(address _user, uint256 _amount) external override {
        require(_msgSender() == indexFund, "FORBIDDEN");
        _burn(_user, _amount);
        IERC20(stakingToken).safeTransfer(_msgSender(), _amount);
    ...
    function _afterTokenTransfer(address _from, address _to, uint256 _amount) internal override {
        if (_from != address(0) && _from != address(0xdead)) {
            ITokenRewards(poolRewards).setShares(_from, _amount, true);
        }
        if (_to != address(0) && _to != address(0xdead)) {
            ITokenRewards(poolRewards).setShares(_to, _amount, false);
        }
    }
}
...
// ============================================================================
// FILE: src/interfaces/IDecentralizedIndex.sol
// ============================================================================
...
contract TokenRewards is ITokenRewards, Context {
    ...
    function depositFromETH(uint256 _ethAmount) public payable override {
        require(_ethAmount > 0, "NEEDTKN");
        _depositFromETH(_ethAmount);
    }

    function _depositFromETH(uint256 _ethAmount) internal {
        uint256 rewardsAmount = ISwapRouter(V3_ROUTER).exactInputSingle{
            value: _ethAmount
        }(
    ...
    function depositRewardsFromIndexFund(uint256 _amount) external override {
        require(_msgSender() == INDEX_FUND, "UNAUTHORIZED");
        _depositRewards(_amount);
    }
    ...
    function _depositRewards(uint256 _amountTotal) internal {
        ...
        if (totalShares == 0) {
            IERC20(rewardsToken).transfer(
                Ownable(address(rewardsToken)).owner(),
...
// ============================================================================
// FILE: src/libraries/BokkyPooBahsDateTimeLibrary.sol
// ============================================================================
