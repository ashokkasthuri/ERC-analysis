...
// Source: 0x122b726b0818a745064159383c5efa90442d4450
// Contract Name: NFTMarketplace
// Generated on: 2026-05-14 12:03:38
...
interface IFeeDistributor {
     ...
     *
     * Also, can emit an {EtherReceived} event if a caller sends an execessive native
     * token amount.
     */
    function distributeFees(Fee[] calldata fees) external payable;

    /**
     * @dev Returns the token address by its symbol.
     */
...
interface INFTFactory {
     ...
     * with the minter role (e.g. the {PassClaim} contract).
     *
     * A caller might additionally pass a list of token specific royalties with
     * payees and their shares. It causes creation a royalty fowrader contract bound
     * to a royalty splitter contract. See the {RoyaltyForwarder} and
...
interface INFTMarketplace {
    /**
     * @dev Defines data related to offchain item listing.
     *  - `transactionId` is an transaction identifier required to track it on
     *    the caller's side.
     *  - `collection` is an NFT collection address.
     *  - `tokenId` is the token identifier within this NFT collection.
     *  - `expirationTimestamp` is a Unix timestamp specifying when an NFT item
     *     listing is to be expired
     *  - `fees` a list of primary fees required to be distributed.
     *  - `price` a user friendly representation of an NFT item price.
     */
    struct Listing {
        uint256 transactionId;
        address collection;
    ...
    /**
     * @dev Defines data required to buy an NFT item.
     *  - `transactionId` is an transaction identifier required to track it on
     *    the caller's side.
     *  - `listing` is listing data. See `Listing` struct for more details.
     *  - `tokenReceiver` is an address to receive a bought NFT item.
     *  - `fees` a list of secondary fees required to be distributed.
     *  - `sellerSignature` is a seller's signature proving listing ownership
     *    of an NFT item.
     */
    ...
    /**
     * @dev Defines data required to cancel an NFT item.
     *  - `transactionId` is an transaction identifier required to track it on
     *    the caller's side.
     *  - `hashes` is a list of hashes for items to be canceled.
     *  - `hashSignatures` is a list of corresponding hash signatures for items to
     *    be canceled.
     */
    struct CancelItemsParams {
        uint256 transactionId;
        bytes32[] hashes;
        bytes[] hashSignatures;
    }
    ...
    /**
     * @dev Emitted when an allowed range of items to be canceled is set.
     */
    event CanceledItemRangeSet(uint8[2] canceledItemRange);

    /**
     * @dev Emitted when an NFT item is bought.
     */
    event ItemBought(address indexed collection, uint256 indexed tokenId, address indexed seller, address buyer);

    /**
     * @dev Emitted when an NFT item listing is canceled.
     */
    event ItemsCanceled(uint256 itemCount, address indexed lister);

    /**
     * @dev Emitted when an NFT item is bought or its listing is canceled.
     */
    event TransactionProcessed(uint256 indexed transactionId);
    ...
    /**
     * @dev The zero address of an NFT collection is given.
     */
    error ZeroCollection();

    /**
     * @dev The zero address of a seller is given.
     */
    error ZeroSeller();

    /**
     * @dev The zero address of a token receiver is given.
     */
    error ZeroTokenReceiver();
    ...
    /**
     * @dev No listing fees during item buying are given.
     */
    error NoListingFeesFound();

    /**
     * @dev No hashes during item caneling are given.
     */
    error NoHashesFound();

    /**
     * @dev Some hash(es) have have no corresponding signatures during item canceling.
     */
    error HashAndSignatureCountMismatched(uint256 hashCount, uint256 hashSignatureCount);

    /**
     * @dev An NFT item listing is expired.
     */
    error ItemListingExpired(uint256 expirationTimestamp, uint256 currentTimestamp);

    /**
     * @dev An allowed NFT item ranged is missed.
     */
    error CanceledItemRangeMissed(uint8[2] canceledItemRange, uint256 canceledItemCount);

    /**
     * @dev An NFT item is attempted to be bought by the seller.
     */
    error UnexpectedTokenReceiver(address seller);

    /**
     * @dev The seller of an NFT item is not verified.
     */
    error InvalidNFTOwner(address collection, uint256 tokenId, address currentOwner, address claimedOnwer);
     ...
     * @dev The seller of an NFT item is not verified.
     */
    error SellerNotVerified(address seller, bytes32 hash, bytes signature);

    /**
     * @dev The lister of an NFT item is not verified.
     */
    error ListerNotVerified(address lister, bytes32 hash, bytes signature);

    /**
     * @dev An NFT item listing has been already canceled.
     */
    error ItemAlreadyCanceled(bytes32 hash);
    ...
    /**
     * @dev Sets an allowed range of NFT item to be canceled.
     *
     * Emits an {CanceledItemRangeSet} event.
     */
    function setCanceledItemRange(uint8[2] calldata canceledItemRange) external;
    ...
    /**
     * @dev Buys an NFT item that has been already listed on the marketplace.
     *
     * Requirements:
     *
     * - Before calling this method, it is required to generate a hash from the hash
     ...
     *   registerd at the {Verifier} contract.
     *
     * Emits {ItemBought} and {TransactionProcessed} events.
     *
     * Also, can emit a {FeesDistributed} event if some fees were sent along.
     */
    function buyItem(BuyItemParams calldata params, bytes calldata signature) external payable;

    /**
     * @dev Cancel an NFT item that has been already listed on the marketplace.
     *
     * Requirements:
     *
     * - Before calling this method, it is required to generate a hash from the hash
     ...
     *   registerd at the {Verifier} contract.
     *
     * Emits {ItemCanceled} and {TransactionProcessed} events.
     */
    function cancelItems(CancelItemsParams calldata params, bytes calldata signature) external;

    /**
     * @dev Generates a hash for an operation to list an NFT which happens offchain.
     */
    function listingHash(Listing calldata listing) external view returns (bytes32);

    /**
     * @dev Generates a hash in order to call the method to buy an NFT item.
     */
    function buyItemHash(BuyItemParams calldata params, address sender) external pure returns (bytes32);

    /**
     * @dev Generates a hash in order to call the method to cancel an NFT item.
     */
    function cancelItemsHash(CancelItemsParams calldata params, address sender) external pure returns (bytes32);
}


// ============================================================================
// FILE: contracts/interfaces/INFTOperator.sol
// ============================================================================
...
// ============================================================================
// FILE: contracts/interfaces/IPassClaim.sol
// ============================================================================
...
// ============================================================================
// FILE: contracts/libraries/LFeeDistributor.sol
// ============================================================================
...
library LFeeDistributor {
    using AddressUpgradeable for address payable;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    function distributeFeesForPayer(
        IFeeDistributor feeDistributor,
        IFeeDistributor.Fee[] memory fees,
        address payer
    ) internal {
        uint256 feeCount = fees.length;

        for (uint256 i = 0; i < feeCount; ) {
            IFeeDistributor.Fee memory fee = fees[i];

            address tokenAddress = feeDistributor.tokenAddresses(fee.token);
            // slither-disable-next-line arbitrary-send-erc20
            tokenAddress == address(0)
                ? fee.payee.sendValue(fee.amount)
                : IERC20Upgradeable(tokenAddress).safeTransferFrom(payer, fee.payee, fee.amount);
...
// ============================================================================
// FILE: contracts/libraries/LParamEncoder.sol
// ============================================================================
...
import { IFeeDistributor } from "./interfaces/IFeeDistributor.sol";
import { LFeeDistributor } from "./libraries/LFeeDistributor.sol";
import { LParamEncoder } from "./libraries/LParamEncoder.sol";

/**
 * @dev This contract implements an NFT marketplace. The NFT item listing operation
 * takes place offchain, while the buying and listing cancelations ones take place
 * onchain.
 */
contract NFTMarketplace is
    ...
    EIP712Upgradeable
{
    using LFeeDistributor for IFeeDistributor;

    bytes32 public constant VERSION = "1.2.0";
    bytes32 private constant _MESSAGE_TYPEHASH =
        keccak256(
            // solhint-disable-next-line max-line-length
            "Message(uint256 transactionId,address collection,uint256 tokenId,uint256 expirationTimestamp,Fee[] fees,string price)Fee(address payee,bytes32 token,uint256 amount)"
        );
    bytes32 private constant _FEE_TYPEHASH = keccak256("Fee(address payee,bytes32 token,uint256 amount)");

    IVerifier public verifier;
    INFTOperator public nftOperator;
    IFeeDistributor public feeDistributor;
    mapping(bytes32 hash => address canceledLister) public canceledListers;
    uint8[2] public canceledItemRange;
    ...
    function initialize(
        ...
        string calldata eip712DomainName,
        string calldata eip712DomainVersion,
        uint8[2] calldata canceledItemRange_
    ) public initializer {
        AccessControlUpgradeable.__AccessControl_init();
        PausableUpgradeable.__Pausable_init();
        ReentrancyGuardUpgradeable.__ReentrancyGuard_init();
        PullPaymentUpgradeable.__PullPayment_init();
        EIP712Upgradeable.__EIP712_init(eip712DomainName, eip712DomainVersion);
        ...
        if (canceledItemRange_[0] > canceledItemRange_[1]) {
            revert InconsistentNFTItemRange(canceledItemRange_);
        }
        canceledItemRange = canceledItemRange_;
    }
     ...
     * Emits an {CanceledItemRangeSet} event.
     */
    function setCanceledItemRange(uint8[2] calldata canceledItemRange_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (canceledItemRange_[0] > canceledItemRange_[1]) {
            revert InconsistentNFTItemRange(canceledItemRange_);
        }
        canceledItemRange = canceledItemRange_;

        emit CanceledItemRangeSet(canceledItemRange_);
    }
     ...
     */
    function buyItem(
        BuyItemParams calldata params,
        bytes calldata signature
    ) external payable nonReentrant whenNotPaused {
        bytes32 hash = _buyItemHash(params, msg.sender);
    ...
    function cancelItems(
        CancelItemsParams calldata params,
        bytes calldata signature
    ) external nonReentrant whenNotPaused {
        bytes32 hash = _cancelItemsHash(params, msg.sender);
     ...
     */
    function buyItemHash(BuyItemParams calldata params, address sender) external pure returns (bytes32) {
        return _buyItemHash(params, sender);
    }
     ...
     */
    function cancelItemsHash(CancelItemsParams calldata params, address sender) external pure returns (bytes32) {
        return _cancelItemsHash(params, sender);
    }

    /**
     * @dev Generates a hash for an operation to list an NFT which happens offchain.
     */
    function listingHash(Listing calldata listing) public view returns (bytes32) {
        return
            EIP712Upgradeable._hashTypedDataV4(
                keccak256(
                    abi.encode(
                        ...
                        keccak256(abi.encodePacked(_feeHashes(listing.fees))),
                        keccak256(bytes(listing.price))
                    )
                )
            );
    }
     ...
     * @dev Buys an NFT item that has been already listed on the marketplace.
     *
     * See the {buyItem} method for more details.
     */
    function _buyItem(BuyItemParams calldata params) private {
        address seller = _validateBuyItemParams(params);
        Listing calldata listing = params.listing;

        bytes32 hash = listingHash(listing);
        if (canceledListers[hash] != address(0)) {
            revert ItemAlreadyCanceled(hash);
        }

        _verifySeller(seller, hash, params.sellerSignature);

        address collection = listing.collection;
        uint256 tokenId = listing.tokenId;

        address currentOwner = IERC721Upgradeable(collection).ownerOf(tokenId);
        if (currentOwner != seller) {
            revert InvalidNFTOwner(collection, tokenId, currentOwner, seller);
        }

        canceledListers[hash] = seller;

        nftOperator.safeTransfer(collection, tokenId, seller, params.tokenReceiver);

        IFeeDistributor.Fee[] memory fees = _mergeFees(params.listing.fees, params.fees);
        _distributeFees(fees, msg.sender);

        emit ItemBought(collection, tokenId, seller, params.tokenReceiver);
     ...
     * distributions can be intermixed in a single request.
     *
     * A caller can be an externally owned account or a Coinflow contract.
     *
     * WARNING: If the caller is the Coinflow contract the fees are disributed
     * in place on behalf of the current contract, because the Coinflow contract sets
     * ERC20 approvals directly on it.
     *
     * Emits a {FeesDistributed} event.
     *
     ...
     * token amount.
     */
    function _distributeFees(IFeeDistributor.Fee[] memory fees, address caller) private {
        uint256 feeCount = fees.length;
        if (feeCount == 0) {
            return;
        }
        ...
        if (tx.origin == caller) {
            ...
            feeDistributor.distributeFeesForPayer(fees, caller);
     ...
     * @dev Cancel an NFT item that has been already listed on the marketplace.
     *
     * See the {cancelItems} method for more details.
     */
    function _cancelItems(CancelItemsParams calldata params) private {
        bytes32[] calldata hashes = params.hashes;
        uint256 hashCount = hashes.length;
        _checkCaneledItemRange(hashCount);

        bytes[] calldata hashSignatures = params.hashSignatures;

        for (uint256 i = 0; i < hashCount; ) {
            bytes32 hash = hashes[i];
            _verifyLister(msg.sender, hash, hashSignatures[i]);

            if (canceledListers[hash] != address(0)) {
                revert ItemAlreadyCanceled(hash);
            }

            canceledListers[hash] = msg.sender;
            unchecked {
                i++;
            }
        }

        emit ItemsCanceled(hashCount, msg.sender);
        emit TransactionProcessed(params.transactionId);
    }

    /**
     * @dev Validates listing and other parameters required for NFT item buying.
     */
    function _validateBuyItemParams(BuyItemParams calldata params) private view returns (address) {
        Listing calldata listing = params.listing;
        if (listing.fees.length == 0) {
            revert NoListingFeesFound();
        }
        if (listing.collection == address(0)) {
            revert ZeroCollection();
        }
        /* solhint-disable not-rely-on-time */
        // slither-disable-next-line timestamp
        if (listing.expirationTimestamp < block.timestamp) {
            revert ItemListingExpired(listing.expirationTimestamp, block.timestamp);
        }
        /* solhint-enable not-rely-on-time */

        address seller = listing.fees[0].payee;
        if (seller == address(0)) {
            revert ZeroSeller();
        }
        if (params.tokenReceiver == address(0)) {
            revert ZeroTokenReceiver();
        }
        if (seller == params.tokenReceiver) {
            revert UnexpectedTokenReceiver(seller);
        }
    ...
    /**
     * @dev Checks that given item count is within an allowed range.
     */
    function _checkCaneledItemRange(uint256 canceledItemCount) private view {
        if (canceledItemCount < canceledItemRange[0] || canceledItemCount > canceledItemRange[1]) {
            revert CanceledItemRangeMissed(canceledItemRange, canceledItemCount);
        }
    }

    /**
     * @dev Verifies the seller of an NFT item by recovering her from a signed hash.
     */
    function _verifySeller(address seller, bytes32 hash, bytes calldata signature) private pure {
        address signer = ECDSAUpgradeable.recover(hash, signature);
        if (signer != seller) {
            revert SellerNotVerified(seller, hash, signature);
        }
    }

    /**
     * @dev Verifies the lister of an NFT item by recovering her from a signed hash.
     */
    function _verifyLister(address lister, bytes32 hash, bytes calldata signature) private pure {
        address signer = ECDSAUpgradeable.recover(hash, signature);
        if (signer != lister) {
            revert ListerNotVerified(lister, hash, signature);
        }
    }
     ...
     * @dev Generates a hash in order to call the method to buy an NFT item.
     */
    function _buyItemHash(BuyItemParams calldata params, address sender) private pure returns (bytes32) {
        Listing calldata listing = params.listing;
            ...
            keccak256(
                abi.encode(
                    ...
                    abi.encode(
                        ...
                        listing.tokenId,
                        listing.expirationTimestamp,
                        LParamEncoder.encodeFees(listing.fees),
                        listing.price
                    ),
                    LParamEncoder.encodeFees(params.fees),
     ...
     * @dev Generates a hash in order to call the method to cancel an NFT item.
     */
    function _cancelItemsHash(CancelItemsParams calldata params, address sender) private pure returns (bytes32) {
        uint256 hashCount = params.hashes.length;
        if (hashCount == 0) {
            revert NoHashesFound();
        }
        if (hashCount != params.hashSignatures.length) {
            revert HashAndSignatureCountMismatched(hashCount, params.hashSignatures.length);
        }

        bytes memory hashAndSignatures = "";
        for (uint256 i = 0; i < hashCount; ) {
            bytes.concat(hashAndSignatures, abi.encode(params.hashes[i], params.hashSignatures[i]));
            unchecked {
                i++;
            }
        }

        return keccak256(abi.encode(params.transactionId, hashAndSignatures, sender));
    }

    /**
     * @dev Generates a list of fee hashes.
     */
    function _feeHashes(IFeeDistributor.Fee[] calldata fees) private pure returns (bytes32[] memory) {
        uint256 feeCount = fees.length;
        bytes32[] memory feeHashes = new bytes32[](feeCount);

        for (uint256 i = 0; i < feeCount; ) {
            feeHashes[i] = _feeHash(fees[i]);
            unchecked {
                i++;
            }
        }
    ...
    /**
     * @dev Generates a fee hash.
     */
    function _feeHash(IFeeDistributor.Fee calldata fee) private pure returns (bytes32) {
        return keccak256(abi.encode(_FEE_TYPEHASH, fee.payee, fee.token, fee.amount));
    }

    /**
     * @dev Merges primary and secondary fees.
     */
    function _mergeFees(
        IFeeDistributor.Fee[] calldata primaryFees,
        IFeeDistributor.Fee[] calldata secondaryFees
    ) private pure returns (IFeeDistributor.Fee[] memory) {
        uint256 secondaryFeeCount = secondaryFees.length;
        if (secondaryFeeCount == 0) {
            return primaryFees;
        }

        uint256 primaryFeeCount = primaryFees.length;
        IFeeDistributor.Fee[] memory mergedFees = new IFeeDistributor.Fee[](primaryFeeCount + secondaryFeeCount);

        uint256 i = 0;
        while (i < primaryFeeCount) {
            mergedFees[i] = primaryFees[i];
            unchecked {
                i++;
            }
        }
        ...
        uint256 j = 0;
        while (j < secondaryFeeCount) {
            mergedFees[i] = secondaryFees[j];
            unchecked {
                i++;
                j++;
            }
