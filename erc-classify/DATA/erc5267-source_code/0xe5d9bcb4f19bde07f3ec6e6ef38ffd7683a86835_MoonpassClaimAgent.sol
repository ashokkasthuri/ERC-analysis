...
// Source: 0xe5d9bcb4f19bde07f3ec6e6ef38ffd7683a86835
// Contract Name: MoonpassClaimAgent
// Generated on: 2026-05-14 11:56:14


// ============================================================================
// FILE: src/claim/MoonpassClaimAgent.sol
// ============================================================================
...
import { IMoonpassClaimAgent, NFTClaimRequest, AccountClaimRequest, StakedNFTClaimRequest, UnstakeNFTRequest } from "../interfaces/claim/IMoonpassClaimAgent.sol";
...
contract MoonpassClaimAgent is
  IMoonpassClaimAgent,
  NoncesUpgradeable,
  MoonpassContract
{
  bytes32 internal constant NFT_TYPEHASH = keccak256("NFTClaimRequest(address distribution,uint256 tokenId,address to,uint256 amount,address feeCollector,uint256 value,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant ACCOUNT_TYPEHASH = keccak256("AccountClaimRequest(address distribution,address account,address to,uint256 amount,address feeCollector,uint256 value,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant STAKED_NFT_TYPEHASH = keccak256("StakedNFTClaimRequest(address distribution,uint256 tokenId,address to,uint256 amount,address taxCollector,uint256 tax,address feeCollector,uint256 value,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant UNSTAKE_NFT_TYPEHASH = keccak256("UnstakeNFTRequest(uint256 nftNetworkId,address nftAddress,address[] distributions,uint256 tokenId,address taxCollector,uint256[] tax,address feeCollector,uint256 value,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  function claimFromNFTAllocation(
    NFTClaimRequest calldata request,
    ...
    whenNotPaused
  {
    ...
    _checkRole(MoonpassRoles.CLAIM_TRUSTEE_ROLE, _signerFrom(data, signature));

    _collectClaimFee(request.feeCollector);

    _useCheckedNonce(_msgSender(), signature.nonce);

    IMoonpassNFTDistribution distribution = IMoonpassNFTDistribution(request.distribution);

    distribution.withdraw(request.tokenId, request.to, request.amount);
  }

  function claimFromAccountAllocation(
    AccountClaimRequest calldata request,
    RequestSignature calldata signature
  )
    external
    payable
    whenNotPaused
  {
    ...
    IMoonpassAccountDistribution distribution = IMoonpassAccountDistribution(request.distribution);

    distribution.withdraw(request.account, request.to, request.amount);
  }

  function claimFromStakedNFTAllocation(
    StakedNFTClaimRequest calldata request,
    RequestSignature calldata signature
  )
    external
    payable
    whenNotPaused
  {
    ...
    IMoonpassNFTDistribution distribution = IMoonpassNFTDistribution(request.distribution);
    if(request.tax > 0) {
      _claimFromStakedNFT(distribution, tokenId, request.taxCollector, request.tax);
    }
    _claimFromStakedNFT(distribution, tokenId, request.to, request.amount);
  }

  function claimStakedNFT(
    UnstakeNFTRequest calldata request,
    RequestSignature calldata signature
  ) 
    external
    payable
    whenNotPaused
  {
    if(request.distributions.length != request.tax.length) {
      revert InvalidRequestError();
    }
    bytes memory data = abi.encode(
      UNSTAKE_NFT_TYPEHASH,
      request.nftNetworkId,
      request.nftAddress,
      keccak256(abi.encodePacked(request.distributions)),
      request.tokenId,
      request.taxCollector,
      keccak256(abi.encodePacked(request.tax)),
      request.feeCollector,
      msg.value,
      _encodeContext(signature)
    );
    ...
    IMoonpassNFTDistribution distribution;
    uint256 tax;
    uint256 tokenId = request.tokenId;
    for(uint256 i = 0; i < request.distributions.length; i++) {
      tax = request.tax[i];
      distribution = IMoonpassNFTDistribution(request.distributions[i]);
      if(tax > 0) {
        _claimFromStakedNFT(distribution, tokenId, request.taxCollector, tax);
      }
    }

    emit StakedNFTClaim(request.nftNetworkId, request.nftAddress, tokenId);
  }

  function _claimFromStakedNFT(
    IMoonpassNFTDistribution distribution,
    uint256 tokenId,
    address to,
    uint256 amount
    ...
    internal
  {
    distribution.withdraw(tokenId, to, amount);
    emit StakedNFTAllocationClaim(address(distribution), tokenId, amount);
  }

  function _collectClaimFee(address to)
    internal
  {
...
// ============================================================================
// FILE: src/interfaces/claim/IMoonpassClaimAgent.sol
// ============================================================================
...
interface IMoonpassClaimAgent
{
  function claimFromNFTAllocation(NFTClaimRequest calldata request, RequestSignature calldata signature) external payable;

  function claimFromAccountAllocation(AccountClaimRequest calldata request, RequestSignature calldata signature) external payable;

  function claimFromStakedNFTAllocation(StakedNFTClaimRequest calldata request, RequestSignature calldata signature) external payable;

  function claimStakedNFT(UnstakeNFTRequest calldata request, RequestSignature calldata signature) external payable;

  error UnableCollectFeeError();
  event StakedNFTAllocationClaim(address indexed distribution, uint256 indexed tokenId, uint256 indexed amount);
  event StakedNFTClaim(uint256 indexed nftNetworkId, address indexed nftAddress, uint256 indexed tokenId);
}

struct NFTClaimRequest {
...
// ============================================================================
// FILE: src/interfaces/token-distribution/nft/IMoonpassNFTDistribution.sol
// ============================================================================
...
// ============================================================================
// FILE: src/interfaces/token-distribution/account/IMoonpassAccountDistribution.sol
// ============================================================================
...
// ============================================================================
// FILE: src/interfaces/token-distribution/IMoonpassTokenDistribution.sol
// ============================================================================
