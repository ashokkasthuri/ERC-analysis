...
// Source: 0x006274d6f31a19d8afcf8023b84ace5108b39991
// Contract Name: StableTokenV1
// Generated on: 2026-05-14 11:58:46


// ============================================================================
// FILE: src/StableTokenV1.sol
// ============================================================================
...
/// @custom:security-contact security@bitcoinpoint.com
/// @dev Extends ERC20Upgradeable, ERC20PausableUpgradeable,
/// ERC20PermitUpgradeable, UUPSUpgradeable, and Bannable
    ...
    Bannable
{
    ...
    function mint(address to, uint256 amount) external onlyOwner notBanned(to) {
        _mint(to, amount);
    }

    /// @notice Burns tokens from a specified address with permission.
    /// @dev Can only be called by the ContractOwner and if approval given by wallet.
    /// @param from Location to burn tokens from.
    ...
    function rescueERC20(IERC20 tokenContract, address to, uint256 amount) external onlyOwner notBanned(to) {
        tokenContract.safeTransfer(to, amount);
        emit TokenRescued(address(tokenContract), to, amount);
    }
    ...
    /// @notice Sets approval based on a provided signature, with checks against account bans.
    /// @dev Overrides the default permit function to include a ban check for both the owner and spender.
    /// @param owner The address of the token holder granting permission.
    /// @param spender The address which is being given approval to spend tokens on behalf of the owner.
    /// @param value The number of tokens the spender is allowed to spend.
    /// @param deadline The time at which the permit will expire (as a Unix timestamp).
...
abstract contract Bannable is OwnableUpgradeable {
    ...
    function ban(address account) external onlyOwner {
        bool success = banList.add(account);
    ...
    function unban(address account) external onlyOwner {
        bool success = banList.remove(account);
    ...
    function getBan(uint256 index) external view returns (address) {
        return banList.at(index);
    }

    /// @notice Lists all banned addresses.
    /// @return An array of banned addresses.
    function listBanned() external view returns (address[] memory) {
        return banList.values();
    }
    ...
    /// @notice Bans a list of accounts.
    /// @param accounts The addresses to ban.
    /// @dev Can only be called by the contract owner.
    function batchBan(address[] calldata accounts) external onlyOwner {
        uint256 length = accounts.length;

        for (uint256 i; i < length;) {
            if (!isBanned(accounts[i])) {
                banList.add(accounts[i]);
                emit Banned(accounts[i]);
            }
