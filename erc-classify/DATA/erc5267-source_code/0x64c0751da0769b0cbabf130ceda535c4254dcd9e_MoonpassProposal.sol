...
// Source: 0x64c0751da0769b0cbabf130ceda535c4254dcd9e
// Contract Name: MoonpassProposal
// Generated on: 2026-05-14 11:56:16


// ============================================================================
// FILE: src/governance/MoonpassProposal.sol
// ============================================================================
...
import { IMoonpassProposal, ProposalInitializer, VotingResult, VoteOptionSummary, VoteRequest, VotingStatus, SetNameRequest, SetDescriptionRequest, SetVotingPeriodRequest, SetRequiredQuorumRequest } from "../interfaces/governance/IMoonpassProposal.sol";
import { MoonpassContract } from "../MoonpassContract.sol";
import { MoonpassRoles } from "../auth/MoonpassRoles.sol";

contract MoonpassProposal is
  IMoonpassProposal,
  NoncesUpgradeable,
  MoonpassContract
{

  bytes32 internal constant VOTE_TYPEHASH = keccak256("VoteRequest(address owner,uint8 option,uint256 weight,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  bytes32 internal constant SET_DESCRIPTION_TYPEHASH = keccak256("SetDescriptionRequest(bytes32 descriptionHash,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant SET_VOTING_PERIOD_TYPEHASH = keccak256("SetVotingPeriodRequest(uint256 votingStart,uint256 votingEnd,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant SET_REQUIRED_QUORUM_TYPEHASH = keccak256("SetRequiredQuorumRequest(uint256 requiredQuorum,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  struct MoonpassProposalStorage {
    string _name;
    ...
    mapping(uint8 => VoteOption) _results;
  }

  bytes32 private constant MoonpassProposalStorageLocation = 0xdca850f6e30dd1db2c0a8d6ff41fbc3aed5465f126202f26180ce17172b812ef;

  function _getMoonpassProposalStorage()
    internal
    pure 
    returns (MoonpassProposalStorage storage $) 
  {
    assembly {
      $.slot := MoonpassProposalStorageLocation
    }
  }

  function initialize(
    ProposalInitializer calldata data,
    ...
    initializer
  {
    ...
    _setName(data.name);
    _setDescription(data.descriptionHash);
    _setVotingPeriod(data.votingStart, data.votingEnd);
    _setRequiredQuorum(data.requiredQuorum);
  }
  ...
  {
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
  ...
  {
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    Vote storage ownerVote = $._votes[owner];
    hasVoted = ownerVote.weight != 0;
  }
    ...
    )
  {
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    Vote storage ownerVote = $._votes[owner];
    if(ownerVote.weight == 0) {
      revert VoteNotFoundError();
    }
    option = ownerVote.option;
    weight = ownerVote.weight;
  }
  ...
  {
    if(option > 2) {
      revert InvalidVotingOptionError();
    }
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    VoteOption storage voteOption = $._results[option];
    count = voteOption.count;
    ...
    )
  {
    ...
    uint8 winnerOption;
    uint256 maxWeight = 0;
    VoteOptionSummary[] memory votes = new VoteOptionSummary[](3);
    VoteOption storage option;
    for(uint8 i = 0; i < 3;) {
      option = $._results[i];
      votes[i] = VoteOptionSummary(i, option.count, option.weight);
    ...
    (uint256 actualQuorum, uint256 requiredQuorum) = quorum();
    if(requiredQuorum > actualQuorum) {
      winnerOption = 3;
    }

    result = VotingResult(votingStatus(), actualQuorum, requiredQuorum, $._votesCount, winnerOption, votes);
  }
    ...
    )
  {
    ...
    if($._committed == 0) {
      return VotingStatus.DRAFT;
    }
    if($._votingStart > ts) {
      return VotingStatus.UPCOMING;
    }
    if($._votingStart < ts && $._votingEnd > ts) {
    ...
    external
  {
    ...
    _checkRole(MoonpassRoles.PROPOSAL_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _vote(request.owner, request.option, request.weight);
  }
    ...
    internal
  {
    if(votingStatus() != VotingStatus.OPEN) {
      revert VotesAreClosedError();
    }
    if(voted(owner)) {
      revert DuplicatedVoteError();
    }
    if(option > 2) {
      revert InvalidVotingOptionError();
    }
    if(weight == 0) {
      revert InsufficientVotingWeightError();
    }

    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    $._quorum += weight;
    $._votesCount++;
    $._votes[owner].option = option;
    $._votes[owner].weight = weight;
    $._results[option].weight += weight;
    $._results[option].count++;

    emit ProposalVote(owner, option, weight);
  }
  ...
  function commit()
    external
    onlyRole(MoonpassRoles.PROPOSAL_MANAGER_ROLE)
  {
    _commit();
  }
  ...
  function setName(
    string calldata proposalName
  )
    external
    onlyRole(MoonpassRoles.PROPOSAL_MANAGER_ROLE)
  {
    _setName(proposalName);
  }
  ...
  function setDescription(
    SetDescriptionRequest calldata request,
    RequestSignature calldata signature
  )
    external
  {
    bytes memory data = abi.encode(
      SET_DESCRIPTION_TYPEHASH,
      request.descriptionHash,
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.PROPOSAL_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _setDescription(request.descriptionHash);
  }
  ...
  function setDescription(
    ...
    onlyRole(MoonpassRoles.PROPOSAL_MANAGER_ROLE)
  {
    _setDescription(descriptionHash);
  }
  ...
  function setVotingPeriod(
    SetVotingPeriodRequest calldata request,
    RequestSignature calldata signature
  )
    external
  {
    bytes memory data = abi.encode(
      SET_VOTING_PERIOD_TYPEHASH,
      request.votingStart,
      request.votingEnd,
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.PROPOSAL_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _setVotingPeriod(request.votingStart, request.votingEnd);
  }
  ...
  function _setVotingPeriod(
    ...
    whenNotCommitted
  {
    if(votingStart < block.timestamp || votingEnd <= votingStart) {
      revert InvalidVotingPeriodError();
    }
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    $._votingStart = votingStart;
    $._votingEnd = votingEnd;

    emit ProposalChange();
  }

  function setRequiredQuorum(
    SetRequiredQuorumRequest calldata request,
    RequestSignature calldata signature
  )
    external
  {
    bytes memory data = abi.encode(
      SET_REQUIRED_QUORUM_TYPEHASH,
      request.requiredQuorum,
      _encodeContext(signature)
    );
    _checkRole(MoonpassRoles.PROPOSAL_TRUSTEE_ROLE, _signerFrom(data, signature));
    _useCheckedNonce(_msgSender(), signature.nonce);
    _setRequiredQuorum(request.requiredQuorum);
  }
  ...
  function setRequiredQuorum(
    ...
    onlyRole(MoonpassRoles.PROPOSAL_MANAGER_ROLE)
  {
    _setRequiredQuorum(requiredQuorum);
  }

  function _setRequiredQuorum(
    uint256 requiredQuorum
  ) 
    internal
    whenNotCommitted
  ...
  modifier whenNotCommitted {
    MoonpassProposalStorage storage $ = _getMoonpassProposalStorage();
    if($._committed == 1) {
      revert ProposalIsCommittedError();
    }
    _;
  }
...
// ============================================================================
// FILE: src/interfaces/governance/IMoonpassProposal.sol
// ============================================================================
