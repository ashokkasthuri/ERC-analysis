...
// Source: 0xe9b76e6bd99437d61fedabc7368dce5bcb406dc3
// Contract Name: MurePoolFactory
// Generated on: 2026-05-14 12:05:41
...
import {MureConfig, Config} from "../src/MureConfig.sol";
import {MureErrors} from "../src/libraries/MureErrors.sol";
import {AppMember, Role, APP_MEMBER_HASH} from "./shared/Types.sol";

/**
 * @title MurePoolFactory
 * @notice Factory contract for deploying applications as instances of the `BeaconProxy` contract
 * within the Mure protocol.
 *
 * @author Mure
 *
 * @dev Facilitates the secure and efficient deployment of upgradeable app contracts using the
 * beacon proxy pattern.
 *
 * Implements the EIP-712 standard for signature validation and Ownable for access control.
 * The factory contract relies on the MureConfig contract to verify EIP-712 signatures and manage
 * the deployment process.
 *
 * Signatures are added with an expiry to enhance the security of the deployment process by ensuring
 * that the provided signature is only valid within a certain time frame.
 */
contract MurePoolFactory is Ownable, EIP712 {
        ...
        EIP712(name_, version_)
    {
        ...
        if (config_ == address(0) || !ERC165Upgradeable(config_).supportsInterface(type(Config).interfaceId)) {
     ...
     * @param appMembers array of app members and their roles for the app contract instance
     * @param deadline expiration time of the signature
     * @param sig signature generated for the user
     * @param data data to be emitted on creation
     */
    function deployAppProxy(
        ...
        if (!_validateAdminRoleExists(appMembers)) {
            revert MureErrors.NoAdmin();
        }

        config.verifyMureSignature(_hash(appName, version, appMembers, deadline, data), sig);

        unchecked {
            ++nonces[_msgSender()];
        }

        bytes memory initializerData = abi.encodeCall(MurePool(address(0)).initialize, (appName, version, appMembers));
        proxy = address(new BeaconProxy(beaconAddress, initializerData));

        {
            (address[] memory appAdmins, address[] memory appOperators) = _extractAppRoles(appMembers);

            emit Deployment(appName, appAdmins, appOperators, proxy, data);
        }
    }
    ...
    /**
     * @dev Generates a hashed representation of the app name, version, members, deadline, along with the sender's nonce.
     * @param appName name of the app contract
     * @param version version of the app contract
     * @param appMembers array of admin addresses and their corresponding roles for the app contract
    ...
    /**
     * @notice Computes the keccak256 hash of an array of AppMember structs.
     * @param appMembers The array of AppMember structs to be hashed.
     */
    function _hashAppMembers(AppMember[] calldata appMembers) private pure returns (bytes32) {
        bytes32[] memory encodedMembers = new bytes32[](appMembers.length);

        for (uint256 i = 0; i < appMembers.length; i++) {
            encodedMembers[i] = keccak256(abi.encode(APP_MEMBER_HASH, appMembers[i].member, appMembers[i].role));
        }

        return keccak256(abi.encodePacked(encodedMembers));
    }

    /**
     * @notice Extracts and categorizes members into admins and operators based on their roles.
     * @param appMembers An array of AppMember structs containing the member address and role.
     */
    ...
    {
        address[] memory appAdmins = new address[](appMembers.length);
        address[] memory appOperators = new address[](appMembers.length);

        uint256 adminsCount = 0;
        uint256 operatorsCount = 0;

        for (uint256 i = 0; i < appMembers.length; i++) {
            if (appMembers[i].role == Role.POOL_OPERATOR) {
                appOperators[operatorsCount++] = appMembers[i].member;
            } else if (appMembers[i].role == Role.DEFAULT_ADMIN) {
                appAdmins[adminsCount++] = appMembers[i].member;
            }
        }

        // Resize the arrays to remove empty elements
        assembly {
            mstore(appAdmins, adminsCount)
            mstore(appOperators, operatorsCount)
        }

        return (appAdmins, appOperators);
    }

    /**
     * @notice Validates that there is at least one member with the DEFAULT_ADMIN role in the provided appMembers array.
     * @param appMembers An array of AppMember structs containing the member address and role.
     */
    function _validateAdminRoleExists(AppMember[] calldata appMembers) private pure returns (bool) {
        for (uint256 i = 0; i < appMembers.length; i++) {
            if (appMembers[i].role == Role.DEFAULT_ADMIN) {
                return true;
            }
        }
...
import {AppMember, Role} from "./shared/Types.sol";
    ...
    PoolApp
{
    ...
    /**
     * @notice Grants roles to the members based on their specified roles in the struct.
     * @param members An array of `AppMember` structs containing the member's address
     * and their role to be granted.
     */
    function _grantRolesToMembers(AppMember[] calldata members) private {
        for (uint256 i = 0; i < members.length; i++) {
            if (members[i].role == Role.POOL_OPERATOR) {
                _grantRole(POOL_OPERATOR_ROLE, members[i].member);
            } else if (members[i].role == Role.DEFAULT_ADMIN) {
                _grantRole(DEFAULT_ADMIN_ROLE, members[i].member);
            }
        }
    }
...
import {MureErrors} from "./libraries/MureErrors.sol";
import {Delegatable} from "./interfaces/Delegatable.sol";
import {FeeConfig} from "./interfaces/FeeConfig.sol";
import {Config} from "./interfaces/Config.sol";
import {PoolApp} from "./interfaces/PoolApp.sol";
 ...
 * @title MureConfig
 * @author Mure
 * @dev Upgradeable configuration contract governing Mure protocol settings, signers, and app delegates.
 *
 ...
 */
contract MureConfig is EIP712Upgradeable, OwnableUpgradeable, UUPSUpgradeable, ERC165Upgradeable, Config {
    ...
    function setAppDelegate(address app, address delegate) external onlyOwner {
        if (!ERC165Upgradeable(app).supportsInterface(type(PoolApp).interfaceId)) {
            revert MureErrors.InvalidAddress(app);
        }
    ...
    /**
     * @notice Sets the fee contract address.
    ...
    function setMureFeeContract(address feeContractAddress) external onlyOwner {
        if (feeContractAddress == address(0) || !_supportsFeeConfigInterface(feeContractAddress)) {
            revert MureErrors.InvalidAddress(feeContractAddress);
        }

        _getStorage().mureFeeContract = feeContractAddress;

        emit FeeContractSet(feeContractAddress);
    }
    ...
    /**
     * @notice Retrieves the fee contract address.
     */
    function getFeeContractAddress() external view returns (address) {
        return _getStorage().mureFeeContract;
    }
    ...
    {
        address feeContractAddress = _getStorage().mureFeeContract;

        if (feeContractAddress == address(0)) {
            return (0, address(0));
        } else {
            return FeeConfig(feeContractAddress).getPoolFee(poolApp, poolName, amount);
        }
    }
    ...
    /**
     * @dev Retrieves the storage for the MureConfig contract.
     */
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(Config).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
     * @dev Checks if fee config supports the fee config interface.
     */
    function _supportsFeeConfigInterface(address feeConfig) private view returns (bool) {
        return FeeConfig(feeConfig).supportsInterface(type(FeeConfig).interfaceId);
    }
...
library MureErrors {
    ...
    /**
     * @dev thrown when no member has a `DEFAULT_ADMIN_ROLE` role
     */
    error NoAdmin();
}
...
/**
 * @dev Enum for pool member roles
 */
enum Role {
    DEFAULT_ADMIN,
    POOL_OPERATOR
...
// ============================================================================
// FILE: src/interfaces/FeeConfig.sol
// ============================================================================
...
/**
 * @dev Structure of a pool fee config
 */
...
/**
 * @dev Interface for fee configuration with support for ERC-165 detection.
 * @author Mure
 */
interface FeeConfig is IERC165 {
    function getPoolFee(address poolApp, string calldata poolName, uint256 amount)
        external
        returns (uint256 feeAmount, address recipient);
    ...
    function poolFeePercentage(address poolApp, string calldata poolName)
        external
        view
