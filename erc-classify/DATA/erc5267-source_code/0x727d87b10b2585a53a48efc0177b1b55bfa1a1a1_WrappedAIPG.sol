...
contract WrappedAIPG is ERC20, AccessControl, Pausable, EIP712 {
    ...
    bytes32 private constant MINT_TYPEHASH = keccak256("MintRequest(address to,uint256 amount,uint256 nonce,uint256 deadline)");
    ...
    // Mapping to keep track of nonces for each address to prevent replay attacks
    ...
    function mint(MintRequest calldata request, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 structHash = keccak256(abi.encode(MINT_TYPEHASH, request.to, request.amount, request.nonce, request.deadline));
        bytes32 digest = _hashTypedDataV4(structHash);
        address signer = ecrecover(digest, v, r, s);
        require(hasRole(MINTER_ROLE, signer), "Invalid signature or unauthorized");
        require(nonces[request.to] == request.nonce, "Invalid nonce");
        nonces[request.to]++; // Increment nonce after use
