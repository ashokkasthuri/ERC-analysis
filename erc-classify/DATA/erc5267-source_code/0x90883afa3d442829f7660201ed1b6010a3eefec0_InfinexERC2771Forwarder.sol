...
// Source: 0x90883afa3d442829f7660201ed1b6010a3eefec0
// Contract Name: InfinexERC2771Forwarder
// Generated on: 2026-05-14 11:58:31
...
// ============================================================================
// FILE: src/forwarder/InfinexERC2771Forwarder.sol
// ============================================================================
 ...
 * * `nonce`: A unique transaction identifier (non-ordered) to avoid replayability and request invalidation.
 ...
 */
contract InfinexERC2771Forwarder is EIP712 {
    ...
    // Stores the nonces that have been used for each address
    ...
    /**
     * @dev This nonce has already been used for a previous message from this address.
    ...
    /**
     * @dev Gas shrink from forwarder detected
     */
    error InfinexERC2771ForwarderGasShrinkDetected();

    /**
     * @dev Reverts when the address provided is the zero address.
     */
    error ZeroAddress();
     ...
     */
    constructor() EIP712("InfinexERC2771Forwarder", "1") { }

    /**
     * @dev Returns `true` if a request is valid for a provided `signature` at the current block timestamp.
     *
     * A transaction is considered valid when the target trusts this forwarder, the nonce hasn't been used,
     * and the signer matches the `from` parameter of the signed request.
     *
    ...
    function execute(ForwardRequestData calldata request) public payable virtual {
        ...
        if (msg.value != request.value) {
            revert InfinexERC2771ForwarderMismatchedValue(request.value, msg.value);
        }

        (bool success, bytes memory returndata) = _execute(request, true);
        Address.verifyCallResultFromTarget(request.to, success, returndata);
    }
     ...
     *
     * If `atomic` is set to `true`, this function will revert when at least
     ...
     * atomic can be set to true to prevent execution when using a service that avoids
     ...
     *
     * NOTE: Setting `requireValidRequests` to false guarantees an all-or-nothing requests execution only for
     ...
     */
    function executeBatch(ForwardRequestData[] calldata requests, address payable refundReceiver, bool atomic)
        public
        payable
        virtual
    {
        ...
        for (uint256 i; i < requests.length; ++i) {
            ...
            (bool success, bytes memory returndata) = _execute(requests[i], atomic);
            if (atomic) {
                Address.verifyCallResultFromTarget(requests[i].to, success, returndata);
            }
            if (!success) {
        ...
        if (requestsValue != msg.value) {
            revert InfinexERC2771ForwarderMismatchedValue(requestsValue, msg.value);
        }
        ...
        returns (bool active, bool signerMatch, address signer)
    {
        ...
        return (nonces[recovered][request.nonce] == false, isValid && recovered == request.from, recovered);
    }
     ...
     */
    function _recoverForwardRequestSigner(ForwardRequestData calldata request) internal view virtual returns (bool, address) {
    ...
    {
        (bool newNonce, bool signerMatch, address signer) = _validate(request);

        // Need to explicitly specify if a revert is required since non-reverting is default for
        // batches and reversion is opt-in since it could be useful in some scenarios
        if (requireValidRequest) {
            if (!newNonce) {
                revert InfinexERC2771ForwarderNonceAlreadyUsed(request.from, request.nonce);
            }

            if (!signerMatch) {
                revert InfinexERC2771ForwarderInvalidSigner(signer, request.from);
            }
        }

        // Ignore an invalid request because requireValidRequest = false
        if (newNonce && signerMatch) {
            // Nonce should be used before the call to prevent reusing by reentrancy
            nonces[signer][request.nonce] = true;

            bytes memory data = abi.encodePacked(request.data, request.from);
            // slither-disable-next-line arbitrary-send-eth,low-level-calls,return-bomb
            (success, returndata) = request.to.call{ gas: request.gas, value: request.value }(data);
    ...
    function _checkForwardedGas(uint256 gasLeft, ForwardRequestData calldata request) private pure {
        ...
        if (gasLeft < request.gas / 63) {
            revert InfinexERC2771ForwarderGasShrinkDetected();

            // Original code from OpenZepplin
            //
