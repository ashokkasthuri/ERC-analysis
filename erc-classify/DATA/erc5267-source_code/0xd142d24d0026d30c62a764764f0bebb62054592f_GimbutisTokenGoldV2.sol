...
// Source: 0xd142d24d0026d30c62a764764f0bebb62054592f
// Contract Name: GimbutisTokenGoldV2
// Generated on: 2026-05-14 11:57:56
...
// ============================================================================
// FILE: contracts/GimbutisTokenGoldV2.sol
// ============================================================================
...
contract GimbutisTokenGoldV2 is
    OwnableUpgradeable,
    ...
    ERC20PermitUpgradeable
{
    ...
    function initialize(
        ...
        __Ownable_init();
        __ERC20_init("Gimbutis Gold", "GXAU");
        __ERC20Permit_init("Gimbutis Gold");
        emit Initialized(msg.sender, block.number);
    }

    modifier onlyActive() {
        require(toggleStatus, "GXAU: contract is not available right now ");
        _;
    }
    ...
    modifier onlyASWM() {
        require(msg.sender == aswmAddress, "GXAU: only aswm");
        _;
    }
    ...
    function setGoldRatio(uint256 _goldRatio) external onlyAdmin {
        goldRatio = _goldRatio;
        require(_goldRatio >= 100, "GXAU: SiverRatio less than 100");
     ...
     */ //XXX, uint256 expectedGXAU
    function buy(address token, uint256 amount) external onlyActive {
        require(
            token != address(this),
            "GXAU: Buy GXAU for GXAU is prohibited"
        );
        require(amount > 0, "GXAU: buy amount is null");
        uint256 _userBalance = IERC20Upgradeable(token).balanceOf(msg.sender);
        require(_userBalance >= amount, "GXAU: Invalid user balance for buy");
        ...
        if (token == usdcAddress || token == usdtAddress) {
            ...
            require(
                ...
                "GXAU: USDC pair with this token not exist"
            );
    ...
    function addReserve(address _holder, uint256 _amount) external onlyASWM {
        require(
            reserves[_holder] == 0,
            "GXAU: already have redeem with this address"
        );
        require(
            super.balanceOf(_holder) >= _amount && minReserveAmount < _amount,
            "GXAU: Invalid amount for redeem"
        );
    ...
    function releaseReserve(address _holder) external onlyASWM {
        require(reserves[_holder] != 0, "GXAU: Redeem is null");
    ...
    /**
     * Returns the latest price Gimbutis Gold for buying
     */
    function getTokenPrice() public view returns (uint256 price) {
        ...
        require(_price > 0, "GXAU: Aggregator price less or equal 0");
        price =
            (uint256(_price) *
    ...
    /**
     * @notice Functon to buy gxau tokens with usdc/usdt
    ...
    function _buyTokenWithUsd(
        ...
        uint256 _gxauAmount = (amount * 1e30) / tokenPrice;

        require(
            _gxauAmount >= minBuyAmount,
            "GXAU: Too small amount to buy minimum GXAU"
        );
        require(
            super.balanceOf(address(this)) >= _gxauAmount,
            "GXAU: Not enough GXAU on balance"
        );
        ...
        _transfer(address(this), buyer, _gxauAmount);
    }

    /**
     * @notice Functon to buy gxau tokens with ERC20
    ...
    function _buyToken(
        ...
        uint256 _gxauUsdPrice = getTokenPrice(); // 18 decimals
        uint256 _erc20UsdPrice = _getERC20Price(_pairAddress, _token, amount);
        uint256 _gxauAmount = (_erc20UsdPrice * 1e18) / _gxauUsdPrice;

        require(
            _gxauAmount >= minBuyAmount,
            "GXAU: Too small GXAU amount for buy"
        );
        require(
            super.balanceOf(address(this)) >= _gxauAmount,
            "GXAU: Not enough GXAU tokens on contract balance"
        );
        ...
        _transfer(address(this), _sender, _gxauAmount);
    }
    ...
    /**
     * @notice Functon to sell Gimbutis Gold tokens
     * @param token USDC/USDT contract address
     * @param _gxauAmount amount of GXAU tokens
     */
    function sell(uint256 _gxauAmount, address token) external onlyActive {
        require(
            super.balanceOf(msg.sender) >= _gxauAmount,
            "GXAU: Too small user balance"
        );
        require(
            token == usdcAddress || token == usdtAddress,
            "GXAU: Sell for USDC or USDT only"
        );
        ...
        uint256 _usdAmount = (_gxauAmount * adjustedPrice) /
            10 ** (aggregator.decimals() + 12);

        require(
            IERC20Upgradeable(token).balanceOf(address(this)) >= _usdAmount,
            "GXAU: Too small contract balance for sell"
        );
        ...
        _transfer(msg.sender, address(this), _gxauAmount);
    }
    ...
    function claim(address token, uint256 amount) external onlyAdmin {
        require(
            ...
            "GXAU: Too small contract balance to claim"
        );
