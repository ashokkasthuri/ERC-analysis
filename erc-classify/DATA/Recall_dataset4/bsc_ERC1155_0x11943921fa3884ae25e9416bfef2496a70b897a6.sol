...
// File: contracts/WalletMonstersApusV3.sol
...
contract WalletMonstersApusV3 is ERC1155, Ownable, IERC1155Receiver{
    ...
    mapping(address => mapping(uint => uint)) coolDown; 
    mapping(address => mapping(uint => bool)) alreadyMinted; 
    ...
    modifier userCantMint(uint256 tokenId) {
        require(alreadyMinted[msg.sender][tokenId] == false, "already minted this period");
        _;
    }
    ...
    modifier hasNoPendingBattle
        {
            require(hasPendingBattle[msg.sender] == false, "settle of previous battle");
            _;
        }
   ...
   //Utility functions
    ...
    function buyEnjimon(uint id) payable public mintingPeriod returns(bool success){
        ...
        function enjimonMarketBuy(uint id) payable public returns(bool success){
        require(balanceOf(address(this), id) > 0, "No More $Enjimon at this id");
        require(msg.value >= marketEnjimonPrice, "Not enough to cover tx");
