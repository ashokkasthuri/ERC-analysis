...
// File: contracts/CantBeEvilCuriosUpgradeable.sol
// SPDX-License-Identifier: MIT
// a16z Contracts v0.0.1 (CantBeEvil.sol)
pragma solidity ^0.8.13;
...
contract CantBeEvilCuriosUpgradeable is ERC165, Initializable {
    using Strings for uint;
    string internal constant _BASE_LICENSE_URI =
        "ar://zmc1WTspIhFyVY82bwfAIcIExLFH5lUcHHUN0wXg4W8/";
    LicenseVersion internal licenseVersion;

    function __CantBeEvilCurios_init(
        LicenseVersion _licenseVersion
    ) internal onlyInitializing {
        __CantBeEvilCurios_init_unchained(_licenseVersion);
    }

    function __CantBeEvilCurios_init_unchained(
        LicenseVersion _licenseVersion
    ) internal onlyInitializing {
        licenseVersion = _licenseVersion;
    }
...
import "./Initialization/Initialization.sol";
import "./CantBeEvilCuriosUpgradeable.sol";
import "hardhat/console.sol";

contract Curios721 is
    OwnableUpgradeable,
    ...
    CantBeEvilCuriosUpgradeable
{
    ...
    constructor(Initialization.Initialization721 memory init) {
        initialize(init);
    }

    function initialize(Initialization.Initialization721 memory init)
        public
        initializer
    {
        __Ownable_init();
        ...
        _setDefaultRoyalty(init.royaltyRecipient, init.royaltyAmount);
        ...
        for (uint256 i = 0; i < init.uriSetters.length; i++) {
            _grantRole(URI_SETTER_ROLE, init.uriSetters[i]);
        }

        for (uint256 i = 0; i < init.minters.length; i++) {
            _grantRole(MINTER_ROLE, init.minters[i]);
        }

        for (uint256 i = 0; i < init.burners.length; i++) {
            _grantRole(BURNER_ROLE, init.burners[i]);
        }

        for (uint256 i = 0; i < init.pausers.length; i++) {
            _grantRole(PAUSER_ROLE, init.pausers[i]);
        }

        __CantBeEvilCurios_init(init.licenseVersion);

        maxSupply = init.maxSupply;
        isPublicMint = init.isPublicMint;
        mintPrice = init.mintPrice;
    }
    ...
    function publicMint(address receiver) external payable {
        console.log("I FINALLY RUN %o", isPublicMint);

        require(isPublicMint, "PM1");
        require(msg.value >= mintPrice, "PM2");
        _mintTo(receiver);
    }

    //may make a new role as this should probably only ever be allowed to be called succesfully
    //by one multi-sig or ledger.
    //at time of withdrawel, if curios fees are turned on, then send some % to curios, rest to wallet address.
    //Otherwise all to wallet address
    function withdrawPublicMintFunds(address wallet)
        external
        payable
    ...
    function mintTo(address addr) public onlyRole(MINTER_ROLE) {
        _mintTo(addr);
    }

    function _mintTo(address addr) internal whenNotPaused {
        if (maxSupply == 0) {
            _safeMint(addr, totalSupply(), "");
        } else {
            require(totalSupply() < maxSupply, "M1");
            _safeMint(addr, totalSupply(), "");
        }
    }

    //research burner role requirement
    ...
    //Add forced royalties on all transfers. flat number like $1
    function safeTransferFrom(
        address from,
        address to,
        uint256 tokenId,
...
// File: contracts/CuriosFactory721.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;
...
import "./Initialization/Initialization.sol";
import "./Curios721.sol";

//add pausable
contract CuriosFactory721 is AccessControl {
    ...
    mapping(uint256 => Curios721) public instances;

    event CreatedNewInstance(address);

    constructor(address[] memory _admins, address[] memory _controllers, address _implementation) {
        ...
        for (uint256 i = 0; i < _controllers.length; i++) {
            _setupRole(DEPLOYER_ROLE, _controllers[i]);
        }
    ...
    function createNewInstance(
        Initialization.Initialization721 memory init
    ) external onlyRole(DEPLOYER_ROLE) {
        address clone = Clones.clone(address(implementation));

        init.admins = addSelfToArray(init.admins);

        Curios721 instance = Curios721(clone);

        instance.initialize(init);
    ...
    // function publicMint(uint256 index) external payable {
    //     instances[index].publicMint();
    // }

    // function mintTo(uint256 index, address addr) external {
    //     require(instances[index].hasRole(instances[index].DEFAULT_ADMIN_ROLE(), msg.sender), "Cannot mint tokens!");
    //     instances[index].mintTo(addr);
    // }

    // function mint(uint256 index) external {
    //     require(instances[index].hasRole(instances[index].DEFAULT_ADMIN_ROLE(), msg.sender), "Cannot mint tokens!");
    //     instances[index].mint();
    // }

    // function setBaseURI(uint256 index, string memory baseURI) external {
    //     require(instances[index].hasRole(instances[index].DEFAULT_ADMIN_ROLE(), msg.sender), "Cannot mint tokens!");
    //     instances[index].setBaseURI(baseURI);
    // }

    // function tokenURI(uint256 index, uint256 tokenId) external view returns(string memory) {
    //     return instances[index].tokenURI(tokenId);
    // }

    // function getOwnerOf(uint256 _index, uint256 tokenId) external view returns(address) {
    //     return instances[_index].ownerOf(tokenId);
    // }

    // function getName(uint256 _index) external view returns(string memory) {
    //     return instances[_index].name();
    // }

    // function getSymbol(uint256 _index) external view returns(string memory) {
    //     return instances[_index].symbol();
    // }

    // FACTORY HELPER FUNCTIONS
    function addSelfToArray(
        address[] memory _arr
...
// File: contracts/Initialization/Initialization.sol
// SPDX-License-Identifier: MIT
...
library Initialization {
    ...
    struct Initialization1155 {
        string baseURI;
        TokenType[] tokenTypes;
        address[] admins;
        address[] uriSetters;
        address[] tokenCreators;
        address[] minters;
        address[] burners;
        address[] pausers;
        address[] upgraders;
