...
contract CGPTNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
     ...
     */
    function mint(uint256 tokenID) external  nonReentrant onlyOwner {
     ...
     */
    function bulkMint(uint256[] memory tokenIDs) external  nonReentrant onlyOwner {
        require(isMintable, "Mint is not enabled.");
...
// File: contracts/CGPTNFTFactory.sol
// SPDX-License-Identifier: MIT
...
contract CGPTNFTFactory is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function create(
        ...
        cgptnft.initialize(_name, _symbol, _uri, maxSupply, tokenIDs, _isMintable, msg.sender);

        cNFTs.push(address(cgptnft));
        length += 1;

        emit Create(address(cgptnft), msg.sender, _name, _symbol, _uri, maxSupply, _isMintable, block.timestamp);
