...
// File: contracts/locker/interfaces/IDepositHandler.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/locker/interfaces/IVaultFactory.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.23;
...
// File: contracts/token-launcher/interfaces/ITokenLauncherLiquidityPoolFactory.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.23;
...
// File: contracts/token-launcher/payment/TokenLauncherLiquidityPoolFactoryV1.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.23;
...
contract TokenLauncherLiquidityPoolFactoryV1 is ITokenLauncherLiquidityPoolFactory, AccessControlEnumerable {
    ...
    function createV2LiquidityPool(CreateV2Input memory input) external payable override onlyRole(DEFAULT_ADMIN_ROLE) returns (CreateV2Output memory) {
        ...
        return CreateV2Output({ liquidityPoolToken: pairAddress, liquidity: liquidity });
    }

    function _addLiquidity(CreateV2Input memory input, uint256 sourceEarnedAmount) private returns (uint256 liquidity) {
        ...
        if (input.liquidityPoolDetails.pairedToken == router.WETH()) {
            require(input.liquidityPoolDetails.amountPairedToken == msg.value, "TokenLauncherLiquidityPoolFactoryV1: INVALID_ETH_AMOUNT");
    ...
    function _lockLiquidity(CreateV2Input memory input, address pairAddress) private {
        require(input.lockLPDetails.lockLPTokenPercentage <= MULTIPLIER_BASIS, "TokenLauncherLiquidityPoolFactoryV1: INVALID_LOCK_PERCENTAGE");
        uint256 lpBalance = IERC20(pairAddress).balanceOf(address(this));
        uint256 lpBalanceToLock = (lpBalance * input.lockLPDetails.lockLPTokenPercentage) / MULTIPLIER_BASIS;
        uint256 lpBalanceToTreasury = lpBalance - lpBalanceToLock;
        ...
        IERC20(pairAddress).safeApprove(IVaultFactory(vaultFactory).paymentModule(), lpBalanceToLock);
    ...
    function setVaultFactory(address _vaultFactory) external onlyRole(DEFAULT_ADMIN_ROLE) {
        address oldVaultFactory = vaultFactory;
        vaultFactory = _vaultFactory;
        emit VaultFactoryUpdated(oldVaultFactory, _vaultFactory);
    }

    function emergencyWithdrawERC20(address tokenAddress, address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        IERC20(tokenAddress).safeTransfer(to, amount);
    }

    function emergencyWithdrawETH(address payable to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        to.transfer(amount);
    }
