// File: contracts/LegendaryShield.sol
...
contract LegendaryShield is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintCompliance() {
    ...
    require(IERC20(usdcAddress).balanceOf(msg.sender) >= cost, 'insufficient funds.');
    _;
  }
  ...
  function setMaxSupply(uint _maxSupply) external onlyOwner {
    require(totalSupply() <= _maxSupply, 'cannot set maxSupply below current supply.');
    maxSupply = _maxSupply;
  }

  // [.√.] sets: signed for a given `_tokenId` //
  function setSigned(uint _tokenId, string memory _email) external onlyOwner() {
    require(block.timestamp <= signBy, 'signBy date has passed.');

    // gets: `holderInfo` from token by tokenId //
    Holders storage holder = holderInfo[_tokenId];

    // sets: email address and marks signed //
    holder.email = _email;
    holder.signed = true;
  }
  ...
  function setEmail(uint _tokenId, string memory _email) public onlyHolder(_tokenId) {
    ...
    require(!holder.signed, 'already signed, must request update.');
  ...
  function withdraw() external {
    require(msg.sender == devAddress || msg.sender == treasuryAddress, 'only dev or treasury.');
    uint usdcBalance = IERC20(usdcAddress).balanceOf(address(this));
    // sends 10% to dev //
    IERC20(usdcAddress).safeTransfer(devAddress, usdcBalance * 10 / 100);
    // sends 90% to treasury //
    IERC20(usdcAddress).safeTransfer(treasuryAddress, usdcBalance * 90 / 100);
  }

  // rescues: given token //
  function unstuck(address _tokenAddress) external onlyOwner {
    // prevents: funny business //
    require(_tokenAddress != usdcAddress, 'cannot remove usdc.');
    // gets: token balance //
  ...
  function unstuck() external nonReentrant onlyOwner {
    // sends: stuck native funds to treasury //
  ...
  function _buyNFT() internal returns (bool success) {
    ...
    require(IERC20(usdcAddress).balanceOf(msg.sender) >= cost, 'insufficient funds.');
    // transfers: usdc from sender to contract //
    IERC20(usdcAddress).safeTransferFrom(msg.sender, address(this), cost);
  ...
  function imageURI(uint _tokenId) public view returns (string memory) {
    ...
    string memory baseURI = _baseImgURI();
    return bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, _tokenId.toString(), imgSuffix))
        : '';
  }
  ...
  function hasSigned(uint _tokenId) public view returns (bool) {
    return holderInfo[_tokenId].signed;
  }
...
// File: node_modules/erc721a/contracts/extensions/ERC721AQueryable.sol
// SPDX-License-Identifier: MIT
// ERC721A Contracts v3.3.0
// Creator: Chiru Labs
