...
// File: contracts/marketplace/executions/CollectionOffer.sol
// SPDX-License-Identifier: MIT
...
contract CollectionOffer is Initializable {
    ...
    event CollectionOfferAccepted(address offerer, bytes32 salt, address collection, uint256 token_id, address seller);
     ...
     * @param _amount total NFTs
     * @param _endTime offer end time
     * @param _token_id NFT id to sell
     * @param _salt offer salt
     */
    function acceptCollectionOffer(
        ...
        uint8 _iteration = collectionOffers[_offerer][_salt];

        ++collectionOffers[_offerer][_salt];

        (bool success, bytes memory result) = address(validator).call(
            abi.encodeWithSignature(
                "acceptCollectionOffer(bytes,address,address,uint256,uint8,uint256,bytes32,uint256,uint8,address)",
                _signature,
                _offerer,
                _collection,
                _price,
        ...
        emit CollectionOfferAccepted(_offerer, _salt, _collection, _token_id, msg.sender);
    }
    ...
    function batchAcceptCollectionOffer(Types.CollectionOffer[] calldata _offers) external {
        ...
        for (uint8 i = 0; i < _offers.length; i++) {
            ...
            emit CollectionOfferAccepted(_offers[i].offerer, _offers[i].salt, _offers[i].collection, _offers[i].token_id, msg.sender);
        }
    }
...
// File: contracts/marketplace/executions/Offer.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
contract Offer is Initializable {
    ...
    event OfferAccepted(address offerer, bytes32 salt, address collection, uint256 token_id, address seller);
    ...
    function acceptOffer(
        ...
        bool _completed = offers[_offerer][_salt];

        offers[_offerer][_salt] = true;

        (bool success, bytes memory result) = address(validator).call(
            abi.encodeWithSignature(
                "acceptOffer(bytes,address,address,uint256,uint256,bytes32,uint256,bool,address)",
                _signature,
                _offerer,
                _collection,
                _price,
        ...
        emit OfferAccepted(_offerer, _salt, _collection, _token_id, msg.sender);
    }
    ...
    function batchAcceptOffer(Types.Offer[] calldata _offers) external {
        ...
        for (uint8 i = 0; i < _offers.length; i++) {
            ...
            emit OfferAccepted(_offers[i].offerer, _offers[i].salt, _offers[i].collection, _offers[i].token_id, msg.sender);
        }
    }
...
// File: contracts/marketplace/executions/Swap.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
contract Swap is Initializable {
    ...
    event Swapped(address seller, bytes32 salt, address collection, uint256 token_id, address buyer);
     ...
     * @param _signature order signature
     * @param _seller NFT seller
     * @param _collection collection
     * @param _currency currency
     * @param _price price per NFT
     * @param _endTime order end time
     * @param _token_id NFT id to sell
     * @param _salt order salt
     */
    function buy(
        bytes calldata _signature,
        address _seller,
        ...
        bool _completed = orders[_seller][_salt];

        orders[_seller][_salt] = true;

        (bool success, bytes memory result) = address(validator).call{value: msg.value}(
            abi.encodeWithSignature(
                "buy(bytes,address,address,address,uint256,uint256,bytes32,uint256,bool,address)",
                _signature,
                _seller,
                _collection,
                _currency,
        ...
        emit Swapped(_seller, _salt, _collection, _token_id, msg.sender);
    }
    ...
    function batchBuy(Types.Order[] memory _orders) external payable {
        ...
        for (uint8 i = 0; i < _orders.length; i++) {
            ...
            emit Swapped(_orders[i].seller, _orders[i].salt, _orders[i].collection, _orders[i].token_id, msg.sender);
        }
    }
...
// File: contracts/marketplace/libraries/Checker.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
// File: contracts/marketplace/libraries/Types.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
...
// File: contracts/security/Auth.sol
